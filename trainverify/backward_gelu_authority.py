"""Authenticate exact-erf BW_gelu against the original nnScaler mirror.

No saved-output substitution, approximation conversion, or value premise. This
is source-domain authentication, not a proof of PyTorch floating-point refinement.
"""
...
def _fail(reason):
    raise ValueError('bw-gelu ' + reason)
...
def bind(cells, source_index):
    ...
    if not isinstance(cell.ir, IRBpOperation) or cell.opname.name != 'BW_gelu':
    ...
    if fw.opname.name != 'FW_gelu' or fw.ir.signature != 'torch.nn.functional.gelu':
    ...
    for obj, name in ((cell, 'BW.' + fw.ir.name), (fw, fw.ir.name)):
        ...
        if set(kwargs) - {'approximate', '__consts'} or not _same_typed(kwargs.get('__consts', []), []):
        ...
        if not _same_typed(kwargs.get('approximate', 'none'), 'none'):
            _fail('Denote exact erf derivative requires approximate=none')
    ...
    if not shapes[0] or any(type(n) is not int or n <= 0 for n in shapes[0]) or any(sh != shapes[0] for sh in shapes):
        _fail('equal positive elementwise operand/gradient shapes required')
    return dict(source_index=source_index, fw_source_index=fi,
                ...
                approximate='none', derivative='geluDerivScalar(saved_input)',
