"""Original BW_softmax [G, saved INPUT logits] -> [dX] source contract.
...
def _fail(reason):
    raise ValueError('bw-softmax ' + reason)
...
def bind(cells, source_index):
    ...
    if not isinstance(cell.ir, IRBpOperation) or cell.opname.name != 'BW_softmax':
    ...
    if fw.opname.name != 'FW_softmax' or fw.ir.signature != 'torch.softmax':
    ...
    ndim = len(fw._input_irs[0].shape)
    for obj in (cell, fw):
        for kwargs in (obj.kwargs, obj.ir.kwargs):
            if (set(kwargs) - {'dim', 'dtype', '__consts'}
                    or type(kwargs.get('dim')) is not int
                    or kwargs['dim'] not in (-1, ndim - 1)
                    or kwargs.get('dtype') is not None
                    or not _same_typed(kwargs.get('__consts', []), [])):
                raise ValueError('bw-softmax unsupported last-axis/dtype source kwargs')
    return dict(source_index=source_index, fw_source_index=fi,
                ...
                derivative='p * (G - sum_last(G * p)); p = softmax(saved_input)',
                obligations=['cotangent provenance', 'saved primal input value reconstruction'],
