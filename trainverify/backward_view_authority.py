"""Authenticate original BW_view [G,saved FW input] as inverse reshape.

Shape metadata determines the target, never the saved tensor values.
...
def _fail(reason):
    raise ValueError('bw-view ' + reason)
...
def bind(cells, source_index):
    ...
    if not isinstance(cell.ir, IRBpOperation) or cell.opname.name != 'BW_view':
    ...
    if fw.opname.name != 'FW_view' or fw.ir.signature != 'torch.Tensor.view':
    ...
    for obj, name in ((cell, 'BW.' + fw.ir.name), (fw, fw.ir.name)):
        ...
        if set(kwargs) - {'size', '__consts'} or not _same_typed(kwargs.get('__consts', []), []):
        ...
        if not _same_typed(kwargs, {k: v for k, v in fw.kwargs.items() if k != '__consts'}):
            _fail('original FW/BW size kwargs mismatch')
    ...
    g, x, dx, fx, fy = shapes
    if g != fy or x != fx or dx != x or prod(g) != prod(x):
        _fail('inverse reshape original gradient/input/output shape mismatch')
    size = fw.kwargs.get('size')
    if (type(size) not in (tuple, list) or not size
            or any(type(n) is not int or n == 0 or n < -1 for n in size)
            or list(size).count(-1) > 1):
        _fail('original view size must be nonempty integer dimensions')
    resolved = list(size)
    if -1 in resolved:
        divisor = prod(n for n in resolved if n != -1)
        if prod(x) % divisor:
            _fail('original inferred view size is nondivisible')
        resolved[resolved.index(-1)] = prod(x) // divisor
    if tuple(resolved) != fy:
        _fail('original view size disagrees with FW output')
    return dict(source_index=source_index, fw_source_index=fi,
                ...
                derivative='reshape(cotangent, saved_input.shape)',
