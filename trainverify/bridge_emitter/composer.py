...
def render_closed_joined_view_segment(ir: GoalIR, relation, segment_id: str) -> str:
    ...
    if len(state_ids) != len(set(state_ids)):
        raise ValueError("joined view states contain duplicate authority")
    states = {item.state_id: item for item in chain.states}
    try:
        before, after = states[segment.pre_state_id], states[segment.post_state_id]
    except KeyError as exc:
        raise ValueError("joined view framing state is not materialized") from exc
    ...
    def writer(name: str, graph: str, store: str, final: str, nodes_name: str,
        ...
        return [
            ...
            f"          [{before_nodes}] [{after_nodes}] {_node_text(node)}",
...
def render_closed_k_rank_local_segment(ir: GoalIR, relation, segment_id: str) -> str:
    """Render one exact list-indexed K-rank local transition."""
    try:
    ...
    segment = next((item for item in chain.segments if item.segment_id == segment_id), None)
    if segment is None or len(segment.transition_ids) != 1:
        raise ValueError("K-rank local renderer requires one exact transition")
    transition = next(
        (item for item in relation.transition_specs
         if item.transition_id == segment.transition_ids[0]), None
    )
    if transition is None:
        raise ValueError("K-rank local renderer lacks its exact transition")
    identity = transition.transition_id.split(":", 2)
    if len(identity) != 3 or identity[2] != transition.rule_id:
        raise ValueError("K-rank local transition has malformed certificate identity")
    cert_type = {
        "KRankLocalRelationCertificate": KRankLocalRelationCertificate,
    }.get(identity[1])
    ...
    exact_backends = {
        ...
        (KRankLocalRelationCertificate, "layernorm-sharded-k-rank-dim1", "FW_layernorm", 1):
            "TrainVerify.Denote.fw_layernorm_distribute_allGatherPrimDimN_dim1_K_3d",
    }
    expected_theorem = exact_backends.get((cert_type, cert.rule_id, cert.op, cert.gather_dim))
    if expected_theorem is None:
        raise ValueError("no checked K-rank local theorem for this certificate")
    if cert.lean_theorem != expected_theorem or transition.lean_theorem != expected_theorem:
        raise ValueError("K-rank local exact theorem identity mismatch")

    records = {item.source: item for item in chain.relation_facts}
    states = {item.state_id: item for item in chain.states}
    before, after = states[segment.pre_state_id], states[segment.post_state_id]
    expected_pre = (cert.input_fact,)
    if transition.pre_facts != tuple(sorted(set(expected_pre))) or transition.post_facts != (cert.output_fact,):
        raise ValueError("K-rank local transition/certificate fact identity mismatch")
    try:
        pre_records = tuple(records[fact] for fact in expected_pre)
        post = records[cert.output_fact]
    except KeyError as exc:
        raise ValueError(f"K-rank local relation fact is unresolved: {exc}") from exc
    if any(record.fact_id not in before.fact_ids for record in pre_records):
        raise ValueError("K-rank local input relation is not live")
    if any(record.kind != "sharded" or record.gather_dim != 1 for record in (*pre_records, post)):
        raise ValueError("K-rank local renderer requires exact dim1 ShardedRel facts")

    k = len(post.pm_tids)
    if k == 0 or cert.rank_count != k or len(cert.pm_step_ids) != k:
        raise ValueError("K-rank local rank count is not derived from the ordered PM list")
    if any(len(record.pm_tids) != k for record in pre_records):
        raise ValueError("K-rank local input/output PM list lengths disagree")
    ...
    if tuple(node.rank for node in pm_nodes) != tuple(range(k)):
        raise ValueError("K-rank local PM writers are not ordered ranks 0..K-1")
    if any(node.op != cert.op or len(node.outs) != 1 or node.params for node in pm_nodes):
        raise ValueError("K-rank local PM writer signature mismatch")
    if tuple(node.outs[0] for node in pm_nodes) != post.pm_tids:
        raise ValueError("K-rank local output relation does not preserve ordered PM TIDs")
    if sm_node.ins[0] != pre_records[0].sm_tid:
        raise ValueError("K-rank local SM dynamic input does not match its relation")
    for rank, node in enumerate(pm_nodes):
        if node.ins[0] != pre_records[0].pm_tids[rank]:
            raise ValueError("K-rank local PM dynamic inputs do not preserve ordered PM TIDs")
    arity = 2 if cert.op == "FW_linear" else 3
    if len(sm_node.ins) != arity or any(len(node.ins) != arity for node in pm_nodes):
        raise ValueError("K-rank local writer arity mismatch")
    external = tuple(zip(cert.external_tids, cert.external_shapes))
    if len(external) != arity - 1:
        raise ValueError("K-rank local external authority arity mismatch")
    expected_external = tuple(tid for tid, _shape in external)
    if tuple(sm_node.ins[1:]) != expected_external or any(
        tuple(node.ins[1:]) != expected_external for node in pm_nodes
    ):
        raise ValueError("K-rank local external tensor bindings disagree")

    authorities = tuple(chain.authority_facts)
    external_rows = []
    for tid, shape in external:
        eq = [item for item in authorities if (
            getattr(item, "kind", None) == "tensor_eq"
            and item.left_side == "sm" and item.left_tid == tid
            and item.right_side == "pm" and item.right_tid == tid
        )]
        sh = [item for item in authorities if (
            getattr(item, "kind", None) == "tensor_shape"
            and item.side == "pm" and item.tid == tid and tuple(item.shape) == tuple(shape)
        )]
        if len(eq) != 1 or len(sh) != 1 or eq[0].fact_id not in before.fact_ids or sh[0].fact_id not in before.fact_ids:
    ...
    def semantic(node, store):
        args = [f"({store} {tid})" for tid in node.ins]
        if cert.op == "FW_linear":
            return f"fw_linear {args[0]} {args[1]}"
        return f"fw_layernorm {args[0]} {args[1]} {args[2]}"

    def apply_lemma(node, graph, store):
        common = [
            "          rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective",
            "            (hshuffle := by simp) (hunshuffle := by simp) (hattn := by simp)]",
            "          simp [applyNodeDistributed, applyNodeRingAttn]",
        ...
        if cert.op == "FW_linear":
            last = f"          exact applyNode_fw_linear_out {graph} {store} {node.rank} {node.ins[0]} {node.ins[1]} {node.outs[0]}"
        else:
            last = f"          exact applyNode_fw_layernorm_out {graph} {store} {node.rank} {node.ins[0]} {node.ins[1]} {node.ins[2]} {node.outs[0]} []"
        return common + [last]

    def writer_lines(name, side, pos, node):
        graph = ir.sm_graph_ref if side == "sm" else ir.pm_graph_ref
        store = "smStore" if side == "sm" else "pmStore"
        nodes = "smNodes" if side == "sm" else "pmNodes"
        final = "smFinal" if side == "sm" else "pmFinal"
        result = [
            f"    have {name} : {final} {node.outs[0]} = {semantic(node, store)} := by",
            ...
            f"        {_node_text(node)} {node.outs[0]} (fun t => {semantic(node, 't')}) (by",
            "          intro t",
        ]
        result += apply_lemma(node, graph, "t")
        result += ["        ) (by native_decide) (by native_decide)]"]
        rewrites = [
            f"foldl_applyNodeDistributedFaithful_at_not_written {graph} ({nodes}.take {pos}) {store} {tid} (by native_decide) (by native_decide)"
            for tid in node.ins
        ]
        result.append("      rw [" + ",\n        ".join(rewrites) + "]")
        return result
    ...
    for index, record in enumerate(pre_records):
        lines += [
            f"    have hIn{index} : {record.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
            f"    change ShardedRel (smStore {record.sm_tid}) {in_lists[index]} {record.gather_dim} {_shape_text(list(record.full_shape))} {_shape_text(list(record.shard_shape))} at hIn{index}",
        ]
    for index, (tid, shape, eq, sh) in enumerate(external_rows):
        lines += [
            f"    have hExternalEq{index} : {eq.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
            f"    change smStore {tid} = pmStore {tid} at hExternalEq{index}",
    ...
    if len(shard) != 3 or any(value <= 0 for value in shard):
        raise ValueError("K-rank local theorem requires a positive rank-3 shard shape")
    b, seq, inner = shard
    hshape0 = "(fun x hx => hIn0.shard_shapes x hx)"
    if cert.op == "FW_linear":
        out_width = external_rows[0][1][0]
        comm_args = (
            f"({expected_theorem} (K := {in_lists[0]}.length) (b := {b}) (s := {seq}) "
            f"(i := {inner}) (o := {out_width}) (xs := {in_lists[0]}) (w := pmStore {external_rows[0][0]}) "
            f"(by simp) (by omega) (by omega) (by omega) "
            f"(by omega) rfl {hshape0} hExternalShape0)"
        )
    elif cert.op == "FW_layernorm":
        comm_args = (
            f"({expected_theorem} (K := {in_lists[0]}.length) (b := {b}) (s := {seq}) "
            f"(d := {inner}) (xs := {in_lists[0]}) (gamma := pmStore {external_rows[0][0]}) "
            f"(beta := pmStore {external_rows[1][0]}) (by simp) (by omega) "
            f"(by omega) (by omega) rfl {hshape0} hExternalShape0 hExternalShape1)"
        )
    lines += [
        f"    have hComm := {comm_args}",
        f"    have hOutValue : smFinal {post.sm_tid} = allGatherPrimDimN 1 {out_list}.length 0 {out_list} := by",
        "      rw [hSmWriter]",
    ]
    for index in range(len(external_rows)):
        lines.append(f"      rw [hExternalEq{index}]")
    for index in range(len(pre_records)):
        lines.append(f"      rw [hIn{index}.full_value]")
    lines += [
        "      rw [hComm]",
        "      simp only [List.map, List.length_cons, List.length_nil]",
    ]
    lines.append("      rw [" + ", ".join(f"← hPmWriter{rank}" for rank in range(k)) + "]")

    for rank, node in enumerate(pm_nodes):
        input_membership = "(by simp)"
        input_shape = f"(hIn0.shard_shapes (pmStore {pre_records[0].pm_tids[rank]}) {input_membership})"
        lines += [f"    have hOutShape{rank} : (pmFinal {node.outs[0]}).shape = {_shape_text(list(post.shard_shape))} := by",
                  f"      rw [hPmWriter{rank}]"]
        if cert.op == "FW_linear":
            lines.append(f"      exact fw_linear_3d_shape {b} {seq} {inner} {external_rows[0][1][0]} _ _ {input_shape} hExternalShape0")
        else:
            lines += ["      unfold fw_layernorm", f"      rw [{input_shape}]", "      rfl"]

    lines += [
        f"    have hout : {post.fact_id}.Holds smFinal pmFinal := by",
        f"      change ShardedRel (smFinal {post.sm_tid}) {out_list} 1 {_shape_text(list(post.full_shape))} {_shape_text(list(post.shard_shape))}",
        "      refine {",
        ...
        "      }",
        "      · rw [hOutValue]",
        f"        rw [allGatherPrimDimN_shape 1 {out_list}.length {out_list} {_shape_text(list(post.shard_shape))}]",
        "        · norm_num",
        "        · simp only [List.head?, Option.map, Option.getD]",
        "          exact hOutShape0",
        "      · intro shard hmem",
    ...
    lines += [f"        · exact hOutShape{rank}" for rank in range(k)]
    fresh = [post.fact_id]
    lines += [
        "    intro fact hfact",
...
def _k_rank_segment_context(ir, relation, segment_id: str, rule_id: str,
    ...
    certificate = _select_exact_typed_certificate(
        relation, transition, rule_id, theorem, certificate_type, certificate_facts
    )
    records = {item.source: item for item in chain.relation_facts}
    try:
        pre = records[transition.pre_facts[0]]
...
def _membership_cases(
    names: list[str], indent: str = "      ", binder: str = "shard"
) -> list[str]:
    ...
    for index, name in enumerate(names):
        lines += [f"{indent}· subst {binder}", f"{indent}  exact {name}"]
    return lines
...
def render_closed_k_rank_output_sharded_linear_segment(
    ...
    """Replay one SM and ordered K PM output-sharded 3D linear writers."""
    try:
    ...
    certificate = _select_exact_typed_certificate(
        relation, transition, rule_id, theorem, KRankOutputShardedLinearCertificate,
        lambda cert: (
    ...
    try:
        ...
        weight = records[certificate.weight_fact]
        output = records[certificate.output_fact]
    except KeyError as exc:
        raise ValueError("output-sharded linear relation fact is not materialized") from exc
    states = {item.state_id: item for item in chain.states}
    before, after = states[segment.pre_state_id], states[segment.post_state_id]
    if not {activation.fact_id, weight.fact_id} <= set(before.fact_ids):
        raise ValueError("output-sharded linear input facts are not live")
    ...
    k = len(weight.pm_tids)
    if (k <= 0 or len(output.pm_tids) != k or certificate.rank_count != k
            or certificate.output_gather_dim != 2):
        raise ValueError("output-sharded linear rank count is not ordered PM authority")
    if (tuple(activation.full_shape) != tuple(certificate.activation_shape)
            or tuple(activation.shard_shape) != tuple(certificate.activation_shape)
            ...
            or tuple(output.full_shape) != tuple(certificate.output_full_shape)
            or tuple(output.shard_shape) != tuple(certificate.output_shard_shape)):
        raise ValueError("output-sharded linear closed shapes disagree with its certificate")
    ...
    if (len(activation_shape) != 3 or len(weight_full) != 2
            or len(weight_shard) != 2 or len(output_full) != 3
            or len(output_shard) != 3):
        raise ValueError("output-sharded linear requires exact rank-3/rank-2 shapes")
    b, seq, inner = activation_shape
    local_out = weight_shard[0]
    if (min(b, seq, inner, local_out) <= 0
            or weight_shard != (local_out, inner)
            or weight_full != (local_out * k, inner)
    ...
    sm_node = ir.sm_nodes[exact_sm[0]]
    pm_nodes = tuple(ir.pm_nodes[index] for index in exact_pm)
    if sm_node.rank != 0 or sm_node.op != "FW_linear":
        raise ValueError("output-sharded linear SM writer is not exact")
    if tuple(node.rank for node in pm_nodes) != tuple(range(k)):
        raise ValueError("output-sharded linear PM writers are not ordered ranks")
    writers = (sm_node, *pm_nodes)
    if any(node.params for node in writers):
        raise ValueError("output-sharded linear FW_linear writers require no parameters")
    if any(len(node.ins) != 2 or len(node.outs) != 1 for node in writers):
        raise ValueError("output-sharded linear writers must be binary singleton-output nodes")
    if sm_node.ins[0] != activation.sm_tid:
        raise ValueError("output-sharded linear SM activation TID disagrees with its fact")
    if sm_node.ins[1] != weight.sm_tid:
    ...
    if tuple(node.ins[1] for node in pm_nodes) != tuple(weight.pm_tids):
        raise ValueError("output-sharded linear PM weight TIDs do not preserve ordered authority")
    if tuple(node.outs[0] for node in pm_nodes) != tuple(output.pm_tids):
        raise ValueError("output-sharded linear PM output TIDs do not preserve ordered authority")
    ...
    lines += [
        ...
        "      · native_decide", "      · native_decide", "      · native_decide", "      · native_decide",
        f"    have hActivation : {activation.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
        f"    change smStore {activation.sm_tid} = pmStore {shared_activation} ∧ (smStore {activation.sm_tid}).shape = {_shape_text(list(activation_shape))} ∧ (pmStore {shared_activation}).shape = {_shape_text(list(activation_shape))} at hActivation",
        f"    have hWeight : {weight.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
        f"    change ShardedRel (smStore {weight.sm_tid}) [{', '.join(f'pmStore {tid}' for tid in weight.pm_tids)}] 0 {_shape_text(list(weight_full))} {_shape_text(list(weight_shard))} at hWeight",
    ]
    ...
    def writer(name, side, pos, node, node_name):
        ...
        return [
            ...
            f"      rw [foldl_faithful_middle_writer {graph} {store} ({nodes}.take {pos}) ({nodes}.drop {pos + 1})",
            f"        {node_name} {node.outs[0]} (fun t => fw_linear (t {node.ins[0]}) (t {node.ins[1]})) (by",
            "          intro t",
            "          rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective",
            "            (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]",
            "          simp [applyNodeDistributed, applyNodeRingAttn]",
    ...
    lines += [
        ...
        f"      (x := pmStore {shared_activation}) (ws := {pm_weights})",
        ...
        f"    have hOutValue : smFinal {output.sm_tid} = allGatherPrimDimN 2 {pm_outputs}.length 0 {pm_outputs} := by",
    ...
    shape_names = []
    for rank, node in enumerate(pm_nodes):
        name = f"hOutShape{rank}"; shape_names.append(name)
        lines += [
            f"    have {name} : (pmFinal {node.outs[0]}).shape = {_shape_text(list(output_shard))} := by",
            f"      rw [hPmWriter{rank}]",
            f"      exact fw_linear_3d_shape {b} {seq} {inner} {local_out} _ _ hActivation.2.2",
            f"        (hWeight.shard_shapes (pmStore {weight.pm_tids[rank]}) (by simp))",
        ]
    lines += [
        f"    have hout : {output.fact_id}.Holds smFinal pmFinal := by",
        f"      change ShardedRel (smFinal {output.sm_tid}) {pm_outputs} 2 {_shape_text(list(output_full))} {_shape_text(list(output_shard))}",
        "      refine {", "        full_value := hOutValue", "        full_shape := ?_",
        ...
        "      · rw [hOutValue]",
        f"        rw [allGatherPrimDimN_shape 2 {pm_outputs}.length {pm_outputs} {_shape_text(list(output_shard))}]",
        "        · simp only [List.length_cons, List.length_nil]", "          native_decide",
        "        · simp only [List.head?, Option.map, Option.getD]",
        f"          exact {shape_names[0]}",
        "      · intro shard hmem",
        "        simp only [List.mem_cons, List.not_mem_nil, or_false] at hmem",
        "        rcases hmem with " + " | ".join("rfl" for _ in range(k)),
    ]
...
def render_closed_k_rank_matmul_head_axis_segment(ir: GoalIR, relation, segment_id: str) -> str:
    ...
    try:
        ...
        raise ValueError("K-rank head-axis matmul relation fact is not materialized") from exc
    states = {item.state_id: item for item in chain.states}
    before, after = states[segment.pre_state_id], states[segment.post_state_id]
    if first.fact_id not in before.fact_ids or second.fact_id not in before.fact_ids:
        raise ValueError("K-rank head-axis matmul input facts are not live")
    if output.fact_id not in after.fact_ids:
        raise ValueError("K-rank head-axis matmul output fact is not live")
    if (first.kind != "sharded" or second.kind != "sharded" or output.kind != "sharded"
            or first.gather_dim != 1 or second.gather_dim != 1 or output.gather_dim != 1):
        raise ValueError("K-rank head-axis matmul requires exact dim1 ShardedRel facts")
    k = len(output.pm_tids)
    if (k < 2 or cert.rank_count != k or len(first.pm_tids) != k
            or len(second.pm_tids) != k or cert.output_gather_dim != 1):
        raise ValueError("K-rank head-axis matmul rank count is not derived from ordered PM authority")
    if (tuple(first.full_shape) != tuple(cert.first_operand_full_shape)
            or tuple(first.shard_shape) != tuple(cert.first_operand_shard_shape)
        ...
        raise ValueError("K-rank head-axis matmul closed rank-4 shapes disagree with its certificate")
    x_full, x_shard = tuple(first.full_shape), tuple(first.shard_shape)
    y_full, y_shard = tuple(second.full_shape), tuple(second.shard_shape)
    out_full, out_shard = tuple(output.full_shape), tuple(output.shard_shape)
    if any(len(shape) != 4 for shape in (x_full, x_shard, y_full, y_shard, out_full, out_shard)):
        raise ValueError("K-rank head-axis matmul requires exact rank-4 shapes")
    b, local_h, q, inner = x_shard
    m = y_shard[3]
    if (local_h <= 0 or q <= 0 or inner <= 0 or m <= 0
            or x_full != (b, local_h * k, q, inner)
            or y_full != (b, local_h * k, inner, m)
            or y_shard != (b, local_h, inner, m)
            or out_full != (b, local_h * k, q, m)
            or out_shard != (b, local_h, q, m)):
        raise ValueError("K-rank head-axis matmul exact rank-4 dimensions do not compose")
    if len(transition.sm_node_indices) != 1 or len(transition.pm_node_indices) != k:
    ...
    if tuple(node.rank for node in pm_nodes) != tuple(range(k)):
        raise ValueError("K-rank head-axis matmul PM writers are not exact ordered ranks")
    writers = (sm_node, *pm_nodes)
    if any(node.params for node in writers):
        raise ValueError("K-rank head-axis FW_matmul writers require no parameters")
    if any(len(node.ins) != 2 or len(node.outs) != 1 for node in writers):
        raise ValueError("K-rank head-axis FW_matmul writers must be binary singleton-output nodes")
    if sm_node.ins != [first.sm_tid, second.sm_tid] or sm_node.outs[0] != output.sm_tid:
        raise ValueError("K-rank head-axis matmul SM writer disagrees with operand/output facts")
    if tuple(node.ins[0] for node in pm_nodes) != tuple(first.pm_tids):
        raise ValueError("K-rank head-axis matmul PM first operands do not preserve ordered TIDs")
    if tuple(node.ins[1] for node in pm_nodes) != tuple(second.pm_tids):
        raise ValueError("K-rank head-axis matmul PM second operands do not preserve ordered TIDs")
    if tuple(node.outs[0] for node in pm_nodes) != tuple(output.pm_tids):
        raise ValueError("K-rank head-axis matmul PM outputs do not preserve ordered TIDs")
    ...
    lines += [
        ...
        f"    change ShardedRel (smStore {first.sm_tid}) [{', '.join(f'pmStore {tid}' for tid in first.pm_tids)}] 1 {_shape_text(list(x_full))} {_shape_text(list(x_shard))} at hFirst",
    ...
    pm_outputs = "[" + ", ".join(f"pmFinal {tid}" for tid in output.pm_tids) + "]"
    lines += [
        "    have htransport := ShardedRel.fw_matmul_head_axis_rank4",
        f"      hFirst hSecond (K := {pm_xs}.length)",
        f"      (b := {b}) (h := {local_h}) (q := {q}) (k := {inner}) (m := {m})",
        "      (by simp) (by native_decide) (by native_decide) (by native_decide) (by native_decide)",
        "      (by simp) (by simp)",
        f"    have hout : {output.fact_id}.Holds smFinal pmFinal := by",
        f"      change ShardedRel (smFinal {output.sm_tid}) {pm_outputs} 1 {_shape_text(list(out_full))} {_shape_text(list(out_shard))}",
...
def render_closed_k_rank_matmul_query_axis_segment(ir: GoalIR, relation, segment_id: str) -> str:
    ...
    if (k < 2 or cert.rank_count != k or len(first.pm_tids) != k or cert.output_gather_dim != 2):
        raise ValueError("K-rank query-axis matmul rank count is not derived from ordered PM authority")
    if (tuple(first.full_shape) != tuple(cert.first_operand_full_shape)
            or tuple(first.shard_shape) != tuple(cert.first_operand_shard_shape)
            or tuple(second.full_shape) != tuple(cert.second_operand_shape)
            or tuple(second.shard_shape) != tuple(cert.second_operand_shape)
            or tuple(output.full_shape) != tuple(cert.output_full_shape)
            or tuple(output.shard_shape) != tuple(cert.output_shard_shape)):
        raise ValueError("K-rank query-axis matmul closed rank-4 shapes disagree with its certificate")
    x_full_shape, x_shard_shape = tuple(first.full_shape), tuple(first.shard_shape)
    y_shape = tuple(second.full_shape)
    out_full_shape, out_shard_shape = tuple(output.full_shape), tuple(output.shard_shape)
    if any(len(shape) != 4 for shape in (x_full_shape, x_shard_shape, y_shape, out_full_shape, out_shard_shape)):
        raise ValueError("K-rank query-axis matmul requires exact rank-4 shapes")
    b, heads, local_q, inner = x_shard_shape
    m = y_shape[3]
    if (local_q <= 0 or inner <= 0 or m <= 0
            or x_full_shape != (b, heads, local_q * k, inner)
            or y_shape != (b, heads, inner, m)
            or out_full_shape != (b, heads, local_q * k, m)
            or out_shard_shape != (b, heads, local_q, m)):
        raise ValueError("K-rank query-axis matmul exact rank-4 dimensions do not compose")
    if len(transition.sm_node_indices) != 1 or len(transition.pm_node_indices) != k:
        raise ValueError("K-rank matmul transition footprint is not exact 1xK")
    sm_start, sm_end = segment.sm_range
    pm_start, pm_end = segment.pm_range
    ...
    if tuple(node.outs[0] for node in pm_nodes) != tuple(output.pm_tids):
        raise ValueError("K-rank query-axis matmul PM outputs do not preserve ordered TIDs")
    ...
    lines += [
        ...
        "      · native_decide", "      · native_decide", "      · native_decide", "      · native_decide",
        f"    have hFirst : {first.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
        f"    change ShardedRel (smStore {first.sm_tid}) [{', '.join(f'pmStore {tid}' for tid in first.pm_tids)}] 2 {_shape_text(list(x_full_shape))} {_shape_text(list(x_shard_shape))} at hFirst",
        f"    have hSecond : {second.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
    ...
    lines += [
        ...
        f"      (hrel := hFirst) (hjoined := hSecond) (K := {pm_inputs}.length)",
        f"      (b := {b}) (h := {heads}) (q := {local_q}) (k := {inner}) (m := {m})",
        "      (by simp) (by native_decide) (by native_decide) (by native_decide) (by simp)",
        "    rw [hSecond.1] at htransport",
        f"    have hout : {output.fact_id}.Holds smFinal pmFinal := by",
        f"      change ShardedRel (smFinal {output.sm_tid}) {pm_outputs} 2 {_shape_text(list(out_full_shape))} {_shape_text(list(out_shard_shape))}",
        "      rw [hSmWriter, " + ", ".join(f"hPmWriter{rank}" for rank in range(k)) + ", hSecond.1]",
        "      simpa using htransport",
        "    intro fact hfact",
        f"    have covered : fact ∈ [{output.fact_id}] ++ {before.state_id}.facts := by",
        f"      exact (show {after.state_id}.facts ⊆ [{output.fact_id}] ++ {before.state_id}.facts by native_decide) hfact",
...
def render_closed_k_rank_matmul_contraction_segment(ir: GoalIR, relation, segment_id: str) -> str:
    ...
    try:
        ...
        y_fact = records[certificate.second_operand_fact]
        output = records[certificate.output_fact]
    except KeyError as exc:
        raise ValueError("K-rank contraction relation fact is not materialized") from exc
    states = {item.state_id: item for item in chain.states}
    before, after = states[segment.pre_state_id], states[segment.post_state_id]
    if x_fact.kind != "sharded" or y_fact.kind != "sharded" or output.kind != "reduction":
        raise ValueError("K-rank contraction requires two sharded inputs and reduction output")
    k = len(output.pm_tids)
    if (k < 2 or certificate.rank_count != k or len(x_fact.pm_tids) != k
            or len(y_fact.pm_tids) != k or x_fact.gather_dim != 3 or y_fact.gather_dim != 2):
        raise ValueError("K-rank contraction rank count/layout authority is not exact")
    if y_fact.fact_id not in before.fact_ids or output.fact_id not in after.fact_ids:
        raise ValueError("K-rank contraction relation facts are not live")
    x_full, x_shard = tuple(x_fact.full_shape), tuple(x_fact.shard_shape)
    y_full, y_shard = tuple(y_fact.full_shape), tuple(y_fact.shard_shape)
    out_shape = tuple(output.full_shape)
    if (x_full != tuple(certificate.first_operand_full_shape)
            or x_shard != tuple(certificate.first_operand_shard_shape)
            or y_full != tuple(certificate.second_operand_full_shape)
            or y_shard != tuple(certificate.second_operand_shard_shape)
            or out_shape != tuple(certificate.output_shape)):
        raise ValueError("K-rank contraction closed shapes disagree with certificate")
    if any(len(shape) != 4 for shape in (x_full, x_shard, y_full, y_shard, out_shape)):
        raise ValueError("K-rank contraction requires exact rank-4 shapes")
    b, heads, q, local_k = x_shard
    m = y_shard[3]
    if (x_full != (b, heads, q, local_k * k)
            or y_full != (b, heads, local_k * k, m)
            or y_shard != (b, heads, local_k, m)
            or out_shape != (b, heads, q, m)):
        raise ValueError("K-rank contraction dimensions do not compose")
    if len(transition.sm_node_indices) != 1 or len(transition.pm_node_indices) != k:
    ...
    if tuple(node.rank for node in pm_nodes) != tuple(range(k)):
        raise ValueError("K-rank contraction PM writers are not exact ordered ranks")
    writers = (sm_node, *pm_nodes)
    if any(node.params for node in writers):
        raise ValueError("K-rank FW_matmul contraction writers require no parameters")
    if any(len(node.ins) != 2 for node in writers):
        raise ValueError("K-rank FW_matmul contraction writers must be binary")
    if any(len(node.outs) != 1 for node in writers):
        raise ValueError("K-rank FW_matmul contraction writers must be singleton-output")
    if sm_node.ins != [x_fact.sm_tid, y_fact.sm_tid] or sm_node.outs != [output.sm_tid]:
        raise ValueError("K-rank contraction SM writer disagrees with relation facts")
    expected_pairs = tuple(zip(x_fact.pm_tids, y_fact.pm_tids, output.pm_tids))
    live_pairs = tuple((node.ins[0], node.ins[1], node.outs[0]) for node in pm_nodes)
    if live_pairs != expected_pairs:
        raise ValueError("K-rank contraction PM writers do not preserve exact rank-wise zip")
    ...
    lines += [
        ...
        "      apply RelationState.Holds.fold_frame smNodes pmNodes smStore pmStore hstate",
        "      · native_decide", "      · native_decide", "      · native_decide", "      · native_decide",
        f"    have hX : {x_fact.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
        f"    change ShardedRel (smStore {x_fact.sm_tid}) [{', '.join(f'pmStore {tid}' for tid in x_fact.pm_tids)}] 3 {_shape_text(list(x_full))} {_shape_text(list(x_shard))} at hX",
        f"    have hY : {y_fact.fact_id}.Holds smStore pmStore := hstate _ (by native_decide)",
    ...
    lines += [
        ...
        f"      (x := smStore {x_fact.sm_tid}) (y := smStore {y_fact.sm_tid})",
        f"      (xs := {x_list}) (ys := {y_list}) (K := {x_list}.length)",
        f"      (b := {b}) (h := {heads}) (q := {q}) (k := {local_k}) (m := {m})",
        "      hX hY (by simp) (by native_decide) (by native_decide) (by native_decide)",
        "      (by simp) (by simp)",
        f"    change ReductionRel (fw_matmul (smStore {x_fact.sm_tid}) (smStore {y_fact.sm_tid})) (List.zipWith fw_matmul {x_list} {y_list}) {_shape_text(list(out_shape))} at htransport",
        f"    have hout : {output.fact_id}.Holds smFinal pmFinal := by",
        f"      change ReductionRel (smFinal {output.sm_tid}) {out_list} {_shape_text(list(out_shape))}",
        "      rw [hSmWriter, " + ", ".join(f"hPmWriter{rank}" for rank in range(k)) + "]",
...
def render_closed_k_rank_contiguous_segment(ir: GoalIR, relation, segment_id: str) -> str:
    ...
    if pre.kind != "sharded" or post.kind != "sharded":
        raise ValueError("K-rank contiguous requires exact ShardedRel facts")
    k = len(post.pm_tids)
    if (k < 2 or certificate.rank_count != k or len(pre.pm_tids) != k
            or certificate.gather_dim != pre.gather_dim or pre.gather_dim != post.gather_dim
            or tuple(certificate.full_shape) != tuple(pre.full_shape)
            or tuple(certificate.full_shape) != tuple(post.full_shape)
            or tuple(certificate.shard_shape) != tuple(pre.shard_shape)
            or tuple(certificate.shard_shape) != tuple(post.shard_shape)):
    ...
    if (sm_node.ins[0] != pre.sm_tid or sm_node.outs[0] != post.sm_tid
            or tuple(node.ins[0] for node in pm_nodes) != tuple(pre.pm_tids)
            or tuple(node.outs[0] for node in pm_nodes) != tuple(post.pm_tids)):
        raise ValueError("K-rank contiguous live writers disagree with ordered relation TIDs")
    ...
    def writer_lines(name, side, pos, node, target_name):
        ...
        return [
            ...
            f"      rw [show {nodes} = {nodes}.take {pos} ++ [{target_name}] ++ {nodes}.drop {pos + 1} by native_decide]",
            f"      rw [foldl_faithful_middle_writer {graph} {store} ({nodes}.take {pos}) ({nodes}.drop {pos + 1})",
            f"        {target_name} {node.outs[0]} (fun t => fw_contiguous (t {node.ins[0]})) (by",
            "          intro t",
            "          rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective",
            "            (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]",
            "          simp [applyNodeDistributed, applyNodeRingAttn]",
...
def render_closed_k_rank_allgather_segment(ir: GoalIR, relation, segment_id: str) -> str:
    """Render one exact ordered K-rank PM AllGather reconstruction writer."""
    chain = relation.dependent_chain_plan
    if chain is None or not chain.complete:
    ...
    if rank_count < 2 or ir.sm_num_ranks != 1 or ir.pm_num_ranks != rank_count:
        raise ValueError("K-rank AllGather graph rank authority disagrees with certificate")
    if (writer.op != "AllGatherPrim" or writer.rank != 0
            or len(writer.outs) != 1 or tuple(writer.params or ()) != (gather_dim,)
    ...
    if (before.kind != "sharded" or before.gather_dim != gather_dim
        ...
        raise ValueError("K-rank AllGather ordered input TIDs disagree with pre relation")
    if (after.kind != "joined" or after.sm_tid != before.sm_tid
            or after.joined_pm_tid != writer.outs[0]):
        raise ValueError("K-rank AllGather output TIDs disagree with joined relation")
    if (tuple(before.full_shape) != tuple(certificate.full_shape)
            or tuple(before.shard_shape) != tuple(certificate.shard_shape)
            or tuple(after.full_shape) != tuple(certificate.full_shape)):
        raise ValueError("K-rank AllGather materialized shapes disagree with certificate")
    reconstructed = list(before.shard_shape)
    if gather_dim < 0 or gather_dim >= len(reconstructed):
        raise ValueError("K-rank AllGather gather dimension is outside shard shape")
    reconstructed[gather_dim] *= rank_count
    if tuple(reconstructed) != tuple(before.full_shape):
        raise ValueError("K-rank AllGather shape contract is not reconstructed")

    states = {item.state_id: item for item in chain.states}
    pre_state, post_state = states[segment.pre_state_id], states[segment.post_state_id]
    if before.fact_id not in pre_state.fact_ids or after.fact_id not in post_state.fact_ids:
        raise ValueError("K-rank AllGather relation facts are not live")
    if not set(post_state.fact_ids) <= ({after.fact_id} | set(pre_state.fact_ids)):
        raise ValueError("K-rank AllGather state introduces an unproved fact")
    ...
    shard_shape = _shape_text(list(before.shard_shape))
    read_proofs = []
    read_names = []
    ...
    lines = [
        ...
        *read_proofs,
        ...
        f"  change ShardedRel ({sm_final} {before.sm_tid}) ({input_text}.map {pm_final})",
        f"    {gather_dim} {full_shape} {shard_shape} at hin",
        f"  have hWriter := {writer_name} pmStore",
        f"  have hJoined : {sm_final} {before.sm_tid} = {pm_final} {writer.outs[0]} :=",
        "    (ShardedRel.to_joined_allGather hin).trans hWriter.symm",
...
def render_closed_k_rank_vocab_embedding_segment(ir: GoalIR, relation, segment_id: str) -> str:
    ...
    if k <= 0 or certificate.rank_count != k or len(post.pm_tids) != k:
        raise ValueError("K-rank vocab embedding rank authority disagrees with facts")
    if certificate.weight_fact != transition.pre_facts[0] or certificate.output_fact != transition.post_facts[0]:
        raise ValueError("K-rank vocab embedding certificate facts disagree with transition")
    if (tuple(pre.full_shape) != tuple(certificate.full_weight_shape)
            or tuple(pre.shard_shape) != tuple(certificate.shard_weight_shape)
            or tuple(post.full_shape) != tuple(certificate.ids_shape) + (certificate.hidden_size,)):
        raise ValueError("K-rank vocab embedding materialized shapes disagree with certificate")
    shard_rows, hidden = certificate.shard_rows, certificate.hidden_size
    if shard_rows <= 0 or hidden <= 0 or tuple(pre.full_shape) != (k * shard_rows, hidden):
    ...
    if certificate.sm_step_id != f"sm:{sm_indices[0]}:0" or tuple(certificate.pm_step_ids) != tuple(
        ...
        raise ValueError("K-rank vocab embedding certificate footprint is not exact")
    ids_tid = certificate.ids_tid
    ...
    eq_facts = [fact for fact in relation.dependent_chain_plan.authority_facts
                if getattr(fact, "kind", None) == "tensor_eq"
                and (fact.left_side, fact.left_tid, fact.right_side, fact.right_tid)
                == ("sm", ids_tid, "pm", ids_tid)]
    shape_facts = [fact for fact in relation.dependent_chain_plan.authority_facts
                   if getattr(fact, "kind", None) == "tensor_shape"
                   and (fact.side, fact.tid, tuple(fact.shape)) == ("pm", ids_tid, tuple(certificate.ids_shape))]
    if len(eq_facts) != 1 or len(shape_facts) != 1:
    ...
    output_tids = "[" + ", ".join(str(tid) for tid in post.pm_tids) + "]"
    full_weight_shape = _shape_text(list(pre.full_shape))
    shard_weight_shape = _shape_text(list(pre.shard_shape))
    ...
    lines = [
        ...
        f"    let smNodes : List NodeDecl := {sm_text}",
        f"    let pmNodes : List NodeDecl := {pm_text}",
        f"    let pmWeightTids : List Tid := {weight_tids}",
        ...
        "      apply RelationState.Holds.fold_frame smNodes pmNodes smStore pmStore hstate",
        "      · native_decide", "      · native_decide", "      · native_decide", "      · native_decide",
        f"    have hWeight : {pre.fact_id}.Holds smFinal pmFinal := hframe {pre.fact_id} (by native_decide)",
        f"    change ShardedRel (smFinal {pre.sm_tid}) (pmWeightTids.map pmFinal) 0 {full_weight_shape} {shard_weight_shape} at hWeight",
        f"    have hIdsEqStore : smStore {ids_tid} = pmStore {ids_tid} := by",
        f"      have h := hstate {ids_eq.fact_id} (by native_decide)",
        f"      change smStore {ids_tid} = pmStore {ids_tid} at h", "      exact h",
        f"    have hIdsShapeStore : (pmStore {ids_tid}).shape = {_shape_text(list(certificate.ids_shape))} := by",
        f"      have h := hstate {ids_shape_fact.fact_id} (by native_decide)",
        f"      change (pmStore {ids_tid}).shape = {_shape_text(list(certificate.ids_shape))} at h", "      exact h",
        f"    have hSmIds : smFinal {ids_tid} = smStore {ids_tid} := by",
        "      unfold smFinal",
        f"      exact foldl_applyNodeDistributedFaithful_at_not_written {ir.sm_graph_ref} smNodes smStore {ids_tid}",
        "        (by native_decide) (by native_decide)",
        f"    have hSmWeight : smFinal {pre.sm_tid} = smStore {pre.sm_tid} := by",
        "      unfold smFinal",
        f"      exact foldl_applyNodeDistributedFaithful_at_not_written {ir.sm_graph_ref} smNodes smStore {pre.sm_tid}",
        "        (by native_decide) (by native_decide)",
        f"    have hPmIds : pmFinal {ids_tid} = pmStore {ids_tid} := by",
        "      unfold pmFinal",
        f"      exact foldl_applyNodeDistributedFaithful_at_not_written {ir.pm_graph_ref} pmNodes pmStore {ids_tid}",
        "        (by native_decide) (by native_decide)",
        f"    have hIdsEq : smFinal {ids_tid} = pmFinal {ids_tid} := by",
        "      rw [hSmIds, hPmIds]", "      exact hIdsEqStore",
    ...
    for rank, node in enumerate(pm_nodes):
        ...
        lines += [
            ...
            "      unfold pmFinal",
            f"      exact foldl_applyNodeDistributedFaithful_at_not_written {ir.pm_graph_ref} pmNodes pmStore {node.ins[1]}",
            "        (by native_decide) (by native_decide)",
            f"    have {writer_name} : pmFinal {node.outs[0]} = fw_embedding_offset {node.params[0]} (pmFinal {ids_tid}) (pmFinal {node.ins[1]}) := by",
            "      calc",
            f"        pmFinal {node.outs[0]} = fw_embedding_offset {node.params[0]} (({before_nodes}).foldl",
            f"            (applyNodeDistributedFaithful {ir.pm_graph_ref}) pmStore {ids_tid}) (({before_nodes}).foldl",
            f"            (applyNodeDistributedFaithful {ir.pm_graph_ref}) pmStore {node.ins[1]}) := by",
            f"          change (pmNodes.foldl (applyNodeDistributedFaithful {ir.pm_graph_ref}) pmStore) {node.outs[0]} = _",
            f"          rw [show pmNodes = {before_nodes} ++ [{_node_text(node)}] ++ {after_nodes} by native_decide]",
            f"          apply foldl_faithful_middle_writer {ir.pm_graph_ref} pmStore {before_nodes} {after_nodes}",
            f"            {_node_text(node)} {node.outs[0]} (fun t => fw_embedding_offset {node.params[0]} (t {ids_tid}) (t {node.ins[1]}))",
            "          · intro t",
            "            rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective",
            "              (hshuffle := by decide) (hunshuffle := by decide) (hattn := by decide)]",
            "            unfold applyNodeDistributed", "            rw [if_neg (by decide), applyNodeRingAttn_eq_applyNode_of_not_ring]",
            ...
            f"          rw [foldl_applyNodeDistributedFaithful_at_not_written {ir.pm_graph_ref} {before_nodes} pmStore {ids_tid} (by native_decide) (by native_decide)]",
            f"          rw [foldl_applyNodeDistributedFaithful_at_not_written {ir.pm_graph_ref} {before_nodes} pmStore {node.ins[1]} (by native_decide) (by native_decide)]",
            f"        _ = fw_embedding_offset {node.params[0]} (pmFinal {ids_tid}) (pmFinal {node.ins[1]}) := by rw [{preserve_name}, hPmIds]",
            f"    have {shape_name} : (pmFinal {node.ins[1]}).shape = {shard_weight_shape} :=",
            f"      hWeight.shard_shapes _ (by simp [pmWeightTids])",
        ]
    lines += [
        f"    have hFullShape : (smFinal {post.sm_tid}).shape = {output_shape} := by",
        ...
        f"      have hBase := {theorem}",
        "        (numParts := rankCount) (shard := " + str(shard_rows) + ") (hidden := " + str(hidden) + ")",
        "        (hparts := by simp [rankCount, pmWeightTids]) (hshard := by omega) (hhid := by omega)",
        f"        (ids := pmFinal {ids_tid}) (Ws := pmWeightTids.map pmFinal)",
    ...
    for rank, shape_name in enumerate(weight_shape_names):
        lines.append(f"          | {rank} => simpa [pmWeightTids, List.getD] using {shape_name}")
    lines += [
        f"          | n + {k} => omega)",
        ...
        "      simp at hBase ⊢",
        "      rw [hBase]", f"      rw [{', '.join('← ' + name for name in writer_names)}]",
        f"    have hout : {post.fact_id}.Holds smFinal pmFinal := by",
        f"      change ReductionRel (smFinal {post.sm_tid}) (pmOutputTids.map pmFinal) {output_shape}",
    ...
    contribution_shape_names = []
    for rank, writer_name in enumerate(writer_names):
        name = f"hContributionShape{rank}"; contribution_shape_names.append(name)
        lines += [
            f"        have {name} : (pmFinal {post.pm_tids[rank]}).shape = {output_shape} := by",
            f"          rw [{writer_name}, fw_embedding_offset_shape, hIdsShape, {weight_shape_names[rank]}]", "          rfl",
        ]
    lines += _membership_cases(contribution_shape_names, indent="        ", binder="contribution")
    lines += [
