...
def render_closed_sum_fw_bw_segment(ir, relation, segment_id: str) -> str:
    try:
        from .composer import _node_text, _shape_text, _render_mixed_final_value, _typed_certificate_digest
    ...
    transitions = tuple(sorted(transitions, key=lambda t: t.rule_id != "sum-producer-sharded-k-rank-dim1"))
    if tuple(item.rule_id for item in transitions) != (
        "sum-producer-sharded-k-rank-dim1",
        "bw-sum-scalar-broadcast-dim1-k-rank",
    ):
    ...
    bw_theorem = "TrainVerify.Denote.bw_sum_allGatherPrimDimN_dim1_rank3"
    ...
    if (k < 1 or sum_cert.rank_count != k or bw_cert.rank_count != k
            ...
            or full_shape_meta != (shard_shape_meta[0], shard_shape_meta[1] * k, shard_shape_meta[2])
            ...
            or bw_post.kind != "sharded" or bw_post.gather_dim != 1
    ...
    if len(sum_transition.sm_node_indices) != 1 or len(bw_transition.sm_node_indices) != 1:
        raise ValueError("sum/BW_sum requires one SM writer per transition")
    ...
    if (len(sum_transition.pre_facts) != 1 or len(bw_transition.pre_facts) != 2
            or len(sum_transition.post_facts) != 1 or len(bw_transition.post_facts) != 1
            or bw_cert.activation_fact != activation.source or bw_cert.gradient_fact != gradient.source
            or sum_cert.gather_dim != 1 or bw_cert.gather_dim != 1
            or sum_cert.full_shape != activation.full_shape or sum_cert.shard_shape != activation.shard_shape
            or (sum_cert.sm_sum_step, *sum_cert.pm_sum_steps) != sum_post.source.step_triple
            or (bw_cert.sm_step_id, *bw_cert.pm_step_ids) != bw_post.source.step_triple
            or any(t.certificate_digest != _typed_certificate_digest(c)
                   for t,c in ((sum_transition,sum_cert),(bw_transition,bw_cert)))):
        raise ValueError("sum/BW_sum exact certificate role/shape/step/digest mismatch")
    if any(n.params for n in (*sm_frame,*pm_frame)) or any(n.rank != 0 for n in sm_frame):
        raise ValueError("sum/BW_sum writer rank/parameters mismatch")
    if (not 0 <= sm_start < sm_end <= len(ir.sm_nodes)
            or not 0 <= pm_start < pm_end <= len(ir.pm_nodes)):
        raise ValueError("sum/BW_sum frame bounds mismatch")
    def source(ref, side, tid, limit, rank):
        nodes = ir.sm_nodes if side == "sm" else ir.pm_nodes
        parts = ref.split(":")
        if ref == f"init:{tid}":
            index = -1
        elif len(parts) == 3 and parts[0] == side and all(x.isdigit() for x in parts[1:]):
            index, projection = map(int,parts[1:])
            if (ref != f"{side}:{index}:{projection}" or not 0 <= index < limit
                    or projection >= len(nodes[index].outs) or nodes[index].outs[projection] != tid
                    or nodes[index].rank != rank):
                raise ValueError("sum/BW_sum source writer/projection/rank mismatch")
        else:
            raise ValueError("sum/BW_sum source axis/reference mismatch")
        if any(tid in n.outs for n in nodes[index+1:limit]):
            raise ValueError("sum/BW_sum source is not latest at its read point")
    for rec in (activation,gradient,sum_post,bw_post):
        f = rec.source
        if (f.layout != rec.kind or f.gather_dim != rec.gather_dim
                or f.source_step_triples or rec.source_tid_triples
                or f.joined_pm_step is not None or rec.joined_pm_tid is not None
                or rec.metadata_tid is not None or rec.metadata_region_id is not None
                or len(f.step_triple) != len(rec.pm_tids)+1):
            raise ValueError("sum/BW_sum source layout/axis/arity mismatch")
        initial = rec.fact_id in before.fact_ids
        source(f.step_triple[0],"sm",rec.sm_tid,sm_start if initial else sm_end,0)
        for rank,(ref,tid) in enumerate(zip(f.step_triple[1:],rec.pm_tids)):
            source(ref,"pm",tid,pm_start if initial else pm_end,rank)
    for t,rec in ((sum_transition,sum_post),(bw_transition,bw_post)):
        expected=(f"sm:{t.sm_node_indices[0]}:0",*(f"pm:{i}:0" for i in t.pm_node_indices))
        if rec.source.step_triple != expected:
            raise ValueError("sum/BW_sum transition/source writer mismatch")
    # The proof frames all pre-state facts, including live public anchors.
    protected = {"sm":set(),"pm":set()}
    known = set()
    for rec in chain.relation_facts:
        if rec.fact_id not in before.fact_ids: continue
        known.add(rec.fact_id)
        protected["sm"].add(rec.sm_tid)
        protected["pm"].update(rec.pm_tids)
        if rec.metadata_tid is not None: protected["sm"].add(rec.metadata_tid)
        if rec.joined_pm_tid is not None: protected["pm"].add(rec.joined_pm_tid)
        for sm,pm0,pm1 in rec.source_tid_triples:
            protected["sm"].add(sm);protected["pm"].update((pm0,pm1))
    for rec in (*chain.authority_facts, *((chain.anchor_fact,) if chain.anchor_fact is not None else ())):
        if rec.fact_id not in before.fact_ids: continue
        known.add(rec.fact_id)
        if rec.kind == "tensor_eq":
            protected[rec.left_side].add(rec.left_tid);protected[rec.right_side].add(rec.right_tid)
        elif rec.kind in ("tensor_shape","packed_cu","label_bound"):
            protected[rec.side].add(rec.tid)
        elif rec.kind == "gather":
            protected["sm"].add(rec.sm_tid);protected["pm"].update((rec.pm_rank0_tid,rec.pm_rank1_tid))
        else: raise ValueError("sum/BW_sum unsupported live authority")
    if known != set(before.fact_ids): raise ValueError("sum/BW_sum unknown live fact")
    for side,frame in (("sm",sm_frame),("pm",pm_frame)):
        outputs=[tid for n in frame for tid in n.outs]
        if len(outputs) != len(set(outputs)) or protected[side].intersection(outputs):
            raise ValueError("sum/BW_sum same-store overwrite of live fact")
    ...
    lines.extend([
        ...
        f"  change ShardedRel (smFinal {activation.sm_tid}) {activation_list} 1 {full} {shard} at hx",
        f"  change ReductionRel (smFinal {gradient.sm_tid}) [pmFinal {gradient.pm_tids[0]}] [1] at hg",
        f"  have hxValue : smFinal {activation.sm_tid} = allGatherPrimDimN 1 {k} 0 {activation_list} := by",
    ...
    lines.extend([
        f"  have hSumComm := {sum_theorem} 1 {k} {activation_list} rfl (by simp) {shard}",
    ...
    lines.extend(["    · rw [← hSumValue]", "      rw [hSumSm]", "      exact fw_sum_shape _",
                  ...
                  f"      (allGatherPrimDimN 1 {k} 0 {activation_list}) =",
                  f"      allGatherPrimDimN 1 {k} 0 {mapped_bw_list} := by",
                  ...
                  f"    change ShardedRel (smFinal {bw_post.sm_tid}) {bw_list} 1 {full} {shard}",
                  "    constructor", f"    · change smFinal {bw_post.sm_tid} = allGatherPrimDimN 1 {k} 0 {bw_list}",
