"""One ordered atomic frame for hidden-sharded and sharded-ID embeddings.

Chunk relations are derived from actual internal writers, never pre-state facts.
"""
from __future__ import annotations


def render_closed_embedding_hidden_ids_segment(ir, relation, segment_id: str) -> str:
    try:
        from .composer import (
            _node_text, _shape_text, _render_mixed_final_value,
            _select_exact_typed_certificate,
        )
        from .relation_compiler import get_closed_rule_spec, KRankHiddenShardedEmbeddingCertificate, KRankShardedIdsEmbeddingCertificate
    except ImportError:
        from composer import (
            _node_text, _shape_text, _render_mixed_final_value,
            _select_exact_typed_certificate,
    ...
    if len(segment.transition_ids) != 2:
        raise ValueError("hidden/IDs embedding requires exactly two transitions")
    transitions = []
    for tid in segment.transition_ids:
        matches = [t for t in relation.transition_specs if t.transition_id == tid]
        if len(matches) != 1:
            raise ValueError("missing or duplicated transition identity")
        transitions.append(matches[0])
    by_rule = {t.rule_id: t for t in transitions}
    if set(by_rule) != {
        "embedding-hidden-sharded-k-rank", "embedding-sharded-ids-k-rank"
    }:
        raise ValueError("hidden/IDs embedding family mismatch")
    hidden_transition = by_rule["embedding-hidden-sharded-k-rank"]
    transition = by_rule["embedding-sharded-ids-k-rank"]
    hidden_theorem = "TrainVerify.Denote.fw_embedding_hidden_shards_k_rank"
    hidden_cert = _select_exact_typed_certificate(
        relation, hidden_transition, "embedding-hidden-sharded-k-rank", hidden_theorem,
        KRankHiddenShardedEmbeddingCertificate,
        lambda c: ((c.weight_fact,), (c.output_fact,)),
    )
    ...
    try:
        hidden_weight = records[hidden_cert.weight_fact]
        hidden_post = records[hidden_cert.output_fact]
    ...
    def source_tid(ref, side):
        ...
        match = re.fullmatch(r"(sm|pm):(0|[1-9][0-9]*):(0|[1-9][0-9]*)", ref)
        if match is None or match[1] != side:
            raise ValueError("embedding source reference is malformed")
        nodes = ir.sm_nodes if side == "sm" else ir.pm_nodes
        index, slot = int(match[2]), int(match[3])
        if index >= len(nodes) or slot >= len(nodes[index].outs):
            raise ValueError("embedding source writer is outside graph")
        return nodes[index].outs[slot]
    for record in (hidden_weight, hidden_post, ids_post, post):
        source = record.source
        refs = source.step_triple
        if (source.layout != record.kind or source.gather_dim != record.gather_dim
                or source.source_step_triples or source.joined_pm_step is not None
                or record.source_tid_triples or record.joined_pm_tid is not None
                or record.metadata_tid is not None or record.metadata_region_id is not None
                or record.row_shard_shape is not None or len(refs) != len(record.pm_tids) + 1):
            raise ValueError("embedding source/record axis metadata mismatch")
        if (source_tid(refs[0], "sm") != record.sm_tid
                or tuple(source_tid(ref, "pm") for ref in refs[1:]) != record.pm_tids):
            raise ValueError("embedding source/record ordered TID mismatch")
    if post.fact_id not in after.fact_ids:
        raise ValueError(f"{rule} post fact is not live")
    fresh_records = tuple(
        item for item in (ids_post, post, hidden_post) if item.fact_id in after.fact_ids
    )
    ...
    if (hidden_cert.rank_count != k or len(hidden_cert.ids_shape) != 2
            or hidden_weight.kind != "sharded" or hidden_weight.gather_dim != 1
            or hidden_post.kind != "sharded" or hidden_post.gather_dim != 2
            or hidden_weight.full_shape != hidden_cert.full_weight_shape
            or hidden_weight.shard_shape != hidden_cert.shard_weight_shape
            or hidden_post.full_shape != hidden_cert.full_output_shape
            or hidden_post.shard_shape != hidden_cert.shard_output_shape
            or len(hidden_weight.pm_tids) != k or len(hidden_post.pm_tids) != k
            or hidden_weight.fact_id not in before.fact_ids):
        raise ValueError("hidden embedding relation/rank authority mismatch")
    hb, ht = hidden_cert.ids_shape
    if len(hidden_cert.shard_weight_shape) != 2:
        raise ValueError("hidden weight must have rank two")
    hv, hd = hidden_cert.shard_weight_shape
    if (min(hb, ht, hv, hd) <= 0
            or hidden_cert.full_weight_shape != (hv, hd*k)
            or hidden_cert.full_output_shape != (hb, ht, hd*k)
            or hidden_cert.shard_output_shape != (hb, ht, hd)):
        raise ValueError("hidden embedding shape outside theorem domain")
    ...
    if not (0 <= segment.sm_range[0] <= segment.sm_range[1] <= len(ir.sm_nodes)
            and 0 <= segment.pm_range[0] <= segment.pm_range[1] <= len(ir.pm_nodes)):
        raise ValueError("atomic embedding frame is outside graph authority")
    ...
    if (len(hidden_transition.sm_node_indices) != 1
            or len(hidden_transition.pm_node_indices) != k
            or not set(hidden_transition.sm_node_indices) <= set(sm_all)
            or not set(hidden_transition.pm_node_indices) <= set(pm_all)
            or hidden_cert.sm_step_id != f"sm:{hidden_transition.sm_node_indices[0]}:0"
            or hidden_cert.pm_step_ids != tuple(f"pm:{i}:0" for i in hidden_transition.pm_node_indices)
            or set(hidden_transition.sm_node_indices) & set(sm_writer_indices)
            or set(hidden_transition.pm_node_indices) & set(pm_writer_indices)):
        raise ValueError("hidden embedding sparse writer partition mismatch")
    hidden_sm = ir.sm_nodes[hidden_transition.sm_node_indices[0]]
    hidden_pm = tuple(ir.pm_nodes[i] for i in hidden_transition.pm_node_indices)
    def plain(node, rank, weight_tid, output_tid):
        return (node.op == "FW_embedding" and node.rank == rank and not node.params
                and tuple(node.ins) == (hidden_cert.ids_tid, weight_tid)
                and tuple(node.outs) == (output_tid,))
    if (not plain(hidden_sm, 0, hidden_weight.sm_tid, hidden_post.sm_tid)
            or any(not plain(n, r, hidden_weight.pm_tids[r], hidden_post.pm_tids[r])
                   for r, n in enumerate(hidden_pm))):
        raise ValueError("hidden embedding writer signature mismatch")
    if (ids_post.sm_tid != certificate.ids_tid
            or tuple(ir.pm_nodes[i].outs[0] for i in chunk_indices) != ids_post.pm_tids
            or any(c >= e for c, e in zip(chunk_indices, embedding_indices))):
        raise ValueError("IDs chunk writer authority/order mismatch")
    ...
    # All old facts, including retained public authority and anchor, must frame.
    all_records = {x.fact_id: x for x in chain.relation_facts}
    all_records.update((x.fact_id, x) for x in chain.authority_facts)
    all_records[chain.anchor_fact.fact_id] = chain.anchor_fact
    protected = {"sm": set(), "pm": set()}
    for fid in before.fact_ids:
        if fid not in all_records:
            raise ValueError("unresolved pre-state fact")
        fact = all_records[fid]
        if fact.kind == "tensor_eq":
            protected[fact.left_side].add(fact.left_tid)
            protected[fact.right_side].add(fact.right_tid)
        elif fact.kind in ("tensor_shape", "packed_cu", "label_bound"):
            protected[fact.side].add(fact.tid)
        elif fact.kind == "gather":
            protected["sm"].add(fact.sm_tid)
            protected["pm"].update((fact.pm_rank0_tid, fact.pm_rank1_tid))
        else:
            protected["sm"].add(fact.sm_tid)
            protected["pm"].update(fact.pm_tids)
            if fact.metadata_tid is not None:
                protected["pm"].add(fact.metadata_tid)
            if fact.joined_pm_tid is not None:
                protected["pm"].add(fact.joined_pm_tid)
            for st, p0, p1 in fact.source_tid_triples:
                protected["sm"].add(st); protected["pm"].update((p0, p1))
    for side, nodes in (("sm", sm_nodes), ("pm", pm_nodes)):
        written = [tid for n in nodes for tid in n.outs]
        if len(written) != len(set(written)) or set(written) & protected[side]:
            raise ValueError("full frame overwrites pre-state authority or repeats writer")
        producers = {tid: pos for pos, n in enumerate(nodes) for tid in n.outs}
        for pos, node in enumerate(nodes):
            if any(tid in producers and producers[tid] >= pos for tid in node.ins):
                raise ValueError("full frame reads an internal writer out of order")
    ...
    hidden_eq = authority("tensor_eq", lambda x:
        (x.left_side, x.left_tid, x.right_side, x.right_tid)
        == ("sm", hidden_cert.ids_tid, "pm", hidden_cert.ids_tid))
    hidden_shape = authority("tensor_shape", lambda x:
        x.side == "pm" and x.tid == hidden_cert.ids_tid and tuple(x.shape) == hidden_cert.ids_shape)
    ...
    lines = ["set_option maxHeartbeats 500000",
    ...
    hidden_sm_helper = add_writer_theorem(
        "hHiddenSm", ir.sm_graph_ref, "smStore", sm_final_name,
        sm_nodes_name, sm_nodes, hidden_transition.sm_node_indices[0]-segment.sm_range[0],
        hidden_sm, tuple(hidden_sm.ins),
        f"fw_embedding ({{store}} {hidden_cert.ids_tid}) ({{store}} {hidden_weight.sm_tid})",
        embedding_apply(ir.sm_graph_ref, hidden_sm))
    hidden_pm_helpers = [add_writer_theorem(
        f"hHiddenPm{r}", ir.pm_graph_ref, "pmStore", pm_final_name,
        pm_nodes_name, pm_nodes, index-segment.pm_range[0], node, tuple(node.ins),
        f"fw_embedding ({{store}} {hidden_cert.ids_tid}) ({{store}} {hidden_weight.pm_tids[r]})",
        embedding_apply(ir.pm_graph_ref, node))
        for r, (index, node) in enumerate(zip(hidden_transition.pm_node_indices, hidden_pm))]
    ...
    weights = "[" + ", ".join(f"pmFinal {t}" for t in hidden_weight.pm_tids) + "]"
    outputs = "[" + ", ".join(f"pmFinal {t}" for t in hidden_post.pm_tids) + "]"
    lines += [
        "set_option maxHeartbeats 500000 in",
        f"private theorem {segment_id}_hidden_out (smStore pmStore : Store)",
        f"    (hw : {hidden_weight.fact_id}.Holds ({sm_final_name} smStore) ({pm_final_name} pmStore))",
        f"    (hidEq : ({sm_final_name} smStore) {hidden_cert.ids_tid} = ({pm_final_name} pmStore) {hidden_cert.ids_tid})",
        f"    (hidShape : (({pm_final_name} pmStore) {hidden_cert.ids_tid}).shape = {shape(hidden_cert.ids_shape)}) :",
        f"    {hidden_post.fact_id}.Holds ({sm_final_name} smStore) ({pm_final_name} pmStore) := by",
        f"  let smFinal := {sm_final_name} smStore",
        f"  let pmFinal := {pm_final_name} pmStore",
        f"  change smFinal {hidden_cert.ids_tid} = pmFinal {hidden_cert.ids_tid} at hidEq",
        f"  change (pmFinal {hidden_cert.ids_tid}).shape = {shape(hidden_cert.ids_shape)} at hidShape",
        f"  change ShardedRel (smFinal {hidden_weight.sm_tid}) {weights} 1 {shape(hidden_cert.full_weight_shape)} {shape(hidden_cert.shard_weight_shape)} at hw",
        f"  have hsm := {hidden_sm_helper} smStore",
        f"  change smFinal {hidden_post.sm_tid} = fw_embedding (smFinal {hidden_cert.ids_tid}) (smFinal {hidden_weight.sm_tid}) at hsm",
    ]
    for r, helper in enumerate(hidden_pm_helpers):
        lines += [f"  have hp{r} := {helper} pmStore",
            f"  change pmFinal {hidden_post.pm_tids[r]} = fw_embedding (pmFinal {hidden_cert.ids_tid}) (pmFinal {hidden_weight.pm_tids[r]}) at hp{r}",
            f"  have hshape{r} : (pmFinal {hidden_post.pm_tids[r]}).shape = {shape(hidden_cert.shard_output_shape)} := by rw [hp{r}, fw_embedding_shape, hidShape, hw.shard_shapes (pmFinal {hidden_weight.pm_tids[r]}) (by simp)]; rfl"]
    lines += [
        f"  have hcomm := {hidden_theorem} (K := {k}) (b := {hb}) (tokens := {ht}) (vocab := {hv}) (hidden := {hd}) (ids := pmFinal {hidden_cert.ids_tid}) (Ws := {weights}) (by omega) (by omega) (by omega) (by omega) (by omega) (by simp) hidShape (by intro W hW; exact hw.shard_shapes W hW)",
        f"  have hvalue : smFinal {hidden_post.sm_tid} = allGatherPrimDimN 2 {k} 0 {outputs} := by rw [hsm, hidEq, hw.full_value]; simp only [List.length_cons, List.length_nil]; rw [hcomm]; simp only [List.map]; rw [{', '.join('←hp'+str(r) for r in range(k))}]",
        f"  change ShardedRel (smFinal {hidden_post.sm_tid}) {outputs} 2 {shape(hidden_cert.full_output_shape)} {shape(hidden_cert.shard_output_shape)}",
        ...
        *(f"    · exact hshape{r}" for r in range(k)), "",
    ]

    fresh_fact_ids = tuple(item.fact_id for item in fresh_records)
    fresh_proofs = tuple(
        "hIdsOut" if item is ids_post else "hHidden" if item is hidden_post else "hout" for item in fresh_records
    )
    ...
    lines += [
        ...
        "      exact hEmbedding",
        f"    have hHiddenEq := hframe {hidden_eq.fact_id} (by native_decide)",
        f"    have hHiddenShape := hframe {hidden_shape.fact_id} (by native_decide)",
        f"    change smFinal {hidden_cert.ids_tid} = pmFinal {hidden_cert.ids_tid} at hHiddenEq",
        f"    change (pmFinal {hidden_cert.ids_tid}).shape = {shape(hidden_cert.ids_shape)} at hHiddenShape",
        f"    have hHidden := {segment_id}_hidden_out smStore pmStore (hframe {hidden_weight.fact_id} (by native_decide)) hHiddenEq hHiddenShape",
        "    intro fact hfact",
