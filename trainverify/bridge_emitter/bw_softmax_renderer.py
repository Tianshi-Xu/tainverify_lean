"""Closed sparse/full-frame renderer for typed BW_softmax sharding."""
from __future__ import annotations


def render_closed_k_rank_bw_softmax_segment(ir, relation, segment_id: str) -> str:
    try:
    ...
    if segment is None or len(segment.transition_ids) != 1:
        raise ValueError("BW_softmax requires one atomic transition")
    transition = {item.transition_id: item for item in relation.transition_specs}.get(
        segment.transition_ids[0]
    )
    if transition is None:
        raise ValueError("BW_softmax transition authority is missing")
    if transition.rule_id not in {"bw-softmax-sharded-dim1-k-rank", "bw-softmax-sharded-dim2-k-rank"}:
        raise ValueError("BW_softmax theorem family is not an orthogonal axis")
    ...
    # Pre-state facts are framed to final stores, never assumed anew.
    live = [item for item in chain.relation_facts if item.fact_id in before.fact_ids]
    for frame,tids in ((sm_frame,{item.sm_tid for item in live}),
                       (pm_frame,{tid for item in live for tid in item.pm_tids})):
        if any(tid in tids for node in frame for tid in node.outs):
            raise ValueError("BW_softmax frame overwrites live input authority")
