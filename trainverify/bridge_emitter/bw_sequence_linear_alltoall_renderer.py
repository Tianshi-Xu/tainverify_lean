"""Atomic C02: shared sequence dX/dW writers plus independent AllToAll.

Uses the general sequence-dual proof skeleton and mixed middle-writer machinery.
Both projections and the collective reference exactly one complete frame per store.
"""
...
def render_closed_bw_sequence_linear_alltoall_segment(ir, relation, segment_id: str) -> str:
    try:
        from .composer import _node_text, _shape_text, _render_mixed_final_value, _select_exact_typed_certificate
        from .relation_compiler import KRankBWLinearDwReductionCertificate, KRankBWLinearDxCertificate, KRankAllToAllRelationCertificate
    except ImportError:
        from composer import _node_text, _shape_text, _render_mixed_final_value, _select_exact_typed_certificate
        from relation_compiler import KRankBWLinearDwReductionCertificate, KRankBWLinearDxCertificate, KRankAllToAllRelationCertificate
    chain = relation.dependent_chain_plan
    seg = next((s for s in chain.segments if s.segment_id == segment_id), None)
    if seg is None or len(seg.transition_ids) != 3:
    ...
    if len({tr.rule_id for tr in ts}) != 3 or {tr.rule_id for tr in ts} != {RULE, DX_RULE, a_rule}:
        raise ValueError("C02 requires exactly shared dX/dW and one AllToAll")
    at = next(tr for tr in ts if tr.rule_id == a_rule)
    ac = _select_exact_typed_certificate(relation,at,a_rule,a_theorem,KRankAllToAllRelationCertificate,
        lambda c:((c.input_fact,),(c.output_fact,)))
    ts = tuple(sorted((tr for tr in ts if tr.rule_id != a_rule),key=lambda tr:tr.rule_id))
    selected = {}
    for t in ts:
        if t.rule_id == RULE:
    ...
    ai,ao = records[ac.input_fact],records[ac.output_fact]
    di,do = ac.input_gather_dim,ac.output_gather_dim
    if (type(di) is not int or type(do) is not int or di==do or di<0 or do<0
            or di>=len(ai.shard_shape) or do>=len(ao.shard_shape)):
        raise ValueError("AllToAll axis authority mismatch")
    for record,axis in ((ai,di),(ao,do)):
        full=list(record.shard_shape);full[axis]*=k
        check_record(record,"sharded",axis,tuple(full),record.shard_shape,k)
    if (ac.rank_count!=k or ai.sm_tid!=ao.sm_tid or ai.full_shape!=ao.full_shape
            or ac.output_fact.step_triple!=(ac.input_fact.step_triple[0],*ac.pm_step_ids)
            or at.sm_node_indices or len(at.pm_node_indices)!=k
            or tuple(sorted(set(at.pm_node_indices)))!=at.pm_node_indices
            or tuple(sorted(set(t.pm_node_indices)))!=t.pm_node_indices
            or set(at.pm_node_indices)&set(t.pm_node_indices)
            or not set(at.pm_node_indices)<=set(range(ps,pe))
            or ac.pm_step_ids!=tuple(f"pm:{p}:0" for p in at.pm_node_indices)):
        raise ValueError("AllToAll ordered authority mismatch")
    anodes=tuple(ir.pm_nodes[p] for p in at.pm_node_indices)
    for rank,n in enumerate(anodes):
        if (n.op!="AllToAllPrim" or n.rank!=rank or n.ins!=list(ai.pm_tids)
                or n.outs!=[ao.pm_tids[rank]] or n.params!=[di,do]):
            raise ValueError("AllToAll writer roles/ranks/axis mismatch")
    states={z.state_id:z for z in chain.states}
    before,after=states[seg.pre_state_id],states[seg.post_state_id]
    fresh={r.fact_id for r in outputs.values()} | {ao.fact_id}
    if (not {g.fact_id,x.fact_id,w.fact_id,ai.fact_id}<=set(before.fact_ids) or not fresh<=set(after.fact_ids)
    ...
    # Actual C02 consumes the previous BW writer's OTHER projection, not local dX/dW.
    check_read_source(ai.source.step_triple[0],"sm",ss)
    for p in at.pm_node_indices:
        for ref in ai.source.step_triple[1:]:check_read_source(ref,"pm",p)
    ...
    for frame,start,owned,live,old,allowed in ((smframe,ss,set(t.sm_node_indices),livesm,oldsm,{r.sm_tid for r in outputs.values()}),
            (pmframe,ps,set(t.pm_node_indices)|set(at.pm_node_indices),livepm,oldpm,{u for r in outputs.values() for u in r.pm_tids}|set(ao.pm_tids))):
    ...
    freshtext=", ".join([*(outputs[q].fact_id for q in selected),ao.fact_id])
    lines.extend([" intro fact hfact",f" have hc : fact ∈ [{freshtext}] ++ {before.state_id}.facts := (show {after.state_id}.facts ⊆ [{freshtext}] ++ {before.state_id}.facts by native_decide) hfact"," simp only [List.mem_append] at hc"," rcases hc with fresh | old"," · simp only [List.mem_cons,List.not_mem_nil,or_false] at fresh","   rcases fresh with "+" | ".join("rfl" for _ in range(len(selected)+1)),*(f"   · exact hout{q}" for q in selected),"   · exact houtA"," · exact hframe fact old","",f"private def {segment_id} : ClosedDepSegmentCertificate {ir.sm_graph_ref} {ir.pm_graph_ref} {before.state_id} {after.state_id} where",f" smNodes := {sn}",f" pmNodes := {pn}",f" sound := by intro a b h; have z := {segment_id}_sound a b h; unfold {sf} {pf} at z; exact z",""])
    return "\n".join(lines)
