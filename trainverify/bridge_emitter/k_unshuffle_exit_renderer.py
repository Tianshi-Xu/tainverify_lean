"""Singleton zigzag-K → ordinary-sharded exit with actual collective writers."""
...
def render_closed_k_unshuffle_exit_segment(ir, relation, segment_id):
    ...
    segment, transition, cert, pre, post, before, after = c._k_rank_segment_context(
        ...
        rc.KRankUnshuffleExitCertificate, lambda x: ((x.input_fact,), (x.output_fact,)))
    proof = compile_proof_plan(ir, build_default_registry())
    try:
        authentic, _, _ = rc.advance_k_rank_unshuffle_exit_frontiers(
            ir, proof, (cert.output_step_triple,), ("sharded",))
    ...
    if (pre.kind != "zigzag_k" or post.gather_dim != 0 or post.kind != "sharded"
            ...
            or pre.metadata_tid != tid):
        raise ValueError("K shuffle fact authority mismatch")
    regions = [r for r in relation.zigzag_regions if r.region_id == pre.metadata_region_id]
    if (len(regions) != 1 or cert.input_step_triple not in regions[0].frontier_triples
    ...
    lines = [
        ...
        *['      · native_decide']*4,
        ...
        f'    change ZigzagKRel (smStore {sm.ins[0]}) {sources} (pmStore {tid}) {fs} {ss} at hSource',
        ...
        f'    have hDecode : decodeCuSeqlens (pmStore {tid}) = [0, {k} * {cert.shard_shape[0]}] := hPackedActual.decoded_single',
        f'    have core := ZigzagKRel.to_sharded_unshuffle_single (d := {cert.shard_shape[0] // 2}) hSource (by decide) hDecode',
    ]
    def writer(name, side, graph, all_nodes, node, actual_buddies):
        ...
        expr = lambda s: f'ZigzagCollective.fw_maybe_unshuffle_collective [{", ".join(f"{s} {t}" for t in reads)}] (decodeCuSeqlens ({s} {tid})) {node.params[0]} {node.params[1]}'
        ...
        if cert.op == 'BW_maybe_shuffle':
            ...
            result += ['              rw [applyNodeDistributedFaithful_unshuffle_out]', '              unfold applyNodeFaithfulUnshuffleValue']
        result += [
    ...
    lines += [
        ...
        f'      change ShardedRel (smFinal {sm.outs[0]}) {outputs} 0 {fs} {ss}',
        '      rw [hSmValue, '+', '.join(f'hPm{r}' for r in range(k))+']',
        '      exact core',
        '    intro fact hfact',
