"""Atomic inverse-axis renderer for BW_linear dX plus paired batch-sharded BW_matmul."""
from __future__ import annotations


def render_closed_bw_linear_matmul_batch_segment(ir,relation,segment_id):
 try:
  ...
  from composer import _node_text,_render_mixed_final_value,_select_exact_typed_certificate
  from relation_compiler import KRankBWLinearDxCertificate,KRankBWMatmulCertificate
 lr="bw-linear-dx-sequence-sharded-rank4";mr="bw-matmul-batch-sharded-rank4";lth="TrainVerify.Denote.bw_linear_dx_dp_split_dim1_4_g169";mths={".1":"TrainVerify.Denote.bw_matmul_fst_split_dim1_4_1_4_8_8",".2":"TrainVerify.Denote.bw_matmul_snd_split_batchdim1_1_4_8_8"}
 ...
 if lt is None or len(mts)!=2 or lt.lean_theorem!=lth: raise ValueError("BW_linear/matmul typed family mismatch")
 ...
 if (lg.full_shape,lx.full_shape,lo.full_shape)!=( (1,8,32),)*3 or (lg.shard_shape,lx.shard_shape,lo.shard_shape)!=((1,2,32),)*3 or lw.full_shape!=(32,32): raise ValueError("BW_linear shapes mismatch")
 ...
 if lc.rank_count!=4 or mf.rank_count!=4 or ms.rank_count!=4: raise ValueError("BW_linear/matmul rank mismatch")
 ...
 if lnS.op!="BW_linear" or tuple(lnS.ins)!=(lg.sm_tid,lx.sm_tid,lw.sm_tid) or lnS.outs[0]!=lo.sm_tid or tuple(n.rank for n in lnP)!=(0,1,2,3) or any(n.op!="BW_linear" or tuple(n.ins)!=(lg.pm_tids[r],lx.pm_tids[r],lw.pm_tids[0]) or n.outs[0]!=lo.pm_tids[r] for r,n in enumerate(lnP)): raise ValueError("BW_linear writers mismatch")
 ...
 def helper(name,graph,initial,fn,nn,frame,pos,node,kind,slot=0):
  ...
  if kind=="linear": expr=f"(bw_linear ({{store}} {node.ins[0]}) ({{store}} {node.ins[1]}) ({{store}} {node.ins[2]})).1";lemma="applyNode_bw_linear_fst_out"
 ...
 hlS=helper("hLinearSm",ir.sm_graph_ref,"smStore",smf,smn,smframe,lt.sm_node_indices[0]-ss,lnS,"linear");hlP=[helper(f"hLinearPm{r}",ir.pm_graph_ref,"pmStore",pmf,pmn,pmframe,i-ps,n,"linear") for r,(i,n) in enumerate(zip(lt.pm_node_indices,lnP))];hmS=[helper(f"hMatmulSm{q}",ir.sm_graph_ref,"smStore",smf,smn,smframe,mts[0].sm_node_indices[0]-ss,mnS,"matmul",q) for q in (0,1)];hmP=[[helper(f"hMatmulPm{q}_{r}",ir.pm_graph_ref,"pmStore",pmf,pmn,pmframe,i-ps,n,"matmul",q) for r,(i,n) in enumerate(zip(mts[0].pm_node_indices,mnP))] for q in (0,1)]
 def lst(r): return "["+", ".join(f"pmFinal {u}" for u in r.pm_tids)+"]"
 lgl,lxl,lol,mgl,mxl,myl,mfl,msl=map(lst,(lg,lx,lo,mg,mx,my,mfo,mso))
 ...
 for r in range(4): lines.extend([f" have hLP{r}:={hlP[r]} pmStore",f" change pmFinal {lo.pm_tids[r]} = (bw_linear (pmFinal {lg.pm_tids[r]}) (pmFinal {lx.pm_tids[r]}) (pmFinal {lw.pm_tids[0]})).1 at hLP{r}"])
 ...
 lines.extend([" intro fact hfact",f" have covered:fact∈[{lo.fact_id},{mfo.fact_id},{mso.fact_id}]++{before.state_id}.facts:=by exact (show {after.state_id}.facts⊆[{lo.fact_id},{mfo.fact_id},{mso.fact_id}]++{before.state_id}.facts by native_decide) hfact"," simp only [List.mem_append] at covered"," rcases covered with fresh|old"," · simp only [List.mem_cons,List.not_mem_nil,or_false] at fresh;rcases fresh with rfl|rfl|rfl","   · exact houtLO","   · exact houtMF","   · exact houtMS"," · exact hframe fact old","","set_option maxRecDepth 32768 in",f"private def {segment_id}:ClosedDepSegmentCertificate {ir.sm_graph_ref} {ir.pm_graph_ref} {before.state_id} {after.state_id} where",f" smNodes:={smn}",f" pmNodes:={pmn}"," sound:=by intro smStore pmStore hstate; have h:= "+f"{segment_id}_sound smStore pmStore hstate; unfold "+smf+" "+pmf+" at h; exact h",""])
 return "\n".join(lines)
