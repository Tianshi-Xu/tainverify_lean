...
def render_closed_k_rank_alltoall_tuple_segment(ir, relation, segment_id):
    ...
    lines = [
        ...
        f"    let rankCount := {tid_list_text(list(fact_pairs[0][0].pm_tids))}.length",
    ...
    for number, transition, certificate, pre, post, nodes in validated:
        ...
        lines.extend([
            ...
            f"    have hRankCountXs{number} : rankCount = {values_name}.length := by simp [{input_name}, {output_name}, {values_name}, rankCount]",
