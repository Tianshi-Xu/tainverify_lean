"""Authenticate original contiguous [G,savedX] -> [dX] logical values.

No storage/stride or cross-rank identity claim; incoming collectives remain opaque.
"""
...
def _fail(reason):
    raise ValueError('bw-contiguous ' + reason)
...
def bind(cells, source_index):
    ...
    if not isinstance(cell.ir, IRBpOperation) or cell.opname.name != 'BW_contiguous':
    ...
    if fw.opname.name != 'FW_contiguous' or fw.ir.signature != 'torch.Tensor.contiguous':
    ...
    for obj, name in ((cell, 'BW.' + fw.ir.name), (fw, fw.ir.name)):
        ...
        if not _same_typed(kwargs, {k: v for k, v in fw.kwargs.items() if k != '__consts'}):
            _fail('original FW/BW empty kwargs mismatch')
    ...
    for ref, ir in zip(cell.inputs, cell._input_irs, strict=True):
        ...
        writers = [j for j, c in enumerate(cells[:source_index])
        ...
        input_writers.append(writers[0] if writers else None)
    ...
    g, x, dx, fx, fy = shapes
    if not g == x == dx == fx == fy:
        _fail('original contiguous gradient/input/output shape mismatch')
