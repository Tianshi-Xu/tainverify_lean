...
def bind(cells, source_index):
    ...
    if not isinstance(cell.ir, IRBpOperation) or cell.opname.name != 'BW_add':
    ...
    if fw.opname.name != 'FW_add' or fw.ir.signature != 'torch.add':
    ...
    if (len(cell.inputs) != 3 or len(cell.outputs) != 2
        ...
        _fail('complete two-tensor ordered ports required')
    ...
    for obj, name in ((cell, 'BW.' + fw.ir.name), (fw, fw.ir.name)):
        ...
        if set(kwargs) - {'alpha', '__consts'} or kwargs.get('__consts', []) != []:
        ...
        if type(alpha) not in (int, float) or alpha != 1:
            _fail('Denote requires source alpha one')
    ...
    # This source slice admits equal-shape residual operands only. Supporting
    # broadcast gradients requires a separate source domain and shape gate.
    ...
    if not shapes[0] or any(type(n) is not int or n <= 0 for n in shapes[0]) or any(sh != shapes[0] for sh in shapes):
        _fail('equal positive residual operand/gradient shapes required')
    return dict(source_index=source_index, fw_source_index=fi,
                ...
                input_roles=['cotangent', 'saved_left', 'saved_right'],
                output_roles=['dleft', 'dright'], alpha=1,
                shape_domain='equal-positive-residual',
