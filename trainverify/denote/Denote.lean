...
Index tensors (e.g. embedding ids) store integer row ids in the uniform
`Scalar = ℝ` payload. `scalarToNat` is the floor map `ℝ → ℕ`; for the integer
values actually stored in index tensors it recovers the exact id. The graph
equivalence proofs use it purely as an opaque decoding function, so no specific
arithmetic property of the floor is required. Defined (rather than axiomatized)
so it does not enlarge the trust base. -/
noncomputable def scalarToNat (x : Scalar) : Nat := ⌊x⌋₊
...
/-- General dimension-parametric version of `transposeAxes 1 2` value-indexing for any
    4D shape `[d0,d1,d2,d3]`. The concrete `transposeAxes_1_2_valAt_*` lemmas below are
    derived as corollaries. The batch term `idx/(d2*d1*d3)*(d1*d2*d3)` accounts for d0>1
    (the old per-shape lemmas all had d0=1 and silently dropped it). -/
theorem transposeAxes_1_2_valAt_gen
    (x : Tensor) (d0 d1 d2 d3 idx : Nat)
    (hx : x.shape = [d0, d1, d2, d3])
    (hd1 : d1 ≠ 0) (hd2 : d2 ≠ 0) (hd3 : d3 ≠ 0)
    (hidx : idx < d0 * d2 * d1 * d3) :
    valAt (transposeAxes 1 2 x) idx =
      valAt x (idx / (d2 * d1 * d3) * (d1 * d2 * d3)
        + idx % (d2 * d1 * d3) % (d1 * d3) / d3 * (d2 * d3)
        + idx % (d2 * d1 * d3) / (d1 * d3) * d3
        + idx % (d2 * d1 * d3) % (d1 * d3) % d3) := by
  have hresult_shape : (transposeAxes 1 2 x).shape = [d0, d2, d1, d3] := by
    simp [transposeAxes, Tensor.mkShape, hx, listSwapAt, List.getD, List.set]
  have hprodeq : prodShape (transposeAxes 1 2 x).shape = d0 * d2 * d1 * d3 := by
    rw [hresult_shape]; simp [prodShape]
  have hprod : idx < prodShape (transposeAxes 1 2 x).shape := by
    rw [hprodeq]; exact hidx
  rw [valAt_of_lt _ _ hprod]
  simp only [transposeAxes, Tensor.mkShape, hx, listSwapAt, List.getD, List.set,
    List.getElem?_cons_zero, List.getElem?_cons_succ, Option.getD_some]
  congr 1
  have e213 : d2 * d1 * d3 ≠ 0 := Nat.mul_ne_zero (Nat.mul_ne_zero hd2 hd1) hd3
  have e13 : d1 * d3 ≠ 0 := Nat.mul_ne_zero hd1 hd3
  simp only [flatToMulti, multiToFlat, prodShape, List.foldl,
    List.getElem?_cons_zero, List.getElem?_cons_succ, Option.getD_some,
    List.set, Nat.div_one, Nat.mul_one, Nat.one_mul,
    e213, e13, hd3, if_false, if_neg, Nat.add_zero, reduceIte,
    Nat.one_ne_zero, Nat.reduceEqDiff]
  ring
...
theorem transposeAxes_1_2_valAt_1_8_4_2 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_1_2_valAt_gen x 1 8 4 2 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_1_2_valAt_1_8_1_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_1_2_valAt_gen x 1 8 1 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_1_2_valAt_1_2_4_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_1_2_valAt_gen x 1 2 4 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_1_2_valAt_1_4_8_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_1_2_valAt_gen x 1 4 8 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_1_2_valAt_1_4_8_2 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_1_2_valAt_gen x 1 4 8 2 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_1_2_valAt_1_1_8_8 (x : Tensor) (idx : Nat)
    (hx : x.shape = [1, 1, 8, 8]) (hidx : idx < 64) :
    valAt (transposeAxes 1 2 x) idx = valAt x idx := by
  rw [transposeAxes_1_2_valAt_gen x 1 1 8 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_1_2_valAt_1_4_2_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_1_2_valAt_gen x 1 4 2 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
set_option maxHeartbeats 3200000 in
/-- General dimension-parametric version of `transposeAxes 2 3` value-indexing for any
    4D shape `[d0,d1,d2,d3]` (swaps last two axes). The concrete
    `transposeAxes_2_3_valAt_*` lemmas below are derived as corollaries. -/
theorem transposeAxes_2_3_valAt_gen
    (x : Tensor) (d0 d1 d2 d3 idx : Nat)
    (hx : x.shape = [d0, d1, d2, d3])
    (hd1 : d1 ≠ 0) (hd2 : d2 ≠ 0) (hd3 : d3 ≠ 0)
    (hidx : idx < d0 * d1 * d3 * d2) :
    valAt (transposeAxes 2 3 x) idx =
      valAt x (idx / (d1 * d3 * d2) * (d1 * d3 * d2)
        + idx % (d1 * d3 * d2) / (d3 * d2) * (d3 * d2)
        + idx % (d1 * d3 * d2) % (d3 * d2) % d2 * d3
        + idx % (d1 * d3 * d2) % (d3 * d2) / d2) := by
  have hresult_shape : (transposeAxes 2 3 x).shape = [d0, d1, d3, d2] := by
    simp [transposeAxes, Tensor.mkShape, hx, listSwapAt, List.getD, List.set]
  have hprodeq : prodShape (transposeAxes 2 3 x).shape = d0 * d1 * d3 * d2 := by
    rw [hresult_shape]; simp [prodShape]
  ...
  simp only [transposeAxes, Tensor.mkShape, hx, listSwapAt, List.getD, List.set,
    List.getElem?_cons_zero, List.getElem?_cons_succ, Option.getD_some]
  congr 1
  have e32 : d3 * d2 ≠ 0 := Nat.mul_ne_zero hd3 hd2
  have e132 : d1 * d3 * d2 ≠ 0 := Nat.mul_ne_zero (Nat.mul_ne_zero hd1 hd3) hd2
  simp only [flatToMulti, multiToFlat, prodShape, List.foldl,
    List.getElem?_cons_zero, List.getElem?_cons_succ, Option.getD_some,
    List.set, Nat.div_one, Nat.mul_one, Nat.one_mul,
    e32, e132, hd1, hd2, hd3, if_false, if_neg, Nat.add_zero, reduceIte,
    Nat.one_ne_zero, Nat.reduceEqDiff]
  ring
...
theorem transposeAxes_2_3_valAt_1_4_8_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_2_3_valAt_gen x 1 4 8 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_2_3_valAt_1_4_8_2 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_2_3_valAt_gen x 1 4 8 2 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_2_3_valAt_1_1_8_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_2_3_valAt_gen x 1 1 8 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
theorem transposeAxes_2_3_valAt_1_4_2_8 (x : Tensor) (idx : Nat)
  ...
  rw [transposeAxes_2_3_valAt_gen x 1 4 2 8 idx hx (by decide) (by decide) (by decide) (by omega)]
  congr 1 <;> omega
...
/-- `applyNode` for `FW_multiref` with first output `t` (any number of remaining outputs `rest`)
    and `params = [n]` with `n > 0`: the first output equals the input. -/
theorem applyNode_fw_multiref_first_out_g261
    (g : GraphDecl) (s : Store) (rank n : Nat) (xTid t : Tid) (rest : List Tid) (hn : 0 < n) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := t :: rest, params := [n] } t = s xTid := by
  unfold applyNode
  rw [show ([xTid] : List Tid).map s = [s xTid] from rfl, evalOp_fw_multiref]
  obtain ⟨m, rfl⟩ : ∃ m, n = m + 1 := ⟨n - 1, by omega⟩
  change storeSet s ((t :: rest).zip (List.replicate (m + 1) (s xTid))) t = _
  rw [List.replicate_succ, List.zip_cons_cons]
  unfold storeSet
  simp [List.find?]
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2]` and `params = [2]`: the second
    output equals the input (requires `t1 ≠ t2`). -/
theorem applyNode_fw_multiref2_second_out_g269
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 : Tid) (hne : t1 ≠ t2) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2], params := [2] } t2 = s xTid := by
  unfold applyNode
  ...
  change storeSet s ([t1, t2].zip (List.replicate 2 (s xTid))) t2 = _
  unfold storeSet
  simp [List.zip, List.zipWith, List.replicate, List.find?, hne, Ne.symm hne]

/-- `applyNode` for `FW_multiref` with `outs = [t1, t2]` and `params = [2]`: the second
    output equals the input. -/
theorem applyNode_fw_multiref2_second_out_g273
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2], params := [2] } t2 = s xTid := by
  unfold applyNode
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2, t3]` and `params = [3]`: the second
    output equals the input. -/
theorem applyNode_fw_multiref3_second_out_g277
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 t3 : Tid) (hne : t1 ≠ t2) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2, t3], params := [3] } t2 = s xTid := by
  unfold applyNode
  rw [show ([xTid] : List Tid).map s = [s xTid] from rfl,
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2]` and `params = [2]`: the second
    output equals the input (when `t1 ≠ t2`). -/
theorem applyNode_fw_multiref2_second_out_g283
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 : Tid) (hne : t1 ≠ t2) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2], params := [2] } t2 = s xTid := by
  unfold applyNode
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2]` and `params = [2]`: the second
    output also equals the input. -/
theorem applyNode_fw_multiref2_second_out_g287
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2], params := [2] } t2 = s xTid := by
  unfold applyNode
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2, t3]` and `params = [3]`: the second
    output equals the input (provided `t1 ≠ t2`). -/
theorem applyNode_fw_multiref3_second_out_g291
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 t3 : Tid) (h : t1 ≠ t2) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2, t3], params := [3] } t2 = s xTid := by
  unfold applyNode
  rw [show ([xTid] : List Tid).map s = [s xTid] from rfl,
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2]` and `params = [2]`: the second
    output equals the input (when `t1 ≠ t2`). -/
theorem applyNode_fw_multiref2_second_out_g297
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 : Tid) (h : t1 ≠ t2) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2], params := [2] } t2 = s xTid := by
  unfold applyNode
  rw [show ([xTid] : List Tid).map s = [s xTid] from rfl,
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2]` and `params = [2]`: the second
    output equals the input. -/
theorem applyNode_fw_multiref2_second_out_g301
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2], params := [2] } t2 = s xTid := by
  unfold applyNode
...
/-- `applyNode` for `FW_multiref` with `outs = [t1, t2, t3]` and `params = [3]`: the first
    output equals the input. -/
theorem applyNode_fw_multiref3_first_out_g303
    (g : GraphDecl) (s : Store) (rank : Nat) (xTid t1 t2 t3 : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_multiref", ins := [xTid],
                    outs := [t1, t2, t3], params := [3] } t1 = s xTid := by
  unfold applyNode
...
/-- Gather-after-chunk identity (dim 2): reassembling the dim-2 chunks (each `[1,8,8]`) of a
    `[1,8,32]`-shaped tensor recovers the original.  This is the inverse direction of
    `chunkPrimDimN_allGatherPrimDimN_dim2_4_1_8_8` and is used for `AllToAllPrim` lineage goals
    where the source passes through `FW_multiref`. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim2_4_1_8_32_g303 (x : Tensor)
    (hsh : x.shape = [1, 8, 32]) :
    allGatherPrimDimN 2 4 0
      [chunkPrimDimN 2 4 0 x, chunkPrimDimN 2 4 1 x,
       chunkPrimDimN 2 4 2 x, chunkPrimDimN 2 4 3 x] = x := by
...
/-! ### FW_softmax distribution lemmas (gap-op batch10, suffix `_g18`) -/

/-- `softmax` preserves shape. -/
theorem softmax_shape_g18 (x : Tensor) : (softmax x).shape = x.shape := by
  unfold softmax
...
theorem evalOp_fw_softmax_g18 (numParts rank : Nat) (params : List Nat) (x : Tensor) :
    evalOp numParts rank "OpName.FW_softmax" params [x] = [fw_softmax x] := by
  rfl

/-- `applyNode` for `FW_softmax` with singleton output. -/
theorem applyNode_fw_softmax_out_g18
    (g : GraphDecl) (s : Store) (rank : Nat) (inTid outTid : Tid) (params : List Nat) :
    applyNode g s { rank := rank, op := "OpName.FW_softmax", ins := [inTid], outs := [outTid], params := params } outTid =
      fw_softmax (s inTid) := by
  unfold applyNode
  rw [show ([inTid] : List Tid).map s = [s inTid] from rfl,
      evalOp_fw_softmax_g18]
  change storeSet s [(outTid, fw_softmax (s inTid))] outTid = _
  unfold storeSet
  simp [List.find?]

/-- `valAt` of `softmax x` at a valid index when the last dimension equals 8.
    Softmax normalises within each contiguous block of 8 elements (the last dim). -/
theorem softmax_valAt_d8_g18 (x : Tensor) (pre : List Nat) (idx : Nat)
    (hrev : x.shape.reverse = 8 :: pre) (hidx : idx < prodShape x.shape) :
    valAt (softmax x) idx =
  ...
  have heq : softmax x = Tensor.mkShape x.shape (fun outIdx =>
      if (∑ j ∈ Finset.range 8, expFn (valAt x (outIdx.1 / 8 * 8 + j))) = 0 then 0
      else expFn (valAt x (outIdx.1 / 8 * 8 + outIdx.1 % 8))
           / ∑ j ∈ Finset.range 8, expFn (valAt x (outIdx.1 / 8 * 8 + j))) := by
  ...
  rw [valAt_of_lt _ _ (show idx < prodShape
      (Tensor.mkShape x.shape (fun outIdx =>
        if (∑ j ∈ Finset.range 8, expFn (valAt x (outIdx.1 / 8 * 8 + j))) = 0 then 0
             ...
             / ∑ j ∈ Finset.range 8, expFn (valAt x (outIdx.1 / 8 * 8 + j)))).shape from hidx)]
  rfl

/- `softmax` distributes over `allGatherPrimDimN` on dim 2 for shard shape `[1,4,2,8]`.
    Splitting on dim 2 keeps each softmax row (last dim, size 8) within a single shard,
    so softmax of the gathered tensor equals the gather of per-shard softmaxes. -/
-- maxHeartbeats raised: large constant index arithmetic over a 256-element tensor.
set_option maxHeartbeats 3200000 in
theorem softmax_allGather2_distrib_1_4_2_8_g18 (c0 c1 c2 c3 : Tensor)
    (h0 : c0.shape = [1, 4, 2, 8]) (h1 : c1.shape = [1, 4, 2, 8])
    (h2 : c2.shape = [1, 4, 2, 8]) (h3 : c3.shape = [1, 4, 2, 8]) :
    softmax (allGatherPrimDimN 2 4 0 [c0, c1, c2, c3]) =
      allGatherPrimDimN 2 4 0 [softmax c0, softmax c1, softmax c2, softmax c3] := by
  have hhead : (([c0, c1, c2, c3] : List Tensor).head?.map (fun t => t.shape)).getD [] = [1, 4, 2, 8] := by
    simp [h0]
  have hG_shape : (allGatherPrimDimN 2 4 0 [c0, c1, c2, c3]).shape = [1, 4, 8, 8] := by
    rw [allGatherPrimDimN_shape 2 4 _ [1, 4, 2, 8] hhead]; simp [List.set, List.getD]
  have hsm_shape : ∀ c : Tensor, c.shape = [1, 4, 2, 8] → (softmax c).shape = [1, 4, 2, 8] := by
    intro c hc; rw [softmax_shape_g18, hc]
  have hhead_sm : (([softmax c0, softmax c1, softmax c2, softmax c3] : List Tensor).head?.map
      (fun t => t.shape)).getD [] = [1, 4, 2, 8] := by
    simp [hsm_shape c0 h0]
  have hRHS_shape : (allGatherPrimDimN 2 4 0 [softmax c0, softmax c1, softmax c2, softmax c3]).shape = [1, 4, 8, 8] := by
    rw [allGatherPrimDimN_shape 2 4 _ [1, 4, 2, 8] hhead_sm]; simp [List.set, List.getD]
  apply Tensor.ext
  · rw [softmax_shape_g18, hG_shape, hRHS_shape]
  · intro idx hidx
    have hidx256 : idx < 256 := by
      rw [softmax_shape_g18, hG_shape] at hidx; simpa [prodShape] using hidx
    have hGrev : (allGatherPrimDimN 2 4 0 [c0, c1, c2, c3]).shape.reverse = 8 :: [8, 4, 1] := by
      rw [hG_shape]; rfl
    have hGprod : idx < prodShape (allGatherPrimDimN 2 4 0 [c0, c1, c2, c3]).shape := by
      rw [hG_shape]; simpa [prodShape] using hidx256
    rw [softmax_valAt_d8_g18 _ [8, 4, 1] idx hGrev hGprod]
    rw [allGatherPrimDimN_2_4_valAt_1_4_2_8 _ idx hhead_sm hidx256]
    set r := (idx % 64) / 16 with hr_def
    set loc := (idx / 64) * 16 + ((idx % 16) / 8) * 8 + idx % 8 with hloc_def
    have hgetD_sm : [softmax c0, softmax c1, softmax c2, softmax c3].getD r (zeroTensor [1, 4, 2, 8]) =
        softmax ([c0, c1, c2, c3].getD r (zeroTensor [1, 4, 2, 8])) := by
      rcases (by omega : r = 0 ∨ r = 1 ∨ r = 2 ∨ r = 3) with h | h | h | h <;> rw [h] <;> simp [List.getD]
    rw [hgetD_sm]
    set cr := [c0, c1, c2, c3].getD r (zeroTensor [1, 4, 2, 8]) with hcr_def
    have hcr_shape : cr.shape = [1, 4, 2, 8] := by
      rw [hcr_def]
      rcases (by omega : r = 0 ∨ r = 1 ∨ r = 2 ∨ r = 3) with h | h | h | h <;> rw [h] <;>
        simp [List.getD, h0, h1, h2, h3]
    have hcr_rev : cr.shape.reverse = 8 :: [2, 4, 1] := by rw [hcr_shape]; rfl
    have hloc64 : loc < 64 := by rw [hloc_def]; omega
    have hcr_prod : loc < prodShape cr.shape := by rw [hcr_shape]; simpa [prodShape] using hloc64
    rw [softmax_valAt_d8_g18 _ [2, 4, 1] loc hcr_rev hcr_prod]
    have hden : (∑ j ∈ Finset.range 8, expFn (valAt (allGatherPrimDimN 2 4 0 [c0, c1, c2, c3]) (idx / 8 * 8 + j)))
              = (∑ j ∈ Finset.range 8, expFn (valAt cr (loc / 8 * 8 + j))) := by
      apply Finset.sum_congr rfl
      intro j hj
      have hj8 : j < 8 := Finset.mem_range.mp hj
      have hk : idx / 8 * 8 + j < 256 := by omega
      rw [allGatherPrimDimN_2_4_valAt_1_4_2_8 _ (idx / 8 * 8 + j) hhead hk]
      have heq_r : ((idx / 8 * 8 + j) % 64) / 16 = r := by rw [hr_def]; omega
      have heq_loc : (idx / 8 * 8 + j) / 64 * 16 + ((idx / 8 * 8 + j) % 16) / 8 * 8 + (idx / 8 * 8 + j) % 8
          = loc / 8 * 8 + j := by rw [hloc_def]; omega
      rw [heq_r, heq_loc, ← hcr_def]
    have hnum : valAt (allGatherPrimDimN 2 4 0 [c0, c1, c2, c3]) (idx / 8 * 8 + idx % 8)
              = valAt cr (loc / 8 * 8 + loc % 8) := by
      have hk : idx / 8 * 8 + idx % 8 < 256 := by omega
      rw [allGatherPrimDimN_2_4_valAt_1_4_2_8 _ (idx / 8 * 8 + idx % 8) hhead hk]
      have heq_r : ((idx / 8 * 8 + idx % 8) % 64) / 16 = r := by rw [hr_def]; omega
      have heq_loc : (idx / 8 * 8 + idx % 8) / 64 * 16 + ((idx / 8 * 8 + idx % 8) % 16) / 8 * 8 + (idx / 8 * 8 + idx % 8) % 8
          = loc / 8 * 8 + loc % 8 := by rw [hloc_def]; omega
      rw [heq_r, heq_loc, ← hcr_def]
    rw [hden, hnum]
...
/-! ## BW_softmax (softmaxBwd) distribution lemmas (goal_199 family) -/

/-- `evalOp` unfolding for `BW_softmax`. -/
theorem evalOp_bw_softmax_g199 (numParts rank : Nat) (params : List Nat) (g y : Tensor) :
    evalOp numParts rank "OpName.BW_softmax" params [g, y] = [bw_softmax g y] := rfl

/-- `applyNode` for `BW_softmax` with two inputs and singleton output. -/
theorem applyNode_bw_softmax_out_g199
    (gr : GraphDecl) (s : Store) (rank : Nat) (gTid yTid outTid : Tid) :
    applyNode gr s { rank := rank, op := "OpName.BW_softmax", ins := [gTid, yTid], outs := [outTid] } outTid =
      bw_softmax (s gTid) (s yTid) := by
  unfold applyNode
  rw [show ([gTid, yTid] : List Tid).map s = [s gTid, s yTid] from rfl,
      evalOp_bw_softmax_g199]
  change storeSet s [(outTid, bw_softmax (s gTid) (s yTid))] outTid = _
  unfold storeSet
  simp [List.find?]
...
/-- `bw_softmax g y` as an explicit `mkShape` when the softmax dimension is 8. -/
theorem bw_softmax_unfold8_g199 (g y : Tensor) (pre : Shape) (hy : y.shape = pre ++ [8]) :
    bw_softmax g y = Tensor.mkShape y.shape (fun outIdx =>
      valAt y (outIdx.1 / 8 * 8 + outIdx.1 % 8) *
        (valAt g (outIdx.1 / 8 * 8 + outIdx.1 % 8)
          - ∑ j ∈ Finset.range 8,
              valAt y (outIdx.1 / 8 * 8 + j) * valAt g (outIdx.1 / 8 * 8 + j))) := by
  have hrev : y.shape.reverse = 8 :: pre.reverse := by
    rw [hy, List.reverse_append]; simp
  unfold bw_softmax softmaxBwd
  rw [hrev]
  simp only [show ((8 : Nat) = 0) = False from by simp, if_false]

/-- `valAt` of `bw_softmax g y` when the softmax dimension is 8. -/
theorem bw_softmax_valAt8_g199 (g y : Tensor) (pre : Shape) (idx : Nat)
    (hy : y.shape = pre ++ [8]) (hidx : idx < prodShape y.shape) :
    valAt (bw_softmax g y) idx =
      valAt y idx *
        (valAt g idx -
          ∑ j ∈ Finset.range 8,
            valAt y (idx / 8 * 8 + j) * valAt g (idx / 8 * 8 + j)) := by
  rw [bw_softmax_unfold8_g199 g y pre hy]
  rw [valAt_of_lt _ _ (by simp only [Tensor.mkShape]; exact hidx)]
  simp only [Tensor.mkShape]
  rw [show idx / 8 * 8 + idx % 8 = idx from by omega]

/-- Per-shard equality used by the BW_softmax dim-2 split: the local `bw_softmax`
    of the rank-`r` chunk equals the global `bw_softmax` restricted to the gathered index. -/
theorem bw_softmax_piece_eq_g199 (g y : Tensor) (r idx : Nat)
    (hg : g.shape = [1, 4, 8, 8]) (hy : y.shape = [1, 4, 8, 8])
    (hr : r < 4) (hidx : idx < 256) (hrsel : idx % 64 / 16 = r) :
    valAt (bw_softmax (chunkPrimDimN 2 4 r g) (chunkPrimDimN 2 4 r y))
        (idx / 64 * 16 + idx % 16 / 8 * 8 + idx % 8)
      = valAt (bw_softmax g y) idx := by
  have hcy : (chunkPrimDimN 2 4 r y).shape = [1, 4, 2, 8] := by
    rw [chunkPrimDimN_shape 2 4 r _ _ hy (by omega)]; simp [List.set, List.getD]
  have hcg : (chunkPrimDimN 2 4 r g).shape = [1, 4, 2, 8] := by
    rw [chunkPrimDimN_shape 2 4 r _ _ hg (by omega)]; simp [List.set, List.getD]
  set L := idx / 64 * 16 + idx % 16 / 8 * 8 + idx % 8 with hL
  have hL64 : L < 64 := by rw [hL]; omega
  have hLprod : L < prodShape (chunkPrimDimN 2 4 r y).shape := by
    rw [hcy]; simpa [prodShape] using hL64
  have hidxprod : idx < prodShape y.shape := by rw [hy]; simpa [prodShape] using hidx
  rw [bw_softmax_valAt8_g199 (chunkPrimDimN 2 4 r g) (chunkPrimDimN 2 4 r y) [1, 4, 2] L hcy hLprod,
      bw_softmax_valAt8_g199 g y [1, 4, 8] idx hy hidxprod]
  have hdot :
      (∑ j ∈ Finset.range 8,
          valAt (chunkPrimDimN 2 4 r y) (L / 8 * 8 + j) *
            valAt (chunkPrimDimN 2 4 r g) (L / 8 * 8 + j))
        = ∑ j ∈ Finset.range 8, valAt y (idx / 8 * 8 + j) * valAt g (idx / 8 * 8 + j) := by
    apply Finset.sum_congr rfl
    intro j hj
    have hj8 : j < 8 := Finset.mem_range.mp hj
    have hidxj : L / 8 * 8 + j < 64 := by rw [hL]; omega
    rw [chunkPrimDimN_2_4_valAt_1_4_8_8 y r (L / 8 * 8 + j) hy hr hidxj,
        chunkPrimDimN_2_4_valAt_1_4_8_8 g r (L / 8 * 8 + j) hg hr hidxj]
    have e1 : (L / 8 * 8 + j) / 16 * 64 + r * 16 + (L / 8 * 8 + j) % 16 = idx / 8 * 8 + j := by
      rw [hL]; omega
    rw [e1]
  rw [chunkPrimDimN_2_4_valAt_1_4_8_8 y r L hy hr hL64,
      chunkPrimDimN_2_4_valAt_1_4_8_8 g r L hg hr hL64]
  have e0 : L / 16 * 64 + r * 16 + L % 16 = idx := by rw [hL]; omega
  rw [e0, hdot]

/-- `bw_softmax` distributes over a dim-2 (sequence) split for `[1,4,8,8]` tensors:
    the softmax axis (last dim) is untouched, so gathering the per-shard backward
    results along dim 2 reconstructs the full backward result. -/
theorem bw_softmax_split_dim2_4_1_4_8_8_g199 (g y : Tensor)
    (hg : g.shape = [1, 4, 8, 8]) (hy : y.shape = [1, 4, 8, 8]) :
    bw_softmax g y = allGatherPrimDimN 2 4 0
      [bw_softmax (chunkPrimDimN 2 4 0 g) (chunkPrimDimN 2 4 0 y),
       bw_softmax (chunkPrimDimN 2 4 1 g) (chunkPrimDimN 2 4 1 y),
       bw_softmax (chunkPrimDimN 2 4 2 g) (chunkPrimDimN 2 4 2 y),
       bw_softmax (chunkPrimDimN 2 4 3 g) (chunkPrimDimN 2 4 3 y)] := by
  have hcy : ∀ r, r < 4 → (chunkPrimDimN 2 4 r y).shape = [1, 4, 2, 8] := by
    intro r hr; rw [chunkPrimDimN_shape 2 4 r _ _ hy (by omega)]; simp [List.set, List.getD]
  have hpiece : ∀ r, r < 4 →
      (bw_softmax (chunkPrimDimN 2 4 r g) (chunkPrimDimN 2 4 r y)).shape = [1, 4, 2, 8] := by
    intro r hr
    exact bw_softmax_shape_g199 _ _ [1, 4, 2] 8 (hcy r hr)
  have hhead : (([bw_softmax (chunkPrimDimN 2 4 0 g) (chunkPrimDimN 2 4 0 y),
       bw_softmax (chunkPrimDimN 2 4 1 g) (chunkPrimDimN 2 4 1 y),
       bw_softmax (chunkPrimDimN 2 4 2 g) (chunkPrimDimN 2 4 2 y),
       bw_softmax (chunkPrimDimN 2 4 3 g) (chunkPrimDimN 2 4 3 y)].head?.map
        (fun t => t.shape)).getD []) = [1, 4, 2, 8] := by
    simp [hpiece 0 (by omega)]
  have hlhs : (bw_softmax g y).shape = [1, 4, 8, 8] :=
    bw_softmax_shape_g199 g y [1, 4, 8] 8 hy
  have hrhs : (allGatherPrimDimN 2 4 0
      [bw_softmax (chunkPrimDimN 2 4 0 g) (chunkPrimDimN 2 4 0 y),
       bw_softmax (chunkPrimDimN 2 4 1 g) (chunkPrimDimN 2 4 1 y),
       bw_softmax (chunkPrimDimN 2 4 2 g) (chunkPrimDimN 2 4 2 y),
       bw_softmax (chunkPrimDimN 2 4 3 g) (chunkPrimDimN 2 4 3 y)]).shape = [1, 4, 8, 8] := by
    rw [allGatherPrimDimN_shape 2 4 _ _ hhead]; simp [List.set, List.getD]
  apply Tensor.ext (by rw [hlhs, hrhs])
  intro idx hidx
  have hidx256 : idx < 256 := by simpa [hlhs, prodShape] using hidx
  rw [allGatherPrimDimN_2_4_valAt_1_4_2_8 _ idx hhead hidx256]
  have hr4 : idx % 64 / 16 = 0 ∨ idx % 64 / 16 = 1 ∨ idx % 64 / 16 = 2 ∨ idx % 64 / 16 = 3 := by
    omega
  rcases hr4 with h | h | h | h
  · rw [h]; simp only [List.getD_cons_zero]
    exact (bw_softmax_piece_eq_g199 g y 0 idx hg hy (by omega) hidx256 h).symm
  · rw [h]; simp only [List.getD_cons_succ, List.getD_cons_zero]
    exact (bw_softmax_piece_eq_g199 g y 1 idx hg hy (by omega) hidx256 h).symm
  · rw [h]; simp only [List.getD_cons_succ, List.getD_cons_zero]
    exact (bw_softmax_piece_eq_g199 g y 2 idx hg hy (by omega) hidx256 h).symm
  · rw [h]; simp only [List.getD_cons_succ, List.getD_cons_zero]
    exact (bw_softmax_piece_eq_g199 g y 3 idx hg hy (by omega) hidx256 h).symm

/-- Unfolding lemma for `evalOp` on `FW_div`. -/
theorem evalOp_fw_div_g17 (numParts rank : Nat) (params : List Nat) (x : Tensor) :
    evalOp numParts rank "OpName.FW_div" params [x] =
      [fw_div ((params.head?.getD 1 : Nat) : Scalar) x] := by
  rfl

/-- `applyNode` for `FW_div` with singleton output. -/
theorem applyNode_fw_div_out_g17
    (g : GraphDecl) (s : Store) (rank : Nat) (params : List Nat) (inTid outTid : Tid) :
    applyNode g s
      { rank := rank, op := "OpName.FW_div", ins := [inTid], outs := [outTid], params := params }
        outTid =
...
/-- `fw_div` preserves shape. -/
theorem fw_div_shape_g17 (c : Scalar) (x : Tensor) : (fw_div c x).shape = x.shape := by
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `valAt` of `fw_div` at a valid index. -/
theorem fw_div_valAt_g17 (c : Scalar) (x : Tensor) (idx : Nat) (h : idx < prodShape x.shape) :
    valAt (fw_div c x) idx = valAt x idx / c := by
  rw [valAt_of_lt _ _ (by rw [fw_div_shape_g17]; exact h)]
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `fw_div` distributes over `allGatherPrimDimN`: applying `fw_div` to the
    gathered tensor equals gathering the per-shard `fw_div` results.
    This holds because `fw_div` is a pointwise (elementwise) operation. -/
theorem fw_div_allGatherPrimDimN_eq_g17
    (c : Scalar) (gatherDim numParts : Nat) (xs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen : xs.length = numParts)
    (hhead : (xs.head?.map (fun t => t.shape)).getD [] = shardShape)
    (hxs_shape : ∀ i (hi : i < xs.length), (xs.get ⟨i, hi⟩).shape = shardShape) :
    fw_div c (allGatherPrimDimN gatherDim numParts 0 xs) =
      allGatherPrimDimN gatherDim numParts 0 (xs.map (fw_div c)) := by
  have hlen_map : (xs.map (fw_div c)).length = numParts := by
    rw [List.length_map, hlen]
  have hhead_map : ((xs.map (fw_div c)).head?.map (fun t => t.shape)).getD [] = shardShape := by
    have h0 : (0 : Nat) < xs.length := by omega
    have h0m : (0 : Nat) < (xs.map (fw_div c)).length := by rw [List.length_map]; omega
    rw [List.head?_eq_getElem?]
    rw [List.getElem?_eq_getElem h0m]
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (xs.map (fw_div c))[0]'h0m = fw_div c (xs[0]'h0) := List.getElem_map ..
    rw [hm0, fw_div_shape_g17]
    exact hxs_shape 0 h0
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_shape : (allGatherPrimDimN gatherDim numParts 0 xs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts xs shardShape hhead
  have hlhs_shape : (fw_div c (allGatherPrimDimN gatherDim numParts 0 xs)).shape = outShape := by
    rw [fw_div_shape_g17]; exact hgather_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0 (xs.map (fw_div c))).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_map
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  have hidx_out : idx < prodShape outShape := by rwa [hlhs_shape] at hidx
  have hgather_idx : idx < prodShape (allGatherPrimDimN gatherDim numParts 0 xs).shape := by
    rw [hgather_shape]; exact hidx_out
  rw [fw_div_valAt_g17 _ _ _ hgather_idx]
  rw [valAt_of_lt _ _ hgather_idx]
  conv_rhs => rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead, hhead_map]
  set ds := List.getD shardShape gatherDim 0
  ...
  by_cases hr_lt : r < numParts
  · have hr_xs : r < xs.length := by omega
    have hr_map : r < (xs.map (fw_div c)).length := by omega
    rw [list_getD_of_lt _ _ _ hr_xs, list_getD_of_lt _ _ _ hr_map]
    have hmap_r : (xs.map (fw_div c))[r]'hr_map = fw_div c (xs[r]'hr_xs) := List.getElem_map ..
    rw [hmap_r]
    by_cases hloc_lt : loc < prodShape (xs[r]'hr_xs).shape
    · rw [fw_div_valAt_g17 _ _ _ hloc_lt]
    · have h1 : ¬(loc < prodShape (fw_div c (xs[r]'hr_xs)).shape) := by
        rw [fw_div_shape_g17]; exact hloc_lt
      simp only [valAt, h1, hloc_lt, dite_false]
      simp
  · have hr_xs : ¬(r < xs.length) := by omega
    have hr_map : ¬(r < (xs.map (fw_div c)).length) := by omega
...
/-- Unfolding lemma for `evalOp` on `FW_div` with explicit scalar param. -/
theorem evalOp_fw_div_g42 (numParts rank c : Nat) (x : Tensor) :
    evalOp numParts rank "OpName.FW_div" [c] [x] = [fw_div ((c : Nat) : Scalar) x] := by
  rfl

/-- `applyNode` for `FW_div` with singleton output and explicit scalar param. -/
theorem applyNode_fw_div_out_g42
    (g : GraphDecl) (s : Store) (rank c : Nat) (inTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_div", ins := [inTid], outs := [outTid], params := [c] } outTid =
      fw_div ((c : Nat) : Scalar) (s inTid) := by
  unfold applyNode
  rw [show ([inTid] : List Tid).map s = [s inTid] from rfl,
      evalOp_fw_div_g42]
  change storeSet s [(outTid, fw_div ((c : Nat) : Scalar) (s inTid))] outTid = _
  unfold storeSet
  simp [List.find?]

/-- `fw_div` preserves shape. -/
theorem fw_div_shape_g42 (c : Scalar) (x : Tensor) : (fw_div c x).shape = x.shape := by
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `valAt` of `fw_div` at a valid index. -/
theorem fw_div_valAt_g42 (c : Scalar) (x : Tensor) (idx : Nat) (h : idx < prodShape x.shape) :
    valAt (fw_div c x) idx = valAt x idx / c := by
  rw [valAt_of_lt _ _ (by rw [fw_div_shape_g42]; exact h)]
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `fw_div` distributes over `allGatherPrimDimN`: applying `fw_div` to the
    gathered tensor equals gathering the per-shard `fw_div` results.
    This holds because `fw_div` is a pointwise (elementwise) operation. -/
theorem fw_div_allGatherPrimDimN_eq_g42
    (c : Scalar) (gatherDim numParts : Nat) (xs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen : xs.length = numParts)
  ...
  have hhead_map : ((xs.map (fw_div c)).head?.map (fun t => t.shape)).getD [] = shardShape := by
    ...
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (xs.map (fw_div c))[0]'h0m = fw_div c (xs[0]'h0) := List.getElem_map ..
    rw [hm0, fw_div_shape_g42]
    exact hxs_shape 0 h0
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_shape : (allGatherPrimDimN gatherDim numParts 0 xs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts xs shardShape hhead
  have hlhs_shape : (fw_div c (allGatherPrimDimN gatherDim numParts 0 xs)).shape = outShape := by
    rw [fw_div_shape_g42]; exact hgather_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0 (xs.map (fw_div c))).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_map
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  ...
  have hgather_idx : idx < prodShape (allGatherPrimDimN gatherDim numParts 0 xs).shape := by
    rw [hgather_shape]; exact hidx_out
  rw [fw_div_valAt_g42 _ _ _ hgather_idx]
  rw [valAt_of_lt _ _ hgather_idx]
  conv_rhs => rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead, hhead_map]
  set ds := List.getD shardShape gatherDim 0
  ...
  · have hr_xs : r < xs.length := by omega
    ...
    rw [hmap_r]
    by_cases hloc_lt : loc < prodShape (xs[r]'hr_xs).shape
    · rw [fw_div_valAt_g42 _ _ _ hloc_lt]
    · have h1 : ¬(loc < prodShape (fw_div c (xs[r]'hr_xs)).shape) := by
        rw [fw_div_shape_g42]; exact hloc_lt
      simp only [valAt, h1, hloc_lt, dite_false]
      simp
  · have hr_xs : ¬(r < xs.length) := by omega
    have hr_map : ¬(r < (xs.map (fw_div c)).length) := by omega
...
/-! ## FW_div lemmas (elementwise scalar division) -/

/-- Unfolding lemma for `evalOp` on `FW_div` with `params := [c]`. -/
theorem evalOp_fw_div_g67 (numParts rank c : Nat) (x : Tensor) :
    evalOp numParts rank "OpName.FW_div" [c] [x] = [fw_div ((c : Nat) : Scalar) x] := by
  rfl

/-- `fw_div` preserves shape. -/
theorem fw_div_shape_g67 (c : Scalar) (x : Tensor) : (fw_div c x).shape = x.shape := by
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `valAt` of `fw_div` at a valid index. -/
theorem fw_div_valAt_g67 (c : Scalar) (x : Tensor) (idx : Nat) (h : idx < prodShape x.shape) :
    valAt (fw_div c x) idx = (valAt x idx) / c := by
  rw [valAt_of_lt _ _ (by rw [fw_div_shape_g67]; exact h)]
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `applyNode` for `FW_div` with singleton output and `params := [c]`. -/
theorem applyNode_fw_div_out_g67
    (g : GraphDecl) (s : Store) (rank c : Nat) (inTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_div", ins := [inTid], outs := [outTid], params := [c] } outTid =
      fw_div ((c : Nat) : Scalar) (s inTid) := by
  unfold applyNode
...
/-- `fw_div` distributes over `allGatherPrimDimN`: applying `fw_div c` to the
    gathered tensor equals gathering the per-shard `fw_div c` results.
    This holds because `fw_div` is a pointwise (elementwise) operation. -/
theorem fw_div_allGatherPrimDimN_eq_g67
    (c : Scalar) (gatherDim numParts : Nat) (xs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen : xs.length = numParts)
  ...
  have hhead_map : ((xs.map (fw_div c)).head?.map (fun t => t.shape)).getD [] = shardShape := by
    ...
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (xs.map (fw_div c))[0]'h0m = fw_div c (xs[0]'h0) := List.getElem_map ..
    rw [hm0, fw_div_shape_g67]
    exact hxs_shape 0 h0
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_shape : (allGatherPrimDimN gatherDim numParts 0 xs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts xs shardShape hhead
  have hlhs_shape : (fw_div c (allGatherPrimDimN gatherDim numParts 0 xs)).shape = outShape := by
    rw [fw_div_shape_g67]; exact hgather_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0 (xs.map (fw_div c))).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_map
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  ...
  have hgather_idx : idx < prodShape (allGatherPrimDimN gatherDim numParts 0 xs).shape := by
    rw [hgather_shape]; exact hidx_out
  rw [fw_div_valAt_g67 _ _ _ hgather_idx]
  rw [valAt_of_lt _ _ hgather_idx]
  conv_rhs => rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead, hhead_map]
  set ds := List.getD shardShape gatherDim 0
  ...
  · have hr_xs : r < xs.length := by omega
    ...
    rw [hmap_r]
    by_cases hloc_lt : loc < prodShape (xs[r]'hr_xs).shape
    · rw [fw_div_valAt_g67 _ _ _ hloc_lt]
    · have h1 : ¬(loc < prodShape (fw_div c (xs[r]'hr_xs)).shape) := by
        rw [fw_div_shape_g67]; exact hloc_lt
      simp only [valAt, h1, hloc_lt, dite_false]
      simp
  · have hr_xs : ¬(r < xs.length) := by omega
    have hr_map : ¬(r < (xs.map (fw_div c)).length) := by omega
...
/-- Unfolding lemma for `evalOp` on `FW_div` with an explicit scalar parameter. -/
theorem evalOp_fw_div_g92 (numParts rank c : Nat) (x : Tensor) :
    evalOp numParts rank "OpName.FW_div" [c] [x] = [fw_div (c : Scalar) x] := rfl

/-- `applyNode` for `FW_div` with singleton output and explicit scalar parameter. -/
theorem applyNode_fw_div_out_g92
    (g : GraphDecl) (s : Store) (rank c : Nat) (inTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_div", ins := [inTid], outs := [outTid], params := [c] } outTid =
      fw_div (c : Scalar) (s inTid) := by
  unfold applyNode
  rw [show ([inTid] : List Tid).map s = [s inTid] from rfl,
      evalOp_fw_div_g92]
  change storeSet s [(outTid, fw_div (c : Scalar) (s inTid))] outTid = _
  unfold storeSet
  simp [List.find?]

/-- `fw_div` preserves shape. -/
theorem fw_div_shape_g92 (c : Scalar) (x : Tensor) : (fw_div c x).shape = x.shape := by
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `valAt` of `fw_div` at a valid index. -/
theorem fw_div_valAt_g92 (c : Scalar) (x : Tensor) (idx : Nat) (h : idx < prodShape x.shape) :
    valAt (fw_div c x) idx = valAt x idx / c := by
  rw [valAt_of_lt _ _ (by rw [fw_div_shape_g92]; exact h)]
  simp [fw_div, scalarDiv, Tensor.mkShape]

/-- `fw_div` distributes over `allGatherPrimDimN`: applying `fw_div c` to the
    gathered tensor equals gathering the per-shard `fw_div c` results.
    This holds because `fw_div` is a pointwise (elementwise) operation. -/
theorem fw_div_allGatherPrimDimN_eq_g92
    (c : Scalar) (gatherDim numParts : Nat) (xs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen : xs.length = numParts)
  ...
  have hhead_map : ((xs.map (fw_div c)).head?.map (fun t => t.shape)).getD [] = shardShape := by
    ...
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (xs.map (fw_div c))[0]'h0m = fw_div c (xs[0]'h0) := List.getElem_map ..
    rw [hm0, fw_div_shape_g92]
    exact hxs_shape 0 h0
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_shape : (allGatherPrimDimN gatherDim numParts 0 xs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts xs shardShape hhead
  have hlhs_shape : (fw_div c (allGatherPrimDimN gatherDim numParts 0 xs)).shape = outShape := by
    rw [fw_div_shape_g92]; exact hgather_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0 (xs.map (fw_div c))).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_map
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  ...
  have hgather_idx : idx < prodShape (allGatherPrimDimN gatherDim numParts 0 xs).shape := by
    rw [hgather_shape]; exact hidx_out
  rw [fw_div_valAt_g92 _ _ _ hgather_idx]
  rw [valAt_of_lt _ _ hgather_idx]
  conv_rhs => rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead, hhead_map]
  set ds := List.getD shardShape gatherDim 0
  ...
  · have hr_xs : r < xs.length := by omega
    ...
    rw [hmap_r]
    by_cases hloc_lt : loc < prodShape (xs[r]'hr_xs).shape
    · rw [fw_div_valAt_g92 _ _ _ hloc_lt]
    · have h1 : ¬(loc < prodShape (fw_div c (xs[r]'hr_xs)).shape) := by
        rw [fw_div_shape_g92]; exact hloc_lt
      simp only [valAt, h1, hloc_lt, dite_false]
      simp
  · have hr_xs : ¬(r < xs.length) := by omega
    have hr_map : ¬(r < (xs.map (fw_div c)).length) := by omega
...
/-! ### BW_div lemmas (added for goal 163) -/

/-- Unfolding lemma for `evalOp` on `BW_div` with `params := [c]`. -/
theorem evalOp_bw_div_g163 (numParts rank c : Nat) (g x : Tensor) :
    evalOp numParts rank "OpName.BW_div" [c] [g, x] = [bw_div (c : Scalar) g] := by
  rfl

/-- `applyNode` for `BW_div` with singleton output and `params := [c]`. -/
theorem applyNode_bw_div_out_g163
    (g : GraphDecl) (s : Store) (rank c : Nat) (gTid xTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.BW_div", ins := [gTid, xTid],
                    outs := [outTid], params := [c] } outTid =
      bw_div (c : Scalar) (s gTid) := by
  unfold applyNode
  rw [show ([gTid, xTid] : List Tid).map s = [s gTid, s xTid] from rfl,
      evalOp_bw_div_g163]
  change storeSet s [(outTid, bw_div (c : Scalar) (s gTid))] outTid = _
  unfold storeSet
  simp [List.find?]

/-- `bw_div` preserves the shape of its grad input. -/
theorem bw_div_shape_g163 (c : Scalar) (g : Tensor) : (bw_div c g).shape = g.shape := by
  simp [bw_div, scalarDiv, Tensor.mkShape]

/-- `valAt` of `bw_div` at a valid index. -/
theorem bw_div_valAt_g163 (c : Scalar) (g : Tensor) (idx : Nat) (h : idx < prodShape g.shape) :
    valAt (bw_div c g) idx = valAt g idx / c := by
  have hsh : (bw_div c g).shape = g.shape := bw_div_shape_g163 c g
  rw [valAt_of_lt _ _ (hsh ▸ h)]
  simp [bw_div, scalarDiv, Tensor.mkShape]

/-- `bw_div` distributes over `allGatherPrimDimN`: dividing a gathered tensor by `c`
    equals gathering the per-shard divisions. Holds because `bw_div` is pointwise. -/
theorem bw_div_allGatherPrimDimN_eq_g163
    (c : Scalar) (gatherDim numParts : Nat) (gs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen_g : gs.length = numParts)
  ...
  have hhead_m : ((gs.map (bw_div c)).head?.map (fun t => t.shape)).getD [] = shardShape := by
    have h0m : (0 : Nat) < (gs.map (bw_div c)).length := by omega
    have h0g : (0 : Nat) < gs.length := by omega
    rw [List.head?_eq_getElem?, List.getElem?_eq_getElem h0m]
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (gs.map (bw_div c))[0]'h0m = bw_div c (gs[0]'h0g) := List.getElem_map _
    rw [hm0, bw_div_shape_g163]
    exact hgs_shape 0 h0g
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_g_shape : (allGatherPrimDimN gatherDim numParts 0 gs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts gs shardShape hhead_g
  have hlhs_shape : (bw_div c (allGatherPrimDimN gatherDim numParts 0 gs)).shape = outShape := by
    rw [bw_div_shape_g163]; exact hgather_g_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0 (gs.map (bw_div c))).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_m
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  have hidx_out : idx < prodShape outShape := by rwa [hlhs_shape] at hidx
  rw [bw_div_valAt_g163 _ _ idx (by rw [hgather_g_shape]; exact hidx_out)]
  rw [valAt_of_lt _ _ (by rw [hgather_g_shape]; exact hidx_out)]
  rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead_g, hhead_m]
  set ds := List.getD shardShape gatherDim 0
  ...
  · have hr_g : r < gs.length := by omega
    have hr_m : r < (gs.map (bw_div c)).length := by omega
    rw [list_getD_of_lt _ _ _ hr_m, list_getD_of_lt _ _ _ hr_g]
    have hm_elem : (gs.map (bw_div c))[r]'hr_m = bw_div c (gs[r]'hr_g) := List.getElem_map _
    rw [hm_elem]
    by_cases hloc : loc < prodShape (gs[r]'hr_g).shape
    · exact (bw_div_valAt_g163 _ _ loc hloc).symm
    · have hbw_sh : (bw_div c (gs[r]'hr_g)).shape = (gs[r]'hr_g).shape := bw_div_shape_g163 _ _
      have h1 : ¬(loc < prodShape (bw_div c (gs[r]'hr_g)).shape) := by rw [hbw_sh]; exact hloc
      simp [valAt, h1, hloc, zero_div]
  · have hr_g : ¬(r < gs.length) := by omega
    have hr_m : ¬(r < (gs.map (bw_div c)).length) := by omega
    rw [list_getD_of_ge _ _ _ hr_m, list_getD_of_ge _ _ _ hr_g]
    simp [zeroTensor, Tensor.mkShape, valAt, prodShape, zero_div]

/-! ### BW_div lemmas (elementwise division by scalar; reads only the gradient input) -/

/-- Unfolding lemma for `BW_div` (params carry the scalar divisor). -/
theorem evalOp_bw_div_g198 (numParts rank : Nat) (c : Nat) (g x : Tensor) :
    evalOp numParts rank "OpName.BW_div" [c] [g, x] = [bw_div (c : Scalar) g] := by
  rfl

/-- `applyNode` for `BW_div` with singleton output. The second input is ignored. -/
theorem applyNode_bw_div_out_g198
    (gr : GraphDecl) (s : Store) (rank : Nat) (c : Nat) (gTid xTid outTid : Tid) :
    applyNode gr s { rank := rank, op := "OpName.BW_div", ins := [gTid, xTid], outs := [outTid], params := [c] } outTid =
      bw_div (c : Scalar) (s gTid) := by
  unfold applyNode
  rw [show ([gTid, xTid] : List Tid).map s = [s gTid, s xTid] from rfl,
      evalOp_bw_div_g198]
...
/-- `bw_div` preserves the shape of its gradient input. -/
theorem bw_div_shape_g198 (c : Scalar) (g : Tensor) : (bw_div c g).shape = g.shape := by
  simp [bw_div, scalarDiv, Tensor.mkShape]

/-- `valAt` of `bw_div` at a valid index. -/
theorem bw_div_valAt_g198 (c : Scalar) (g : Tensor) (idx : Nat) (h : idx < prodShape g.shape) :
    valAt (bw_div c g) idx = valAt g idx / c := by
  have hsh : (bw_div c g).shape = g.shape := bw_div_shape_g198 c g
  rw [valAt_of_lt _ _ (hsh ▸ h)]
  simp [bw_div, scalarDiv, Tensor.mkShape]

/-- `bw_div` distributes over `allGatherPrimDimN`: dividing the gathered gradient by
    the scalar equals gathering the per-shard divisions. This holds because `bw_div`
    is pointwise on its gradient input. -/
theorem bw_div_allGatherPrimDimN_eq_g198
    (c : Scalar) (gatherDim numParts : Nat) (gs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen_g : gs.length = numParts)
    ...
    (hgs_shape : ∀ i (hi : i < gs.length), (gs.get ⟨i, hi⟩).shape = shardShape) :
    bw_div c (allGatherPrimDimN gatherDim numParts 0 gs) =
      allGatherPrimDimN gatherDim numParts 0 (List.map (bw_div c) gs) := by
  have hlen_m : (List.map (bw_div c) gs).length = numParts := by
    rw [List.length_map, hlen_g]
  have hhead_m : ((List.map (bw_div c) gs).head?.map (fun t => t.shape)).getD [] = shardShape := by
    have h0m : (0 : Nat) < (List.map (bw_div c) gs).length := by omega
    have h0g : (0 : Nat) < gs.length := by omega
    rw [List.head?_eq_getElem?, List.getElem?_eq_getElem h0m]
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (List.map (bw_div c) gs)[0]'h0m = bw_div c (gs[0]'h0g) := List.getElem_map _
    rw [hm0, bw_div_shape_g198]
    exact hgs_shape 0 h0g
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_g_shape : (allGatherPrimDimN gatherDim numParts 0 gs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts gs shardShape hhead_g
  have hlhs_shape : (bw_div c (allGatherPrimDimN gatherDim numParts 0 gs)).shape = outShape := by
    rw [bw_div_shape_g198]; exact hgather_g_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0
      (List.map (bw_div c) gs)).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_m
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  have hidx_out : idx < prodShape outShape := by rwa [hlhs_shape] at hidx
  rw [bw_div_valAt_g198 c _ idx (by rw [hgather_g_shape]; exact hidx_out)]
  rw [valAt_of_lt _ _ (by rw [hgather_g_shape]; exact hidx_out)]
  rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead_g, hhead_m]
  set ds := List.getD shardShape gatherDim 0
  ...
  by_cases hr_lt : r < numParts
  · have hr_g : r < gs.length := by omega
    have hr_m : r < (List.map (bw_div c) gs).length := by omega
    rw [list_getD_of_lt _ _ _ hr_m, list_getD_of_lt _ _ _ hr_g]
    have hm_elem : (List.map (bw_div c) gs)[r]'hr_m = bw_div c (gs[r]'hr_g) :=
      List.getElem_map _
    rw [hm_elem]
    by_cases hloc : loc < prodShape (gs[r]'hr_g).shape
    · rw [bw_div_valAt_g198 c _ loc hloc]
    · have h1 : ¬(loc < prodShape (bw_div c (gs[r]'hr_g)).shape) := by
        rw [bw_div_shape_g198]; exact hloc
      simp [valAt, hloc, h1]
  · have hr_g : ¬(r < gs.length) := by omega
    have hr_m : ¬(r < (List.map (bw_div c) gs).length) := by omega
    rw [list_getD_of_ge _ _ _ hr_m, list_getD_of_ge _ _ _ hr_g]
...
/-! ## BW_div (scalar division backward) lemmas (goal_233) -/

theorem bw_div_shape_g233 (c : Scalar) (g : Tensor) : (bw_div c g).shape = g.shape := by
  simp [bw_div, scalarDiv, Tensor.mkShape]

theorem bw_div_valAt_g233 (c : Scalar) (g : Tensor) (idx : Nat)
    (h : idx < prodShape g.shape) :
    valAt (bw_div c g) idx = valAt g idx / c := by
  rw [valAt_of_lt _ _ (by rw [bw_div_shape_g233]; exact h)]
  simp [bw_div, scalarDiv, Tensor.mkShape]

theorem bw_div_allGatherPrimDimN_eq_g233
    (c : Scalar) (gatherDim numParts : Nat) (xs : List Tensor)
    (shardShape : Shape)
    (hnp : 0 < numParts)
    (hlen : xs.length = numParts)
    (hhead : (xs.head?.map (fun t => t.shape)).getD [] = shardShape)
    (hxs_shape : ∀ i (hi : i < xs.length), (xs.get ⟨i, hi⟩).shape = shardShape) :
    bw_div c (allGatherPrimDimN gatherDim numParts 0 xs) =
      allGatherPrimDimN gatherDim numParts 0 (xs.map (fun t => bw_div c t)) := by
  have hlen_map : (xs.map (fun t => bw_div c t)).length = numParts := by
    rw [List.length_map, hlen]
  have hhead_map : ((xs.map (fun t => bw_div c t)).head?.map (fun t => t.shape)).getD [] = shardShape := by
    have h0 : (0 : Nat) < xs.length := by omega
    have h0m : (0 : Nat) < (xs.map (fun t => bw_div c t)).length := by rw [List.length_map]; omega
    rw [List.head?_eq_getElem?]
    rw [List.getElem?_eq_getElem h0m]
    simp only [Option.map_some, Option.getD_some]
    have hm0 : (xs.map (fun t => bw_div c t))[0]'h0m = bw_div c (xs[0]'h0) := List.getElem_map ..
    rw [hm0, bw_div_shape_g233]
    exact hxs_shape 0 h0
  set outShape := shardShape.set gatherDim (shardShape.getD gatherDim 0 * numParts) with houtShape_def
  have hgather_shape : (allGatherPrimDimN gatherDim numParts 0 xs).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts xs shardShape hhead
  have hlhs_shape : (bw_div c (allGatherPrimDimN gatherDim numParts 0 xs)).shape = outShape := by
    rw [bw_div_shape_g233]; exact hgather_shape
  have hrhs_shape : (allGatherPrimDimN gatherDim numParts 0 (xs.map (fun t => bw_div c t))).shape = outShape :=
    allGatherPrimDimN_shape gatherDim numParts _ shardShape hhead_map
  apply Tensor.ext (by rw [hlhs_shape, hrhs_shape])
  intro idx hidx
  have hidx_out : idx < prodShape outShape := by rwa [hlhs_shape] at hidx
  have hgather_idx : idx < prodShape (allGatherPrimDimN gatherDim numParts 0 xs).shape := by
    rw [hgather_shape]; exact hidx_out
  rw [bw_div_valAt_g233 _ _ _ hgather_idx]
  rw [valAt_of_lt _ _ hgather_idx]
  conv_rhs => rw [valAt_of_lt _ _ (by rw [hrhs_shape]; exact hidx_out)]
  simp only [allGatherPrimDimN, Tensor.mkShape, hhead, hhead_map]
  set ds := List.getD shardShape gatherDim 0
  ...
  by_cases hr_lt : r < numParts
  · have hr_xs : r < xs.length := by omega
    have hr_map : r < (xs.map (fun t => bw_div c t)).length := by omega
    rw [list_getD_of_lt _ _ _ hr_xs, list_getD_of_lt _ _ _ hr_map]
    have hmap_r : (xs.map (fun t => bw_div c t))[r]'hr_map = bw_div c (xs[r]'hr_xs) := List.getElem_map ..
    rw [hmap_r]
    by_cases hloc_lt : loc < prodShape (xs[r]'hr_xs).shape
    · rw [bw_div_valAt_g233 _ _ _ hloc_lt]
    · have h1 : ¬(loc < prodShape (bw_div c (xs[r]'hr_xs)).shape) := by
        rw [bw_div_shape_g233]; exact hloc_lt
      simp only [valAt, h1, hloc_lt, dite_false]
      simp
  · have hr_xs : ¬(r < xs.length) := by omega
    have hr_map : ¬(r < (xs.map (fun t => bw_div c t)).length) := by omega
...
theorem applyNode_bw_div_out_g233
    (g : GraphDecl) (s : Store) (rank : Nat) (gradTid xTid outTid c : Nat) :
    applyNode g s { rank := rank, op := "OpName.BW_div", ins := [gradTid, xTid], outs := [outTid], params := [c] } outTid =
      bw_div ((c : Nat) : Scalar) (s gradTid) := by
  unfold applyNode
  change storeSet s [(outTid, bw_div ((c : Nat) : Scalar) (s gradTid))] outTid = _
  unfold storeSet
  simp [List.find?]
...
/-- `applyNode` for `FW_contiguous` with singleton output: it is the identity,
    returning the stored input tensor unchanged. -/
theorem applyNode_fw_contiguous_out_g71
    (g : GraphDecl) (s : Store) (rank : Nat) (inTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_contiguous", ins := [inTid], outs := [outTid] } outTid =
      s inTid := by
  unfold applyNode
...
/-- Gather-after-chunk identity on dim 3 for shape `[1,8,4,8]`: reassembling the
    dim-3 chunks (each of shape `[1,8,4,2]`) recovers the original tensor. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim3_1_8_4_8_g71 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 3 4 0
      [chunkPrimDimN 3 4 0 x, chunkPrimDimN 3 4 1 x,
       chunkPrimDimN 3 4 2 x, chunkPrimDimN 3 4 3 x] = x := by
...
/-! ## FW_contiguous (identity) helpers for goal 96 -/

/-- `applyNode` for `FW_contiguous` with singleton output. Since `FW_contiguous` is the identity
    op, the stored value is just the input tensor. -/
theorem applyNode_fw_contiguous_out_g96
    (g : GraphDecl) (s : Store) (rank : Nat) (inTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.FW_contiguous", ins := [inTid], outs := [outTid] } outTid =
      s inTid := by
  unfold applyNode
...
/-- `valAt` of an `allGatherPrimDimN` along dim 2 with 4 shards of shape `[1,8,1,8]`. -/
theorem allGatherPrimDimN_2_4_valAt_1_8_1_8_g96 (xs : List Tensor) (idx : Nat)
    (hhead : (xs.head?.map (fun t => t.shape)).getD [] = [1, 8, 1, 8]) (hidx : idx < 256) :
    valAt (allGatherPrimDimN 2 4 0 xs) idx =
      valAt (xs.getD ((idx % 32) / 8) (zeroTensor [1, 8, 1, 8]))
        ((idx / 32) * 8 + idx % 8) := by
  have hresult_shape : (allGatherPrimDimN 2 4 0 xs).shape = [1, 8, 4, 8] := by
...
/-- `valAt` of a `chunkPrimDimN` along dim 2 (4 parts) on a tensor of shape `[1,8,4,8]`. -/
theorem chunkPrimDimN_2_4_valAt_1_8_4_8_g96 (x : Tensor) (r idx : Nat)
    (hx : x.shape = [1, 8, 4, 8]) (hr : r < 4) (hidx : idx < 64) :
    valAt (chunkPrimDimN 2 4 r x) idx =
      valAt x ((idx / 8) * 32 + r * 8 + idx % 8) := by
  ...
  simp only [chunkPrimDimN, Tensor.mkShape, hx, List.getD, List.getElem?_cons_zero,
    ...
    show (1 : Nat) ≠ 0 from by omega, show (32 : Nat) ≠ 0 from by omega,
    show (64 : Nat) ≠ 0 from by omega, show (4 / 4 : Nat) = 1 by norm_num,
    ite_false]
  have hrm : r % 4 = r := Nat.mod_eq_of_lt hr
  rw [hrm]
  congr 1
...
/-- Gather-after-chunk identity: reassembling dim-2 chunks (of shape `[1,8,1,8]`) recovers the
    original tensor of shape `[1,8,4,8]`. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim2_4_1_8_4_8_g96 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 2 4 0
      [chunkPrimDimN 2 4 0 x, chunkPrimDimN 2 4 1 x,
       chunkPrimDimN 2 4 2 x, chunkPrimDimN 2 4 3 x] = x := by
  ...
  rw [hsh] at hidx
  have hidx256 : idx < 256 := by simpa [prodShape] using hidx
  rw [allGatherPrimDimN_2_4_valAt_1_8_1_8_g96 _ idx hhead hidx256]
  have hgetD : ∀ (i : Nat) (hi : i < 4),
      [chunkPrimDimN 2 4 0 x, chunkPrimDimN 2 4 1 x,
       chunkPrimDimN 2 4 2 x, chunkPrimDimN 2 4 3 x].getD i (zeroTensor [1, 8, 1, 8]) =
        chunkPrimDimN 2 4 i x := by
    ...
    rcases h4 with rfl | rfl | rfl | rfl <;>
      simp [List.getD, List.getElem?_cons_zero, List.getElem?_cons_succ]
  have hr : (idx % 32) / 8 < 4 := by omega
  have hloc : ((idx / 32) * 8 + idx % 8) / 8 * 32 + ((idx % 32) / 8) * 8 + ((idx / 32) * 8 + idx % 8) % 8 = idx := by omega
  rw [hgetD ((idx % 32) / 8) hr]
...
/-! ## BW_contiguous + AllToAll helpers (goal_167 family) -/

/-- `applyNode` for `BW_contiguous`: returns the first input (gradient), ignoring the second. -/
theorem applyNode_bw_contiguous_out_g167
    (g : GraphDecl) (s : Store) (rank : Nat) (gTid xTid outTid : Tid) :
    applyNode g s { rank := rank, op := "OpName.BW_contiguous", ins := [gTid, xTid], outs := [outTid] } outTid =
      s gTid := by
  unfold applyNode
  change storeSet s [(outTid, s gTid)] outTid = _
  unfold storeSet
...
/-- Gather-after-chunk identity: reassembling dim-2 chunks (of shape [1,8,1,8]) recovers the
    original tensor of shape [1,8,4,8]. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim2_4_1_8_4_8_g167 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 2 4 0
      [chunkPrimDimN 2 4 0 x, chunkPrimDimN 2 4 1 x,
       chunkPrimDimN 2 4 2 x, chunkPrimDimN 2 4 3 x] = x := by
...
/-- Gather-after-chunk identity: reassembling dim-3 chunks (of shape [1,8,4,2]) recovers the
    original tensor of shape [1,8,4,8]. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim3_4_1_8_4_8_g167 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 3 4 0
      [chunkPrimDimN 3 4 0 x, chunkPrimDimN 3 4 1 x,
       chunkPrimDimN 3 4 2 x, chunkPrimDimN 3 4 3 x] = x := by
...
/-- `applyNode` for `BW_contiguous`: the gradient passthrough returns its first input
    (the gradient `g`), ignoring the reference input `x`. -/
theorem applyNode_bw_contiguous_out_g202
    (gr : GraphDecl) (s : Store) (rank : Nat) (gTid xTid outTid : Tid) :
    applyNode gr s { rank := rank, op := "OpName.BW_contiguous", ins := [gTid, xTid], outs := [outTid] } outTid =
      s gTid := by
  unfold applyNode
  change storeSet s [(outTid, s gTid)] outTid = _
  unfold storeSet
...
/-- Gather-after-chunk identity (dim 3): reassembling dim-3 chunks (of shape [1,8,4,2])
    recovers the original tensor of shape [1,8,4,8]. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim3_1_8_4_8_g202 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 3 4 0
      [chunkPrimDimN 3 4 0 x, chunkPrimDimN 3 4 1 x,
       chunkPrimDimN 3 4 2 x, chunkPrimDimN 3 4 3 x] = x := by
...
/-- Gather-after-chunk identity (dim 1): reassembling dim-1 chunks (of shape [1,2,4,8])
    recovers the original tensor of shape [1,8,4,8]. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim1_1_8_4_8_g202 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 1 4 0
      [chunkPrimDimN 1 4 0 x, chunkPrimDimN 1 4 1 x,
       chunkPrimDimN 1 4 2 x, chunkPrimDimN 1 4 3 x] = x := by
  ...
  have hgetD : ∀ (i : Nat) (hi : i < 4),
    ...
    rcases h4 with rfl | rfl | rfl | rfl <;>
      simp [List.getD, List.getElem?_cons_zero, List.getElem?_cons_succ]
  have hr : idx / 64 < 4 := by omega
  have hloc : idx % 64 < 64 := by omega
  rw [hgetD (idx / 64) hr]
  rw [chunkPrimDimN_1_4_valAt_1_8_4_8 x (idx / 64) (idx % 64) hsh hr hloc]
  congr 1
  omega

/-- Unfolding lemma for `BW_contiguous`: identity on the gradient (first) input. -/
theorem evalOp_bw_contiguous_g237 (numParts rank : Nat) (params : List Nat) (g x : Tensor) :
    evalOp numParts rank "OpName.BW_contiguous" params [g, x] = [g] := by
  rfl

/-- `applyNode` for `BW_contiguous` with two inputs and singleton output: returns the
    gradient (first) input unchanged. -/
theorem applyNode_bw_contiguous_out_g237
  ...
  unfold applyNode
  rw [show ([gTid, xTid] : List Tid).map s = [s gTid, s xTid] from rfl,
      evalOp_bw_contiguous_g237]
  change storeSet s [(outTid, s gTid)] outTid = _
  unfold storeSet
  simp [List.find?]

/-- Gather-chunk roundtrip for dim 1 on shape [1,8,4,8]:
    gathering the 4 dim-1 chunks of x along dim 1 gives back x. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim1_4_1_8_4_8_g237 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 1 4 0
      [chunkPrimDimN 1 4 0 x, chunkPrimDimN 1 4 1 x,
       chunkPrimDimN 1 4 2 x, chunkPrimDimN 1 4 3 x] = x := by
...
/-- Gather-chunk roundtrip for dim 2 on shape [1,8,4,8]:
    gathering the 4 dim-2 chunks of x along dim 2 gives back x. -/
theorem allGatherPrimDimN_chunkPrimDimN_id_dim2_4_1_8_4_8_g237 (x : Tensor)
    (hsh : x.shape = [1, 8, 4, 8]) :
    allGatherPrimDimN 2 4 0
      [chunkPrimDimN 2 4 0 x, chunkPrimDimN 2 4 1 x,
       chunkPrimDimN 2 4 2 x, chunkPrimDimN 2 4 3 x] = x := by
