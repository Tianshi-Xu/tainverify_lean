...
/-- Autograd for forward shuffle is the exact inverse collective. -/
theorem bw_maybe_shuffle_collective_eq_fw_unshuffle
    (grads : List Tensor) (cu : List Nat) (cpSize cpRank : Nat) :
    TrainVerify.Denote.bw_maybe_shuffle_collective grads cu cpSize cpRank =
      fw_maybe_unshuffle_collective grads cu cpSize cpRank := by
  rfl

/-- Autograd for forward unshuffle is the exact forward collective. -/
theorem bw_maybe_unshuffle_collective_eq_fw_shuffle
    (grads : List Tensor) (cu : List Nat) (cpSize cpRank : Nat) :
    TrainVerify.Denote.bw_maybe_unshuffle_collective grads cu cpSize cpRank =
      fw_maybe_shuffle_collective grads cu cpSize cpRank := by
  rfl
...
noncomputable def fw_attn_zigzag_collective
...
@[simp] theorem fw_attn_zigzag_collective_cpSize_one
  ...
  simp [fw_attn_zigzag_collective]
...
theorem fw_rms_norm_allGather0_commute_2_core_3d
  ...
  · intro idx hidx
    ...
    have hLval : valAt (fw_rms_norm (allGatherPrimDimN 0 2 0 [a, b]) w)
      ...
      simp [hidxGather, hdivIdx, hmodIdx]
    rw [hLval, hmean]
    rw [allGatherPrimDimN0_valAt_3D 2 shard middle hidden [a, b]
    ...
    have hmodLocal : ((i * middle + j) * hidden + col) % hidden = col := by
      rw [show (i * middle + j) * hidden + col = col + hidden * (i * middle + j) by ring,
        Nat.add_mul_mod_self_left, Nat.mod_eq_of_lt hcolLt]
    simp [hlocBound, hdivLocal, hmodLocal]
