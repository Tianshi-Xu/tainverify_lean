import denote.gpt_ly4_regen.Goal_248_Data

open TrainVerify.Denote
open TrainVerify.Denote.Generated

namespace TrainVerify.Denote.GeneratedGoals

-- Split out of Goal_248.lean and restructured for Lean v4.31.
-- The v4.27 proof reduced this against the full 12-node fold; under v4.31 the
-- `by decide` side goals of `repeat rw [applyNode_eq_of_not_mem_outs]`, and the
-- trailing `rfl`, make the kernel accumulate def-eq state without converging
-- (~3.5 GB/min; 27+ GB RSS measured on Goal_124). Truncating to the prefix that
-- ends at the writing node puts it at the head of the fold, so the same rewrites
-- close it against a much smaller graph.
-- Proof content is otherwise the v4.27 proof from commit c4f01699.
set_option maxRecDepth 100000 in
set_option maxHeartbeats 400000 in
theorem hpm3base_248 (initPM : Store) :
    (denoteGraph pm_goal_248 initPM) 3284 = allToAllPrimWithDims 4 3 [(denoteGraph pm_goal_248 initPM) 3305, (denoteGraph pm_goal_248 initPM) 3307, (denoteGraph pm_goal_248 initPM) 3309, (denoteGraph pm_goal_248 initPM) 3311] 2 1 := by
  have hL : denoteGraph pm_goal_248 initPM 3284
          = denoteGraph {pm_goal_248 with nodes := pm_goal_248.nodes.take 12} initPM 3284 :=
    denoteGraph_tid_eq_of_suffix_no_writes pm_goal_248 initPM 3284 _ _
      (List.take_append_drop 12 _).symm (by decide)
  have hR : ∀ t : Tid, t = 3305 ∨ t = 3307 ∨ t = 3309 ∨ t = 3311 →
      denoteGraph pm_goal_248 initPM t
        = denoteGraph {pm_goal_248 with nodes := pm_goal_248.nodes.take 11} initPM t := by
    rintro t (rfl | rfl | rfl | rfl) <;>
      exact denoteGraph_tid_eq_of_suffix_no_writes pm_goal_248 initPM _ _ _
        (List.take_append_drop 11 _).symm (by decide)
  rw [hL, hR 3305 (by tauto), hR 3307 (by tauto), hR 3309 (by tauto), hR 3311 (by tauto)]
  simp only [pm_goal_248, denoteGraph, GraphDecl.nodes, List.take, List.foldl, List.map]
  rw [applyNode_allToAllPrimWithDims_out]
  rfl

end TrainVerify.Denote.GeneratedGoals
