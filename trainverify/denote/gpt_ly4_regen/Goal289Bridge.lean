/- goal_289 bridge (prereqs=[2..55,257-285 odd]=69). FW_multiref params=[3] FIRST-output
   + AllGather dim1 tail -> single-tp 999. SM=FW_multiref(640)->[1004,1008,1012] node 61 take first 1004=640.
   PM=4xFW_multiref(2229+r)->[3607+,3609+,2313+] node 396-399 take FIRST out 3607/3617/3627/3637
   -> AllGatherPrim(node 403)->999 single tp. 640=goal_55 output [1,8,32] dim1-gathered tps 2229-2232 [1,2,32].
   Core semantics (multiref first-out copy + AllGather reconstruct) in prove_goal_289_cut. Bridge = frame only.
   Template = Goal265Bridge (multiref+AllGather single-out, 3rd-out) adapted to FIRST output via
   applyNode_fw_multiref3_first_out_g289 (no ineq args). Multiref nodes 396-399 adjacent, AllGather node 403. -/
...
-- ========== mini sm_goal_289 computes 1004 (multiref 1st-out = s 640) ==========
theorem denote_sm_goal_289_1004 (s : Store) :
    denoteGraph sm_goal_289 s 1004 = s 640 := by
  simp only [sm_goal_289, denoteGraph, List.foldl]
  rw [applyNode_fw_multiref3_first_out_g289]

-- ========== mini pm_goal_289 computes 999 (multiref 1st -> AllGather) ==========
theorem denote_pm_goal_289_999 (s : Store) :
    denoteGraph pm_goal_289 s 999 = allGatherPrimDimN 1 4 0 [s 2229, s 2230, s 2231, s 2232] := by
  simp only [pm_goal_289, denoteGraph, List.foldl]
  rw [applyNode_allGatherPrimDimN_out_thm]
  simp only [List.map]
  congr 1 <;>
    simp only [applyNode_fw_multiref3_first_out_g289] <;>
    rfl

-- ========== SM self-frame: full computes 1004 (node 61) ==========
theorem sm_frame_1004_self (initSM : Store) :
    denoteGraph sm initSM 1004 = denoteGraph sm_goal_289 (denoteGraph sm initSM) 1004 := by
  rw [denote_sm_goal_289_1004]
  rw [sm_val initSM 61 1004 (by native_decide) (by native_decide)]
...
-- ========== full pm computes multiref 1st-out 3607-3637 (node 396-399) = Spm ins ==========
theorem pm_full_g289_3607 (initPM : Store) :
    denoteGraph pm initPM 3607 = denoteGraph pm initPM 2229 := by
  rw [pm_val initPM 396 3607 (by native_decide) (by native_decide)]
...
theorem pm_full_g289_3617 (initPM : Store) :
    denoteGraph pm initPM 3617 = denoteGraph pm initPM 2230 := by
  rw [pm_val initPM 397 3617 (by native_decide) (by native_decide)]
...
theorem pm_full_g289_3627 (initPM : Store) :
    denoteGraph pm initPM 3627 = denoteGraph pm initPM 2231 := by
  rw [pm_val initPM 398 3627 (by native_decide) (by native_decide)]
...
theorem pm_full_g289_3637 (initPM : Store) :
    denoteGraph pm initPM 3637 = denoteGraph pm initPM 2232 := by
  rw [pm_val initPM 399 3637 (by native_decide) (by native_decide)]
...
-- ========== PM self-frame: 999 (AllGather node 403, ins 3607-3637) ==========
theorem pm_frame_999_self (initPM : Store) :
  ...
  rw [pm_val initPM 403 999 (by native_decide) (by native_decide)]
  rw [show pm.nodes[403]'(by native_decide)
      = { rank := 0, op := "OpName.AllGatherPrim", ins := [3607, 3617, 3627, 3637], outs := [999], params := [1] }
      from by native_decide]
  rw [applyNode_allGatherPrimDimN_out_thm]
  simp only [List.map]
  rw [pm_prefix_eq initPM 403 3607 (by native_decide),
      pm_prefix_eq initPM 403 3617 (by native_decide),
      pm_prefix_eq initPM 403 3627 (by native_decide),
      pm_prefix_eq initPM 403 3637 (by native_decide)]
  rw [pm_full_g289_3607, pm_full_g289_3617, pm_full_g289_3627, pm_full_g289_3637]
  rw [show pm.numRanks = 4 from by native_decide]

lemma goal_289_hInitCut_helper (Ssm Spm : Store)
    ...
    : InitGoalsHold pm_goal_289.numRanks goal_289_cut_initGoals Ssm Spm := by
  have hnr : pm_goal_289.numRanks = pm.numRanks := by native_decide
  rw [hnr]
  simp only [InitGoalsHold] at hinitC ⊢
  simp only [goal_289_cut_initGoals, goal_289_prereqs, List.forall_mem_append,
...
theorem goal_289_cut_to_full (h : goal_289_stmt_cut) : goal_289_stmt := by
  ...
  have hnr : pm_goal_289.numRanks = pm.numRanks := by native_decide
  have hInitCut : InitGoalsHold pm_goal_289.numRanks goal_289_cut_initGoals Ssm Spm :=
    goal_289_hInitCut_helper Ssm Spm hinitC hg2 hg3 hg4 hg5 hg6 hg7 hg8 hg9 hg10 hg11 hg12 hg13 hg14 hg15 hg16 hg17 hg18 hg19 hg20 hg21 hg22 hg23 hg24 hg25 hg26 hg27 hg28 hg29 hg30 hg31 hg32 hg33 hg34 hg35 hg36 hg37 hg38 hg39 hg40 hg41 hg42 hg43 hg44 hg45 hg46 hg47 hg48 hg49 hg50 hg51 hg52 hg53 hg54 hg55 hg257 hg259 hg261 hg263 hg265 hg267 hg269 hg271 hg273 hg275 hg277 hg279 hg281 hg283 hg285
  ...
  have hSM289 : StoreShapesHold Ssm sm_goal_289InitEnv := by
    intro tid sh hsh
    rw [sm_goal_289InitEnv] at hsh
    have hmem := mem_of_shapeEnvOfList_eq_some hsh
    simp only [sm_goal_289InitShapes, List.mem_cons, List.not_mem_nil, or_false,
               Prod.mk.injEq] at hmem
    rcases hmem with ⟨rfl, rfl⟩
    · exact h640_smsh
  have hPM289 : StoreShapesHold Spm pm_goal_289InitEnv := by
    intro tid sh hsh
    rw [pm_goal_289InitEnv] at hsh
    have hmem := mem_of_shapeEnvOfList_eq_some hsh
    simp only [pm_goal_289InitShapes, List.mem_cons, List.not_mem_nil, or_false,
               Prod.mk.injEq] at hmem
    ...
    · exact h2231sh
    · exact h2232sh
  have hcut := h Ssm Spm hSM289 hPM289 hInitCut
  have hsmf : Ssm 1004 = denoteGraph sm_goal_289 Ssm 1004 := by
    rw [hSsm]; exact sm_frame_1004_self initSM
  have hpm999 : Spm 999 = denoteGraph pm_goal_289 Spm 999 := by
    rw [denote_pm_goal_289_999]
    rw [hSpm]
    exact pm_frame_999_self initPM
  rw [hnr] at hcut
  simp only [goal_289, List.map] at hcut ⊢
  rw [hsmf, hpm999]
  exact hcut

theorem goal_289_intermediate (initSM initPM : Store)
    (hSM : StoreShapesHold initSM smInitEnv) (hPM : StoreShapesHold initPM pmInitEnv)
    (hInit : InitGoalsHold pm.numRanks initGoals initSM initPM) :
    InitGoalHolds pm.numRanks goal_289 (denoteGraph sm initSM) (denoteGraph pm initPM) := by
  have hfull : goal_289_stmt := goal_289_cut_to_full prove_goal_289_cut
