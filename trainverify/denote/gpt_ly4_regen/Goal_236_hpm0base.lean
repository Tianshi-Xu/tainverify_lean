...
theorem hpm0base_236 (initPM : Store) :
    (denoteGraph pm_goal_236 initPM) 3083 = allToAllPrimWithDims 4 0
  ...
  have hL : denoteGraph pm_goal_236 initPM 3083
          = denoteGraph {pm_goal_236 with nodes := pm_goal_236.nodes.take 9} initPM 3083 :=
    denoteGraph_tid_eq_of_suffix_no_writes pm_goal_236 initPM 3083 _ _
      (List.take_append_drop 9 _).symm (by decide)
  have hR : ∀ t : Tid, t = 3109 ∨ t = 3111 ∨ t = 3113 ∨ t = 3115 →
      denoteGraph pm_goal_236 initPM t
        = denoteGraph {pm_goal_236 with nodes := pm_goal_236.nodes.take 8} initPM t := by
