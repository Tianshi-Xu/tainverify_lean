/- goal_56 bridge (prereqs=[2..55,257-285 odd,289]=70). Column-parallel FW_linear
   (shared input 999 + column-sharded weight) + AllGather dim2 -> single-tp 642.
   SM node 62 FW_linear(1004,641)->642 [1,8,32]. PM nodes 406-409 4xFW_linear(999,2257+r)->2261-2264
   (shared input 999, per-rank weight slice [8,32]) + node 415 AllGatherPrim(range 2261+r, params=[2] dim2)->642 single out.
   input 1004=999 from goal_289 single-tp [1,8,32]; weights 641->2257-2260 [8,32] from initGoal_641 (in initGoals, column-sharded dim0).
   Each rank computes a feature-column slice [1,8,8]; AllGather dim2 reconstructs [1,8,32] = full linear.
   Core column-parallel semantics (fw_linear_column_parallel_4_1_8_32_8) in prove_goal_56_cut. Bridge = frame only.
   Template = Goal50Bridge frame (per-rank op + final AllGather single-out, range-form full-pm ins)
   adapted op FW_layernorm->FW_linear (2 ins, shared input), AllGather dim1->dim2; large-prereq hInitCut helper. -/
...
-- ========== mini sm_goal_56 computes 642 (FW_linear 1004,641) ==========
theorem denote_sm_goal_56_642 (s : Store) :
    denoteGraph sm_goal_56 s 642 = fw_linear (s 1004) (s 641) := by
  simp only [sm_goal_56, denoteGraph, List.foldl]
  rw [applyNode_fw_linear_out]

-- ========== mini pm_goal_56 computes 642 (AllGather dim2 of 4x FW_linear) ==========
theorem denote_pm_goal_56_642 (s : Store) :
    denoteGraph pm_goal_56 s 642
      = allGatherPrimDimN 2 4 0
          [fw_linear (s 999) (s 2257),
           fw_linear (s 999) (s 2258),
           fw_linear (s 999) (s 2259),
           fw_linear (s 999) (s 2260)] := by
  simp only [pm_goal_56, denoteGraph, List.foldl]
  rw [applyNode_allGatherPrimDimN_out_thm]
  simp only [List.map]
  congr 1

-- ========== SM self-frame: full computes 642 (node 62 FW_linear) ==========
theorem sm_frame_642_self (initSM : Store) :
    denoteGraph sm initSM 642 = denoteGraph sm_goal_56 (denoteGraph sm initSM) 642 := by
  rw [denote_sm_goal_56_642]
  rw [sm_val initSM 62 642 (by native_decide) (by native_decide)]
  rw [show sm.nodes[62]'(by native_decide)
      = { rank := 0, op := "OpName.FW_linear", ins := [1004, 641], outs := [642] }
      from by native_decide]
  rw [applyNode_fw_linear_out]
  rw [sm_prefix_eq initSM 62 1004 (by native_decide),
      sm_prefix_eq initSM 62 641 (by native_decide)]

-- ========== full pm computes per-rank FW_linear 2261-2264 (node 406-409) ==========
theorem pm_full_g56_2261 (initPM : Store) :
    denoteGraph pm initPM 2261
      = fw_linear (denoteGraph pm initPM 999) (denoteGraph pm initPM 2257) := by
  rw [pm_val initPM 406 2261 (by native_decide) (by native_decide)]
  rw [show pm.nodes[406]'(by native_decide)
      = { rank := 0, op := "OpName.FW_linear", ins := [999, 2257], outs := [2261] }
      from by native_decide]
  rw [applyNode_fw_linear_out]
  rw [pm_prefix_eq initPM 406 999 (by native_decide),
      pm_prefix_eq initPM 406 2257 (by native_decide)]

theorem pm_full_g56_2262 (initPM : Store) :
    denoteGraph pm initPM 2262
      = fw_linear (denoteGraph pm initPM 999) (denoteGraph pm initPM 2258) := by
  rw [pm_val initPM 407 2262 (by native_decide) (by native_decide)]
  rw [show pm.nodes[407]'(by native_decide)
      = { rank := 1, op := "OpName.FW_linear", ins := [999, 2258], outs := [2262] }
      from by native_decide]
  rw [applyNode_fw_linear_out]
  rw [pm_prefix_eq initPM 407 999 (by native_decide),
      pm_prefix_eq initPM 407 2258 (by native_decide)]

theorem pm_full_g56_2263 (initPM : Store) :
    denoteGraph pm initPM 2263
      = fw_linear (denoteGraph pm initPM 999) (denoteGraph pm initPM 2259) := by
  rw [pm_val initPM 408 2263 (by native_decide) (by native_decide)]
  rw [show pm.nodes[408]'(by native_decide)
      = { rank := 2, op := "OpName.FW_linear", ins := [999, 2259], outs := [2263] }
      from by native_decide]
  rw [applyNode_fw_linear_out]
  rw [pm_prefix_eq initPM 408 999 (by native_decide),
      pm_prefix_eq initPM 408 2259 (by native_decide)]

theorem pm_full_g56_2264 (initPM : Store) :
    denoteGraph pm initPM 2264
      = fw_linear (denoteGraph pm initPM 999) (denoteGraph pm initPM 2260) := by
  rw [pm_val initPM 409 2264 (by native_decide) (by native_decide)]
  rw [show pm.nodes[409]'(by native_decide)
      = { rank := 3, op := "OpName.FW_linear", ins := [999, 2260], outs := [2264] }
      from by native_decide]
  rw [applyNode_fw_linear_out]
  rw [pm_prefix_eq initPM 409 999 (by native_decide),
      pm_prefix_eq initPM 409 2260 (by native_decide)]

-- ========== PM self-frame: 642 (AllGather node 415, range-form ins, dim2) ==========
theorem pm_frame_642_self (initPM : Store) :
    denoteGraph pm initPM 642 = denoteGraph pm_goal_56 (denoteGraph pm initPM) 642 := by
  rw [denote_pm_goal_56_642]
  rw [pm_val initPM 415 642 (by native_decide) (by native_decide)]
  rw [show pm.nodes[415]'(by native_decide)
  ...
  rw [pm_prefix_eq initPM 415 2261 (by native_decide),
      pm_prefix_eq initPM 415 2262 (by native_decide),
      pm_prefix_eq initPM 415 2263 (by native_decide),
      pm_prefix_eq initPM 415 2264 (by native_decide)]
  rw [pm_full_g56_2261, pm_full_g56_2262, pm_full_g56_2263, pm_full_g56_2264]
  rw [show pm.numRanks = 4 from by native_decide]

lemma goal_56_hInitCut_helper (Ssm Spm : Store)
    ...
    : InitGoalsHold pm_goal_56.numRanks goal_56_cut_initGoals Ssm Spm := by
  have hnr : pm_goal_56.numRanks = pm.numRanks := by native_decide
  rw [hnr]
  simp only [InitGoalsHold] at hinitC ⊢
  simp only [goal_56_cut_initGoals, goal_56_prereqs, List.forall_mem_append,
    List.forall_mem_cons, List.forall_mem_nil, and_true]
  exact ⟨hinitC, hg2, hg3, hg4, hg5, hg6, hg7, hg8, hg9, hg10, hg11, hg12, hg13, hg14, hg15, hg16, hg17, hg18, hg19, hg20, hg21, hg22, hg23, hg24, hg25, hg26, hg27, hg28, hg29, hg30, hg31, hg32, hg33, hg34, hg35, hg36, hg37, hg38, hg39, hg40, hg41, hg42, hg43, hg44, hg45, hg46, hg47, hg48, hg49, hg50, hg51, hg52, hg53, hg54, hg55, hg257, hg259, hg261, hg263, hg265, hg267, hg269, hg271, hg273, hg275, hg277, hg279, hg281, hg283, hg285, hg289, List.forall_mem_nil _⟩

theorem goal_56_cut_to_full (h : goal_56_stmt_cut) : goal_56_stmt := by
  ...
  rw [← hSsm, ← hSpm] at hg2 hg3 hg4 hg5 hg6 hg7 hg8 hg9 hg10 hg11 hg12 hg13 hg14 hg15 hg16 hg17 hg18 hg19 hg20 hg21 hg22 hg23 hg24 hg25 hg26 hg27 hg28 hg29 hg30 hg31 hg32 hg33 hg34 hg35 hg36 hg37 hg38 hg39 hg40 hg41 hg42 hg43 hg44 hg45 hg46 hg47 hg48 hg49 hg50 hg51 hg52 hg53 hg54 hg55 hg257 hg259 hg261 hg263 hg265 hg267 hg269 hg271 hg273 hg275 hg277 hg279 hg281 hg283 hg285 hg289 hinitC
  have hnr : pm_goal_56.numRanks = pm.numRanks := by native_decide
  have hInitCut : InitGoalsHold pm_goal_56.numRanks goal_56_cut_initGoals Ssm Spm :=
    goal_56_hInitCut_helper Ssm Spm hinitC hg2 hg3 hg4 hg5 hg6 hg7 hg8 hg9 hg10 hg11 hg12 hg13 hg14 hg15 hg16 hg17 hg18 hg19 hg20 hg21 hg22 hg23 hg24 hg25 hg26 hg27 hg28 hg29 hg30 hg31 hg32 hg33 hg34 hg35 hg36 hg37 hg38 hg39 hg40 hg41 hg42 hg43 hg44 hg45 hg46 hg47 hg48 hg49 hg50 hg51 hg52 hg53 hg54 hg55 hg257 hg259 hg261 hg263 hg265 hg267 hg269 hg271 hg273 hg275 hg277 hg279 hg281 hg283 hg285 hg289
  -- input 1004 [1,8,32] (sm ts) from goal_289; 999 [1,8,32] (pm tp) from goal_289
  have h1004_smsh : (Ssm 1004).shape = [1, 8, 32] := by
    have h := hg289.1; simp only [goal_289] at h; exact h
  have h999_pmsh : (Spm 999).shape = [1, 8, 32] := by
    have h := hg289.2.1; simp only [goal_289, List.map] at h
    have := congrArg List.head? h; simpa using this
  -- weight 641 [32,32] (sm) + 2257-2260 [8,32] (pm) from initGoal_641 (in initGoals)
  have hg641 := hinitC initGoal_641 (by simp only [initGoals]; decide)
  have h641_smsh : (Ssm 641).shape = [32, 32] := by
    have h := hg641.1; simp only [initGoal_641] at h; exact h
  have hpmW : (Spm 2257).shape = [8,32] ∧ (Spm 2258).shape = [8,32] ∧
              (Spm 2259).shape = [8,32] ∧ (Spm 2260).shape = [8,32] := by
    have h := hg641.2.1
    simp only [initGoal_641, List.map, List.cons.injEq, and_true] at h
    exact ⟨h.1, h.2.1, h.2.2.1, h.2.2.2⟩
  obtain ⟨h2257sh, h2258sh, h2259sh, h2260sh⟩ := hpmW
  have hSM56 : StoreShapesHold Ssm sm_goal_56InitEnv := by
    intro tid sh hsh
    rw [sm_goal_56InitEnv] at hsh
    have hmem := mem_of_shapeEnvOfList_eq_some hsh
    simp only [sm_goal_56InitShapes, List.mem_cons, List.not_mem_nil, or_false,
               Prod.mk.injEq] at hmem
    rcases hmem with ⟨rfl, rfl⟩ | ⟨rfl, rfl⟩
    · exact h641_smsh
    · exact h1004_smsh
  have hPM56 : StoreShapesHold Spm pm_goal_56InitEnv := by
    intro tid sh hsh
    rw [pm_goal_56InitEnv] at hsh
    have hmem := mem_of_shapeEnvOfList_eq_some hsh
    simp only [pm_goal_56InitShapes, List.mem_cons, List.not_mem_nil, or_false,
               Prod.mk.injEq] at hmem
    ...
    · exact h2259sh
    · exact h2260sh
  have hcut := h Ssm Spm hSM56 hPM56 hInitCut
  have hsmf : Ssm 642 = denoteGraph sm_goal_56 Ssm 642 := by
    rw [hSsm]; exact sm_frame_642_self initSM
  have hpm642 : Spm 642 = denoteGraph pm_goal_56 Spm 642 := by
    rw [hSpm]; exact pm_frame_642_self initPM
  rw [hnr] at hcut
  simp only [goal_56, List.map] at hcut ⊢
  rw [hsmf, hpm642]
  exact hcut

theorem goal_56_intermediate (initSM initPM : Store)
    (hSM : StoreShapesHold initSM smInitEnv) (hPM : StoreShapesHold initPM pmInitEnv)
    (hInit : InitGoalsHold pm.numRanks initGoals initSM initPM) :
    InitGoalHolds pm.numRanks goal_56 (denoteGraph sm initSM) (denoteGraph pm initPM) := by
  have hfull : goal_56_stmt := goal_56_cut_to_full prove_goal_56_cut
