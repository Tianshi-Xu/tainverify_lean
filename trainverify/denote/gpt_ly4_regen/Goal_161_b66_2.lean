...
theorem b66_2_161 (initPM : Store) :
    (denoteGraph pm_goal_161 initPM) 1970 =
      (bw_matmul ((denoteGraph pm_goal_161 initPM) 789) ((denoteGraph pm_goal_161 initPM) 1947)
        ((denoteGraph pm_goal_161 initPM) 1951)).2 := by
  have hL : denoteGraph pm_goal_161 initPM 1970
          = denoteGraph {pm_goal_161 with nodes := pm_goal_161.nodes.take 11} initPM 1970 :=
    denoteGraph_tid_eq_of_suffix_no_writes pm_goal_161 initPM 1970 _ _
      (List.take_append_drop 11 _).symm (by decide)
  have hR : ∀ t : Tid, t = 1947 ∨ t = 1951 →
      denoteGraph pm_goal_161 initPM t
        = denoteGraph {pm_goal_161 with nodes := pm_goal_161.nodes.take 10} initPM t := by
    rintro t (rfl | rfl) <;>
      exact denoteGraph_tid_eq_of_suffix_no_writes pm_goal_161 initPM _ _ _
        (List.take_append_drop 10 _).symm (by decide)
  rw [hL, hR 1947 (by tauto), hR 1951 (by tauto)]
