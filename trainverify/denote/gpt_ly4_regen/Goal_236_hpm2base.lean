...
theorem hpm2base_236 (initPM : Store) :
    (denoteGraph pm_goal_236 initPM) 3089 = allToAllPrimWithDims 4 2
  ...
  have hL : denoteGraph pm_goal_236 initPM 3089
          = denoteGraph {pm_goal_236 with nodes := pm_goal_236.nodes.take 11} initPM 3089 :=
    denoteGraph_tid_eq_of_suffix_no_writes pm_goal_236 initPM 3089 _ _
      (List.take_append_drop 11 _).symm (by decide)
  have hR : ∀ t : Tid, t = 3109 ∨ t = 3111 ∨ t = 3113 ∨ t = 3115 →
      denoteGraph pm_goal_236 initPM t
        = denoteGraph {pm_goal_236 with nodes := pm_goal_236.nodes.take 10} initPM t := by
