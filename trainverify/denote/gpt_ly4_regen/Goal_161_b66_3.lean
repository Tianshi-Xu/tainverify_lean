...
theorem b66_3_161 (initPM : Store) :
    (denoteGraph pm_goal_161 initPM) 1972 =
      (bw_matmul ((denoteGraph pm_goal_161 initPM) 789) ((denoteGraph pm_goal_161 initPM) 1948)
        ((denoteGraph pm_goal_161 initPM) 1952)).2 := by
  have hL : denoteGraph pm_goal_161 initPM 1972
          = denoteGraph {pm_goal_161 with nodes := pm_goal_161.nodes.take 12} initPM 1972 :=
    denoteGraph_tid_eq_of_suffix_no_writes pm_goal_161 initPM 1972 _ _
      (List.take_append_drop 12 _).symm (by decide)
  have hR : ∀ t : Tid, t = 1948 ∨ t = 1952 →
      denoteGraph pm_goal_161 initPM t
        = denoteGraph {pm_goal_161 with nodes := pm_goal_161.nodes.take 11} initPM t := by
    rintro t (rfl | rfl) <;>
      exact denoteGraph_tid_eq_of_suffix_no_writes pm_goal_161 initPM _ _ _
        (List.take_append_drop 11 _).symm (by decide)
  rw [hL, hR 1948 (by tauto), hR 1952 (by tauto)]
