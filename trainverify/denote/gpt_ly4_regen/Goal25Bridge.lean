...
theorem goal_25_cut_to_full (h : goal_25_stmt_cut) : goal_25_stmt := by
  ...
  have h594_pmsh : (Spm 594).shape = [32] := by
    ...
    -- h : Ssm 594 = Spm 594  (both as denoteGraph forms)
    show (denoteGraph pm initPM 594).shape = [32]
    rw [← h]; exact h594_smsh
  have h595_pmsh : (Spm 595).shape = [32] := by
    have h := h595_full.2.2
    simp only [initGoal_595, List.map] at h
    rw [reconstructWithDim_singleton] at h
    show (denoteGraph pm initPM 595).shape = [32]
  ...
  have hInitCut : InitGoalsHold pm_goal_25.numRanks goal_25_cut_initGoals Ssm Spm := by
    rw [hnr]; intro g hg
    simp only [goal_25_cut_initGoals, goal_25_prereqs, List.mem_append] at hg
    rcases hg with hg | hg
    · exact hinitC g hg
    · simp only [List.mem_cons, List.not_mem_nil, or_false] at hg
      rcases hg with rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl
      · exact hg2
      · exact hg3
      · exact hg4
      · exact hg5
...
theorem goal_25_intermediate (initSM initPM : Store)
  ...
  simpa [InitGoalHolds, goal_25] using this
