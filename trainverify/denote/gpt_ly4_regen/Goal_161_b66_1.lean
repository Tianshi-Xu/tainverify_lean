...
theorem b66_1_161 (initPM : Store) :
    (denoteGraph pm_goal_161 initPM) 1968 =
      (bw_matmul ((denoteGraph pm_goal_161 initPM) 789) ((denoteGraph pm_goal_161 initPM) 1946)
        ((denoteGraph pm_goal_161 initPM) 1950)).2 := by
  have hL : denoteGraph pm_goal_161 initPM 1968
          = denoteGraph {pm_goal_161 with nodes := pm_goal_161.nodes.take 10} initPM 1968 :=
    denoteGraph_tid_eq_of_suffix_no_writes pm_goal_161 initPM 1968 _ _
      (List.take_append_drop 10 _).symm (by decide)
  have hR : ∀ t : Tid, t = 1946 ∨ t = 1950 →
      denoteGraph pm_goal_161 initPM t
        = denoteGraph {pm_goal_161 with nodes := pm_goal_161.nodes.take 9} initPM t := by
    rintro t (rfl | rfl) <;>
      exact denoteGraph_tid_eq_of_suffix_no_writes pm_goal_161 initPM _ _ _
        (List.take_append_drop 9 _).symm (by decide)
  rw [hL, hR 1946 (by tauto), hR 1950 (by tauto)]
