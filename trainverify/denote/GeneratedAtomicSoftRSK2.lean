...
def pmGraph : GraphDecl := { numRanks := 2, nodes := [{ rank := 0, op := "OpName.BW_softmax", ins := [100, 200], outs := [300], params := [3] }, { rank := 0, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [700], params := [2] }, { rank := 1, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [701], params := [2] }, { rank := 1, op := "OpName.BW_softmax", ins := [101, 201], outs := [301], params := [3] }] }


private def fg : RelationFact :=
  .sharded 10 [100, 101] 2 [2, 3, 10, 7] [2, 3, 5, 7]

private def fx : RelationFact :=
  .sharded 20 [200, 201] 2 [2, 3, 10, 7] [2, 3, 5, 7]

private def fo : RelationFact :=
  .sharded 30 [300, 301] 2 [2, 3, 10, 7] [2, 3, 5, 7]

private def fw : RelationFact :=
  .reduction 50 [600, 601] [2, 3, 10, 7]

private def fj : RelationFact :=
  .sharded 50 [700, 701] 2 [2, 3, 10, 7] [2, 3, 5, 7]
...
private def segment_000000_pm_nodes : List NodeDecl := [{ rank := 0, op := "OpName.BW_softmax", ins := [100, 200], outs := [300], params := [3] }, { rank := 0, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [700], params := [2] }, { rank := 1, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [701], params := [2] }, { rank := 1, op := "OpName.BW_softmax", ins := [101, 201], outs := [301], params := [3] }]
...
private theorem segment_000000_hPmWriter1 (pmStore : Store) :
  ...
  have hfinal : (segment_000000_pm_final pmStore) = segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore := by
    unfold segment_000000_pm_final
    rfl
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 3) ++ [{ rank := 1, op := "OpName.BW_softmax", ins := [101, 201], outs := [301], params := [3] }] ++ (segment_000000_pm_nodes.drop 4) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 301 = bw_softmax (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 101) (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 201) := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
  ...
  have hout_read_0 : ((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 101 = (segment_000000_pm_final pmStore) 101 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 3) ({ rank := 1, op := "OpName.BW_softmax", ins := [101, 201], outs := [301], params := [3] } :: (segment_000000_pm_nodes.drop 4)) 101
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 201 = (segment_000000_pm_final pmStore) 201 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 3) ({ rank := 1, op := "OpName.BW_softmax", ins := [101, 201], outs := [301], params := [3] } :: (segment_000000_pm_nodes.drop 4)) 201
      (by native_decide) (by native_decide)
  have hout : (segment_000000_pm_final pmStore) 301 = bw_softmax ((segment_000000_pm_final pmStore) 101) ((segment_000000_pm_final pmStore) 201) := by
    calc
      _ = bw_softmax (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 101) (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 201) := hout_prefix
      _ = bw_softmax ((segment_000000_pm_final pmStore) 101) ((segment_000000_pm_final pmStore) 201) := by rw [hout_read_0, hout_read_1]
  exact hout

private theorem segment_000000_scatter_writer0 (pmStore : Store) : (segment_000000_pm_final pmStore) 700 = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 0 [(segment_000000_pm_final pmStore) 600, (segment_000000_pm_final pmStore) 601] := by
  ...
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 1) ++ [{ rank := 0, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [700], params := [2] }] ++ (segment_000000_pm_nodes.drop 2) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 700 = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 0 [((segment_000000_pm_nodes.take 1)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 600, ((segment_000000_pm_nodes.take 1)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 601] := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 1) (segment_000000_pm_nodes.drop 2)
      { rank := 0, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [700], params := [2] } 700
      (fun t => reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 0 [t 600, t 601]) (by
        intro t
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_reduceScatterPrim_out SyntheticBWSoftmaxReduceScatter.pmGraph t 0 2 [600, 601] 700
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 1)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 600 = (segment_000000_pm_final pmStore) 600 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 1) ({ rank := 0, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [700], params := [2] } :: (segment_000000_pm_nodes.drop 2)) 600
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 1)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 601 = (segment_000000_pm_final pmStore) 601 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 1) ({ rank := 0, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [700], params := [2] } :: (segment_000000_pm_nodes.drop 2)) 601
      (by native_decide) (by native_decide)
  have hout : (segment_000000_pm_final pmStore) 700 = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 0 [(segment_000000_pm_final pmStore) 600, (segment_000000_pm_final pmStore) 601] := by
    calc
      _ = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 0 [((segment_000000_pm_nodes.take 1)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 600, ((segment_000000_pm_nodes.take 1)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 601] := hout_prefix
      _ = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 0 [(segment_000000_pm_final pmStore) 600, (segment_000000_pm_final pmStore) 601] := by rw [hout_read_0, hout_read_1]
  exact hout

private theorem segment_000000_scatter_writer1 (pmStore : Store) : (segment_000000_pm_final pmStore) 701 = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 1 [(segment_000000_pm_final pmStore) 600, (segment_000000_pm_final pmStore) 601] := by
  have hfinal : (segment_000000_pm_final pmStore) = segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore := by unfold segment_000000_pm_final; rfl
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 2) ++ [{ rank := 1, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [701], params := [2] }] ++ (segment_000000_pm_nodes.drop 3) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 701 = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 1 [((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 600, ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 601] := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 2) (segment_000000_pm_nodes.drop 3)
      { rank := 1, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [701], params := [2] } 701
      (fun t => reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 1 [t 600, t 601]) (by
        intro t
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_reduceScatterPrim_out SyntheticBWSoftmaxReduceScatter.pmGraph t 1 2 [600, 601] 701
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 600 = (segment_000000_pm_final pmStore) 600 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 2) ({ rank := 1, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [701], params := [2] } :: (segment_000000_pm_nodes.drop 3)) 600
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 601 = (segment_000000_pm_final pmStore) 601 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWSoftmaxReduceScatter.pmGraph pmStore
      (segment_000000_pm_nodes.take 2) ({ rank := 1, op := "OpName.ReduceScatterPrim", ins := [600, 601], outs := [701], params := [2] } :: (segment_000000_pm_nodes.drop 3)) 601
      (by native_decide) (by native_decide)
  have hout : (segment_000000_pm_final pmStore) 701 = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 1 [(segment_000000_pm_final pmStore) 600, (segment_000000_pm_final pmStore) 601] := by
    calc
      _ = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 1 [((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 600, ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful SyntheticBWSoftmaxReduceScatter.pmGraph) pmStore 601] := hout_prefix
      _ = reduceScatterPrimDimN 2 SyntheticBWSoftmaxReduceScatter.pmGraph.numRanks 1 [(segment_000000_pm_final pmStore) 600, (segment_000000_pm_final pmStore) 601] := by rw [hout_read_0, hout_read_1]
  exact hout
...
private theorem segment_000000_sound (smStore pmStore : Store)
    ...
    change ShardedRel (smFinal 10) [pmFinal 100, pmFinal 101] 2 [2, 3, 10, 7] [2, 3, 5, 7] at hg
    have hx : fx.Holds smFinal pmFinal := hframe _ (by native_decide)
    change ShardedRel (smFinal 20) [pmFinal 200, pmFinal 201] 2 [2, 3, 10, 7] [2, 3, 5, 7] at hx
    have hgValue : smFinal 10 = allGatherPrimDimN 2 2 0 [pmFinal 100, pmFinal 101] := by
      simpa only [List.length_cons, List.length_nil] using hg.full_value
    have hxValue : smFinal 20 = allGatherPrimDimN 2 2 0 [pmFinal 200, pmFinal 201] := by
    ...
    have hOutShape1 : (pmFinal 301).shape = [2, 3, 5, 7] := by
      rw [hPmWriter1]
      exact bw_softmax_shape_g199 _ _ [2, 3, 5] 7 (hx.shard_shapes _ (by simp))
    have hcomm := TrainVerify.Denote.bw_softmax_allGatherPrimDimN_dim2_rank4 [pmFinal 100, pmFinal 101] [pmFinal 200, pmFinal 201] 2 2 3 5 7
      (by omega) (by omega) (by omega) (by omega) (by omega)
      (by simp) (by simp) hg.shard_shapes hx.shard_shapes
    have hOutValue : smFinal 30 = allGatherPrimDimN 2 2 0 [pmFinal 300, pmFinal 301] := by
      rw [hSmWriter, hgValue, hxValue, hcomm]
      simp only [List.zipWith]
      rw [← hPmWriter0, ← hPmWriter1]
    have hOutValueList : smFinal 30 = allGatherPrimDimN 2 [pmFinal 300, pmFinal 301].length 0 [pmFinal 300, pmFinal 301] := by
      simpa only [List.length_cons, List.length_nil] using hOutValue
    have hFullShape : (smFinal 30).shape = [2, 3, 10, 7] := by
      rw [hSmWriter]
      exact bw_softmax_shape_g199 _ _ [2, 3, 10] 7 hx.full_shape
    have hout : fo.Holds smFinal pmFinal := by
      change ShardedRel (smFinal 30) [pmFinal 300, pmFinal 301] 2 [2, 3, 10, 7] [2, 3, 5, 7]
      refine {
    ...
    change ReductionRel (smFinal 50) [pmFinal 600, pmFinal 601] [2, 3, 10, 7] at hred
    have hw0 := segment_000000_scatter_writer0 pmStore
    change pmFinal 700 = reduceScatterPrimDimN 2 2 0 [pmFinal 600, pmFinal 601] at hw0
    have hr0 : smFinal 50 = allReducePrim 2 0 [pmFinal 600, pmFinal 601] := by simpa only [List.length_cons, List.length_nil, allReducePrim] using hred.full_value
    have hc0 : pmFinal 700 = chunkPrimDimN 2 2 0 (smFinal 50) := by rw [hw0]; unfold reduceScatterPrimDimN; rw [← hr0]
    have hw1 := segment_000000_scatter_writer1 pmStore
    change pmFinal 701 = reduceScatterPrimDimN 2 2 1 [pmFinal 600, pmFinal 601] at hw1
    have hr1 : smFinal 50 = allReducePrim 2 1 [pmFinal 600, pmFinal 601] := by simpa only [List.length_cons, List.length_nil, allReducePrim] using hred.full_value
    have hc1 : pmFinal 701 = chunkPrimDimN 2 2 1 (smFinal 50) := by rw [hw1]; unfold reduceScatterPrimDimN; rw [← hr1]
    have hordered : [pmFinal 700, pmFinal 701] = List.ofFn (fun r : Fin 2 => chunkPrimDimN 2 2 r.1 (smFinal 50)) := by
      change [pmFinal 700, pmFinal 701] = [chunkPrimDimN 2 2 0 (smFinal 50), chunkPrimDimN 2 2 1 (smFinal 50)]
      rw [hc0, hc1]
    have hvalue : smFinal 50 = allGatherPrimDimN 2 [pmFinal 700, pmFinal 701].length 0 [pmFinal 700, pmFinal 701] := by
      rw [hordered]
      simp only [List.length_ofFn]
      symm
      exact allGatherPrimDimN_chunks_ofFn 2 2 (smFinal 50) (by omega) (by rw [hred.full_shape]; native_decide) (by rw [hred.full_shape]; native_decide)
    have hs0 : (pmFinal 700).shape = [2, 3, 5, 7] := by
      rw [hc0, chunkPrimDimN_shape 2 2 0 (smFinal 50) [2, 3, 10, 7] hred.full_shape (by omega)]
      native_decide
    have hs1 : (pmFinal 701).shape = [2, 3, 5, 7] := by
      rw [hc1, chunkPrimDimN_shape 2 2 1 (smFinal 50) [2, 3, 10, 7] hred.full_shape (by omega)]
      native_decide
    have houtScatter : fj.Holds smFinal pmFinal := by
      change ShardedRel (smFinal 50) [pmFinal 700, pmFinal 701] 2 [2, 3, 10, 7] [2, 3, 5, 7]
