...
def pmGraph : GraphDecl := { numRanks := 3, nodes := [{ rank := 0, op := "OpName.BW_softmax", ins := [1000, 2000], outs := [3000] }, { rank := 0, op := "OpName.BW_transpose", ins := [4000, 99], outs := [5000], params := [1, 2] }, { rank := 1, op := "OpName.BW_softmax", ins := [1001, 2001], outs := [3001] }, { rank := 1, op := "OpName.BW_transpose", ins := [4001, 99], outs := [5001], params := [1, 2] }, { rank := 2, op := "OpName.BW_softmax", ins := [1002, 2002], outs := [3002] }, { rank := 2, op := "OpName.BW_transpose", ins := [4002, 99], outs := [5002], params := [1, 2] }] }
...
def fti : RelationFact := .sharded 400 [4000, 4001, 4002] 2 [2, 3, 15, 7] [2, 3, 5, 7]
def fo : RelationFact := .sharded 300 [3000, 3001, 3002] 1 [2, 9, 5, 7] [2, 3, 5, 7]
def fto : RelationFact := .sharded 500 [5000, 5001, 5002] 1 [2, 15, 3, 7] [2, 5, 3, 7]
...
private theorem segment_000000_sound (smStore pmStore : Store) (hstate : state_000000.Holds smStore pmStore) : state_000001.Holds (segment_000000_sm_final smStore) (segment_000000_pm_final pmStore) := by
    ...
    change ShardedRel (smFinal 400) [pmFinal 4000, pmFinal 4001, pmFinal 4002] 2 [2, 3, 5 * [pmFinal 4000, pmFinal 4001, pmFinal 4002].length, 7] [2, 3, 5, 7] at hti
    ...
    change ShardedRel (smFinal 200) [pmFinal 2000, pmFinal 2001, pmFinal 2002] 1 [2, 9, 5, 7] [2, 3, 5, 7] at hy
    ...
    have houtT : fto.Holds smFinal pmFinal := by
      change ShardedRel (smFinal 500) [pmFinal 5000, pmFinal 5001, pmFinal 5002] 1 [2, 15, 3, 7] [2, 5, 3, 7]
      rw [hTsm, hTpm0, hTpm1, hTpm2]
      simpa using htRaw
    have hgValue : smFinal 100 = allGatherPrimDimN 1 3 0 [pmFinal 1000, pmFinal 1001, pmFinal 1002] := by simpa only [List.length_cons, List.length_nil] using hg.full_value
    have hyValue : smFinal 200 = allGatherPrimDimN 1 3 0 [pmFinal 2000, pmFinal 2001, pmFinal 2002] := by simpa only [List.length_cons, List.length_nil] using hy.full_value
    ...
    have hSValue : smFinal 300 = allGatherPrimDimN 1 3 0 [pmFinal 3000, pmFinal 3001, pmFinal 3002] := by
      rw [hSsm, hgValue, hyValue, hcomm]
      simp only [List.zipWith]
      rw [← hSpm0, ← hSpm1, ← hSpm2]
    have hSValueList : smFinal 300 = allGatherPrimDimN 1 [pmFinal 3000, pmFinal 3001, pmFinal 3002].length 0 [pmFinal 3000, pmFinal 3001, pmFinal 3002] := by simpa only [List.length_cons, List.length_nil] using hSValue
    have hSFullShape : (smFinal 300).shape = [2, 9, 5, 7] := by rw [hSsm]; exact bw_softmax_shape_g199 _ _ [2, 9, 5] 7 hy.full_shape
    have hSShapes : ∀ z ∈ [pmFinal 3000, pmFinal 3001, pmFinal 3002], z.shape = [2, 3, 5, 7] := by
      simp only [List.forall_mem_cons]
      exact ⟨hSShape0, hSShape1, hSShape2, List.forall_mem_nil _⟩
