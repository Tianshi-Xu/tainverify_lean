...
def pmGraph : GraphDecl := { numRanks := 2, nodes := [{ rank := 0, op := "OpName.BW_softmax", ins := [1000, 2000], outs := [3000] }, { rank := 0, op := "OpName.BW_transpose", ins := [4000, 99], outs := [5000], params := [1, 2] }, { rank := 1, op := "OpName.BW_softmax", ins := [1001, 2001], outs := [3001] }, { rank := 1, op := "OpName.BW_transpose", ins := [4001, 99], outs := [5001], params := [1, 2] }] }
def fg : RelationFact := .sharded 100 [1000, 1001] 1 [2, 6, 5, 7] [2, 3, 5, 7]
def fx : RelationFact := .sharded 200 [2000, 2001] 1 [2, 6, 5, 7] [2, 3, 5, 7]
def fti : RelationFact := .sharded 400 [4000, 4001] 2 [2, 3, 10, 7] [2, 3, 5, 7]
def fo : RelationFact := .sharded 300 [3000, 3001] 1 [2, 6, 5, 7] [2, 3, 5, 7]
def fto : RelationFact := .sharded 500 [5000, 5001] 1 [2, 10, 3, 7] [2, 5, 3, 7]
...
private def segment_000000_pm_nodes : List NodeDecl := [{ rank := 0, op := "OpName.BW_softmax", ins := [1000, 2000], outs := [3000] }, { rank := 0, op := "OpName.BW_transpose", ins := [4000, 99], outs := [5000], params := [1, 2] }, { rank := 1, op := "OpName.BW_softmax", ins := [1001, 2001], outs := [3001] }, { rank := 1, op := "OpName.BW_transpose", ins := [4001, 99], outs := [5001], params := [1, 2] }]
...
private theorem segment_000000_sound (smStore pmStore : Store) (hstate : state_000000.Holds smStore pmStore) : state_000001.Holds (segment_000000_sm_final smStore) (segment_000000_pm_final pmStore) := by
    ...
    change ShardedRel (smFinal 400) [pmFinal 4000, pmFinal 4001] 2 [2, 3, 5 * [pmFinal 4000, pmFinal 4001].length, 7] [2, 3, 5, 7] at hti
    have hg : fg.Holds smFinal pmFinal := hframe _ (by native_decide)
    change ShardedRel (smFinal 100) [pmFinal 1000, pmFinal 1001] 1 [2, 6, 5, 7] [2, 3, 5, 7] at hg
    have hy : fx.Holds smFinal pmFinal := hframe _ (by native_decide)
    change ShardedRel (smFinal 200) [pmFinal 2000, pmFinal 2001] 1 [2, 6, 5, 7] [2, 3, 5, 7] at hy
    ...
    have houtT : fto.Holds smFinal pmFinal := by
      change ShardedRel (smFinal 500) [pmFinal 5000, pmFinal 5001] 1 [2, 10, 3, 7] [2, 5, 3, 7]
      rw [hTsm, hTpm0, hTpm1]
      simpa using htRaw
    have hgValue : smFinal 100 = allGatherPrimDimN 1 2 0 [pmFinal 1000, pmFinal 1001] := by simpa only [List.length_cons, List.length_nil] using hg.full_value
    have hyValue : smFinal 200 = allGatherPrimDimN 1 2 0 [pmFinal 2000, pmFinal 2001] := by simpa only [List.length_cons, List.length_nil] using hy.full_value
    have hcomm := TrainVerify.Denote.bw_softmax_allGatherPrimDimN_dim1_rank4 [pmFinal 1000, pmFinal 1001] [pmFinal 2000, pmFinal 2001] 2 2 3 5 7
      (by omega) (by omega) (by omega) (by omega) (by omega)
      (by simp) (by simp) hg.shard_shapes hy.shard_shapes
    have hSValue : smFinal 300 = allGatherPrimDimN 1 2 0 [pmFinal 3000, pmFinal 3001] := by
      rw [hSsm, hgValue, hyValue, hcomm]
      simp only [List.zipWith]
      rw [← hSpm0, ← hSpm1]
    have hSValueList : smFinal 300 = allGatherPrimDimN 1 [pmFinal 3000, pmFinal 3001].length 0 [pmFinal 3000, pmFinal 3001] := by simpa only [List.length_cons, List.length_nil] using hSValue
    have hSFullShape : (smFinal 300).shape = [2, 6, 5, 7] := by rw [hSsm]; exact bw_softmax_shape_g199 _ _ [2, 6, 5] 7 hy.full_shape
    have hSShapes : ∀ z ∈ [pmFinal 3000, pmFinal 3001], z.shape = [2, 3, 5, 7] := by
      simp only [List.forall_mem_cons]
      exact ⟨hSShape0, hSShape1, List.forall_mem_nil _⟩
    have houtS : fo.Holds smFinal pmFinal := by
      change ShardedRel (smFinal 300) [pmFinal 3000, pmFinal 3001] 1 [2, 6, 5, 7] [2, 3, 5, 7]
