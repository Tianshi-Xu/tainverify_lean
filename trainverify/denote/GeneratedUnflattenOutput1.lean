...
def sm : GraphDecl := { numRanks := 1, nodes := [{ rank := 0, op := "OpName.FW_linear", ins := [20000, 20002], outs := [20001] }, { rank := 0, op := "OpName.FW_linear", ins := [30000, 30002], outs := [30001] }, { rank := 0, op := "OpName.FW_view", ins := [10000], outs := [10001], params := [2, 10, 3, 7] }] }
...
private def y0 : RelationFact :=
  .sharded 10001 [10200, 10201] 1 [2, 10, 3, 7] [2, 5, 3, 7]
...
private def segment_000000_smNodes : List NodeDecl := [{ rank := 0, op := "OpName.FW_linear", ins := [20000, 20002], outs := [20001] }, { rank := 0, op := "OpName.FW_linear", ins := [30000, 30002], outs := [30001] }, { rank := 0, op := "OpName.FW_view", ins := [10000], outs := [10001], params := [2, 10, 3, 7] }]
...
private theorem segment_000000_t2_hSmWriter(smStore:Store):(segment_000000_smFinal smStore) 10001=fw_view [2, 10, 3, 7] ((segment_000000_smFinal smStore) 10000):=by
  ...
  have hout_nodes : segment_000000_smNodes = (segment_000000_smNodes.take 2) ++ [{ rank := 0, op := "OpName.FW_view", ins := [10000], outs := [10001], params := [2, 10, 3, 7] }] ++ (segment_000000_smNodes.drop 3) := by
    native_decide
  have hout_prefix : (segment_000000_smFinal smStore) 10001 = fw_view [2, 10, 3, 7] (((segment_000000_smNodes.take 2)).foldl (applyNodeDistributedFaithful UnflattenOutputK2N1M2Graph.sm) smStore 10000) := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer UnflattenOutputK2N1M2Graph.sm smStore
      (segment_000000_smNodes.take 2) (segment_000000_smNodes.drop 3)
      { rank := 0, op := "OpName.FW_view", ins := [10000], outs := [10001], params := [2, 10, 3, 7] } 10001
      (fun t => fw_view [2, 10, 3, 7] (t 10000)) (by
        intro t
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle:=by native_decide) (hunshuffle:=by native_decide) (hattn:=by native_decide)]
        simp [applyNodeDistributed,applyNodeRingAttn]
        exact applyNode_fw_view_out UnflattenOutputK2N1M2Graph.sm t 0 2 [10, 3, 7] 10000 10001
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_smNodes.take 2)).foldl (applyNodeDistributedFaithful UnflattenOutputK2N1M2Graph.sm) smStore 10000 = (segment_000000_smFinal smStore) 10000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final UnflattenOutputK2N1M2Graph.sm smStore
      (segment_000000_smNodes.take 2) ({ rank := 0, op := "OpName.FW_view", ins := [10000], outs := [10001], params := [2, 10, 3, 7] } :: (segment_000000_smNodes.drop 3)) 10000
      (by native_decide) (by native_decide)
  have hout : (segment_000000_smFinal smStore) 10001 = fw_view [2, 10, 3, 7] ((segment_000000_smFinal smStore) 10000) := by
    calc
...
private theorem segment_000000_t2_out(smStore pmStore:Store)(hframe:state_before.Holds (segment_000000_smFinal smStore) (segment_000000_pmFinal pmStore)):
    ...
    have hSm:smFinal 10001=fw_view [2, 10, 3, 7] (smFinal 10000):=segment_000000_t2_hSmWriter smStore
    ...
    have hvalue:smFinal 10001=allGatherPrimDimN 1 2 0 [pmFinal 10200, pmFinal 10201]:=by
      rw [hSm,hxV,hcomm]
      simp only [List.map]
      rw [← hPm0, ← hPm1]
    have hout:y0.Holds smFinal pmFinal:=by
      change ShardedRel (smFinal 10001) [pmFinal 10200, pmFinal 10201] 1 [2, 10, 3, 7] [2, 5, 3, 7]
