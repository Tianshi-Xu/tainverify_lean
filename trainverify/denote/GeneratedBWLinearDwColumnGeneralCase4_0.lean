...
private def segment_000000_sm_final (store : Store) : Store :=
  segment_000000_sm_nodes.foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) store
private def segment_000000_pm_final (store : Store) : Store :=
  segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) store
...
private theorem segment_000000_hSmWriter (smStore : Store) :
  ...
  have hfinal : (segment_000000_sm_final smStore) = segment_000000_sm_nodes.foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore := by
  ...
  have hout_nodes : segment_000000_sm_nodes = (segment_000000_sm_nodes.take 0) ++ [{ rank := 0, op := "OpName.BW_linear", ins := [100, 200, 300], outs := [400, 401] }] ++ (segment_000000_sm_nodes.drop 1) := by
    native_decide
  have hout_prefix : (segment_000000_sm_final smStore) 401 = (bw_linear (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 100) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 200) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 300)).2 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer ColumnDwCase4_0.smGraph smStore
      ...
      (fun t => (bw_linear (t 100) (t 200) (t 300)).2) (by
        ...
        exact applyNode_bw_linear_snd_out ColumnDwCase4_0.smGraph t 0 100 200 300 400 401 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 100 = (segment_000000_sm_final smStore) 100 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.smGraph smStore
      ...
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 200 = (segment_000000_sm_final smStore) 200 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.smGraph smStore
      ...
      (by native_decide) (by native_decide)
  have hout_read_2 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 300 = (segment_000000_sm_final smStore) 300 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.smGraph smStore
  ...
  have hout : (segment_000000_sm_final smStore) 401 = (bw_linear ((segment_000000_sm_final smStore) 100) ((segment_000000_sm_final smStore) 200) ((segment_000000_sm_final smStore) 300)).2 := by
    calc
      _ = (bw_linear (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 100) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 200) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 300)).2 := hout_prefix
...
private theorem segment_000000_hPmWriter0 (pmStore : Store) :
  ...
  have hfinal : (segment_000000_pm_final pmStore) = segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore := by
  ...
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 0) ++ [{ rank := 0, op := "OpName.BW_linear", ins := [1000, 2000, 3000], outs := [4000, 5000] }] ++ (segment_000000_pm_nodes.drop 1) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 5000 = (bw_linear (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 2000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 3000)).2 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer ColumnDwCase4_0.pmGraph pmStore
      ...
      (fun t => (bw_linear (t 1000) (t 2000) (t 3000)).2) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_bw_linear_snd_out ColumnDwCase4_0.pmGraph t 0 1000 2000 3000 4000 5000 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000 = (segment_000000_pm_final pmStore) 1000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
      ...
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 2000 = (segment_000000_pm_final pmStore) 2000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
      ...
      (by native_decide) (by native_decide)
  have hout_read_2 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 3000 = (segment_000000_pm_final pmStore) 3000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
  ...
  have hout : (segment_000000_pm_final pmStore) 5000 = (bw_linear ((segment_000000_pm_final pmStore) 1000) ((segment_000000_pm_final pmStore) 2000) ((segment_000000_pm_final pmStore) 3000)).2 := by
    calc
      _ = (bw_linear (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 2000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 3000)).2 := hout_prefix
...
private theorem segment_000000_hPmWriter1 (pmStore : Store) :
  ...
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 2) ++ [{ rank := 1, op := "OpName.BW_linear", ins := [1000, 2001, 3001], outs := [4001, 5001] }] ++ (segment_000000_pm_nodes.drop 3) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 5001 = (bw_linear (((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000) (((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 2001) (((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 3001)).2 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer ColumnDwCase4_0.pmGraph pmStore
      ...
      (fun t => (bw_linear (t 1000) (t 2001) (t 3001)).2) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_bw_linear_snd_out ColumnDwCase4_0.pmGraph t 1 1000 2001 3001 4001 5001 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000 = (segment_000000_pm_final pmStore) 1000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
      ...
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 2001 = (segment_000000_pm_final pmStore) 2001 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
      ...
      (by native_decide) (by native_decide)
  have hout_read_2 : ((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 3001 = (segment_000000_pm_final pmStore) 3001 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
  ...
  have hout : (segment_000000_pm_final pmStore) 5001 = (bw_linear ((segment_000000_pm_final pmStore) 1000) ((segment_000000_pm_final pmStore) 2001) ((segment_000000_pm_final pmStore) 3001)).2 := by
    calc
      _ = (bw_linear (((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000) (((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 2001) (((segment_000000_pm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 3001)).2 := hout_prefix
...
private theorem segment_000000_hViewSm(smStore:Store):(segment_000000_sm_final smStore) 110=fw_view [1, 16, 64] ((segment_000000_sm_final smStore) 100):=by
  have hfinal:(segment_000000_sm_final smStore)=segment_000000_sm_nodes.foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore:=by unfold segment_000000_sm_final;rfl
  have hout_nodes : segment_000000_sm_nodes = (segment_000000_sm_nodes.take 2) ++ [{ rank := 0, op := "OpName.BW_view", ins := [100, 100], outs := [110], params := [1, 16, 64] }] ++ (segment_000000_sm_nodes.drop 3) := by
    native_decide
  have hout_prefix : (segment_000000_sm_final smStore) 110 = fw_view [1, 16, 64] (((segment_000000_sm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 100) := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer ColumnDwCase4_0.smGraph smStore
      ...
      (fun t => fw_view [1, 16, 64] (t 100)) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle:=by native_decide) (hunshuffle:=by native_decide) (hattn:=by native_decide)]
        simp [applyNodeDistributed,applyNodeRingAttn]
        exact applyNode_bw_view_out ColumnDwCase4_0.smGraph t 0 1 [16, 64] 100 100 110
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_sm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 100 = (segment_000000_sm_final smStore) 100 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.smGraph smStore
  ...
  have hout : (segment_000000_sm_final smStore) 110 = fw_view [1, 16, 64] ((segment_000000_sm_final smStore) 100) := by
    calc
      _ = fw_view [1, 16, 64] (((segment_000000_sm_nodes.take 2)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.smGraph) smStore 100) := hout_prefix
...
private theorem segment_000000_hViewPm(pmStore:Store):(segment_000000_pm_final pmStore) 1100=fw_view [1, 16, 64] ((segment_000000_pm_final pmStore) 1000):=by
  have hfinal:(segment_000000_pm_final pmStore)=segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore:=by unfold segment_000000_pm_final;rfl
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 3) ++ [{ rank := 0, op := "OpName.BW_view", ins := [1000, 1000], outs := [1100], params := [1, 16, 64] }] ++ (segment_000000_pm_nodes.drop 4) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 1100 = fw_view [1, 16, 64] (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000) := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer ColumnDwCase4_0.pmGraph pmStore
      ...
      (fun t => fw_view [1, 16, 64] (t 1000)) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle:=by native_decide) (hunshuffle:=by native_decide) (hattn:=by native_decide)]
        simp [applyNodeDistributed,applyNodeRingAttn]
        exact applyNode_bw_view_out ColumnDwCase4_0.pmGraph t 0 1 [16, 64] 1000 1000 1100
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000 = (segment_000000_pm_final pmStore) 1000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final ColumnDwCase4_0.pmGraph pmStore
  ...
  have hout : (segment_000000_pm_final pmStore) 1100 = fw_view [1, 16, 64] ((segment_000000_pm_final pmStore) 1000) := by
    calc
      _ = fw_view [1, 16, 64] (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful ColumnDwCase4_0.pmGraph) pmStore 1000) := hout_prefix
...
private theorem segment_000000_sound (smStore pmStore : Store)
    ...
    have hOutShape0 : (pmFinal 5000).shape = [64, 32] := by
      rw [hPmWriter0]
      exact segment_000000_dw_shape _ _ _ 64 32 hg.2.2 hxShape0 hwShape0
    ...
    have hOutShape1 : (pmFinal 5001).shape = [64, 32] := by
      rw [hPmWriter1]
      exact segment_000000_dw_shape _ _ _ 64 32 hg.2.2 hxShape1 hwShape1
    have hcomm := TrainVerify.Denote.bw_linear_dw_column_allGather_rank3 2 1 16 64 32
...
private def segment_000000 :
    ClosedDepSegmentCertificate ColumnDwCase4_0.smGraph ColumnDwCase4_0.pmGraph state_000000 state_000001 where
