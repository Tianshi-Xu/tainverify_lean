...
namespace TrainVerify.Denote.SourceRank4ReverseExchange
...
private theorem gather3_shape (T B S H C : Nat) (xs : List Tensor)
    (hT : 0 < T) (hlen : xs.length = T)
    (hxs : ∀ x ∈ xs, x.shape = [B, S, H, C]) :
    (allGatherPrimDimN 3 T 0 xs).shape = [B, S, H, C * T] := by
  rw [allGatherPrimDimN_shape 3 T xs _ (head_shape T xs _ hT hlen hxs)]
  simp [List.set, List.getD]

private theorem destination_shape (T B S H C : Nat) (xs : List Tensor) (dst : Fin T)
    (hT : 0 < T) (hlen : xs.length = T)
    (hxs : ∀ x ∈ xs, x.shape = [B, S * T, H, C]) :
    (AllToAllSourceFaithful.tensor T dst.val 3 1 xs).shape = [B, S, H, C * T] := by
  unfold AllToAllSourceFaithful.tensor
  apply gather3_shape T B S H C _ hT (by rw [List.length_map, hlen])
  intro x hx
  obtain ⟨y, hy, rfl⟩ := List.mem_map.mp hx
  exact chunk1_shape T B S H C dst.val y hT (hxs y hy)

/-- Rank-4 reverse exchange: the faithful axis-1 sender split followed by
axis-3 receiver gather equals an axis-1 chunk of the original axis-3 gather.
The gather-then-chunk equality is proved from the faithful definition. -/
theorem axis3_destination_eq_chunk1
    (T B S H C : Nat) (xs : List Tensor) (dst : Fin T)
    (hT : 0 < T) (hB : 0 < B) (hS : 0 < S) (hH : 0 < H) (hC : 0 < C)
    (hlen : xs.length = T) (hxs : ∀ x ∈ xs, x.shape = [B, S * T, H, C]) :
    AllToAllSourceFaithful.tensor T dst.val 3 1 xs =
      chunkPrimDimN 1 T dst.val (allGatherPrimDimN 3 T 0 xs) := by
  ...
  have hgshape := gather3_shape T B (S * T) H C xs hT hlen hxs
  have hlshape := destination_shape T B S H C xs dst hT hlen hxs
  have hrshape := chunk1_shape T B S H (C * T) dst.val
    (allGatherPrimDimN 3 T 0 xs) hT hgshape
  apply Tensor.ext (by rw [hlshape, hrshape])
  intro idx hidx
  have hbound : idx < B * S * H * (C * T) := by
    rw [hlshape] at hidx
    simpa only [prodShape, List.foldl, Nat.one_mul] using hidx
  obtain ⟨b, s, h, q, hb, hs, hh, hq, hidxEq⟩ :=
    coordinates4 idx B S H (C * T) hS hH (Nat.mul_pos hC hT) hbound
  obtain ⟨r, c, hr, hc, hqEq⟩ := split_index q T C hC
    (by simpa only [Nat.mul_comm] using hq)
  have hcol : r * C + c < C * T := by rw [← hqEq]; exact hq
  have hpre : (b * S + s) * H + h < B * S * H := flat_lt (flat_lt hb hs) hh
  have hseq : dst.val * S + s < S * T := by
    simpa only [Nat.mul_comm] using flat_lt dst.isLt hs
  have hfullpre : (b * (S * T) + (dst.val * S + s)) * H + h <
      B * (S * T) * H := flat_lt (flat_lt hb hseq) hh
  ...
  rw [hidxEq]
  unfold AllToAllSourceFaithful.tensor
  change valAt (allGatherPrimDimN 3 T 0 pieces) _ = _
  rw [allGatherPrimDimN_dim3_4d_valAt pieces T B S H C ((b * S + s) * H + h)
    r c hT hC hpre hr hc hphead, hpget]
  rw [chunk1_valAt T B S H C sender dst b s h c hT hS hH hC hsender hb hs hh hc]
  rw [chunk1_valAt T B S H (C * T) (allGatherPrimDimN 3 T 0 xs)
    dst b s h (r * C + c) hT hS hH (Nat.mul_pos hC hT) hgshape hb hs hh hcol]
  rw [allGatherPrimDimN_dim3_4d_valAt xs T B (S * T) H C
    ((b * (S * T) + (dst.val * S + s)) * H + h) r c
    hT hC hfullpre hr hc hxhead, hget]

/-- Gathering the faithful destinations in destination order on axis 1
reconstructs the original ordered sender gather on axis 3. -/
theorem axis3_exchange
    (T B S H C : Nat) (xs : List Tensor)
    (hT : 0 < T) (hB : 0 < B) (hS : 0 < S) (hH : 0 < H) (hC : 0 < C)
    (hlen : xs.length = T) (hxs : ∀ x ∈ xs, x.shape = [B, S * T, H, C]) :
    allGatherPrimDimN 1 T 0
        (List.ofFn fun dst : Fin T => AllToAllSourceFaithful.tensor T dst.val 3 1 xs) =
      allGatherPrimDimN 3 T 0 xs := by
  have hdest :
      (List.ofFn fun dst : Fin T => AllToAllSourceFaithful.tensor T dst.val 3 1 xs) =
    ...
    funext dst
    exact axis3_destination_eq_chunk1 T B S H C xs dst hT hB hS hH hC hlen hxs
  rw [hdest]
  apply allGatherPrimDimN_chunks_ofFn 1 T (allGatherPrimDimN 3 T 0 xs) hT
  · rw [gather3_shape T B (S * T) H C xs hT hlen hxs]
    change 1 < 4
    decide
  · rw [gather3_shape T B (S * T) H C xs hT hlen hxs]
    simp [List.getD]

/-- Old-run-independent output adapter. Only the predecessor reconstruction is
assumed; faithful ordered outputs determine local shapes and reconstruction.
DP ownership and source-renderer integration are not additional hypotheses. -/
theorem axis3_output_facts
    (D u T B S H C : Nat) (global : Tensor) (xs outputs : List Tensor)
    (hT : 0 < T) (hB : 0 < B) (hS : 0 < S) (hH : 0 < H) (hC : 0 < C)
    (hlen : xs.length = T)
    (hxs : ∀ x ∈ xs, x.shape = [B, S * T, H, C])
    (hglobal : global.shape = [B * D, S * T, H, C * T])
    (hpre : chunkPrimDimN 0 D u global = allGatherPrimDimN 3 T 0 xs)
    (houtputs : outputs = List.ofFn (fun dst : Fin T =>
      AllToAllSourceFaithful.tensor T dst.val 3 1 xs)) :
    global.shape = [B * D, S * T, H, C * T] ∧
    (∀ x ∈ outputs, x.shape = [B, S, H, C * T]) ∧
  ...
  · rw [houtputs]
    exact hpre.trans (axis3_exchange T B S H C xs hT hB hS hH hC hlen hxs).symm

#print axioms axis3_destination_eq_chunk1
#print axioms axis3_exchange
#print axioms axis3_output_facts

end
end TrainVerify.Denote.SourceRank4ReverseExchange
