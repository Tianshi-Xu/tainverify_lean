...
def pmGraph : GraphDecl := { numRanks := 1, nodes := [{ rank := 0, op := "OpName.BW_linear", ins := [1000, 2000, 206], outs := [4000, 5000] }, { rank := 0, op := "OpName.FW_neg", ins := [9000], outs := [9001] }] }
def fact_g : RelationFact := .sharded 100 [1000] 1 [3, 2, 1] [3, 2, 1]
def fact_x : RelationFact := .sharded 200 [2000] 1 [3, 2, 4] [3, 2, 4]
def fact_w : RelationFact := .sharded 206 [206] 0 [1, 4] [1, 4]
def fact_dw : RelationFact := .reduction 401 [5000] [1, 4]
...
private def segment_000000_pm_nodes : List NodeDecl := [{ rank := 0, op := "OpName.BW_linear", ins := [1000, 2000, 206], outs := [4000, 5000] }, { rank := 0, op := "OpName.FW_neg", ins := [9000], outs := [9001] }]
@[irreducible] private def segment_000000_sm_final (s : Store) : Store := segment_000000_sm_nodes.foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) s
@[irreducible] private def segment_000000_pm_final (s : Store) : Store := segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) s
...
private theorem segment_000000_hSmWriter1 (store : Store) : (segment_000000_sm_final store) 401 = (bw_linear ((segment_000000_sm_final store) 100) ((segment_000000_sm_final store) 200) ((segment_000000_sm_final store) 206)).2 := by
  have hfinal : (segment_000000_sm_final store) = segment_000000_sm_nodes.foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store := by unfold segment_000000_sm_final; rfl
  have hout_nodes : segment_000000_sm_nodes = (segment_000000_sm_nodes.take 0) ++ [{ rank := 0, op := "OpName.BW_linear", ins := [100, 200, 206], outs := [400, 401] }] ++ (segment_000000_sm_nodes.drop 1) := by
    native_decide
  have hout_prefix : (segment_000000_sm_final store) 401 = (bw_linear (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 100) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 200) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 206)).2 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SequenceDwCase0_1.smGraph store
      ...
      (fun t => (bw_linear (t 100) (t 200) (t 206)).2) (by
        ...
        exact applyNode_bw_linear_snd_out SequenceDwCase0_1.smGraph t 0 100 200 206 400 401 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 100 = (segment_000000_sm_final store) 100 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.smGraph store
      ...
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 200 = (segment_000000_sm_final store) 200 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.smGraph store
      ...
      (by native_decide) (by native_decide)
  have hout_read_2 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 206 = (segment_000000_sm_final store) 206 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.smGraph store
  ...
  have hout : (segment_000000_sm_final store) 401 = (bw_linear ((segment_000000_sm_final store) 100) ((segment_000000_sm_final store) 200) ((segment_000000_sm_final store) 206)).2 := by
    calc
      _ = (bw_linear (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 100) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 200) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 206)).2 := hout_prefix
...
private theorem segment_000000_hPmWriter1_0 (store : Store) : (segment_000000_pm_final store) 5000 = (bw_linear ((segment_000000_pm_final store) 1000) ((segment_000000_pm_final store) 2000) ((segment_000000_pm_final store) 206)).2 := by
  have hfinal : (segment_000000_pm_final store) = segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store := by unfold segment_000000_pm_final; rfl
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 0) ++ [{ rank := 0, op := "OpName.BW_linear", ins := [1000, 2000, 206], outs := [4000, 5000] }] ++ (segment_000000_pm_nodes.drop 1) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final store) 5000 = (bw_linear (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 1000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 2000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 206)).2 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SequenceDwCase0_1.pmGraph store
      ...
      (fun t => (bw_linear (t 1000) (t 2000) (t 206)).2) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_bw_linear_snd_out SequenceDwCase0_1.pmGraph t 0 1000 2000 206 4000 5000 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 1000 = (segment_000000_pm_final store) 1000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.pmGraph store
      ...
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 2000 = (segment_000000_pm_final store) 2000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.pmGraph store
      ...
      (by native_decide) (by native_decide)
  have hout_read_2 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 206 = (segment_000000_pm_final store) 206 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.pmGraph store
  ...
  have hout : (segment_000000_pm_final store) 5000 = (bw_linear ((segment_000000_pm_final store) 1000) ((segment_000000_pm_final store) 2000) ((segment_000000_pm_final store) 206)).2 := by
    calc
      _ = (bw_linear (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 1000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 2000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 206)).2 := hout_prefix
...
private theorem segment_000000_hSmWriter0 (store : Store) : (segment_000000_sm_final store) 400 = (bw_linear ((segment_000000_sm_final store) 100) ((segment_000000_sm_final store) 200) ((segment_000000_sm_final store) 206)).1 := by
  ...
  have hout_nodes : segment_000000_sm_nodes = (segment_000000_sm_nodes.take 0) ++ [{ rank := 0, op := "OpName.BW_linear", ins := [100, 200, 206], outs := [400, 401] }] ++ (segment_000000_sm_nodes.drop 1) := by
    native_decide
  have hout_prefix : (segment_000000_sm_final store) 400 = (bw_linear (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 100) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 200) (((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 206)).1 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SequenceDwCase0_1.smGraph store
      (segment_000000_sm_nodes.take 0) (segment_000000_sm_nodes.drop 1)
      ...
      (fun t => (bw_linear (t 100) (t 200) (t 206)).1) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_bw_linear_fst_out SequenceDwCase0_1.smGraph t 0 100 200 206 400 401 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_sm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.smGraph) store 100 = (segment_000000_sm_final store) 100 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.smGraph store
...
private theorem segment_000000_hPmWriter0_0 (store : Store) : (segment_000000_pm_final store) 4000 = (bw_linear ((segment_000000_pm_final store) 1000) ((segment_000000_pm_final store) 2000) ((segment_000000_pm_final store) 206)).1 := by
  ...
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 0) ++ [{ rank := 0, op := "OpName.BW_linear", ins := [1000, 2000, 206], outs := [4000, 5000] }] ++ (segment_000000_pm_nodes.drop 1) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final store) 4000 = (bw_linear (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 1000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 2000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 206)).1 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SequenceDwCase0_1.pmGraph store
      (segment_000000_pm_nodes.take 0) (segment_000000_pm_nodes.drop 1)
      ...
      (fun t => (bw_linear (t 1000) (t 2000) (t 206)).1) (by
        ...
        rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
        simp [applyNodeDistributed, applyNodeRingAttn]
        exact applyNode_bw_linear_fst_out SequenceDwCase0_1.pmGraph t 0 1000 2000 206 4000 5000 (by native_decide)
      ) (by native_decide) (by native_decide)
  have hout_read_0 : ((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 1000 = (segment_000000_pm_final store) 1000 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SequenceDwCase0_1.pmGraph store
  ...
  have hout : (segment_000000_pm_final store) 4000 = (bw_linear ((segment_000000_pm_final store) 1000) ((segment_000000_pm_final store) 2000) ((segment_000000_pm_final store) 206)).1 := by
    calc
      _ = (bw_linear (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 1000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 2000) (((segment_000000_pm_nodes.take 0)).foldl (applyNodeDistributedFaithful SequenceDwCase0_1.pmGraph) store 206)).1 := hout_prefix
...
private theorem segment_000000_sound (smStore pmStore : Store) (hstate : state_before.Holds smStore pmStore) : state_after.Holds (segment_000000_sm_final smStore) (segment_000000_pm_final pmStore) := by
 ...
 change ShardedRel (smFinal 206) [pmFinal 206] 0 [1, 4] [1, 4] at hw
 ...
 have hxV : smFinal 200 = allGatherPrimDimN 1 1 0 [pmFinal 2000] := by simpa only [List.length_cons,List.length_nil] using hx.full_value
 ...
 have hcomm1 := TrainVerify.Denote.bw_linear_dw_sequence_reduction_rank3 1 3 2 1 4 [pmFinal 1000] [pmFinal 2000] (pmFinal 206)
 ...
 have hV1 : smFinal 401 = tensorSum [pmFinal 5000] := by
   rw [hS1, hgV, hxV, hwEq, hcomm1]
   rw [← hP1_0]
 have hFull1 : (smFinal 401).shape = [1, 4] := by rw [hS1]; exact bw_linear_3d_snd_shape 3 2 1 4 _ _ _ hg.full_shape hx.full_shape hw.full_shape
 have hShape1_0 : (pmFinal 5000).shape = [1, 4] := by rw [hP1_0]; exact bw_linear_3d_snd_shape 3 2 1 4 _ _ _ (hg.shard_shapes _ (by simp)) (hx.shard_shapes _ (by simp)) (hw.shard_shapes _ (by simp))
 have hout1 : fact_dw.Holds smFinal pmFinal := by
   change ReductionRel (smFinal 401) [pmFinal 5000] [1, 4]
   have hReduce : smFinal 401 = allReducePrim [pmFinal 5000].length 0 [pmFinal 5000] := by rw [hV1]; rfl
   refine { full_value := hReduce, full_shape := hFull1, contributions_nonempty := by simp, contribution_shapes := ?_, reduced_shape := ?_ }
   · simp only [List.forall_mem_cons]; exact ⟨hShape1_0, List.forall_mem_nil _⟩
 ...
 have hcomm0 := TrainVerify.Denote.bw_linear_dx_sequence_allGather_rank3 1 3 2 1 4 [pmFinal 1000] [pmFinal 2000] (smFinal 200) (pmFinal 206)
 ...
 have hV0 : smFinal 400 = allGatherPrimDimN 1 1 0 [pmFinal 4000] := by
   rw [hS0, hgV, hwEq, hcomm0]
   rw [← hP0_0]
 have hFull0 : (smFinal 400).shape = [3, 2, 4] := by rw [hS0]; exact bw_linear_3d_fst_shape 3 2 1 4 _ _ _ hg.full_shape hx.full_shape hw.full_shape
 have hShape0_0 : (pmFinal 4000).shape = [3, 2, 4] := by rw [hP0_0]; exact bw_linear_3d_fst_shape 3 2 1 4 _ _ _ (hg.shard_shapes _ (by simp)) (hx.shard_shapes _ (by simp)) (hw.shard_shapes _ (by simp))
 have hout0 : fact_out.Holds smFinal pmFinal := by
   ...
   · simp only [List.forall_mem_cons]; exact ⟨hShape0_0, List.forall_mem_nil _⟩
...
private def segment_000000 : ClosedDepSegmentCertificate SequenceDwCase0_1.smGraph SequenceDwCase0_1.pmGraph state_before state_after where
