/- Canonical Goal 1, layer 15: graph alignment of the K projection. -/
...
private theorem cL16K_init_singleton_eq (initSM initPM : Store)
...
private theorem cL16K_sm_weight_value (initSM : Store) (tid : Tid)
...
private theorem cL16K_pm_weight_value (initPM : Store) (tid : Tid)
...
private theorem cL16K_rms_weight_eq (initSM initPM : Store)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM) :
    denoteGraphDistributedFaithful sm_goal_1 initSM 5596 =
      denoteGraphDistributedFaithful pm_goal_1 initPM 5596 := by
  have hi := cL16K_init_singleton_eq initSM initPM hInit initGoal_5596
    (by native_decide) 5596 rfl rfl rfl rfl
  have hs := cL16K_sm_weight_value initSM 5596 (by native_decide)
  have hp := cL16K_pm_weight_value initPM 5596 (by native_decide)
  rw [hs, hp, hi]

private theorem cL16K_proj_weight_eq (initSM initPM : Store)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM) :
    denoteGraphDistributedFaithful sm_goal_1 initSM 5598 =
      denoteGraphDistributedFaithful pm_goal_1 initPM 5598 := by
  have hi := cL16K_init_singleton_eq initSM initPM hInit initGoal_5598
    (by native_decide) 5598 rfl rfl rfl rfl
  have hs := cL16K_sm_weight_value initSM 5598 (by native_decide)
  have hp := cL16K_pm_weight_value initPM 5598 (by native_decide)
  rw [hs, hp, hi]

private theorem cL16K_proj_weight_shape (initPM : Store)
...
/-- The complete ordinary canonical L16 K graph relation `5932 ↔ 10696/10697`.
All projection-weight facts are derived from the generated init contracts. -/
theorem canonical_l16_k_ordinary_relation
    (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM)
    (hCache : Gather2Rel
  ...
  have hRmsW := cL16K_rms_weight_eq initSM initPM hInit
  have hKW := cL16K_proj_weight_eq initSM initPM hInit
  have hKShape := cL16K_proj_weight_shape initPM hPM
  have hSemantic := canonical_l16_k_ordinary_semantic hCache hRmsW hKW hKShape
  rw [canonical_l16_k_sm_reduce initSM]
  rw [canonical_l16_k_pm0_reduce initPM]
  rw [canonical_l16_k_pm1_reduce initPM]
  exact hSemantic

#print axioms canonical_l16_k_ordinary_relation
