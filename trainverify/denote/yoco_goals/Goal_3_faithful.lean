/-
  Goal_3_faithful.lean — 2026-07-06 Pattern_3 upstream-reshape-fidelity variant.

  Same underlying model (YOCO-MoE-A0.4B, 24-layer) and same goal target
  (tid 4675) as Goal_3.lean, but ALL FW_reshape nodes carry
  `params := [target_shape]` (thanks to Verdict/graph_to_lean.py 2026-07-06 patch),
  making Denote's reshape semantics faithful to PyTorch's `.reshape()` (fw_view)
  rather than the old identity model. This is prerequisite for Pattern_3 goal_3
  to be TRUE (see ANALYSIS_V2.md for the full root-cause analysis).

  Namespace: `TrainVerify.Denote.GeneratedGoals` (same as Goal_3.lean) —
  suffix-disambiguated identifiers (sm_goal_3_faithful, pm_goal_3_faithful, etc.)
  to coexist with the legacy Goal_3.lean.
-/
...
/-! ## Init goals (copied from Goal_3.lean; tid references identical) -/

/-! ## Init goals + goal statement (adapted from Goal_3.lean; references sm_goal_3_faithful now) -/
...
/-- Ring-attention-aware goal statement, adapted from Pattern_3.lean. -/
