...
theorem l10_ordinary_moe_logits_from_norm_input (initSM initPM : Store)
    ...
    Gather2Rel
      ...
      [4096, 64] [2048, 64] := by
  have hwEq := l10OMr_weight_eq initSM initPM hInit
  have hwShape := l10OMr_weight_shape initPM hPM
  ...
  rw [l10OMr_red_sm5512 initSM, l10OMr_red_sm5510 initSM,
    l10OMr_red_sm8296 initSM, hwEq,
    l10OMr_red_pm9480 initPM, l10OMr_red_pm9481 initPM,
    l10OMr_red_pm5512 initPM, l10OMr_red_pm5510 initPM,
    l10OMr_red_pm12062 initPM, l10OMr_red_pm15376 initPM,
    l10OMr_red_pm15378 initPM,
    ...
    l10OMr_chunk_gather0 _ _ hp0Shape hp1Shape,
    l10OMr_chunk_gather1 _ _ hp0Shape hp1Shape]
  exact hLocal
...
theorem l10_ordinary_moe_router_from_norm_input (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks initGoals initSM initPM)
    (hNorm : Gather2Rel
      (denoteGraphDistributedFaithful sm_goal_1 initSM 5509)
    ...
    Gather2Rel
      ...
      [4096, 64] [2048, 64] := by
  have hLogits := l10_ordinary_moe_logits_from_norm_input initSM initPM hPM hInit hNorm
  ...
  rw [l10OMr_red_sm5513 initSM hLogits.full_shape,
    l10OMr_red_pm9482 initPM hLogits.shard0_shape,
    l10OMr_red_pm9483 initPM hLogits.shard1_shape,
    l10OMr_red_sm5514 initSM hLogits.full_shape,
    l10OMr_red_pm9484 initPM hLogits.shard0_shape,
    l10OMr_red_pm9485 initPM hLogits.shard1_shape]
  exact ⟨hProbs, hMap⟩

/-- The real scalar-gate branch preserves ordinary dim-0 layout. -/
theorem l10_ordinary_moe_gate_from_norm_input (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks initGoals initSM initPM)
    (hNorm : Gather2Rel
      (denoteGraphDistributedFaithful sm_goal_1 initSM 5509)
  ...
  have hSource : Gather2Rel
    ...
    rw [l10OMrg_red_sm8304 initSM, l10OMrg_red_pm13680 initPM,
      l10OMrg_red_pm13681 initPM]
    exact hNorm
  have hReshape : Gather2Rel
      ...
      [4096, 1024] [2048, 1024] := by
    rw [l10OMrg_red_sm5519 initSM, l10OMrg_red_pm9494 initPM,
      l10OMrg_red_pm9495 initPM]
    exact Gather2Rel.view_id hSource
  have hwEq := l10OMrg_weight_eq initSM initPM hInit
  have hwShape := l10OMrg_weight_shape initPM hPM
  have hLinear : Gather2Rel
      ...
      [4096, 1] [2048, 1] := by
    rw [l10OMrg_red_sm5521 initSM, l10OMrg_red_pm9498 initPM,
      l10OMrg_red_pm9499 initPM, hwEq]
  ...
  have hView : Gather2Rel
      ...
      [4096, 1] [2048, 1] := by
    rw [l10OMrg_red_sm5522 initSM, l10OMrg_red_pm9500 initPM,
      l10OMrg_red_pm9501 initPM]
    exact Gather2Rel.view_id hLinear
  rw [l10OMrg_red_sm5523 initSM, l10OMrg_red_pm9502 initPM,
    l10OMrg_red_pm9503 initPM]
...
theorem l10_ordinary_moe_router_from_attention_output (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    ...
    Gather2Rel
      ...
      (denoteGraphDistributedFaithful pm_goal_1 initPM 9485)
      [4096, 64] [2048, 64] := by
  have hNorm := l10_ordinary_moe_norm_from_attention_output
    initSM initPM hInit hAttention
  exact l10_ordinary_moe_router_from_norm_input initSM initPM hPM hInit hNorm

/-- The ordinary scalar gate is closed directly from attention output. -/
theorem l10_ordinary_moe_gate_from_attention_output (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM)
    (hAttention : Gather2Rel
      (denoteGraphDistributedFaithful sm_goal_1 initSM 5507)
  ...
  have hNorm := l10_ordinary_moe_norm_from_attention_output
    initSM initPM hInit hAttention
  exact l10_ordinary_moe_gate_from_norm_input initSM initPM hPM hInit hNorm

#print axioms l10_ordinary_moe_router_from_norm_input
#print axioms l10_ordinary_moe_gate_from_norm_input
#print axioms l10_ordinary_moe_router_from_attention_output
#print axioms l10_ordinary_moe_gate_from_attention_output
