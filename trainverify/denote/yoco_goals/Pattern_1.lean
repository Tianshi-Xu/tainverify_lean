/- Hand proof for Pattern_1: MoE FF block + norm + inner_chunk_ce over context-parallel.
   Pattern: 1
   SM=25 ops, PM=53 ops, with the three local cut boundary contracts consumed by the proof.
-/
import denote.yoco_goals.Goal_1_Cut
...
def goal_1_stmt_with_labels : Prop :=
  ∀ (initSM initPM : Store),
    ...
    let smStore := denoteGraph sm_goal_1_cut initSM
    let pmStore := denoteGraph pm_goal_1_cut initPM
    let ts := smStore goal_1_cut_goal.ts
    let tps := goal_1_cut_goal.tps.map (fun p => pmStore p.tid)
    ts.shape = goal_1_cut_goal.tsShape ∧
      (tps.map (fun t => t.shape)) = goal_1_cut_goal.tpShapes ∧
      ts = reconstructWithDim goal_1_cut_goal.gatherDim pm_goal_1_cut.numRanks 0 tps
...
private theorem sm_fold_boundary_read (initSM : Store) (tid : Tid)
    (h : ∀ n ∈ sm_goal_1_cut.nodes, tid ∉ n.outs) :
    (sm_goal_1_cut.nodes.foldl (applyNode sm_goal_1_cut) initSM) tid = initSM tid :=
  foldl_applyNode_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM tid h

/-! ## SM machinery

`sm_goal_1_cut` has 25 nodes computing final loss `[4096]` from boundary inputs
...
/-- Full unfolding of `denoteGraph sm_goal_1_cut initSM 4673` in terms of `initSM` values. -/
theorem denote_sm_goal_1_cut_4673 (initSM : Store)
    (h5898 : (initSM 5898).shape.reverse.head? = some 64) :
    denoteGraph sm_goal_1_cut initSM 4673 =
      (fw_inner_chunk_ce
        (fw_rms_norm
  ...
  have h_split : sm_goal_1_cut.nodes = sm_goal_1_cut.nodes.take 24 ++
      [{ rank := 0, op := "OpName.FW_inner_chunk_ce", ins := [5930, 5931, 4678], outs := [4673, 4674], params := [1024] }] := by
    show sm_goal_1_cut.nodes = _
    rfl
  unfold denoteGraph
  rw [h_split, List.foldl_append, List.foldl_cons, List.foldl_nil]
  set S24 : Store := (sm_goal_1_cut.nodes.take 24).foldl (applyNode sm_goal_1_cut) initSM
  rw [applyNode_fw_inner_chunk_ce_fst_out_1p sm_goal_1_cut S24 0 5930 5931 4678 4673 4674 [1024]]
  set S1 : Store := (sm_goal_1_cut.nodes.take 1).foldl (applyNode sm_goal_1_cut) initSM with hS1_def
  set S2 : Store := (sm_goal_1_cut.nodes.take 2).foldl (applyNode sm_goal_1_cut) initSM with hS2_def
  set S3 : Store := (sm_goal_1_cut.nodes.take 3).foldl (applyNode sm_goal_1_cut) initSM with hS3_def
  set S4 : Store := (sm_goal_1_cut.nodes.take 4).foldl (applyNode sm_goal_1_cut) initSM with hS4_def
  set S5 : Store := (sm_goal_1_cut.nodes.take 5).foldl (applyNode sm_goal_1_cut) initSM with hS5_def
  set S6 : Store := (sm_goal_1_cut.nodes.take 6).foldl (applyNode sm_goal_1_cut) initSM with hS6_def
  set S7 : Store := (sm_goal_1_cut.nodes.take 7).foldl (applyNode sm_goal_1_cut) initSM with hS7_def
  set S8 : Store := (sm_goal_1_cut.nodes.take 8).foldl (applyNode sm_goal_1_cut) initSM with hS8_def
  set S9 : Store := (sm_goal_1_cut.nodes.take 9).foldl (applyNode sm_goal_1_cut) initSM with hS9_def
  set S10 : Store := (sm_goal_1_cut.nodes.take 10).foldl (applyNode sm_goal_1_cut) initSM with hS10_def
  set S11 : Store := (sm_goal_1_cut.nodes.take 11).foldl (applyNode sm_goal_1_cut) initSM with hS11_def
  set S12 : Store := (sm_goal_1_cut.nodes.take 12).foldl (applyNode sm_goal_1_cut) initSM with hS12_def
  set S13 : Store := (sm_goal_1_cut.nodes.take 13).foldl (applyNode sm_goal_1_cut) initSM with hS13_def
  set S14 : Store := (sm_goal_1_cut.nodes.take 14).foldl (applyNode sm_goal_1_cut) initSM with hS14_def
  set S15 : Store := (sm_goal_1_cut.nodes.take 15).foldl (applyNode sm_goal_1_cut) initSM with hS15_def
  set S16 : Store := (sm_goal_1_cut.nodes.take 16).foldl (applyNode sm_goal_1_cut) initSM with hS16_def
  set S17 : Store := (sm_goal_1_cut.nodes.take 17).foldl (applyNode sm_goal_1_cut) initSM with hS17_def
  set S18 : Store := (sm_goal_1_cut.nodes.take 18).foldl (applyNode sm_goal_1_cut) initSM with hS18_def
  set S19 : Store := (sm_goal_1_cut.nodes.take 19).foldl (applyNode sm_goal_1_cut) initSM with hS19_def
  set S20 : Store := (sm_goal_1_cut.nodes.take 20).foldl (applyNode sm_goal_1_cut) initSM with hS20_def
  set S21 : Store := (sm_goal_1_cut.nodes.take 21).foldl (applyNode sm_goal_1_cut) initSM with hS21_def
  set S22 : Store := (sm_goal_1_cut.nodes.take 22).foldl (applyNode sm_goal_1_cut) initSM with hS22_def
  set S23 : Store := (sm_goal_1_cut.nodes.take 23).foldl (applyNode sm_goal_1_cut) initSM with hS23_def

  -- Sj=24 written by node_23 (FW_rms_norm), outs=[5930], tid=5930
  have h_take24_5930 : sm_goal_1_cut.nodes.take 24 = sm_goal_1_cut.nodes.take 23 ++ [{ rank := 0, op := "OpName.FW_rms_norm", ins := [5928, 5929], outs := [5930] }] := by rfl
  have hS24_eq_5930 : S24 = applyNode sm_goal_1_cut S23 { rank := 0, op := "OpName.FW_rms_norm", ins := [5928, 5929], outs := [5930] } := by
    show (sm_goal_1_cut.nodes.take 24).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS24_5930 : S24 5930 = fw_rms_norm (S23 5928) (S23 5929) := by
    rw [hS24_eq_5930]
    exact applyNode_fw_rms_norm_out_1p sm_goal_1_cut S23 0 5928 5929 5930
  have hS24_5931 : S24 5931 = initSM 5931 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 24) initSM 5931
      (by intro n hn; fin_cases hn <;> decide)
  have hS24_4678 : S24 4678 = initSM 4678 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 24) initSM 4678
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=23 written by node_22 (FW_maybe_unshuffle), outs=[5928], tid=5928
  have h_take23_5928 : sm_goal_1_cut.nodes.take 23 = sm_goal_1_cut.nodes.take 22 ++ [{ rank := 0, op := "OpName.FW_maybe_unshuffle", ins := [5926, 5927], outs := [5928], params := [1, 0] }] := by rfl
  have hS23_eq_5928 : S23 = applyNode sm_goal_1_cut S22 { rank := 0, op := "OpName.FW_maybe_unshuffle", ins := [5926, 5927], outs := [5928], params := [1, 0] } := by
    show (sm_goal_1_cut.nodes.take 23).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS23_5928 : S23 5928 = fw_maybe_unshuffle (S22 5926) (S22 5927) 1 0 := by
    rw [hS23_eq_5928]
    exact applyNode_fw_maybe_unshuffle_out_1p sm_goal_1_cut S22 0 5926 5927 5928 [1, 0]
  have hS23_5929 : S23 5929 = initSM 5929 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 23) initSM 5929
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=22 written by node_21 (FW_add), outs=[5926], tid=5926
  have h_take22_5926 : sm_goal_1_cut.nodes.take 22 = sm_goal_1_cut.nodes.take 21 ++ [{ rank := 0, op := "OpName.FW_add", ins := [8580, 5925], outs := [5926] }] := by rfl
  have hS22_eq_5926 : S22 = applyNode sm_goal_1_cut S21 { rank := 0, op := "OpName.FW_add", ins := [8580, 5925], outs := [5926] } := by
    show (sm_goal_1_cut.nodes.take 22).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS22_5926 : S22 5926 = elemwiseAdd (S21 8580) (S21 5925) := by
    rw [hS22_eq_5926]
    exact applyNode_fw_add2_out sm_goal_1_cut S21 0 8580 5925 5926
  have hS22_5927 : S22 5927 = initSM 5927 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 22) initSM 5927
      (by intro n hn; fin_cases hn <;> decide)
  have hS21_8580 : S21 8580 = S1 8580 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 8580 1 21 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=21 written by node_20 (FW_float), outs=[5925], tid=5925
  have h_take21_5925 : sm_goal_1_cut.nodes.take 21 = sm_goal_1_cut.nodes.take 20 ++ [{ rank := 0, op := "OpName.FW_float", ins := [5924], outs := [5925] }] := by rfl
  have hS21_eq_5925 : S21 = applyNode sm_goal_1_cut S20 { rank := 0, op := "OpName.FW_float", ins := [5924], outs := [5925] } := by
    show (sm_goal_1_cut.nodes.take 21).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take21_5925, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS21_5925 : S21 5925 = S20 5924 := by
    rw [hS21_eq_5925]
    exact applyNode_fw_float_out sm_goal_1_cut S20 0 5924 5925 []
  -- Sj=1 written by node_0 (FW_multiref), outs=[8576, 8580], tid=8580
  have h_take1_8580 : sm_goal_1_cut.nodes.take 1 = sm_goal_1_cut.nodes.take 0 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [5893], outs := [8576, 8580], params := [2] }] := by rfl
  have hS1_eq_8580 : S1 = applyNode sm_goal_1_cut initSM { rank := 0, op := "OpName.FW_multiref", ins := [5893], outs := [8576, 8580], params := [2] } := by
    show (sm_goal_1_cut.nodes.take 1).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take1_8580, List.foldl_append, List.foldl_cons, List.foldl_nil]
    rfl
  have hS1_8580 : S1 8580 = initSM 5893 := by
    rw [hS1_eq_8580]
    exact applyNode_fw_multiref2_second_out_g259 sm_goal_1_cut initSM 0 5893 8576 8580 (by decide)
  -- Sj=20 written by node_19 (FW_add), outs=[5924], tid=5924
  have h_take20_5924 : sm_goal_1_cut.nodes.take 20 = sm_goal_1_cut.nodes.take 19 ++ [{ rank := 0, op := "OpName.FW_add", ins := [5904, 5923], outs := [5924] }] := by rfl
  have hS20_eq_5924 : S20 = applyNode sm_goal_1_cut S19 { rank := 0, op := "OpName.FW_add", ins := [5904, 5923], outs := [5924] } := by
    show (sm_goal_1_cut.nodes.take 20).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS20_5924 : S20 5924 = elemwiseAdd (S19 5904) (S19 5923) := by
    rw [hS20_eq_5924]
    exact applyNode_fw_add2_out sm_goal_1_cut S19 0 5904 5923 5924
  have hS19_5904 : S19 5904 = S13 5904 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5904 13 19 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=19 written by node_18 (FW_mul), outs=[5923], tid=5923
  have h_take19_5923 : sm_goal_1_cut.nodes.take 19 = sm_goal_1_cut.nodes.take 18 ++ [{ rank := 0, op := "OpName.FW_mul", ins := [5909, 5922], outs := [5923] }] := by rfl
  have hS19_eq_5923 : S19 = applyNode sm_goal_1_cut S18 { rank := 0, op := "OpName.FW_mul", ins := [5909, 5922], outs := [5923] } := by
    show (sm_goal_1_cut.nodes.take 19).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS19_5923 : S19 5923 = elemwiseMul (S18 5909) (S18 5922) := by
    rw [hS19_eq_5923]
    exact applyNode_fw_mul_out sm_goal_1_cut S18 0 5909 5922 5923
  -- Sj=13 written by node_12 (FW_all2all_moe_gmm), outs=[5904], tid=5904
  have h_take13_5904 : sm_goal_1_cut.nodes.take 13 = sm_goal_1_cut.nodes.take 12 ++ [{ rank := 0, op := "OpName.FW_all2all_moe_gmm", ins := [8591, 5899, 5900, 5902, 5903], outs := [5904], params := [64, 0, 64, 8] }] := by rfl
  have hS13_eq_5904 : S13 = applyNode sm_goal_1_cut S12 { rank := 0, op := "OpName.FW_all2all_moe_gmm", ins := [8591, 5899, 5900, 5902, 5903], outs := [5904], params := [64, 0, 64, 8] } := by
    show (sm_goal_1_cut.nodes.take 13).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS13_5904 : S13 5904 = fw_all2all_moe_gmm (S12 8591) (S12 5899) (S12 5900) (S12 5902) (S12 5903) 64 0 64 8 ((((10 : Nat) : Scalar))) := by
    rw [hS13_eq_5904]
    exact applyNode_fw_all2all_moe_gmm_out_1p sm_goal_1_cut S12 0 8591 5899 5900 5902 5903 5904 [64, 0, 64, 8]
  have hS18_5909 : S18 5909 = S14 5909 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5909 14 18 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=18 written by node_17 (FW_view), outs=[5922], tid=5922
  have h_take18_5922 : sm_goal_1_cut.nodes.take 18 = sm_goal_1_cut.nodes.take 17 ++ [{ rank := 0, op := "OpName.FW_view", ins := [5921], outs := [5922], params := [4096, 1024] }] := by rfl
  have hS18_eq_5922 : S18 = applyNode sm_goal_1_cut S17 { rank := 0, op := "OpName.FW_view", ins := [5921], outs := [5922], params := [4096, 1024] } := by
    show (sm_goal_1_cut.nodes.take 18).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take18_5922, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS18_5922 : S18 5922 = fw_view [4096, 1024] (S17 5921) := by
    rw [hS18_eq_5922]
    exact applyNode_fw_view_out sm_goal_1_cut S17 0 4096 [1024] 5921 5922
  have hS12_8591 : S12 8591 = S2 8591 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 8591 2 12 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS12_5899 : S12 5899 = S9 5899 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5899 9 12 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS12_5900 : S12 5900 = S9 5900 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5900 9 12 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS12_5902 : S12 5902 = initSM 5902 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 12) initSM 5902
      (by intro n hn; fin_cases hn <;> decide)
  have hS12_5903 : S12 5903 = initSM 5903 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 12) initSM 5903
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=14 written by node_13 (FW_sigmoid), outs=[5909], tid=5909
  have h_take14_5909 : sm_goal_1_cut.nodes.take 14 = sm_goal_1_cut.nodes.take 13 ++ [{ rank := 0, op := "OpName.FW_sigmoid", ins := [5908], outs := [5909] }] := by rfl
  have hS14_eq_5909 : S14 = applyNode sm_goal_1_cut S13 { rank := 0, op := "OpName.FW_sigmoid", ins := [5908], outs := [5909] } := by
    show (sm_goal_1_cut.nodes.take 14).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take14_5909, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS14_5909 : S14 5909 = fw_sigmoid (S13 5908) := by
    rw [hS14_eq_5909]
    exact applyNode_fw_sigmoid_out_1p sm_goal_1_cut S13 0 5908 5909
  -- Sj=17 written by node_16 (FW_mix_precision_linear), outs=[5921], tid=5921
  have h_take17_5921 : sm_goal_1_cut.nodes.take 17 = sm_goal_1_cut.nodes.take 16 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5919, 5920], outs := [5921] }] := by rfl
  have hS17_eq_5921 : S17 = applyNode sm_goal_1_cut S16 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5919, 5920], outs := [5921] } := by
    show (sm_goal_1_cut.nodes.take 17).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take17_5921, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS17_5921 : S17 5921 = fw_linear (S16 5919) (S16 5920) := by
    rw [hS17_eq_5921]
    exact applyNode_fw_mix_precision_linear_out_1p sm_goal_1_cut S16 0 5919 5920 5921
  -- Sj=2 written by node_1 (FW_multiref), outs=[8587, 8591, 8595, 8599, 8603], tid=8591
  have h_take2_8591 : sm_goal_1_cut.nodes.take 2 = sm_goal_1_cut.nodes.take 1 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] }] := by rfl
  have hS2_eq_8591 : S2 = applyNode sm_goal_1_cut S1 { rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] } := by
    show (sm_goal_1_cut.nodes.take 2).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take2_8591, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS2_8591 : S2 8591 = S1 5895 := by
    rw [hS2_eq_8591]
    exact applyNode_fw_multiref5_at_pos1_out sm_goal_1_cut S1 0 5895 8587 8591 8595 8599 8603 (by decide)
  ...
  have hS8_5898_sh : (S8 5898).shape.reverse.head? = some 64 := by
    have hS8_5898_eq : S8 5898 = initSM 5898 :=
      foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 8) initSM 5898
        (by intro n hn; fin_cases hn <;> decide)
    rw [hS8_5898_eq]; exact h5898
  have h_take9_5899 : sm_goal_1_cut.nodes.take 9 = sm_goal_1_cut.nodes.take 8 ++ [{ rank := 0, op := "OpName.FW_topk_routing", ins := [5898], outs := [5899, 5900, 5901], params := [8, 64] }] := by rfl
  have hS9_eq_5899 : S9 = applyNode sm_goal_1_cut S8 { rank := 0, op := "OpName.FW_topk_routing", ins := [5898], outs := [5899, 5900, 5901], params := [8, 64] } := by
    show (sm_goal_1_cut.nodes.take 9).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS9_5899 : S9 5899 = (fw_topk_routing (S8 5898) 8 64).fst := by
    rw [hS9_eq_5899, applyNode_fw_topk_routing_probs_out sm_goal_1_cut S8 0 5898 5899 5900 5901 [8, 64], hS8_5898_sh]
    rfl
  -- Sj=9 written by node_8 (FW_topk_routing), outs=[5899, 5900, 5901], tid=5900
  have h_take9_5900 : sm_goal_1_cut.nodes.take 9 = sm_goal_1_cut.nodes.take 8 ++ [{ rank := 0, op := "OpName.FW_topk_routing", ins := [5898], outs := [5899, 5900, 5901], params := [8, 64] }] := by rfl
  have hS9_eq_5900 : S9 = applyNode sm_goal_1_cut S8 { rank := 0, op := "OpName.FW_topk_routing", ins := [5898], outs := [5899, 5900, 5901], params := [8, 64] } := by
    show (sm_goal_1_cut.nodes.take 9).foldl (applyNode sm_goal_1_cut) initSM = _
  ...
  have hS9_5900 : S9 5900 = (fw_topk_routing (S8 5898) 8 64).snd.fst := by
    rw [hS9_eq_5900, applyNode_fw_topk_routing_map_out sm_goal_1_cut S8 0 5898 5899 5900 5901 [8, 64] (by decide), hS8_5898_sh]
    rfl
  have hS13_5908 : S13 5908 = S10 5908 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5908 10 13 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=16 written by node_15 (FW_reshape), outs=[5919], tid=5919
  have h_take16_5919 : sm_goal_1_cut.nodes.take 16 = sm_goal_1_cut.nodes.take 15 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [5918], outs := [5919] }] := by rfl
  have hS16_eq_5919 : S16 = applyNode sm_goal_1_cut S15 { rank := 0, op := "OpName.FW_reshape", ins := [5918], outs := [5919] } := by
    show (sm_goal_1_cut.nodes.take 16).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take16_5919, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS16_5919 : S16 5919 = S15 5918 := by
    rw [hS16_eq_5919]
    exact applyNode_fw_reshape_out sm_goal_1_cut S15 0 5918 5919 []
  have hS16_5920 : S16 5920 = initSM 5920 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 16) initSM 5920
      (by intro n hn; fin_cases hn <;> decide)
  have hS1_5895 : S1 5895 = initSM 5895 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 1) initSM 5895
      (by intro n hn; fin_cases hn <;> decide)
  have hS8_5898 : S8 5898 = initSM 5898 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 8) initSM 5898
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=10 written by node_9 (FW_view), outs=[5908], tid=5908
  have h_take10_5908 : sm_goal_1_cut.nodes.take 10 = sm_goal_1_cut.nodes.take 9 ++ [{ rank := 0, op := "OpName.FW_view", ins := [5907], outs := [5908], params := [4096, 1] }] := by rfl
  have hS10_eq_5908 : S10 = applyNode sm_goal_1_cut S9 { rank := 0, op := "OpName.FW_view", ins := [5907], outs := [5908], params := [4096, 1] } := by
    show (sm_goal_1_cut.nodes.take 10).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take10_5908, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS10_5908 : S10 5908 = fw_view [4096, 1] (S9 5907) := by
    rw [hS10_eq_5908]
    exact applyNode_fw_view_out sm_goal_1_cut S9 0 4096 [1] 5907 5908
  -- Sj=15 written by node_14 (FW_swiglu), outs=[5918], tid=5918
  have h_take15_5918 : sm_goal_1_cut.nodes.take 15 = sm_goal_1_cut.nodes.take 14 ++ [{ rank := 0, op := "OpName.FW_swiglu", ins := [5913, 5917], outs := [5918] }] := by rfl
  have hS15_eq_5918 : S15 = applyNode sm_goal_1_cut S14 { rank := 0, op := "OpName.FW_swiglu", ins := [5913, 5917], outs := [5918] } := by
    show (sm_goal_1_cut.nodes.take 15).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take15_5918, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS15_5918 : S15 5918 = fw_swiglu (S14 5913) (S14 5917) := by
    rw [hS15_eq_5918]
    exact applyNode_fw_swiglu_out_1p sm_goal_1_cut S14 0 5913 5917 5918
  have hS9_5907 : S9 5907 = S6 5907 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5907 6 9 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS14_5913 : S14 5913 = S11 5913 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5913 11 14 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS14_5917 : S14 5917 = S12 5917 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5917 12 14 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=6 written by node_5 (FW_mix_precision_linear), outs=[5907], tid=5907
  have h_take6_5907 : sm_goal_1_cut.nodes.take 6 = sm_goal_1_cut.nodes.take 5 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5905, 5906], outs := [5907] }] := by rfl
  have hS6_eq_5907 : S6 = applyNode sm_goal_1_cut S5 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5905, 5906], outs := [5907] } := by
    show (sm_goal_1_cut.nodes.take 6).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take6_5907, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS6_5907 : S6 5907 = fw_linear (S5 5905) (S5 5906) := by
    rw [hS6_eq_5907]
    exact applyNode_fw_mix_precision_linear_out_1p sm_goal_1_cut S5 0 5905 5906 5907
  -- Sj=11 written by node_10 (FW_view), outs=[5913], tid=5913
  have h_take11_5913 : sm_goal_1_cut.nodes.take 11 = sm_goal_1_cut.nodes.take 10 ++ [{ rank := 0, op := "OpName.FW_view", ins := [5912], outs := [5913], params := [4096, 512] }] := by rfl
  have hS11_eq_5913 : S11 = applyNode sm_goal_1_cut S10 { rank := 0, op := "OpName.FW_view", ins := [5912], outs := [5913], params := [4096, 512] } := by
    show (sm_goal_1_cut.nodes.take 11).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take11_5913, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS11_5913 : S11 5913 = fw_view [4096, 512] (S10 5912) := by
    rw [hS11_eq_5913]
    exact applyNode_fw_view_out sm_goal_1_cut S10 0 4096 [512] 5912 5913
  -- Sj=12 written by node_11 (FW_view), outs=[5917], tid=5917
  have h_take12_5917 : sm_goal_1_cut.nodes.take 12 = sm_goal_1_cut.nodes.take 11 ++ [{ rank := 0, op := "OpName.FW_view", ins := [5916], outs := [5917], params := [4096, 512] }] := by rfl
  have hS12_eq_5917 : S12 = applyNode sm_goal_1_cut S11 { rank := 0, op := "OpName.FW_view", ins := [5916], outs := [5917], params := [4096, 512] } := by
    show (sm_goal_1_cut.nodes.take 12).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take12_5917, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS12_5917 : S12 5917 = fw_view [4096, 512] (S11 5916) := by
    rw [hS12_eq_5917]
    exact applyNode_fw_view_out sm_goal_1_cut S11 0 4096 [512] 5916 5917
  have hS5_5905 : S5 5905 = S3 5905 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5905 3 5 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS5_5906 : S5 5906 = initSM 5906 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 5) initSM 5906
      (by intro n hn; fin_cases hn <;> decide)
  have hS10_5912 : S10 5912 = S7 5912 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5912 7 10 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS11_5916 : S11 5916 = S8 5916 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5916 8 11 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=3 written by node_2 (FW_reshape), outs=[5905], tid=5905
  have h_take3_5905 : sm_goal_1_cut.nodes.take 3 = sm_goal_1_cut.nodes.take 2 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [8595], outs := [5905] }] := by rfl
  have hS3_eq_5905 : S3 = applyNode sm_goal_1_cut S2 { rank := 0, op := "OpName.FW_reshape", ins := [8595], outs := [5905] } := by
    show (sm_goal_1_cut.nodes.take 3).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take3_5905, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS3_5905 : S3 5905 = S2 8595 := by
    rw [hS3_eq_5905]
    exact applyNode_fw_reshape_out sm_goal_1_cut S2 0 8595 5905 []
  -- Sj=7 written by node_6 (FW_mix_precision_linear), outs=[5912], tid=5912
  have h_take7_5912 : sm_goal_1_cut.nodes.take 7 = sm_goal_1_cut.nodes.take 6 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5910, 5911], outs := [5912] }] := by rfl
  have hS7_eq_5912 : S7 = applyNode sm_goal_1_cut S6 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5910, 5911], outs := [5912] } := by
    show (sm_goal_1_cut.nodes.take 7).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take7_5912, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS7_5912 : S7 5912 = fw_linear (S6 5910) (S6 5911) := by
    rw [hS7_eq_5912]
    exact applyNode_fw_mix_precision_linear_out_1p sm_goal_1_cut S6 0 5910 5911 5912
  -- Sj=8 written by node_7 (FW_mix_precision_linear), outs=[5916], tid=5916
  have h_take8_5916 : sm_goal_1_cut.nodes.take 8 = sm_goal_1_cut.nodes.take 7 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5914, 5915], outs := [5916] }] := by rfl
  have hS8_eq_5916 : S8 = applyNode sm_goal_1_cut S7 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [5914, 5915], outs := [5916] } := by
    show (sm_goal_1_cut.nodes.take 8).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take8_5916, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS8_5916 : S8 5916 = fw_linear (S7 5914) (S7 5915) := by
    rw [hS8_eq_5916]
    exact applyNode_fw_mix_precision_linear_out_1p sm_goal_1_cut S7 0 5914 5915 5916
  -- Sj=2 written by node_1 (FW_multiref), outs=[8587, 8591, 8595, 8599, 8603], tid=8595
  have h_take2_8595 : sm_goal_1_cut.nodes.take 2 = sm_goal_1_cut.nodes.take 1 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] }] := by rfl
  have hS2_eq_8595 : S2 = applyNode sm_goal_1_cut S1 { rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] } := by
    show (sm_goal_1_cut.nodes.take 2).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take2_8595, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS2_8595 : S2 8595 = S1 5895 := by
    rw [hS2_eq_8595]
    exact applyNode_fw_multiref5_at_pos2_out sm_goal_1_cut S1 0 5895 8587 8591 8595 8599 8603 (by decide) (by decide)
  have hS6_5910 : S6 5910 = S4 5910 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5910 4 6 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS6_5911 : S6 5911 = initSM 5911 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 6) initSM 5911
      (by intro n hn; fin_cases hn <;> decide)
  have hS7_5914 : S7 5914 = S5 5914 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 5914 5 7 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS7_5915 : S7 5915 = initSM 5915 :=
    foldl_applyNode_at_not_written sm_goal_1_cut (sm_goal_1_cut.nodes.take 7) initSM 5915
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=4 written by node_3 (FW_reshape), outs=[5910], tid=5910
  have h_take4_5910 : sm_goal_1_cut.nodes.take 4 = sm_goal_1_cut.nodes.take 3 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [8599], outs := [5910] }] := by rfl
  have hS4_eq_5910 : S4 = applyNode sm_goal_1_cut S3 { rank := 0, op := "OpName.FW_reshape", ins := [8599], outs := [5910] } := by
    show (sm_goal_1_cut.nodes.take 4).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take4_5910, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS4_5910 : S4 5910 = S3 8599 := by
    rw [hS4_eq_5910]
    exact applyNode_fw_reshape_out sm_goal_1_cut S3 0 8599 5910 []
  -- Sj=5 written by node_4 (FW_reshape), outs=[5914], tid=5914
  have h_take5_5914 : sm_goal_1_cut.nodes.take 5 = sm_goal_1_cut.nodes.take 4 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [8603], outs := [5914] }] := by rfl
  have hS5_eq_5914 : S5 = applyNode sm_goal_1_cut S4 { rank := 0, op := "OpName.FW_reshape", ins := [8603], outs := [5914] } := by
    show (sm_goal_1_cut.nodes.take 5).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take5_5914, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS5_5914 : S5 5914 = S4 8603 := by
    rw [hS5_eq_5914]
    exact applyNode_fw_reshape_out sm_goal_1_cut S4 0 8603 5914 []
  have hS3_8599 : S3 8599 = S2 8599 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 8599 2 3 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hS4_8603 : S4 8603 = S2 8603 :=
    foldl_take_split_at_not_written sm_goal_1_cut sm_goal_1_cut.nodes initSM 8603 2 4 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Sj=2 written by node_1 (FW_multiref), outs=[8587, 8591, 8595, 8599, 8603], tid=8599
  have h_take2_8599 : sm_goal_1_cut.nodes.take 2 = sm_goal_1_cut.nodes.take 1 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] }] := by rfl
  have hS2_eq_8599 : S2 = applyNode sm_goal_1_cut S1 { rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] } := by
    show (sm_goal_1_cut.nodes.take 2).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take2_8599, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS2_8599 : S2 8599 = S1 5895 := by
    rw [hS2_eq_8599]
    exact applyNode_fw_multiref5_at_pos3_out sm_goal_1_cut S1 0 5895 8587 8591 8595 8599 8603 (by decide) (by decide) (by decide)
  -- Sj=2 written by node_1 (FW_multiref), outs=[8587, 8591, 8595, 8599, 8603], tid=8603
  have h_take2_8603 : sm_goal_1_cut.nodes.take 2 = sm_goal_1_cut.nodes.take 1 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] }] := by rfl
  have hS2_eq_8603 : S2 = applyNode sm_goal_1_cut S1 { rank := 0, op := "OpName.FW_multiref", ins := [5895], outs := [8587, 8591, 8595, 8599, 8603], params := [5] } := by
    show (sm_goal_1_cut.nodes.take 2).foldl (applyNode sm_goal_1_cut) initSM = _
    rw [h_take2_8603, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hS2_8603 : S2 8603 = S1 5895 := by
    rw [hS2_eq_8603]
    exact applyNode_fw_multiref5_at_pos4_out sm_goal_1_cut S1 0 5895 8587 8591 8595 8599 8603 (by decide) (by decide) (by decide) (by decide)
  rw [
...
/-- Full unfolding of `denoteGraph pm_goal_1_cut initPM 4673`. -/
theorem denote_pm_goal_1_cut_4673 (initPM : Store)
    (h11621 : (initPM 11621).shape.reverse.head? = some 64)
    (h11622 : (initPM 11622).shape.reverse.head? = some 64) :
    denoteGraph pm_goal_1_cut initPM 4673 =
      allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [(fw_inner_chunk_ce (fw_rms_norm (fw_maybe_unshuffle (elemwiseAdd (initPM 11609) (elemwiseAdd (fw_all2all_moe_gmm_full (initPM 11613) ((fw_topk_routing (initPM 11621) 8 64).fst) ((fw_topk_routing (initPM 11621) 8 64).snd.fst) [initPM 11629, initPM 11630] [initPM 11631, initPM 11632] 64 8 ((((10 : Nat) : Scalar)))) (elemwiseMul (fw_sigmoid (fw_view [2048, 1] (fw_linear (initPM 11613) (initPM 5906)))) (fw_view [2048, 1024] (fw_linear (fw_swiglu (fw_view [2048, 512] (fw_linear (initPM 11613) (initPM 5911))) (fw_view [2048, 512] (fw_linear (initPM 11613) (initPM 5915)))) (initPM 5920)))))) (initPM 5927) 2 0) (initPM 5929)) (initPM 5931) (chunkPrimDimN 0 pm_goal_1_cut.numRanks 0 (initPM 4678)) (((initPM 5931).shape.head?).getD 0) ((((0 : Nat) : Scalar)))).fst, (fw_inner_chunk_ce (fw_rms_norm (fw_maybe_unshuffle (elemwiseAdd (initPM 11610) (elemwiseAdd (fw_all2all_moe_gmm_full (initPM 11614) ((fw_topk_routing (initPM 11622) 8 64).fst) ((fw_topk_routing (initPM 11622) 8 64).snd.fst) [initPM 11629, initPM 11630] [initPM 11631, initPM 11632] 64 8 ((((10 : Nat) : Scalar)))) (elemwiseMul (fw_sigmoid (fw_view [2048, 1] (fw_linear (initPM 11614) (initPM 5906)))) (fw_view [2048, 1024] (fw_linear (fw_swiglu (fw_view [2048, 512] (fw_linear (initPM 11614) (initPM 5911))) (fw_view [2048, 512] (fw_linear (initPM 11614) (initPM 5915)))) (initPM 5920)))))) (initPM 5927) 2 1) (initPM 5929)) (initPM 5931) (chunkPrimDimN 0 pm_goal_1_cut.numRanks 1 (initPM 4678)) (((initPM 5931).shape.head?).getD 0) ((((0 : Nat) : Scalar)))).fst] := by
  have h_split : pm_goal_1_cut.nodes = pm_goal_1_cut.nodes.take 52 ++
      [{ rank := 0, op := "OpName.AllGatherPrim", ins := [11837, 11838], outs := [4673], params := [0] }] := by
    show pm_goal_1_cut.nodes = _
    rfl
  unfold denoteGraph
  rw [h_split, List.foldl_append, List.foldl_cons, List.foldl_nil]
  set P52 : Store := (pm_goal_1_cut.nodes.take 52).foldl (applyNode pm_goal_1_cut) initPM
  set P1 : Store := (pm_goal_1_cut.nodes.take 1).foldl (applyNode pm_goal_1_cut) initPM with hP1_def
  set P2 : Store := (pm_goal_1_cut.nodes.take 2).foldl (applyNode pm_goal_1_cut) initPM with hP2_def
  set P3 : Store := (pm_goal_1_cut.nodes.take 3).foldl (applyNode pm_goal_1_cut) initPM with hP3_def
  set P4 : Store := (pm_goal_1_cut.nodes.take 4).foldl (applyNode pm_goal_1_cut) initPM with hP4_def
  set P5 : Store := (pm_goal_1_cut.nodes.take 5).foldl (applyNode pm_goal_1_cut) initPM with hP5_def
  set P6 : Store := (pm_goal_1_cut.nodes.take 6).foldl (applyNode pm_goal_1_cut) initPM with hP6_def
  set P7 : Store := (pm_goal_1_cut.nodes.take 7).foldl (applyNode pm_goal_1_cut) initPM with hP7_def
  set P8 : Store := (pm_goal_1_cut.nodes.take 8).foldl (applyNode pm_goal_1_cut) initPM with hP8_def
  set P9 : Store := (pm_goal_1_cut.nodes.take 9).foldl (applyNode pm_goal_1_cut) initPM with hP9_def
  set P10 : Store := (pm_goal_1_cut.nodes.take 10).foldl (applyNode pm_goal_1_cut) initPM with hP10_def
  set P11 : Store := (pm_goal_1_cut.nodes.take 11).foldl (applyNode pm_goal_1_cut) initPM with hP11_def
  set P12 : Store := (pm_goal_1_cut.nodes.take 12).foldl (applyNode pm_goal_1_cut) initPM with hP12_def
  set P13 : Store := (pm_goal_1_cut.nodes.take 13).foldl (applyNode pm_goal_1_cut) initPM with hP13_def
  set P14 : Store := (pm_goal_1_cut.nodes.take 14).foldl (applyNode pm_goal_1_cut) initPM with hP14_def
  set P15 : Store := (pm_goal_1_cut.nodes.take 15).foldl (applyNode pm_goal_1_cut) initPM with hP15_def
  set P16 : Store := (pm_goal_1_cut.nodes.take 16).foldl (applyNode pm_goal_1_cut) initPM with hP16_def
  set P17 : Store := (pm_goal_1_cut.nodes.take 17).foldl (applyNode pm_goal_1_cut) initPM with hP17_def
  set P18 : Store := (pm_goal_1_cut.nodes.take 18).foldl (applyNode pm_goal_1_cut) initPM with hP18_def
  set P19 : Store := (pm_goal_1_cut.nodes.take 19).foldl (applyNode pm_goal_1_cut) initPM with hP19_def
  set P20 : Store := (pm_goal_1_cut.nodes.take 20).foldl (applyNode pm_goal_1_cut) initPM with hP20_def
  set P21 : Store := (pm_goal_1_cut.nodes.take 21).foldl (applyNode pm_goal_1_cut) initPM with hP21_def
  set P22 : Store := (pm_goal_1_cut.nodes.take 22).foldl (applyNode pm_goal_1_cut) initPM with hP22_def
  set P23 : Store := (pm_goal_1_cut.nodes.take 23).foldl (applyNode pm_goal_1_cut) initPM with hP23_def
  set P24 : Store := (pm_goal_1_cut.nodes.take 24).foldl (applyNode pm_goal_1_cut) initPM with hP24_def
  set P25 : Store := (pm_goal_1_cut.nodes.take 25).foldl (applyNode pm_goal_1_cut) initPM with hP25_def
  set P26 : Store := (pm_goal_1_cut.nodes.take 26).foldl (applyNode pm_goal_1_cut) initPM with hP26_def
  set P27 : Store := (pm_goal_1_cut.nodes.take 27).foldl (applyNode pm_goal_1_cut) initPM with hP27_def
  set P28 : Store := (pm_goal_1_cut.nodes.take 28).foldl (applyNode pm_goal_1_cut) initPM with hP28_def
  set P29 : Store := (pm_goal_1_cut.nodes.take 29).foldl (applyNode pm_goal_1_cut) initPM with hP29_def
  set P30 : Store := (pm_goal_1_cut.nodes.take 30).foldl (applyNode pm_goal_1_cut) initPM with hP30_def
  set P31 : Store := (pm_goal_1_cut.nodes.take 31).foldl (applyNode pm_goal_1_cut) initPM with hP31_def
  set P32 : Store := (pm_goal_1_cut.nodes.take 32).foldl (applyNode pm_goal_1_cut) initPM with hP32_def
  set P33 : Store := (pm_goal_1_cut.nodes.take 33).foldl (applyNode pm_goal_1_cut) initPM with hP33_def
  set P34 : Store := (pm_goal_1_cut.nodes.take 34).foldl (applyNode pm_goal_1_cut) initPM with hP34_def
  set P35 : Store := (pm_goal_1_cut.nodes.take 35).foldl (applyNode pm_goal_1_cut) initPM with hP35_def
  set P36 : Store := (pm_goal_1_cut.nodes.take 36).foldl (applyNode pm_goal_1_cut) initPM with hP36_def
  set P37 : Store := (pm_goal_1_cut.nodes.take 37).foldl (applyNode pm_goal_1_cut) initPM with hP37_def
  set P38 : Store := (pm_goal_1_cut.nodes.take 38).foldl (applyNode pm_goal_1_cut) initPM with hP38_def
  set P39 : Store := (pm_goal_1_cut.nodes.take 39).foldl (applyNode pm_goal_1_cut) initPM with hP39_def
  set P40 : Store := (pm_goal_1_cut.nodes.take 40).foldl (applyNode pm_goal_1_cut) initPM with hP40_def
  set P41 : Store := (pm_goal_1_cut.nodes.take 41).foldl (applyNode pm_goal_1_cut) initPM with hP41_def
  set P42 : Store := (pm_goal_1_cut.nodes.take 42).foldl (applyNode pm_goal_1_cut) initPM with hP42_def
  set P43 : Store := (pm_goal_1_cut.nodes.take 43).foldl (applyNode pm_goal_1_cut) initPM with hP43_def
  set P44 : Store := (pm_goal_1_cut.nodes.take 44).foldl (applyNode pm_goal_1_cut) initPM with hP44_def
  set P45 : Store := (pm_goal_1_cut.nodes.take 45).foldl (applyNode pm_goal_1_cut) initPM with hP45_def
  set P46 : Store := (pm_goal_1_cut.nodes.take 46).foldl (applyNode pm_goal_1_cut) initPM with hP46_def
  set P47 : Store := (pm_goal_1_cut.nodes.take 47).foldl (applyNode pm_goal_1_cut) initPM with hP47_def
  set P48 : Store := (pm_goal_1_cut.nodes.take 48).foldl (applyNode pm_goal_1_cut) initPM with hP48_def
  set P49 : Store := (pm_goal_1_cut.nodes.take 49).foldl (applyNode pm_goal_1_cut) initPM with hP49_def
  set P50 : Store := (pm_goal_1_cut.nodes.take 50).foldl (applyNode pm_goal_1_cut) initPM with hP50_def
  set P51 : Store := (pm_goal_1_cut.nodes.take 51).foldl (applyNode pm_goal_1_cut) initPM with hP51_def
  rw [applyNode_allGatherPrimDimN_out pm_goal_1_cut P52 0 [11837, 11838] 4673 0]
  simp only [List.map, List.map_cons, List.map_nil]
  have hP52_11837 : P52 11837 = P51 11837 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11837 51 52 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=52 written by pm_node_51 (rank=1 FW_inner_chunk_ce), outs=[11838, 11840], tid=11838
  have h_pmtake52_11838 : pm_goal_1_cut.nodes.take 52 = pm_goal_1_cut.nodes.take 51 ++ [{ rank := 1, op := "OpName.FW_inner_chunk_ce", ins := [11834, 5931, 11836], outs := [11838, 11840], params := [1024] }] := by rfl
  have hP52_eq_11838 : P52 = applyNode pm_goal_1_cut P51 { rank := 1, op := "OpName.FW_inner_chunk_ce", ins := [11834, 5931, 11836], outs := [11838, 11840], params := [1024] } := by
    show (pm_goal_1_cut.nodes.take 52).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP52_11838 : P52 11838 = (fw_inner_chunk_ce (P51 11834) (P51 5931) (P51 11836) ((((P51 5931).shape.head?).getD 0)) ((((0 : Nat) : Scalar)))).fst := by
    rw [hP52_eq_11838]
    exact applyNode_fw_inner_chunk_ce_fst_out_1p pm_goal_1_cut P51 1 11834 5931 11836 11838 11840 [1024]
  -- Pj=51 written by pm_node_50 (rank=0 FW_inner_chunk_ce), outs=[11837, 11839], tid=11837
  have h_pmtake51_11837 : pm_goal_1_cut.nodes.take 51 = pm_goal_1_cut.nodes.take 50 ++ [{ rank := 0, op := "OpName.FW_inner_chunk_ce", ins := [11833, 5931, 11835], outs := [11837, 11839], params := [1024] }] := by rfl
  have hP51_eq_11837 : P51 = applyNode pm_goal_1_cut P50 { rank := 0, op := "OpName.FW_inner_chunk_ce", ins := [11833, 5931, 11835], outs := [11837, 11839], params := [1024] } := by
    show (pm_goal_1_cut.nodes.take 51).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP51_11837 : P51 11837 = (fw_inner_chunk_ce (P50 11833) (P50 5931) (P50 11835) ((((P50 5931).shape.head?).getD 0)) ((((0 : Nat) : Scalar)))).fst := by
    rw [hP51_eq_11837]
    exact applyNode_fw_inner_chunk_ce_fst_out_1p pm_goal_1_cut P50 0 11833 5931 11835 11837 11839 [1024]
  have hP51_11834 : P51 11834 = P50 11834 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11834 50 51 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP51_5931 : P51 5931 = initPM 5931 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 51) initPM 5931
      (by intro n hn; fin_cases hn <;> decide)
  have hP51_11836 : P51 11836 = P2 11836 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11836 2 51 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP50_11833 : P50 11833 = P49 11833 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11833 49 50 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP50_5931 : P50 5931 = initPM 5931 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 50) initPM 5931
      (by intro n hn; fin_cases hn <;> decide)
  have hP50_11835 : P50 11835 = P1 11835 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11835 1 50 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=50 written by pm_node_49 (rank=1 FW_rms_norm), outs=[11834], tid=11834
  have h_pmtake50_11834 : pm_goal_1_cut.nodes.take 50 = pm_goal_1_cut.nodes.take 49 ++ [{ rank := 1, op := "OpName.FW_rms_norm", ins := [11728, 5929], outs := [11834] }] := by rfl
  have hP50_eq_11834 : P50 = applyNode pm_goal_1_cut P49 { rank := 1, op := "OpName.FW_rms_norm", ins := [11728, 5929], outs := [11834] } := by
    show (pm_goal_1_cut.nodes.take 50).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP50_11834 : P50 11834 = fw_rms_norm (P49 11728) (P49 5929) := by
    rw [hP50_eq_11834]
    exact applyNode_fw_rms_norm_out_1p pm_goal_1_cut P49 1 11728 5929 11834
  -- Pj=2 written by pm_node_1 (rank=1 ChunkPrim), outs=[11836], tid=11836
  have h_pmtake2_11836 : pm_goal_1_cut.nodes.take 2 = pm_goal_1_cut.nodes.take 1 ++ [{ rank := 1, op := "OpName.ChunkPrim", ins := [4678], outs := [11836], params := [0] }] := by rfl
  have hP2_eq_11836 : P2 = applyNode pm_goal_1_cut P1 { rank := 1, op := "OpName.ChunkPrim", ins := [4678], outs := [11836], params := [0] } := by
    show (pm_goal_1_cut.nodes.take 2).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake2_11836, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP2_11836 : P2 11836 = chunkPrimDimN 0 pm_goal_1_cut.numRanks 1 (P1 4678) := by
    rw [hP2_eq_11836]
    exact applyNode_chunkPrimDimN_out pm_goal_1_cut P1 1 4678 11836 0
  -- Pj=49 written by pm_node_48 (rank=0 FW_rms_norm), outs=[11833], tid=11833
  have h_pmtake49_11833 : pm_goal_1_cut.nodes.take 49 = pm_goal_1_cut.nodes.take 48 ++ [{ rank := 0, op := "OpName.FW_rms_norm", ins := [11727, 5929], outs := [11833] }] := by rfl
  have hP49_eq_11833 : P49 = applyNode pm_goal_1_cut P48 { rank := 0, op := "OpName.FW_rms_norm", ins := [11727, 5929], outs := [11833] } := by
    show (pm_goal_1_cut.nodes.take 49).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP49_11833 : P49 11833 = fw_rms_norm (P48 11727) (P48 5929) := by
    rw [hP49_eq_11833]
    exact applyNode_fw_rms_norm_out_1p pm_goal_1_cut P48 0 11727 5929 11833
  -- Pj=1 written by pm_node_0 (rank=0 ChunkPrim), outs=[11835], tid=11835
  have h_pmtake1_11835 : pm_goal_1_cut.nodes.take 1 = pm_goal_1_cut.nodes.take 0 ++ [{ rank := 0, op := "OpName.ChunkPrim", ins := [4678], outs := [11835], params := [0] }] := by rfl
  have hP1_eq_11835 : P1 = applyNode pm_goal_1_cut initPM { rank := 0, op := "OpName.ChunkPrim", ins := [4678], outs := [11835], params := [0] } := by
    show (pm_goal_1_cut.nodes.take 1).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake1_11835, List.foldl_append, List.foldl_cons, List.foldl_nil]
    rfl
  have hP1_11835 : P1 11835 = chunkPrimDimN 0 pm_goal_1_cut.numRanks 0 (initPM 4678) := by
    rw [hP1_eq_11835]
    exact applyNode_chunkPrimDimN_out pm_goal_1_cut initPM 0 4678 11835 0
  have hP49_11728 : P49 11728 = P48 11728 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11728 48 49 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP49_5929 : P49 5929 = initPM 5929 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 49) initPM 5929
      (by intro n hn; fin_cases hn <;> decide)
  have hP1_4678 : P1 4678 = initPM 4678 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 1) initPM 4678
      (by intro n hn; fin_cases hn <;> decide)
  have hP48_11727 : P48 11727 = P47 11727 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11727 47 48 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP48_5929 : P48 5929 = initPM 5929 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 48) initPM 5929
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=48 written by pm_node_47 (rank=1 FW_maybe_unshuffle), outs=[11728], tid=11728
  have h_pmtake48_11728 : pm_goal_1_cut.nodes.take 48 = pm_goal_1_cut.nodes.take 47 ++ [{ rank := 1, op := "OpName.FW_maybe_unshuffle", ins := [11722, 5927], outs := [11728], params := [2, 1] }] := by rfl
  have hP48_eq_11728 : P48 = applyNode pm_goal_1_cut P47 { rank := 1, op := "OpName.FW_maybe_unshuffle", ins := [11722, 5927], outs := [11728], params := [2, 1] } := by
    show (pm_goal_1_cut.nodes.take 48).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP48_11728 : P48 11728 = fw_maybe_unshuffle (P47 11722) (P47 5927) 2 1 := by
    rw [hP48_eq_11728]
    exact applyNode_fw_maybe_unshuffle_out_1p pm_goal_1_cut P47 1 11722 5927 11728 [2, 1]
  -- Pj=47 written by pm_node_46 (rank=0 FW_maybe_unshuffle), outs=[11727], tid=11727
  have h_pmtake47_11727 : pm_goal_1_cut.nodes.take 47 = pm_goal_1_cut.nodes.take 46 ++ [{ rank := 0, op := "OpName.FW_maybe_unshuffle", ins := [11721, 5927], outs := [11727], params := [2, 0] }] := by rfl
  have hP47_eq_11727 : P47 = applyNode pm_goal_1_cut P46 { rank := 0, op := "OpName.FW_maybe_unshuffle", ins := [11721, 5927], outs := [11727], params := [2, 0] } := by
    show (pm_goal_1_cut.nodes.take 47).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP47_11727 : P47 11727 = fw_maybe_unshuffle (P46 11721) (P46 5927) 2 0 := by
    rw [hP47_eq_11727]
    exact applyNode_fw_maybe_unshuffle_out_1p pm_goal_1_cut P46 0 11721 5927 11727 [2, 0]
  have hP47_11722 : P47 11722 = P46 11722 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11722 46 47 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP47_5927 : P47 5927 = initPM 5927 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 47) initPM 5927
      (by intro n hn; fin_cases hn <;> decide)
  have hP46_11721 : P46 11721 = P45 11721 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11721 45 46 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP46_5927 : P46 5927 = initPM 5927 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 46) initPM 5927
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=46 written by pm_node_45 (rank=1 FW_add), outs=[11722], tid=11722
  have h_pmtake46_11722 : pm_goal_1_cut.nodes.take 46 = pm_goal_1_cut.nodes.take 45 ++ [{ rank := 1, op := "OpName.FW_add", ins := [16855, 11718], outs := [11722] }] := by rfl
  have hP46_eq_11722 : P46 = applyNode pm_goal_1_cut P45 { rank := 1, op := "OpName.FW_add", ins := [16855, 11718], outs := [11722] } := by
    show (pm_goal_1_cut.nodes.take 46).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP46_11722 : P46 11722 = elemwiseAdd (P45 16855) (P45 11718) := by
    rw [hP46_eq_11722]
    exact applyNode_fw_add2_out pm_goal_1_cut P45 1 16855 11718 11722
  -- Pj=45 written by pm_node_44 (rank=0 FW_add), outs=[11721], tid=11721
  have h_pmtake45_11721 : pm_goal_1_cut.nodes.take 45 = pm_goal_1_cut.nodes.take 44 ++ [{ rank := 0, op := "OpName.FW_add", ins := [16847, 11717], outs := [11721] }] := by rfl
  have hP45_eq_11721 : P45 = applyNode pm_goal_1_cut P44 { rank := 0, op := "OpName.FW_add", ins := [16847, 11717], outs := [11721] } := by
    show (pm_goal_1_cut.nodes.take 45).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP45_11721 : P45 11721 = elemwiseAdd (P44 16847) (P44 11717) := by
    rw [hP45_eq_11721]
    exact applyNode_fw_add2_out pm_goal_1_cut P44 0 16847 11717 11721
  have hP45_16855 : P45 16855 = P4 16855 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16855 4 45 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP45_11718 : P45 11718 = P44 11718 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11718 44 45 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP44_16847 : P44 16847 = P3 16847 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16847 3 44 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP44_11717 : P44 11717 = P43 11717 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11717 43 44 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=4 written by pm_node_3 (rank=1 FW_multiref), outs=[16851, 16855], tid=16855
  have h_pmtake4_16855 : pm_goal_1_cut.nodes.take 4 = pm_goal_1_cut.nodes.take 3 ++ [{ rank := 1, op := "OpName.FW_multiref", ins := [11610], outs := [16851, 16855], params := [2] }] := by rfl
  have hP4_eq_16855 : P4 = applyNode pm_goal_1_cut P3 { rank := 1, op := "OpName.FW_multiref", ins := [11610], outs := [16851, 16855], params := [2] } := by
    show (pm_goal_1_cut.nodes.take 4).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake4_16855, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP4_16855 : P4 16855 = P3 11610 := by
    rw [hP4_eq_16855]
    exact applyNode_fw_multiref2_second_out_g259 pm_goal_1_cut P3 1 11610 16851 16855 (by decide)
  -- Pj=44 written by pm_node_43 (rank=1 FW_float), outs=[11718], tid=11718
  have h_pmtake44_11718 : pm_goal_1_cut.nodes.take 44 = pm_goal_1_cut.nodes.take 43 ++ [{ rank := 1, op := "OpName.FW_float", ins := [11712], outs := [11718] }] := by rfl
  have hP44_eq_11718 : P44 = applyNode pm_goal_1_cut P43 { rank := 1, op := "OpName.FW_float", ins := [11712], outs := [11718] } := by
    show (pm_goal_1_cut.nodes.take 44).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake44_11718, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP44_11718 : P44 11718 = P43 11712 := by
    rw [hP44_eq_11718]
    exact applyNode_fw_float_out pm_goal_1_cut P43 1 11712 11718 []
  -- Pj=3 written by pm_node_2 (rank=0 FW_multiref), outs=[16843, 16847], tid=16847
  have h_pmtake3_16847 : pm_goal_1_cut.nodes.take 3 = pm_goal_1_cut.nodes.take 2 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [11609], outs := [16843, 16847], params := [2] }] := by rfl
  have hP3_eq_16847 : P3 = applyNode pm_goal_1_cut P2 { rank := 0, op := "OpName.FW_multiref", ins := [11609], outs := [16843, 16847], params := [2] } := by
    show (pm_goal_1_cut.nodes.take 3).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake3_16847, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP3_16847 : P3 16847 = P2 11609 := by
    rw [hP3_eq_16847]
    exact applyNode_fw_multiref2_second_out_g259 pm_goal_1_cut P2 0 11609 16843 16847 (by decide)
  -- Pj=43 written by pm_node_42 (rank=0 FW_float), outs=[11717], tid=11717
  have h_pmtake43_11717 : pm_goal_1_cut.nodes.take 43 = pm_goal_1_cut.nodes.take 42 ++ [{ rank := 0, op := "OpName.FW_float", ins := [11711], outs := [11717] }] := by rfl
  have hP43_eq_11717 : P43 = applyNode pm_goal_1_cut P42 { rank := 0, op := "OpName.FW_float", ins := [11711], outs := [11717] } := by
    show (pm_goal_1_cut.nodes.take 43).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake43_11717, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP43_11717 : P43 11717 = P42 11711 := by
    rw [hP43_eq_11717]
    exact applyNode_fw_float_out pm_goal_1_cut P42 0 11711 11717 []
  have hP3_11610 : P3 11610 = initPM 11610 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 3) initPM 11610
      (by intro n hn; fin_cases hn <;> decide)
  have hP43_11712 : P43 11712 = P42 11712 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11712 42 43 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP2_11609 : P2 11609 = initPM 11609 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 2) initPM 11609
      (by intro n hn; fin_cases hn <;> decide)
  have hP42_11711 : P42 11711 = P41 11711 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11711 41 42 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=42 written by pm_node_41 (rank=1 FW_add), outs=[11712], tid=11712
  have h_pmtake42_11712 : pm_goal_1_cut.nodes.take 42 = pm_goal_1_cut.nodes.take 41 ++ [{ rank := 1, op := "OpName.FW_add", ins := [11634, 11708], outs := [11712] }] := by rfl
  have hP42_eq_11712 : P42 = applyNode pm_goal_1_cut P41 { rank := 1, op := "OpName.FW_add", ins := [11634, 11708], outs := [11712] } := by
    show (pm_goal_1_cut.nodes.take 42).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP42_11712 : P42 11712 = elemwiseAdd (P41 11634) (P41 11708) := by
    rw [hP42_eq_11712]
    exact applyNode_fw_add2_out pm_goal_1_cut P41 1 11634 11708 11712
  -- Pj=41 written by pm_node_40 (rank=0 FW_add), outs=[11711], tid=11711
  have h_pmtake41_11711 : pm_goal_1_cut.nodes.take 41 = pm_goal_1_cut.nodes.take 40 ++ [{ rank := 0, op := "OpName.FW_add", ins := [11633, 11707], outs := [11711] }] := by rfl
  have hP41_eq_11711 : P41 = applyNode pm_goal_1_cut P40 { rank := 0, op := "OpName.FW_add", ins := [11633, 11707], outs := [11711] } := by
    show (pm_goal_1_cut.nodes.take 41).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP41_11711 : P41 11711 = elemwiseAdd (P40 11633) (P40 11707) := by
    rw [hP41_eq_11711]
    exact applyNode_fw_add2_out pm_goal_1_cut P40 0 11633 11707 11711
  have hP41_11634 : P41 11634 = P30 11634 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11634 30 41 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP41_11708 : P41 11708 = P40 11708 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11708 40 41 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP40_11633 : P40 11633 = P27 11633 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11633 27 40 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP40_11707 : P40 11707 = P39 11707 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11707 39 40 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=30 written by pm_node_29 (rank=1 FW_all2all_moe_gmm_full), outs=[11634], tid=11634
  have h_pmtake30_11634 : pm_goal_1_cut.nodes.take 30 = pm_goal_1_cut.nodes.take 29 ++ [{ rank := 1, op := "OpName.FW_all2all_moe_gmm_full", ins := [16889, 11624, 11626, 11629, 11630, 11631, 11632], outs := [11634], params := [64, 8] }] := by rfl
  have hP30_eq_11634 : P30 = applyNode pm_goal_1_cut P29 { rank := 1, op := "OpName.FW_all2all_moe_gmm_full", ins := [16889, 11624, 11626, 11629, 11630, 11631, 11632], outs := [11634], params := [64, 8] } := by
    show (pm_goal_1_cut.nodes.take 30).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP30_11634 : P30 11634 = fw_all2all_moe_gmm_full (P29 16889) (P29 11624) (P29 11626)
    ...
    rw [hP30_eq_11634]
    exact applyNode_fw_all2all_moe_gmm_full_out_1p_r2 pm_goal_1_cut P29 1
      16889 11624 11626 11629 11630 11631 11632 11634 [64, 8]
  -- Pj=40 written by pm_node_39 (rank=1 FW_mul), outs=[11708], tid=11708
  have h_pmtake40_11708 : pm_goal_1_cut.nodes.take 40 = pm_goal_1_cut.nodes.take 39 ++ [{ rank := 1, op := "OpName.FW_mul", ins := [11648, 11704], outs := [11708] }] := by rfl
  have hP40_eq_11708 : P40 = applyNode pm_goal_1_cut P39 { rank := 1, op := "OpName.FW_mul", ins := [11648, 11704], outs := [11708] } := by
    show (pm_goal_1_cut.nodes.take 40).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP40_11708 : P40 11708 = elemwiseMul (P39 11648) (P39 11704) := by
    rw [hP40_eq_11708]
    exact applyNode_fw_mul_out pm_goal_1_cut P39 1 11648 11704 11708
  -- Pj=27 written by pm_node_26 (rank=0 FW_all2all_moe_gmm_full), outs=[11633], tid=11633
  have h_pmtake27_11633 : pm_goal_1_cut.nodes.take 27 = pm_goal_1_cut.nodes.take 26 ++ [{ rank := 0, op := "OpName.FW_all2all_moe_gmm_full", ins := [16866, 11623, 11625, 11629, 11630, 11631, 11632], outs := [11633], params := [64, 8] }] := by rfl
  have hP27_eq_11633 : P27 = applyNode pm_goal_1_cut P26 { rank := 0, op := "OpName.FW_all2all_moe_gmm_full", ins := [16866, 11623, 11625, 11629, 11630, 11631, 11632], outs := [11633], params := [64, 8] } := by
    show (pm_goal_1_cut.nodes.take 27).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP27_11633 : P27 11633 = fw_all2all_moe_gmm_full (P26 16866) (P26 11623) (P26 11625)
    ...
    rw [hP27_eq_11633]
    exact applyNode_fw_all2all_moe_gmm_full_out_1p_r2 pm_goal_1_cut P26 0
      16866 11623 11625 11629 11630 11631 11632 11633 [64, 8]
  -- Pj=39 written by pm_node_38 (rank=0 FW_mul), outs=[11707], tid=11707
  have h_pmtake39_11707 : pm_goal_1_cut.nodes.take 39 = pm_goal_1_cut.nodes.take 38 ++ [{ rank := 0, op := "OpName.FW_mul", ins := [11647, 11703], outs := [11707] }] := by rfl
  have hP39_eq_11707 : P39 = applyNode pm_goal_1_cut P38 { rank := 0, op := "OpName.FW_mul", ins := [11647, 11703], outs := [11707] } := by
    show (pm_goal_1_cut.nodes.take 39).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP39_11707 : P39 11707 = elemwiseMul (P38 11647) (P38 11703) := by
    rw [hP39_eq_11707]
    exact applyNode_fw_mul_out pm_goal_1_cut P38 0 11647 11703 11707
  have hP29_16889 : P29 16889 = P6 16889 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16889 6 29 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP29_11624 : P29 11624 = P23 11624 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11624 23 29 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP29_11626 : P29 11626 = P23 11626 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11626 23 29 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP29_11630 : P29 11630 = initPM 11630 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 29) initPM 11630
      (by intro n hn; fin_cases hn <;> decide)
  have hP29_11629 : P29 11629 = initPM 11629 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 29) initPM 11629
      (by intro n hn; fin_cases hn <;> decide)
  have hP29_11632 : P29 11632 = initPM 11632 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 29) initPM 11632
      (by intro n hn; fin_cases hn <;> decide)
  have hP29_11631 : P29 11631 = initPM 11631 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 29) initPM 11631
      (by intro n hn; fin_cases hn <;> decide)
  have hP39_11648 : P39 11648 = P31 11648 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11648 31 39 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP39_11704 : P39 11704 = P38 11704 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11704 38 39 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_16866 : P26 16866 = P5 16866 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16866 5 26 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_11623 : P26 11623 = P19 11623 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11623 19 26 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_11625 : P26 11625 = P19 11625 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11625 19 26 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_11629 : P26 11629 = initPM 11629 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 26) initPM 11629
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_11630 : P26 11630 = initPM 11630 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 26) initPM 11630
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_11631 : P26 11631 = initPM 11631 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 26) initPM 11631
      (by intro n hn; fin_cases hn <;> decide)
  have hP26_11632 : P26 11632 = initPM 11632 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 26) initPM 11632
      (by intro n hn; fin_cases hn <;> decide)
  have hP38_11647 : P38 11647 = P28 11647 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11647 28 38 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP38_11703 : P38 11703 = P37 11703 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11703 37 38 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=6 written by pm_node_5 (rank=1 FW_multiref), outs=[16885, 16889, 16893, 16897, 16901], tid=16889
  have h_pmtake6_16889 : pm_goal_1_cut.nodes.take 6 = pm_goal_1_cut.nodes.take 5 ++ [{ rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] }] := by rfl
  have hP6_eq_16889 : P6 = applyNode pm_goal_1_cut P5 { rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 6).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake6_16889, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP6_16889 : P6 16889 = P5 11614 := by
    rw [hP6_eq_16889]
    exact applyNode_fw_multiref5_at_pos1_out pm_goal_1_cut P5 1 11614 16885 16889 16893 16897 16901 (by decide)
  ...
  have hP22_11622_sh : (P22 11622).shape.reverse.head? = some 64 := by
    have hP22_11622_eq : P22 11622 = initPM 11622 :=
      foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 22) initPM 11622
        (by intro n hn; fin_cases hn <;> decide)
    rw [hP22_11622_eq]; exact h11622
  have h_pmtake23_11624 : pm_goal_1_cut.nodes.take 23 = pm_goal_1_cut.nodes.take 22 ++ [{ rank := 1, op := "OpName.FW_topk_routing", ins := [11622], outs := [11624, 11626, 11628], params := [8, 64] }] := by rfl
  have hP23_eq_11624 : P23 = applyNode pm_goal_1_cut P22 { rank := 1, op := "OpName.FW_topk_routing", ins := [11622], outs := [11624, 11626, 11628], params := [8, 64] } := by
    show (pm_goal_1_cut.nodes.take 23).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP23_11624 : P23 11624 = (fw_topk_routing (P22 11622) 8 64).fst := by
    rw [hP23_eq_11624, applyNode_fw_topk_routing_probs_out pm_goal_1_cut P22 1 11622 11624 11626 11628 [8, 64], hP22_11622_sh]
    rfl
  -- Pj=23 written by pm_node_22 (rank=1 FW_topk_routing), outs=[11624, 11626, 11628], tid=11626
  have h_pmtake23_11626 : pm_goal_1_cut.nodes.take 23 = pm_goal_1_cut.nodes.take 22 ++ [{ rank := 1, op := "OpName.FW_topk_routing", ins := [11622], outs := [11624, 11626, 11628], params := [8, 64] }] := by rfl
  have hP23_eq_11626 : P23 = applyNode pm_goal_1_cut P22 { rank := 1, op := "OpName.FW_topk_routing", ins := [11622], outs := [11624, 11626, 11628], params := [8, 64] } := by
    show (pm_goal_1_cut.nodes.take 23).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP23_11626 : P23 11626 = (fw_topk_routing (P22 11622) 8 64).snd.fst := by
    rw [hP23_eq_11626, applyNode_fw_topk_routing_map_out pm_goal_1_cut P22 1 11622 11624 11626 11628 [8, 64] (by decide), hP22_11622_sh]
    rfl
  -- Pj=31 written by pm_node_30 (rank=1 FW_sigmoid), outs=[11648], tid=11648
  have h_pmtake31_11648 : pm_goal_1_cut.nodes.take 31 = pm_goal_1_cut.nodes.take 30 ++ [{ rank := 1, op := "OpName.FW_sigmoid", ins := [11646], outs := [11648] }] := by rfl
  have hP31_eq_11648 : P31 = applyNode pm_goal_1_cut P30 { rank := 1, op := "OpName.FW_sigmoid", ins := [11646], outs := [11648] } := by
    show (pm_goal_1_cut.nodes.take 31).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake31_11648, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP31_11648 : P31 11648 = fw_sigmoid (P30 11646) := by
    rw [hP31_eq_11648]
    exact applyNode_fw_sigmoid_out_1p pm_goal_1_cut P30 1 11646 11648
  -- Pj=38 written by pm_node_37 (rank=1 FW_view), outs=[11704], tid=11704
  have h_pmtake38_11704 : pm_goal_1_cut.nodes.take 38 = pm_goal_1_cut.nodes.take 37 ++ [{ rank := 1, op := "OpName.FW_view", ins := [11694], outs := [11704], params := [2048, 1024] }] := by rfl
  have hP38_eq_11704 : P38 = applyNode pm_goal_1_cut P37 { rank := 1, op := "OpName.FW_view", ins := [11694], outs := [11704], params := [2048, 1024] } := by
    show (pm_goal_1_cut.nodes.take 38).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP38_11704 : P38 11704 = fw_view [2048, 1024] (P37 11694) := by
    rw [hP38_eq_11704]
    exact applyNode_fw_view_out pm_goal_1_cut P37 1 2048 [1024] 11694 11704
  -- Pj=5 written by pm_node_4 (rank=0 FW_multiref), outs=[16862, 16866, 16870, 16874, 16878], tid=16866
  have h_pmtake5_16866 : pm_goal_1_cut.nodes.take 5 = pm_goal_1_cut.nodes.take 4 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] }] := by rfl
  have hP5_eq_16866 : P5 = applyNode pm_goal_1_cut P4 { rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 5).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake5_16866, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP5_16866 : P5 16866 = P4 11613 := by
    rw [hP5_eq_16866]
    exact applyNode_fw_multiref5_at_pos1_out pm_goal_1_cut P4 0 11613 16862 16866 16870 16874 16878 (by decide)
  ...
  have hP18_11621_sh : (P18 11621).shape.reverse.head? = some 64 := by
    have hP18_11621_eq : P18 11621 = initPM 11621 :=
      foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 18) initPM 11621
        (by intro n hn; fin_cases hn <;> decide)
    rw [hP18_11621_eq]; exact h11621
  have h_pmtake19_11623 : pm_goal_1_cut.nodes.take 19 = pm_goal_1_cut.nodes.take 18 ++ [{ rank := 0, op := "OpName.FW_topk_routing", ins := [11621], outs := [11623, 11625, 11627], params := [8, 64] }] := by rfl
  have hP19_eq_11623 : P19 = applyNode pm_goal_1_cut P18 { rank := 0, op := "OpName.FW_topk_routing", ins := [11621], outs := [11623, 11625, 11627], params := [8, 64] } := by
    show (pm_goal_1_cut.nodes.take 19).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP19_11623 : P19 11623 = (fw_topk_routing (P18 11621) 8 64).fst := by
    rw [hP19_eq_11623, applyNode_fw_topk_routing_probs_out pm_goal_1_cut P18 0 11621 11623 11625 11627 [8, 64], hP18_11621_sh]
    rfl
  -- Pj=19 written by pm_node_18 (rank=0 FW_topk_routing), outs=[11623, 11625, 11627], tid=11625
  have h_pmtake19_11625 : pm_goal_1_cut.nodes.take 19 = pm_goal_1_cut.nodes.take 18 ++ [{ rank := 0, op := "OpName.FW_topk_routing", ins := [11621], outs := [11623, 11625, 11627], params := [8, 64] }] := by rfl
  have hP19_eq_11625 : P19 = applyNode pm_goal_1_cut P18 { rank := 0, op := "OpName.FW_topk_routing", ins := [11621], outs := [11623, 11625, 11627], params := [8, 64] } := by
    show (pm_goal_1_cut.nodes.take 19).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP19_11625 : P19 11625 = (fw_topk_routing (P18 11621) 8 64).snd.fst := by
    rw [hP19_eq_11625, applyNode_fw_topk_routing_map_out pm_goal_1_cut P18 0 11621 11623 11625 11627 [8, 64] (by decide), hP18_11621_sh]
    rfl
  -- Pj=28 written by pm_node_27 (rank=0 FW_sigmoid), outs=[11647], tid=11647
  have h_pmtake28_11647 : pm_goal_1_cut.nodes.take 28 = pm_goal_1_cut.nodes.take 27 ++ [{ rank := 0, op := "OpName.FW_sigmoid", ins := [11645], outs := [11647] }] := by rfl
  have hP28_eq_11647 : P28 = applyNode pm_goal_1_cut P27 { rank := 0, op := "OpName.FW_sigmoid", ins := [11645], outs := [11647] } := by
    show (pm_goal_1_cut.nodes.take 28).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake28_11647, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP28_11647 : P28 11647 = fw_sigmoid (P27 11645) := by
    rw [hP28_eq_11647]
    exact applyNode_fw_sigmoid_out_1p pm_goal_1_cut P27 0 11645 11647
  -- Pj=37 written by pm_node_36 (rank=0 FW_view), outs=[11703], tid=11703
  have h_pmtake37_11703 : pm_goal_1_cut.nodes.take 37 = pm_goal_1_cut.nodes.take 36 ++ [{ rank := 0, op := "OpName.FW_view", ins := [11693], outs := [11703], params := [2048, 1024] }] := by rfl
  have hP37_eq_11703 : P37 = applyNode pm_goal_1_cut P36 { rank := 0, op := "OpName.FW_view", ins := [11693], outs := [11703], params := [2048, 1024] } := by
    show (pm_goal_1_cut.nodes.take 37).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP37_11703 : P37 11703 = fw_view [2048, 1024] (P36 11693) := by
    rw [hP37_eq_11703]
    exact applyNode_fw_view_out pm_goal_1_cut P36 0 2048 [1024] 11693 11703
  have hP5_11614 : P5 11614 = initPM 11614 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 5) initPM 11614
      (by intro n hn; fin_cases hn <;> decide)
  have hP22_11622 : P22 11622 = initPM 11622 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 22) initPM 11622
      (by intro n hn; fin_cases hn <;> decide)
  have hP30_11646 : P30 11646 = P24 11646 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11646 24 30 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP37_11694 : P37 11694 = P36 11694 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11694 36 37 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP4_11613 : P4 11613 = initPM 11613 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 4) initPM 11613
      (by intro n hn; fin_cases hn <;> decide)
  have hP18_11621 : P18 11621 = initPM 11621 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 18) initPM 11621
      (by intro n hn; fin_cases hn <;> decide)
  have hP27_11645 : P27 11645 = P20 11645 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11645 20 27 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP36_11693 : P36 11693 = P35 11693 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11693 35 36 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=24 written by pm_node_23 (rank=1 FW_view), outs=[11646], tid=11646
  have h_pmtake24_11646 : pm_goal_1_cut.nodes.take 24 = pm_goal_1_cut.nodes.take 23 ++ [{ rank := 1, op := "OpName.FW_view", ins := [11640], outs := [11646], params := [2048, 1] }] := by rfl
  have hP24_eq_11646 : P24 = applyNode pm_goal_1_cut P23 { rank := 1, op := "OpName.FW_view", ins := [11640], outs := [11646], params := [2048, 1] } := by
    show (pm_goal_1_cut.nodes.take 24).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP24_11646 : P24 11646 = fw_view [2048, 1] (P23 11640) := by
    rw [hP24_eq_11646]
    exact applyNode_fw_view_out pm_goal_1_cut P23 1 2048 [1] 11640 11646
  -- Pj=36 written by pm_node_35 (rank=1 FW_mix_precision_linear), outs=[11694], tid=11694
  have h_pmtake36_11694 : pm_goal_1_cut.nodes.take 36 = pm_goal_1_cut.nodes.take 35 ++ [{ rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11688, 5920], outs := [11694] }] := by rfl
  have hP36_eq_11694 : P36 = applyNode pm_goal_1_cut P35 { rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11688, 5920], outs := [11694] } := by
    show (pm_goal_1_cut.nodes.take 36).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP36_11694 : P36 11694 = fw_linear (P35 11688) (P35 5920) := by
    rw [hP36_eq_11694]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P35 1 11688 5920 11694
  -- Pj=20 written by pm_node_19 (rank=0 FW_view), outs=[11645], tid=11645
  have h_pmtake20_11645 : pm_goal_1_cut.nodes.take 20 = pm_goal_1_cut.nodes.take 19 ++ [{ rank := 0, op := "OpName.FW_view", ins := [11639], outs := [11645], params := [2048, 1] }] := by rfl
  have hP20_eq_11645 : P20 = applyNode pm_goal_1_cut P19 { rank := 0, op := "OpName.FW_view", ins := [11639], outs := [11645], params := [2048, 1] } := by
    show (pm_goal_1_cut.nodes.take 20).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP20_11645 : P20 11645 = fw_view [2048, 1] (P19 11639) := by
    rw [hP20_eq_11645]
    exact applyNode_fw_view_out pm_goal_1_cut P19 0 2048 [1] 11639 11645
  -- Pj=35 written by pm_node_34 (rank=0 FW_mix_precision_linear), outs=[11693], tid=11693
  have h_pmtake35_11693 : pm_goal_1_cut.nodes.take 35 = pm_goal_1_cut.nodes.take 34 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11687, 5920], outs := [11693] }] := by rfl
  have hP35_eq_11693 : P35 = applyNode pm_goal_1_cut P34 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11687, 5920], outs := [11693] } := by
    show (pm_goal_1_cut.nodes.take 35).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP35_11693 : P35 11693 = fw_linear (P34 11687) (P34 5920) := by
    rw [hP35_eq_11693]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P34 0 11687 5920 11693
  have hP23_11640 : P23 11640 = P16 11640 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11640 16 23 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP35_11688 : P35 11688 = P34 11688 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11688 34 35 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP35_5920 : P35 5920 = initPM 5920 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 35) initPM 5920
      (by intro n hn; fin_cases hn <;> decide)
  have hP19_11639 : P19 11639 = P13 11639 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11639 13 19 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP34_11687 : P34 11687 = P33 11687 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11687 33 34 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP34_5920 : P34 5920 = initPM 5920 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 34) initPM 5920
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=16 written by pm_node_15 (rank=1 FW_mix_precision_linear), outs=[11640], tid=11640
  have h_pmtake16_11640 : pm_goal_1_cut.nodes.take 16 = pm_goal_1_cut.nodes.take 15 ++ [{ rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11636, 5906], outs := [11640] }] := by rfl
  have hP16_eq_11640 : P16 = applyNode pm_goal_1_cut P15 { rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11636, 5906], outs := [11640] } := by
    show (pm_goal_1_cut.nodes.take 16).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP16_11640 : P16 11640 = fw_linear (P15 11636) (P15 5906) := by
    rw [hP16_eq_11640]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P15 1 11636 5906 11640
  -- Pj=34 written by pm_node_33 (rank=1 FW_reshape), outs=[11688], tid=11688
  have h_pmtake34_11688 : pm_goal_1_cut.nodes.take 34 = pm_goal_1_cut.nodes.take 33 ++ [{ rank := 1, op := "OpName.FW_reshape", ins := [11686], outs := [11688] }] := by rfl
  have hP34_eq_11688 : P34 = applyNode pm_goal_1_cut P33 { rank := 1, op := "OpName.FW_reshape", ins := [11686], outs := [11688] } := by
    show (pm_goal_1_cut.nodes.take 34).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake34_11688, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP34_11688 : P34 11688 = P33 11686 := by
    rw [hP34_eq_11688]
    exact applyNode_fw_reshape_out pm_goal_1_cut P33 1 11686 11688 []
  -- Pj=13 written by pm_node_12 (rank=0 FW_mix_precision_linear), outs=[11639], tid=11639
  have h_pmtake13_11639 : pm_goal_1_cut.nodes.take 13 = pm_goal_1_cut.nodes.take 12 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11635, 5906], outs := [11639] }] := by rfl
  have hP13_eq_11639 : P13 = applyNode pm_goal_1_cut P12 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11635, 5906], outs := [11639] } := by
    show (pm_goal_1_cut.nodes.take 13).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP13_11639 : P13 11639 = fw_linear (P12 11635) (P12 5906) := by
    rw [hP13_eq_11639]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P12 0 11635 5906 11639
  -- Pj=33 written by pm_node_32 (rank=0 FW_reshape), outs=[11687], tid=11687
  have h_pmtake33_11687 : pm_goal_1_cut.nodes.take 33 = pm_goal_1_cut.nodes.take 32 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [11685], outs := [11687] }] := by rfl
  have hP33_eq_11687 : P33 = applyNode pm_goal_1_cut P32 { rank := 0, op := "OpName.FW_reshape", ins := [11685], outs := [11687] } := by
    show (pm_goal_1_cut.nodes.take 33).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake33_11687, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP33_11687 : P33 11687 = P32 11685 := by
    rw [hP33_eq_11687]
    exact applyNode_fw_reshape_out pm_goal_1_cut P32 0 11685 11687 []
  have hP15_11636 : P15 11636 = P10 11636 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11636 10 15 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP15_5906 : P15 5906 = initPM 5906 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 15) initPM 5906
      (by intro n hn; fin_cases hn <;> decide)
  have hP33_11686 : P33 11686 = P32 11686 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11686 32 33 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP12_11635 : P12 11635 = P7 11635 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11635 7 12 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP12_5906 : P12 5906 = initPM 5906 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 12) initPM 5906
      (by intro n hn; fin_cases hn <;> decide)
  have hP32_11685 : P32 11685 = P29 11685 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11685 29 32 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=10 written by pm_node_9 (rank=1 FW_reshape), outs=[11636], tid=11636
  have h_pmtake10_11636 : pm_goal_1_cut.nodes.take 10 = pm_goal_1_cut.nodes.take 9 ++ [{ rank := 1, op := "OpName.FW_reshape", ins := [16893], outs := [11636] }] := by rfl
  have hP10_eq_11636 : P10 = applyNode pm_goal_1_cut P9 { rank := 1, op := "OpName.FW_reshape", ins := [16893], outs := [11636] } := by
    show (pm_goal_1_cut.nodes.take 10).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake10_11636, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP10_11636 : P10 11636 = P9 16893 := by
    rw [hP10_eq_11636]
    exact applyNode_fw_reshape_out pm_goal_1_cut P9 1 16893 11636 []
  -- Pj=32 written by pm_node_31 (rank=1 FW_swiglu), outs=[11686], tid=11686
  have h_pmtake32_11686 : pm_goal_1_cut.nodes.take 32 = pm_goal_1_cut.nodes.take 31 ++ [{ rank := 1, op := "OpName.FW_swiglu", ins := [11664, 11682], outs := [11686] }] := by rfl
  have hP32_eq_11686 : P32 = applyNode pm_goal_1_cut P31 { rank := 1, op := "OpName.FW_swiglu", ins := [11664, 11682], outs := [11686] } := by
    show (pm_goal_1_cut.nodes.take 32).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP32_11686 : P32 11686 = fw_swiglu (P31 11664) (P31 11682) := by
    rw [hP32_eq_11686]
    exact applyNode_fw_swiglu_out_1p pm_goal_1_cut P31 1 11664 11682 11686
  -- Pj=7 written by pm_node_6 (rank=0 FW_reshape), outs=[11635], tid=11635
  have h_pmtake7_11635 : pm_goal_1_cut.nodes.take 7 = pm_goal_1_cut.nodes.take 6 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [16870], outs := [11635] }] := by rfl
  have hP7_eq_11635 : P7 = applyNode pm_goal_1_cut P6 { rank := 0, op := "OpName.FW_reshape", ins := [16870], outs := [11635] } := by
    show (pm_goal_1_cut.nodes.take 7).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake7_11635, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP7_11635 : P7 11635 = P6 16870 := by
    rw [hP7_eq_11635]
    exact applyNode_fw_reshape_out pm_goal_1_cut P6 0 16870 11635 []
  -- Pj=29 written by pm_node_28 (rank=0 FW_swiglu), outs=[11685], tid=11685
  have h_pmtake29_11685 : pm_goal_1_cut.nodes.take 29 = pm_goal_1_cut.nodes.take 28 ++ [{ rank := 0, op := "OpName.FW_swiglu", ins := [11663, 11681], outs := [11685] }] := by rfl
  have hP29_eq_11685 : P29 = applyNode pm_goal_1_cut P28 { rank := 0, op := "OpName.FW_swiglu", ins := [11663, 11681], outs := [11685] } := by
    show (pm_goal_1_cut.nodes.take 29).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP29_11685 : P29 11685 = fw_swiglu (P28 11663) (P28 11681) := by
    rw [hP29_eq_11685]
    exact applyNode_fw_swiglu_out_1p pm_goal_1_cut P28 0 11663 11681 11685
  have hP9_16893 : P9 16893 = P6 16893 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16893 6 9 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP31_11664 : P31 11664 = P25 11664 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11664 25 31 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP31_11682 : P31 11682 = P26 11682 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11682 26 31 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP6_16870 : P6 16870 = P5 16870 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16870 5 6 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP28_11663 : P28 11663 = P21 11663 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11663 21 28 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP28_11681 : P28 11681 = P22 11681 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11681 22 28 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=6 written by pm_node_5 (rank=1 FW_multiref), outs=[16885, 16889, 16893, 16897, 16901], tid=16893
  have h_pmtake6_16893 : pm_goal_1_cut.nodes.take 6 = pm_goal_1_cut.nodes.take 5 ++ [{ rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] }] := by rfl
  have hP6_eq_16893 : P6 = applyNode pm_goal_1_cut P5 { rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 6).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake6_16893, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP6_16893 : P6 16893 = P5 11614 := by
    rw [hP6_eq_16893]
    exact applyNode_fw_multiref5_at_pos2_out pm_goal_1_cut P5 1 11614 16885 16889 16893 16897 16901 (by decide) (by decide)
  -- Pj=25 written by pm_node_24 (rank=1 FW_view), outs=[11664], tid=11664
  have h_pmtake25_11664 : pm_goal_1_cut.nodes.take 25 = pm_goal_1_cut.nodes.take 24 ++ [{ rank := 1, op := "OpName.FW_view", ins := [11654], outs := [11664], params := [2048, 512] }] := by rfl
  have hP25_eq_11664 : P25 = applyNode pm_goal_1_cut P24 { rank := 1, op := "OpName.FW_view", ins := [11654], outs := [11664], params := [2048, 512] } := by
    show (pm_goal_1_cut.nodes.take 25).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP25_11664 : P25 11664 = fw_view [2048, 512] (P24 11654) := by
    rw [hP25_eq_11664]
    exact applyNode_fw_view_out pm_goal_1_cut P24 1 2048 [512] 11654 11664
  -- Pj=26 written by pm_node_25 (rank=1 FW_view), outs=[11682], tid=11682
  have h_pmtake26_11682 : pm_goal_1_cut.nodes.take 26 = pm_goal_1_cut.nodes.take 25 ++ [{ rank := 1, op := "OpName.FW_view", ins := [11672], outs := [11682], params := [2048, 512] }] := by rfl
  have hP26_eq_11682 : P26 = applyNode pm_goal_1_cut P25 { rank := 1, op := "OpName.FW_view", ins := [11672], outs := [11682], params := [2048, 512] } := by
    show (pm_goal_1_cut.nodes.take 26).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP26_11682 : P26 11682 = fw_view [2048, 512] (P25 11672) := by
    rw [hP26_eq_11682]
    exact applyNode_fw_view_out pm_goal_1_cut P25 1 2048 [512] 11672 11682
  -- Pj=5 written by pm_node_4 (rank=0 FW_multiref), outs=[16862, 16866, 16870, 16874, 16878], tid=16870
  have h_pmtake5_16870 : pm_goal_1_cut.nodes.take 5 = pm_goal_1_cut.nodes.take 4 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] }] := by rfl
  have hP5_eq_16870 : P5 = applyNode pm_goal_1_cut P4 { rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 5).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake5_16870, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP5_16870 : P5 16870 = P4 11613 := by
    rw [hP5_eq_16870]
    exact applyNode_fw_multiref5_at_pos2_out pm_goal_1_cut P4 0 11613 16862 16866 16870 16874 16878 (by decide) (by decide)
  -- Pj=21 written by pm_node_20 (rank=0 FW_view), outs=[11663], tid=11663
  have h_pmtake21_11663 : pm_goal_1_cut.nodes.take 21 = pm_goal_1_cut.nodes.take 20 ++ [{ rank := 0, op := "OpName.FW_view", ins := [11653], outs := [11663], params := [2048, 512] }] := by rfl
  have hP21_eq_11663 : P21 = applyNode pm_goal_1_cut P20 { rank := 0, op := "OpName.FW_view", ins := [11653], outs := [11663], params := [2048, 512] } := by
    show (pm_goal_1_cut.nodes.take 21).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP21_11663 : P21 11663 = fw_view [2048, 512] (P20 11653) := by
    rw [hP21_eq_11663]
    exact applyNode_fw_view_out pm_goal_1_cut P20 0 2048 [512] 11653 11663
  -- Pj=22 written by pm_node_21 (rank=0 FW_view), outs=[11681], tid=11681
  have h_pmtake22_11681 : pm_goal_1_cut.nodes.take 22 = pm_goal_1_cut.nodes.take 21 ++ [{ rank := 0, op := "OpName.FW_view", ins := [11671], outs := [11681], params := [2048, 512] }] := by rfl
  have hP22_eq_11681 : P22 = applyNode pm_goal_1_cut P21 { rank := 0, op := "OpName.FW_view", ins := [11671], outs := [11681], params := [2048, 512] } := by
    show (pm_goal_1_cut.nodes.take 22).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP22_11681 : P22 11681 = fw_view [2048, 512] (P21 11671) := by
    rw [hP22_eq_11681]
    exact applyNode_fw_view_out pm_goal_1_cut P21 0 2048 [512] 11671 11681
  have hP24_11654 : P24 11654 = P17 11654 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11654 17 24 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP25_11672 : P25 11672 = P18 11672 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11672 18 25 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP20_11653 : P20 11653 = P14 11653 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11653 14 20 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP21_11671 : P21 11671 = P15 11671 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11671 15 21 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=17 written by pm_node_16 (rank=1 FW_mix_precision_linear), outs=[11654], tid=11654
  have h_pmtake17_11654 : pm_goal_1_cut.nodes.take 17 = pm_goal_1_cut.nodes.take 16 ++ [{ rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11650, 5911], outs := [11654] }] := by rfl
  have hP17_eq_11654 : P17 = applyNode pm_goal_1_cut P16 { rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11650, 5911], outs := [11654] } := by
    show (pm_goal_1_cut.nodes.take 17).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP17_11654 : P17 11654 = fw_linear (P16 11650) (P16 5911) := by
    rw [hP17_eq_11654]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P16 1 11650 5911 11654
  -- Pj=18 written by pm_node_17 (rank=1 FW_mix_precision_linear), outs=[11672], tid=11672
  have h_pmtake18_11672 : pm_goal_1_cut.nodes.take 18 = pm_goal_1_cut.nodes.take 17 ++ [{ rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11668, 5915], outs := [11672] }] := by rfl
  have hP18_eq_11672 : P18 = applyNode pm_goal_1_cut P17 { rank := 1, op := "OpName.FW_mix_precision_linear", ins := [11668, 5915], outs := [11672] } := by
    show (pm_goal_1_cut.nodes.take 18).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP18_11672 : P18 11672 = fw_linear (P17 11668) (P17 5915) := by
    rw [hP18_eq_11672]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P17 1 11668 5915 11672
  -- Pj=14 written by pm_node_13 (rank=0 FW_mix_precision_linear), outs=[11653], tid=11653
  have h_pmtake14_11653 : pm_goal_1_cut.nodes.take 14 = pm_goal_1_cut.nodes.take 13 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11649, 5911], outs := [11653] }] := by rfl
  have hP14_eq_11653 : P14 = applyNode pm_goal_1_cut P13 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11649, 5911], outs := [11653] } := by
    show (pm_goal_1_cut.nodes.take 14).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP14_11653 : P14 11653 = fw_linear (P13 11649) (P13 5911) := by
    rw [hP14_eq_11653]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P13 0 11649 5911 11653
  -- Pj=15 written by pm_node_14 (rank=0 FW_mix_precision_linear), outs=[11671], tid=11671
  have h_pmtake15_11671 : pm_goal_1_cut.nodes.take 15 = pm_goal_1_cut.nodes.take 14 ++ [{ rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11667, 5915], outs := [11671] }] := by rfl
  have hP15_eq_11671 : P15 = applyNode pm_goal_1_cut P14 { rank := 0, op := "OpName.FW_mix_precision_linear", ins := [11667, 5915], outs := [11671] } := by
    show (pm_goal_1_cut.nodes.take 15).foldl (applyNode pm_goal_1_cut) initPM = _
  ...
  have hP15_11671 : P15 11671 = fw_linear (P14 11667) (P14 5915) := by
    rw [hP15_eq_11671]
    exact applyNode_fw_mix_precision_linear_out_1p pm_goal_1_cut P14 0 11667 5915 11671
  have hP16_11650 : P16 11650 = P11 11650 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11650 11 16 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP16_5911 : P16 5911 = initPM 5911 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 16) initPM 5911
      (by intro n hn; fin_cases hn <;> decide)
  have hP17_11668 : P17 11668 = P12 11668 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11668 12 17 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP17_5915 : P17 5915 = initPM 5915 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 17) initPM 5915
      (by intro n hn; fin_cases hn <;> decide)
  have hP13_11649 : P13 11649 = P8 11649 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11649 8 13 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP13_5911 : P13 5911 = initPM 5911 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 13) initPM 5911
      (by intro n hn; fin_cases hn <;> decide)
  have hP14_11667 : P14 11667 = P9 11667 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 11667 9 14 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP14_5915 : P14 5915 = initPM 5915 :=
    foldl_applyNode_at_not_written pm_goal_1_cut (pm_goal_1_cut.nodes.take 14) initPM 5915
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=11 written by pm_node_10 (rank=1 FW_reshape), outs=[11650], tid=11650
  have h_pmtake11_11650 : pm_goal_1_cut.nodes.take 11 = pm_goal_1_cut.nodes.take 10 ++ [{ rank := 1, op := "OpName.FW_reshape", ins := [16897], outs := [11650] }] := by rfl
  have hP11_eq_11650 : P11 = applyNode pm_goal_1_cut P10 { rank := 1, op := "OpName.FW_reshape", ins := [16897], outs := [11650] } := by
    show (pm_goal_1_cut.nodes.take 11).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake11_11650, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP11_11650 : P11 11650 = P10 16897 := by
    rw [hP11_eq_11650]
    exact applyNode_fw_reshape_out pm_goal_1_cut P10 1 16897 11650 []
  -- Pj=12 written by pm_node_11 (rank=1 FW_reshape), outs=[11668], tid=11668
  have h_pmtake12_11668 : pm_goal_1_cut.nodes.take 12 = pm_goal_1_cut.nodes.take 11 ++ [{ rank := 1, op := "OpName.FW_reshape", ins := [16901], outs := [11668] }] := by rfl
  have hP12_eq_11668 : P12 = applyNode pm_goal_1_cut P11 { rank := 1, op := "OpName.FW_reshape", ins := [16901], outs := [11668] } := by
    show (pm_goal_1_cut.nodes.take 12).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake12_11668, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP12_11668 : P12 11668 = P11 16901 := by
    rw [hP12_eq_11668]
    exact applyNode_fw_reshape_out pm_goal_1_cut P11 1 16901 11668 []
  -- Pj=8 written by pm_node_7 (rank=0 FW_reshape), outs=[11649], tid=11649
  have h_pmtake8_11649 : pm_goal_1_cut.nodes.take 8 = pm_goal_1_cut.nodes.take 7 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [16874], outs := [11649] }] := by rfl
  have hP8_eq_11649 : P8 = applyNode pm_goal_1_cut P7 { rank := 0, op := "OpName.FW_reshape", ins := [16874], outs := [11649] } := by
    show (pm_goal_1_cut.nodes.take 8).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake8_11649, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP8_11649 : P8 11649 = P7 16874 := by
    rw [hP8_eq_11649]
    exact applyNode_fw_reshape_out pm_goal_1_cut P7 0 16874 11649 []
  -- Pj=9 written by pm_node_8 (rank=0 FW_reshape), outs=[11667], tid=11667
  have h_pmtake9_11667 : pm_goal_1_cut.nodes.take 9 = pm_goal_1_cut.nodes.take 8 ++ [{ rank := 0, op := "OpName.FW_reshape", ins := [16878], outs := [11667] }] := by rfl
  have hP9_eq_11667 : P9 = applyNode pm_goal_1_cut P8 { rank := 0, op := "OpName.FW_reshape", ins := [16878], outs := [11667] } := by
    show (pm_goal_1_cut.nodes.take 9).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake9_11667, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP9_11667 : P9 11667 = P8 16878 := by
    rw [hP9_eq_11667]
    exact applyNode_fw_reshape_out pm_goal_1_cut P8 0 16878 11667 []
  have hP10_16897 : P10 16897 = P6 16897 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16897 6 10 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP11_16901 : P11 16901 = P6 16901 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16901 6 11 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP7_16874 : P7 16874 = P5 16874 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16874 5 7 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  have hP8_16878 : P8 16878 = P5 16878 :=
    foldl_take_split_at_not_written pm_goal_1_cut pm_goal_1_cut.nodes initPM 16878 5 8 (by omega)
      (by intro n hn; fin_cases hn <;> decide)
  -- Pj=6 written by pm_node_5 (rank=1 FW_multiref), outs=[16885, 16889, 16893, 16897, 16901], tid=16897
  have h_pmtake6_16897 : pm_goal_1_cut.nodes.take 6 = pm_goal_1_cut.nodes.take 5 ++ [{ rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] }] := by rfl
  have hP6_eq_16897 : P6 = applyNode pm_goal_1_cut P5 { rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 6).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake6_16897, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP6_16897 : P6 16897 = P5 11614 := by
    rw [hP6_eq_16897]
    exact applyNode_fw_multiref5_at_pos3_out pm_goal_1_cut P5 1 11614 16885 16889 16893 16897 16901 (by decide) (by decide) (by decide)
  -- Pj=6 written by pm_node_5 (rank=1 FW_multiref), outs=[16885, 16889, 16893, 16897, 16901], tid=16901
  have h_pmtake6_16901 : pm_goal_1_cut.nodes.take 6 = pm_goal_1_cut.nodes.take 5 ++ [{ rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] }] := by rfl
  have hP6_eq_16901 : P6 = applyNode pm_goal_1_cut P5 { rank := 1, op := "OpName.FW_multiref", ins := [11614], outs := [16885, 16889, 16893, 16897, 16901], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 6).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake6_16901, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP6_16901 : P6 16901 = P5 11614 := by
    rw [hP6_eq_16901]
    exact applyNode_fw_multiref5_at_pos4_out pm_goal_1_cut P5 1 11614 16885 16889 16893 16897 16901 (by decide) (by decide) (by decide) (by decide)
  -- Pj=5 written by pm_node_4 (rank=0 FW_multiref), outs=[16862, 16866, 16870, 16874, 16878], tid=16874
  have h_pmtake5_16874 : pm_goal_1_cut.nodes.take 5 = pm_goal_1_cut.nodes.take 4 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] }] := by rfl
  have hP5_eq_16874 : P5 = applyNode pm_goal_1_cut P4 { rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 5).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake5_16874, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP5_16874 : P5 16874 = P4 11613 := by
    rw [hP5_eq_16874]
    exact applyNode_fw_multiref5_at_pos3_out pm_goal_1_cut P4 0 11613 16862 16866 16870 16874 16878 (by decide) (by decide) (by decide)
  -- Pj=5 written by pm_node_4 (rank=0 FW_multiref), outs=[16862, 16866, 16870, 16874, 16878], tid=16878
  have h_pmtake5_16878 : pm_goal_1_cut.nodes.take 5 = pm_goal_1_cut.nodes.take 4 ++ [{ rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] }] := by rfl
  have hP5_eq_16878 : P5 = applyNode pm_goal_1_cut P4 { rank := 0, op := "OpName.FW_multiref", ins := [11613], outs := [16862, 16866, 16870, 16874, 16878], params := [5] } := by
    show (pm_goal_1_cut.nodes.take 5).foldl (applyNode pm_goal_1_cut) initPM = _
    rw [h_pmtake5_16878, List.foldl_append, List.foldl_cons, List.foldl_nil]
  have hP5_16878 : P5 16878 = P4 11613 := by
    rw [hP5_eq_16878]
    exact applyNode_fw_multiref5_at_pos4_out pm_goal_1_cut P4 0 11613 16862 16866 16870 16874 16878 (by decide) (by decide) (by decide) (by decide)
  rw [
...
theorem sm_chain_shape_4096 (initSM : Store) (hSM : StoreShapesHold initSM sm_goal_1_cutInitEnv) :
    (denoteGraph sm_goal_1_cut initSM 4673).shape = [4096] := by
  ...
  rw [denote_sm_goal_1_cut_4673 initSM (by rw [hSM 5898 [4096, 64] (by native_decide)]; rfl)]
  exact TrainVerify.Denote.fw_inner_chunk_ce_fst_shape
    _ _ _ _ _ 4096 hL

/-- The PM computation chain (after allGather) has shape [4096]. -/
theorem pm_chain_shape_4096 (initPM : Store) (hPM : StoreShapesHold initPM pm_goal_1_cutInitEnv) :
    (denoteGraph pm_goal_1_cut initPM 4673).shape = [4096] := by
  ...
  have hfst0 : (fw_inner_chunk_ce _ (initPM 5931) (chunkPrimDimN 0 pm_goal_1_cut.numRanks 0 (initPM 4678))
  ...
  have hfst1 : (fw_inner_chunk_ce _ (initPM 5931) (chunkPrimDimN 0 pm_goal_1_cut.numRanks 1 (initPM 4678))
      (((initPM 5931).shape.head?).getD 0) ((((0 : Nat) : Scalar)))).fst.shape = [2048] :=
  ...
  rw [denote_pm_goal_1_cut_4673 initPM (by rw [hPM 11621 [2048, 64] (by native_decide)]; rfl) (by rw [hPM 11622 [2048, 64] (by native_decide)]; rfl)]
  have hpn : pm_goal_1_cut.numRanks = 2 := rfl
  rw [hpn]
...
theorem prove_goal_1 : goal_1_stmt_with_labels := by
  ...
  simp only [goal_1_cut_goal]
  refine ⟨?shape, ?tp_shapes, ?value⟩
  case shape =>
    -- (denoteGraph sm_goal_1_cut initSM 4673).shape = [4096]
    exact sm_chain_shape_4096 initSM hSM
  case tp_shapes =>
    -- List.map shape [(denoteGraph pm_goal_1_cut initPM 4673)] = [[4096]]
    simp only [List.map]
    rw [pm_chain_shape_4096 initPM hPM]
  case value =>
    -- Extract needed intermediate goals.
    have hInit' : InitGoalsHold pm_goal_1_cut.numRanks goal_1_cut_initGoals initSM initPM := hInit
    ...
    have extract_singleton : ∀ (g : LineageGoal) (_ : g ∈ goal_1_cut_initGoals)
      ...
      have hval := h.2.2
      rw [reconstructForGoal_of_not_replicated g pm_goal_1_cut.numRanks
    ...
    have extract_dual : ∀ (g : LineageGoal) (_ : g ∈ goal_1_cut_initGoals)
        ...
        initSM ts = allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [initPM p0, initPM p1] := by
    ...
    have hb_4678 : initSM 4678 = initPM 4678 :=
      extract_singleton goal1CutInitGoal_4678 (by native_decide) 4678 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5906 : initSM 5906 = initPM 5906 :=
      extract_singleton goal1CutInitGoal_5906 (by native_decide) 5906 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5911 : initSM 5911 = initPM 5911 :=
      extract_singleton goal1CutInitGoal_5911 (by native_decide) 5911 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5915 : initSM 5915 = initPM 5915 :=
      extract_singleton goal1CutInitGoal_5915 (by native_decide) 5915 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5920 : initSM 5920 = initPM 5920 :=
      extract_singleton goal1CutInitGoal_5920 (by native_decide) 5920 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5927 : initSM 5927 = initPM 5927 :=
      extract_singleton goal1CutInitGoal_5927 (by native_decide) 5927 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5929 : initSM 5929 = initPM 5929 :=
      extract_singleton goal1CutInitGoal_5929 (by native_decide) 5929 (by rfl) (by rfl) (by rfl) (by rfl)
    have hb_5931 : initSM 5931 = initPM 5931 :=
      extract_singleton goal1CutInitGoal_5931 (by native_decide) 5931 (by rfl) (by rfl) (by rfl) (by rfl)
    ...
    have hb_5893 : initSM 5893 = allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [initPM 11609, initPM 11610] :=
    ...
    have hb_5895 : initSM 5895 = allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [initPM 11613, initPM 11614] :=
    ...
    have hb_5898 : initSM 5898 = allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [initPM 11621, initPM 11622] :=
    ...
    have hb_5902 : initSM 5902 = allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [initPM 11629, initPM 11630] :=
      extract_dual goal1CutInitGoal_5902 (by native_decide) 5902 11629 11630 [32, 1024, 1024]
        (by rfl) (by rfl) (by rfl) (by rfl) h11629_shape (by decide)
    have hb_5903 : initSM 5903 = allGatherPrimDimN 0 pm_goal_1_cut.numRanks 0 [initPM 11631, initPM 11632] :=
      extract_dual goal1CutInitGoal_5903 (by native_decide) 5903 11631 11632 [32, 1024, 512]
    ...
    rw [denote_sm_goal_1_cut_4673 initSM (by rw [hSM 5898 [4096, 64] (by native_decide)]; rfl),
        denote_pm_goal_1_cut_4673 initPM (by rw [hPM 11621 [2048, 64] (by native_decide)]; rfl) (by rw [hPM 11622 [2048, 64] (by native_decide)]; rfl)]
    ...
    -- First reduce pm_goal_1_cut.numRanks = 2 in target for axiom matching.
    have hpmR : pm_goal_1_cut.numRanks = 2 := by rfl
    rw [hpmR]
