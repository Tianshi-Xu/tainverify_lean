/- L10 ordinary MoE composition from attention residual to block output. -/
...
theorem l10_ordinary_moe_composition (initSM initPM : Store)
  ...
  have hActivation := l10_ordinary_moe_activation_from_attention_output
    initSM initPM hInit hAttention
  have hRouter := l10_ordinary_moe_router_from_norm_input
    initSM initPM hPM hInit hNorm
  have hRouterProbs := hRouter.1
  have hRouterMap := hRouter.2
  have hExpert := l10_ordinary_moe_expert_from_branch_inputs
    initSM initPM hSM hPM hInit hActivation hRouterProbs hRouterMap
  have hExpertOrd := ordinary2Rel_of_gather2Rel hExpert
  have hGate := l10_ordinary_moe_gate_from_norm_input
    initSM initPM hPM hInit hNorm
  have hGateOrd := ordinary2Rel_of_gather2Rel hGate
  have hDown := l10_ordinary_moe_down_from_norm_input
    initSM initPM hPM hInit hNormOrd
  have hResidual := l10_ordinary_moe_residual_from_attention_output
    initSM initPM hAttention
  have hResidualOrd := ordinary2Rel_of_gather2Rel hResidual
  have hBoundary := l10_ordinary_moe_boundary_from_branch_inputs
