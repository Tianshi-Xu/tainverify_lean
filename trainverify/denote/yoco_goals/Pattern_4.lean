...
-/
import denote.yoco_goals.Goal_4_Cut
...
/-!
### SM-side machinery: denote sm_goal_4_cut at write tid 4676.

sm_goal_4_cut has 25 nodes: 24 topk_routing (indices 0..23), 1 FW_stack (index 24).
...
private theorem denote_sm_S24_at_scores
    ...
    (hk : k + 1 ≤ 24)
    (h_prefix_eq : sm_goal_4_cut.nodes.take (k+1) =
      (sm_goal_4_cut.nodes.take k) ++
      [{ rank := (0:Nat), op := "OpName.FW_topk_routing", ins := [logitsTid],
         outs := [probsTid, mapTid, scoresTid], params := [8] }])
    (h_prefix_no_write_scores :
      ∀ n ∈ sm_goal_4_cut.nodes.take k, scoresTid ∉ n.outs)
    (h_prefix_no_write_logits :
      ∀ n ∈ sm_goal_4_cut.nodes.take k, logitsTid ∉ n.outs)
    (h_suffix_no_write :
      ∀ n ∈ (sm_goal_4_cut.nodes.drop (k+1)).take (24 - (k+1)), scoresTid ∉ n.outs)
    (h_pne : probsTid ≠ scoresTid) (h_mne : mapTid ≠ scoresTid) :
    ((sm_goal_4_cut.nodes.take 24).foldl (applyNode sm_goal_4_cut) initSM) scoresTid =
    (fw_topk_routing (initSM logitsTid) 8 1).snd.snd := by
  have h_split : sm_goal_4_cut.nodes.take 24 =
      sm_goal_4_cut.nodes.take (k+1) ++ (sm_goal_4_cut.nodes.drop (k+1)).take (24 - (k+1)) := by
    have heq : k + 1 + (24 - (k+1)) = 24 := by omega
    calc sm_goal_4_cut.nodes.take 24
        = sm_goal_4_cut.nodes.take (k+1 + (24 - (k+1))) := by rw [heq]
      _ = sm_goal_4_cut.nodes.take (k+1) ++ (sm_goal_4_cut.nodes.drop (k+1)).take (24 - (k+1)) := by
          rw [List.take_add]
  rw [h_split, List.foldl_append]
  rw [foldl_applyNode_at_not_written sm_goal_4_cut _ _ scoresTid h_suffix_no_write]
  rw [h_prefix_eq, List.foldl_append, List.foldl_cons, List.foldl_nil]
  set S_k : Store := (sm_goal_4_cut.nodes.take k).foldl (applyNode sm_goal_4_cut) initSM
  have h_S_k_logits : S_k logitsTid = initSM logitsTid :=
    foldl_applyNode_at_not_written sm_goal_4_cut _ initSM logitsTid h_prefix_no_write_logits
  have h_apply := applyNode_fw_topk_routing_scores_out sm_goal_4_cut S_k 0
    logitsTid probsTid mapTid scoresTid [8] h_pne h_mne
  rw [h_apply, h_S_k_logits]
  rfl

/-- Main machinery: denote sm_goal_4_cut at 4676 = fw_stack of 24 topk scores. -/
theorem denote_sm_goal_4_cut_4676 (initSM : Store) :
    denoteGraph sm_goal_4_cut initSM 4676 = fw_stack
  ...
  -- sm_goal_4_cut.nodes = topks (24) ++ [stack]. Unfold denoteGraph.
  have h_split_full : sm_goal_4_cut.nodes = sm_goal_4_cut.nodes.take 24 ++
      [{ rank := 0, op := "OpName.FW_stack",
         ins := [4711, 4765, 4819, 4873, 4927, 4981, 5035, 5089, 5143, 5197,
                 ...
                 5754, 5803, 5852, 5901],
         outs := [4676], params := [] }] := by
    show sm_goal_4_cut.nodes = _
    rfl
  unfold denoteGraph
  rw [h_split_full, List.foldl_append, List.foldl_cons, List.foldl_nil]
  set S24 : Store := (sm_goal_4_cut.nodes.take 24).foldl (applyNode sm_goal_4_cut) initSM
  rw [applyNode_fw_stack_out sm_goal_4_cut S24 0 _ 4676 []]
...
/-!
### PM-side machinery: denote pm_goal_4_cut at write tid 4676.

pm_goal_4_cut.nodes has 55 nodes:
...
/-- Helper: for pm_goal_4_cut, S_pm52 at scoresTid of a topk node at position k
...
private theorem denote_pm_S52_at_scores
    ...
    (hk : k + 1 ≤ 52)
    (h_prefix_eq : pm_goal_4_cut.nodes.take (k+1) =
      (pm_goal_4_cut.nodes.take k) ++
      [{ rank := rank_i, op := "OpName.FW_topk_routing", ins := [logitsTid],
         outs := [probsTid, mapTid, scoresTid], params := [8] }])
    (h_logits_val : ((pm_goal_4_cut.nodes.take k).foldl (applyNode pm_goal_4_cut) initPM) logitsTid
                    = logitsVal)
    (h_suffix_no_write :
      ∀ n ∈ (pm_goal_4_cut.nodes.drop (k+1)).take (52 - (k+1)), scoresTid ∉ n.outs)
    (h_pne : probsTid ≠ scoresTid) (h_mne : mapTid ≠ scoresTid) :
    ((pm_goal_4_cut.nodes.take 52).foldl (applyNode pm_goal_4_cut) initPM) scoresTid =
    (fw_topk_routing logitsVal 8 1).snd.snd := by
  have h_split : pm_goal_4_cut.nodes.take 52 =
      pm_goal_4_cut.nodes.take (k+1) ++ (pm_goal_4_cut.nodes.drop (k+1)).take (52 - (k+1)) := by
    have heq : k + 1 + (52 - (k+1)) = 52 := by omega
    calc pm_goal_4_cut.nodes.take 52
        = pm_goal_4_cut.nodes.take (k+1 + (52 - (k+1))) := by rw [heq]
      _ = pm_goal_4_cut.nodes.take (k+1) ++ (pm_goal_4_cut.nodes.drop (k+1)).take (52 - (k+1)) := by
          rw [List.take_add]
  rw [h_split, List.foldl_append]
  rw [foldl_applyNode_at_not_written pm_goal_4_cut _ _ scoresTid h_suffix_no_write]
  rw [h_prefix_eq, List.foldl_append, List.foldl_cons, List.foldl_nil]
  set S_k : Store := (pm_goal_4_cut.nodes.take k).foldl (applyNode pm_goal_4_cut) initPM
  have h_apply := applyNode_fw_topk_routing_scores_out pm_goal_4_cut S_k rank_i
    logitsTid probsTid mapTid scoresTid [8] h_pne h_mne
...
private theorem denote_pm_S52_at_chunk
    ...
    (h_in_val : ((pm_goal_4_cut.nodes.take k).foldl (applyNode pm_goal_4_cut) initPM) inTid
                = inVal)
    (h_suffix_no_write :
      ∀ n ∈ (pm_goal_4_cut.nodes.drop (k+1)).take (52 - (k+1)), outTid ∉ n.outs) :
    ((pm_goal_4_cut.nodes.take 52).foldl (applyNode pm_goal_4_cut) initPM) outTid =
    chunkPrimDimN 0 pm_goal_4_cut.numRanks rank_i inVal := by
  have h_split : pm_goal_4_cut.nodes.take 52 =
      pm_goal_4_cut.nodes.take (k+1) ++ (pm_goal_4_cut.nodes.drop (k+1)).take (52 - (k+1)) := by
    have heq : k + 1 + (52 - (k+1)) = 52 := by omega
    calc pm_goal_4_cut.nodes.take 52
      ...
      _ = pm_goal_4_cut.nodes.take (k+1) ++ (pm_goal_4_cut.nodes.drop (k+1)).take (52 - (k+1)) := by
          rw [List.take_add]
  rw [h_split, List.foldl_append]
  rw [foldl_applyNode_at_not_written pm_goal_4_cut _ _ outTid h_suffix_no_write]
  rw [h_prefix_eq, List.foldl_append, List.foldl_cons, List.foldl_nil]
  set S_k : Store := (pm_goal_4_cut.nodes.take k).foldl (applyNode pm_goal_4_cut) initPM
  rw [applyNode_chunkPrimDimN_out pm_goal_4_cut S_k rank_i inTid outTid 0]
  rw [h_in_val]
...
private theorem pm_prefix_at_chunked
    ...
    (h_take_k : pm_goal_4_cut.nodes.take k =
      (pm_goal_4_cut.nodes.take k_chunk) ++
      [{ rank := rank_i, op := "OpName.ChunkPrim", ins := [srcTid],
         outs := [logitsTid], params := [0] }] ++
      (pm_goal_4_cut.nodes.drop (k_chunk + 1)).take (k - (k_chunk + 1)))
    (h_prefix_no_write_src :
      ∀ n ∈ pm_goal_4_cut.nodes.take k_chunk, srcTid ∉ n.outs)
    (h_between_no_write :
      ∀ n ∈ (pm_goal_4_cut.nodes.drop (k_chunk + 1)).take (k - (k_chunk + 1)),
        logitsTid ∉ n.outs) :
    ((pm_goal_4_cut.nodes.take k).foldl (applyNode pm_goal_4_cut) initPM) logitsTid =
    chunkPrimDimN 0 pm_goal_4_cut.numRanks rank_i (initPM srcTid) := by
  rw [h_take_k, List.foldl_append, List.foldl_append, List.foldl_cons, List.foldl_nil]
  rw [foldl_applyNode_at_not_written pm_goal_4_cut _ _ logitsTid h_between_no_write]
  set S_chunk : Store := (pm_goal_4_cut.nodes.take k_chunk).foldl (applyNode pm_goal_4_cut) initPM
  rw [applyNode_chunkPrimDimN_out pm_goal_4_cut S_chunk rank_i srcTid logitsTid 0]
  have h_src_eq : S_chunk srcTid = initPM srcTid :=
    foldl_applyNode_at_not_written pm_goal_4_cut _ initPM srcTid h_prefix_no_write_src
  rw [h_src_eq]

/-- Main PM machinery: denote pm_goal_4_cut at 4676 = allGather of 2 fw_stacks. -/
theorem denote_pm_goal_4_cut_4676 (initPM : Store) :
    denoteGraph pm_goal_4_cut initPM 4676 =
    allGatherPrimDimN 1 pm_goal_4_cut.numRanks 0
      [fw_stack
        [(fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 0 (initPM 4708)) 8 1).snd.snd,
         (fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 0 (initPM 4762)) 8 1).snd.snd,
       ...
       fw_stack
        [(fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 1 (initPM 4708)) 8 1).snd.snd,
         (fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 1 (initPM 4762)) 8 1).snd.snd,
  ...
  have h_split_full : pm_goal_4_cut.nodes = pm_goal_4_cut.nodes.take 52 ++
      [{ rank := 0, op := "OpName.FW_stack",
    ...
    show pm_goal_4_cut.nodes = _
    rfl
  unfold denoteGraph
  rw [h_split_full, List.foldl_append, List.foldl_cons, List.foldl_cons,
      List.foldl_cons, List.foldl_nil]
  set S52 : Store := (pm_goal_4_cut.nodes.take 52).foldl (applyNode pm_goal_4_cut) initPM
  set S53 : Store := applyNode pm_goal_4_cut S52
    { rank := 0, op := "OpName.FW_stack",
      ins := [7485, 7671, 7857, 8043, 8229, 8415, 8601, 8787, 8973, 9159,
              9345, 9531, 9735, 9907, 10079, 10251, 10423, 10595, 10767,
              10939, 11111, 11283, 11455, 11627],
      outs := [11781], params := [] }
  set S54 : Store := applyNode pm_goal_4_cut S53
    { rank := 1, op := "OpName.FW_stack",
      ins := [7486, 7672, 7858, 8044, 8230, 8416, 8602, 8788, 8974, 9160,
              9346, 9532, 9736, 9908, 10080, 10252, 10424, 10596, 10768,
              10940, 11112, 11284, 11456, 11628],
      outs := [11782], params := [] }
  rw [applyNode_allGatherPrimDimN_out pm_goal_4_cut S54 0 [11781, 11782] 4676 1]
  have h_S54_11781 : S54 11781 = S53 11781 := by
    show applyNode pm_goal_4_cut S53 _ 11781 = S53 11781
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S53 _ 11781 (by decide)
  have h_S54_11782 : S54 11782 = fw_stack ([7486, 7672, 7858, 8044, 8230, 8416, 8602, 8788, 8974, 9160, 9346, 9532, 9736, 9908, 10080, 10252, 10424, 10596, 10768, 10940, 11112, 11284, 11456, 11628].map S53) := by
    show applyNode pm_goal_4_cut S53 _ 11782 = _
    exact applyNode_fw_stack_out pm_goal_4_cut S53 1 _ 11782 []
  have h_S53_11781 : S53 11781 = fw_stack ([7485, 7671, 7857, 8043, 8229, 8415, 8601, 8787, 8973, 9159, 9345, 9531, 9735, 9907, 10079, 10251, 10423, 10595, 10767, 10939, 11111, 11283, 11455, 11627].map S52) := by
    show applyNode pm_goal_4_cut S52 _ 11781 = _
    exact applyNode_fw_stack_out pm_goal_4_cut S52 0 _ 11781 []
  have h_out_7479 : S52 7479 = chunkPrimDimN 0 pm_goal_4_cut.numRanks 0 (initPM 4708) :=
    denote_pm_S52_at_chunk initPM 0 0 4708 7479 _ (by omega)
      (by rfl)
      (by rfl)
      (by native_decide)
  have h_out_7480 : S52 7480 = chunkPrimDimN 0 pm_goal_4_cut.numRanks 1 (initPM 4708) :=
    denote_pm_S52_at_chunk initPM 1 1 4708 7480 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 4708 (by native_decide))
      (by native_decide)
  have h_out_7665 : S52 7665 = chunkPrimDimN 0 pm_goal_4_cut.numRanks 0 (initPM 4762) :=
    denote_pm_S52_at_chunk initPM 4 0 4762 7665 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 4762 (by native_decide))
      (by native_decide)
  have h_out_7666 : S52 7666 = chunkPrimDimN 0 pm_goal_4_cut.numRanks 1 (initPM 4762) :=
    denote_pm_S52_at_chunk initPM 5 1 4762 7666 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 4762 (by native_decide))
      (by native_decide)
  have h_scores_7485 : S52 7485 = (fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 0 (initPM 4708)) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 2 0 7479 7481 7483 7485 _ (by omega)
      ...
      (by native_decide)
      (by decide) (by decide)
  have h_scores_7486 : S52 7486 = (fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 1 (initPM 4708)) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 3 1 7480 7482 7484 7486 _ (by omega)
      ...
      (by native_decide)
      (by decide) (by decide)
  have h_scores_7671 : S52 7671 = (fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 0 (initPM 4762)) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 6 0 7665 7667 7669 7671 _ (by omega)
      ...
      (by native_decide)
      (by decide) (by decide)
  have h_scores_7672 : S52 7672 = (fw_topk_routing (chunkPrimDimN 0 pm_goal_4_cut.numRanks 1 (initPM 4762)) 8 1).snd.snd :=
  ...
  have h_scores_7857 : S52 7857 = (fw_topk_routing (initPM 7851) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 8 0 7851 7853 7855 7857 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 7851 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_7858 : S52 7858 = (fw_topk_routing (initPM 7852) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 9 1 7852 7854 7856 7858 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 7852 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8043 : S52 8043 = (fw_topk_routing (initPM 8037) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 10 0 8037 8039 8041 8043 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8037 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8044 : S52 8044 = (fw_topk_routing (initPM 8038) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 11 1 8038 8040 8042 8044 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8038 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8229 : S52 8229 = (fw_topk_routing (initPM 8223) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 12 0 8223 8225 8227 8229 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8223 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8230 : S52 8230 = (fw_topk_routing (initPM 8224) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 13 1 8224 8226 8228 8230 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8224 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8415 : S52 8415 = (fw_topk_routing (initPM 8409) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 14 0 8409 8411 8413 8415 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8409 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8416 : S52 8416 = (fw_topk_routing (initPM 8410) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 15 1 8410 8412 8414 8416 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8410 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8601 : S52 8601 = (fw_topk_routing (initPM 8595) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 16 0 8595 8597 8599 8601 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8595 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8602 : S52 8602 = (fw_topk_routing (initPM 8596) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 17 1 8596 8598 8600 8602 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8596 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8787 : S52 8787 = (fw_topk_routing (initPM 8781) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 18 0 8781 8783 8785 8787 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8781 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8788 : S52 8788 = (fw_topk_routing (initPM 8782) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 19 1 8782 8784 8786 8788 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8782 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8973 : S52 8973 = (fw_topk_routing (initPM 8967) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 20 0 8967 8969 8971 8973 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8967 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_8974 : S52 8974 = (fw_topk_routing (initPM 8968) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 21 1 8968 8970 8972 8974 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 8968 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9159 : S52 9159 = (fw_topk_routing (initPM 9153) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 22 0 9153 9155 9157 9159 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9153 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9160 : S52 9160 = (fw_topk_routing (initPM 9154) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 23 1 9154 9156 9158 9160 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9154 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9345 : S52 9345 = (fw_topk_routing (initPM 9339) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 24 0 9339 9341 9343 9345 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9339 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9346 : S52 9346 = (fw_topk_routing (initPM 9340) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 25 1 9340 9342 9344 9346 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9340 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9531 : S52 9531 = (fw_topk_routing (initPM 9525) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 26 0 9525 9527 9529 9531 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9525 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9532 : S52 9532 = (fw_topk_routing (initPM 9526) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 27 1 9526 9528 9530 9532 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9526 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9735 : S52 9735 = (fw_topk_routing (initPM 9729) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 28 0 9729 9731 9733 9735 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9729 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9736 : S52 9736 = (fw_topk_routing (initPM 9730) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 29 1 9730 9732 9734 9736 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9730 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9907 : S52 9907 = (fw_topk_routing (initPM 9901) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 30 0 9901 9903 9905 9907 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9901 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_9908 : S52 9908 = (fw_topk_routing (initPM 9902) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 31 1 9902 9904 9906 9908 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 9902 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10079 : S52 10079 = (fw_topk_routing (initPM 10073) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 32 0 10073 10075 10077 10079 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10073 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10080 : S52 10080 = (fw_topk_routing (initPM 10074) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 33 1 10074 10076 10078 10080 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10074 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10251 : S52 10251 = (fw_topk_routing (initPM 10245) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 34 0 10245 10247 10249 10251 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10245 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10252 : S52 10252 = (fw_topk_routing (initPM 10246) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 35 1 10246 10248 10250 10252 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10246 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10423 : S52 10423 = (fw_topk_routing (initPM 10417) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 36 0 10417 10419 10421 10423 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10417 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10424 : S52 10424 = (fw_topk_routing (initPM 10418) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 37 1 10418 10420 10422 10424 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10418 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10595 : S52 10595 = (fw_topk_routing (initPM 10589) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 38 0 10589 10591 10593 10595 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10589 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10596 : S52 10596 = (fw_topk_routing (initPM 10590) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 39 1 10590 10592 10594 10596 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10590 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10767 : S52 10767 = (fw_topk_routing (initPM 10761) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 40 0 10761 10763 10765 10767 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10761 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10768 : S52 10768 = (fw_topk_routing (initPM 10762) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 41 1 10762 10764 10766 10768 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10762 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10939 : S52 10939 = (fw_topk_routing (initPM 10933) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 42 0 10933 10935 10937 10939 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10933 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_10940 : S52 10940 = (fw_topk_routing (initPM 10934) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 43 1 10934 10936 10938 10940 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 10934 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11111 : S52 11111 = (fw_topk_routing (initPM 11105) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 44 0 11105 11107 11109 11111 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11105 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11112 : S52 11112 = (fw_topk_routing (initPM 11106) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 45 1 11106 11108 11110 11112 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11106 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11283 : S52 11283 = (fw_topk_routing (initPM 11277) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 46 0 11277 11279 11281 11283 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11277 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11284 : S52 11284 = (fw_topk_routing (initPM 11278) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 47 1 11278 11280 11282 11284 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11278 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11455 : S52 11455 = (fw_topk_routing (initPM 11449) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 48 0 11449 11451 11453 11455 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11449 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11456 : S52 11456 = (fw_topk_routing (initPM 11450) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 49 1 11450 11452 11454 11456 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11450 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11627 : S52 11627 = (fw_topk_routing (initPM 11621) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 50 0 11621 11623 11625 11627 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11621 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_scores_11628 : S52 11628 = (fw_topk_routing (initPM 11622) 8 1).snd.snd :=
    denote_pm_S52_at_scores initPM 51 1 11622 11624 11626 11628 _ (by omega)
      (by rfl)
      (by exact foldl_applyNode_at_not_written pm_goal_4_cut _ initPM 11622 (by native_decide))
      (by native_decide)
      (by decide) (by decide)
  have h_S53_7486 : S53 7486 = S52 7486 := by
    show applyNode pm_goal_4_cut S52 _ 7486 = S52 7486
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 7486 (by decide)
  have h_S53_7672 : S53 7672 = S52 7672 := by
    show applyNode pm_goal_4_cut S52 _ 7672 = S52 7672
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 7672 (by decide)
  have h_S53_7858 : S53 7858 = S52 7858 := by
    show applyNode pm_goal_4_cut S52 _ 7858 = S52 7858
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 7858 (by decide)
  have h_S53_8044 : S53 8044 = S52 8044 := by
    show applyNode pm_goal_4_cut S52 _ 8044 = S52 8044
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 8044 (by decide)
  have h_S53_8230 : S53 8230 = S52 8230 := by
    show applyNode pm_goal_4_cut S52 _ 8230 = S52 8230
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 8230 (by decide)
  have h_S53_8416 : S53 8416 = S52 8416 := by
    show applyNode pm_goal_4_cut S52 _ 8416 = S52 8416
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 8416 (by decide)
  have h_S53_8602 : S53 8602 = S52 8602 := by
    show applyNode pm_goal_4_cut S52 _ 8602 = S52 8602
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 8602 (by decide)
  have h_S53_8788 : S53 8788 = S52 8788 := by
    show applyNode pm_goal_4_cut S52 _ 8788 = S52 8788
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 8788 (by decide)
  have h_S53_8974 : S53 8974 = S52 8974 := by
    show applyNode pm_goal_4_cut S52 _ 8974 = S52 8974
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 8974 (by decide)
  have h_S53_9160 : S53 9160 = S52 9160 := by
    show applyNode pm_goal_4_cut S52 _ 9160 = S52 9160
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 9160 (by decide)
  have h_S53_9346 : S53 9346 = S52 9346 := by
    show applyNode pm_goal_4_cut S52 _ 9346 = S52 9346
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 9346 (by decide)
  have h_S53_9532 : S53 9532 = S52 9532 := by
    show applyNode pm_goal_4_cut S52 _ 9532 = S52 9532
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 9532 (by decide)
  have h_S53_9736 : S53 9736 = S52 9736 := by
    show applyNode pm_goal_4_cut S52 _ 9736 = S52 9736
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 9736 (by decide)
  have h_S53_9908 : S53 9908 = S52 9908 := by
    show applyNode pm_goal_4_cut S52 _ 9908 = S52 9908
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 9908 (by decide)
  have h_S53_10080 : S53 10080 = S52 10080 := by
    show applyNode pm_goal_4_cut S52 _ 10080 = S52 10080
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 10080 (by decide)
  have h_S53_10252 : S53 10252 = S52 10252 := by
    show applyNode pm_goal_4_cut S52 _ 10252 = S52 10252
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 10252 (by decide)
  have h_S53_10424 : S53 10424 = S52 10424 := by
    show applyNode pm_goal_4_cut S52 _ 10424 = S52 10424
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 10424 (by decide)
  have h_S53_10596 : S53 10596 = S52 10596 := by
    show applyNode pm_goal_4_cut S52 _ 10596 = S52 10596
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 10596 (by decide)
  have h_S53_10768 : S53 10768 = S52 10768 := by
    show applyNode pm_goal_4_cut S52 _ 10768 = S52 10768
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 10768 (by decide)
  have h_S53_10940 : S53 10940 = S52 10940 := by
    show applyNode pm_goal_4_cut S52 _ 10940 = S52 10940
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 10940 (by decide)
  have h_S53_11112 : S53 11112 = S52 11112 := by
    show applyNode pm_goal_4_cut S52 _ 11112 = S52 11112
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 11112 (by decide)
  have h_S53_11284 : S53 11284 = S52 11284 := by
    show applyNode pm_goal_4_cut S52 _ 11284 = S52 11284
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 11284 (by decide)
  have h_S53_11456 : S53 11456 = S52 11456 := by
    show applyNode pm_goal_4_cut S52 _ 11456 = S52 11456
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 11456 (by decide)
  have h_S53_11628 : S53 11628 = S52 11628 := by
    show applyNode pm_goal_4_cut S52 _ 11628 = S52 11628
    exact applyNode_eq_of_not_mem_outs pm_goal_4_cut S52 _ 11628 (by decide)
...
theorem prove_goal_4 : goal_4_stmt_cut := by
  ...
  have hpmR : pm_goal_4_cut.numRanks = 2 := rfl
  ...
  case shape_sm =>
    rw [denote_sm_goal_4_cut_4676 initSM]
    ...
    rfl
  case shape_pm =>
    -- pmStore 4676 has shape [24, 4096, 64] via denote_pm_goal_4_cut_4676 (allGather dim=1 on 2 shards).
    ...
    have hchunk_shape : ∀ (r : Nat) (x : Tensor) (_ : x.shape = [4096, 64]),
        (chunkPrimDimN 0 pm_goal_4_cut.numRanks r x).shape = [2048, 64] := by
      intro r x hx
      rw [chunkPrimDimN_shape 0 pm_goal_4_cut.numRanks r x [4096, 64] hx (by rw [hpmR]; omega)]
      rfl
    simp only [List.map]
    rw [denote_pm_goal_4_cut_4676 initPM]
    ...
    -- But hstack_r0_shape still has `pm_goal_4_cut.numRanks`. Rewrite it too.
  ...
  case value =>
    ...
    have hInit' : InitGoalsHold pm_goal_4_cut.numRanks goal_4_cut_initGoals initSM initPM := hInit
    ...
    have extract_singleton : ∀ (g : LineageGoal) (_ : g ∈ goal_4_cut_initGoals)
      ...
      rw [reconstructForGoal_of_not_replicated g pm_goal_4_cut.numRanks
    ...
    have hb_4708 : initSM 4708 = initPM 4708 := by
      have := extract_singleton goal4CutIntermediateGoal_4708 (by native_decide) [initPM 4708]
        (by simp) (by simp [goal4CutIntermediateGoal_4708]) (by rfl) (by rfl)
      simpa [goal4CutIntermediateGoal_4708, List.head] using this
    have hb_4762 : initSM 4762 = initPM 4762 := by
      have := extract_singleton goal4CutIntermediateGoal_4762 (by native_decide) [initPM 4762]
        (by simp) (by simp [goal4CutIntermediateGoal_4762]) (by rfl) (by rfl)
      simpa [goal4CutIntermediateGoal_4762, List.head] using this
    ...
    have extract_dual : ∀ (g : LineageGoal) (_ : g ∈ goal_4_cut_initGoals)
        ...
        initSM g.ts = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM p0, initPM p1] := by
      ...
      have hrec : reconstructWithDim 0 pm_goal_4_cut.numRanks 0 [initPM p0, initPM p1]
          = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM p0, initPM p1] := by
    ...
    have hb_4816 : initSM 4816 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 7851, initPM 7852] :=
      extract_dual goal4CutIntermediateGoal_4816 (by native_decide) 7851 7852
        (by simp [goal4CutIntermediateGoal_4816]) (by rfl) (by rfl) (by rfl) h7851_pm
    have h8037_pm : (initPM 8037).shape = [2048, 64] := hPM 8037 [2048, 64] (by native_decide)
    have hb_4870 : initSM 4870 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 8037, initPM 8038] :=
      extract_dual goal4CutIntermediateGoal_4870 (by native_decide) 8037 8038
        (by simp [goal4CutIntermediateGoal_4870]) (by rfl) (by rfl) (by rfl) h8037_pm
    have h8223_pm : (initPM 8223).shape = [2048, 64] := hPM 8223 [2048, 64] (by native_decide)
    have hb_4924 : initSM 4924 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 8223, initPM 8224] :=
      extract_dual goal4CutIntermediateGoal_4924 (by native_decide) 8223 8224
        (by simp [goal4CutIntermediateGoal_4924]) (by rfl) (by rfl) (by rfl) h8223_pm
    have h8409_pm : (initPM 8409).shape = [2048, 64] := hPM 8409 [2048, 64] (by native_decide)
    have hb_4978 : initSM 4978 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 8409, initPM 8410] :=
      extract_dual goal4CutIntermediateGoal_4978 (by native_decide) 8409 8410
        (by simp [goal4CutIntermediateGoal_4978]) (by rfl) (by rfl) (by rfl) h8409_pm
    have h8595_pm : (initPM 8595).shape = [2048, 64] := hPM 8595 [2048, 64] (by native_decide)
    have hb_5032 : initSM 5032 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 8595, initPM 8596] :=
      extract_dual goal4CutIntermediateGoal_5032 (by native_decide) 8595 8596
        (by simp [goal4CutIntermediateGoal_5032]) (by rfl) (by rfl) (by rfl) h8595_pm
    have h8781_pm : (initPM 8781).shape = [2048, 64] := hPM 8781 [2048, 64] (by native_decide)
    have hb_5086 : initSM 5086 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 8781, initPM 8782] :=
      extract_dual goal4CutIntermediateGoal_5086 (by native_decide) 8781 8782
        (by simp [goal4CutIntermediateGoal_5086]) (by rfl) (by rfl) (by rfl) h8781_pm
    have h8967_pm : (initPM 8967).shape = [2048, 64] := hPM 8967 [2048, 64] (by native_decide)
    have hb_5140 : initSM 5140 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 8967, initPM 8968] :=
      extract_dual goal4CutIntermediateGoal_5140 (by native_decide) 8967 8968
        (by simp [goal4CutIntermediateGoal_5140]) (by rfl) (by rfl) (by rfl) h8967_pm
    have h9153_pm : (initPM 9153).shape = [2048, 64] := hPM 9153 [2048, 64] (by native_decide)
    have hb_5194 : initSM 5194 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 9153, initPM 9154] :=
      extract_dual goal4CutIntermediateGoal_5194 (by native_decide) 9153 9154
        (by simp [goal4CutIntermediateGoal_5194]) (by rfl) (by rfl) (by rfl) h9153_pm
    have h9339_pm : (initPM 9339).shape = [2048, 64] := hPM 9339 [2048, 64] (by native_decide)
    have hb_5248 : initSM 5248 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 9339, initPM 9340] :=
      extract_dual goal4CutIntermediateGoal_5248 (by native_decide) 9339 9340
        (by simp [goal4CutIntermediateGoal_5248]) (by rfl) (by rfl) (by rfl) h9339_pm
    have h9525_pm : (initPM 9525).shape = [2048, 64] := hPM 9525 [2048, 64] (by native_decide)
    have hb_5302 : initSM 5302 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 9525, initPM 9526] :=
      extract_dual goal4CutIntermediateGoal_5302 (by native_decide) 9525 9526
        (by simp [goal4CutIntermediateGoal_5302]) (by rfl) (by rfl) (by rfl) h9525_pm
    ...
    have hb_5359 : initSM 5359 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 9729, initPM 9730] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5408 : initSM 5408 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 9901, initPM 9902] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5457 : initSM 5457 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 10073, initPM 10074] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5506 : initSM 5506 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 10245, initPM 10246] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5555 : initSM 5555 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 10417, initPM 10418] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5604 : initSM 5604 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 10589, initPM 10590] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5653 : initSM 5653 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 10761, initPM 10762] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5702 : initSM 5702 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 10933, initPM 10934] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5751 : initSM 5751 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 11105, initPM 11106] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5800 : initSM 5800 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 11277, initPM 11278] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5849 : initSM 5849 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 11449, initPM 11450] :=
    ...
    -- This local CUT boundary contract is extracted from hInit and jointly
    -- witnessed; see trainverify/GOAL_3_4_LAYOUT_SPLIT.md.
    have hb_5898 : initSM 5898 = allGatherPrimDimN 0 pm_goal_4_cut.numRanks 0 [initPM 11621, initPM 11622] :=
