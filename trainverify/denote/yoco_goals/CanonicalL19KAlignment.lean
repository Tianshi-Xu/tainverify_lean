/- Canonical Goal 1, layer 19: graph alignment of the K projection. -/
...
private theorem cL19K_init_singleton_eq (initSM initPM : Store)
...
private theorem cL19K_sm_weight_value (initSM : Store) (tid : Tid)
...
private theorem cL19K_pm_weight_value (initPM : Store) (tid : Tid)
...
private theorem cL19K_rms_weight_eq (initSM initPM : Store)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM) :
    denoteGraphDistributedFaithful sm_goal_1 initSM 5596 =
      denoteGraphDistributedFaithful pm_goal_1 initPM 5596 := by
  have hi := cL19K_init_singleton_eq initSM initPM hInit initGoal_5596
    (by native_decide) 5596 rfl rfl rfl rfl
  have hs := cL19K_sm_weight_value initSM 5596 (by native_decide)
  have hp := cL19K_pm_weight_value initPM 5596 (by native_decide)
  rw [hs, hp, hi]

private theorem cL19K_proj_weight_eq (initSM initPM : Store)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM) :
    denoteGraphDistributedFaithful sm_goal_1 initSM 5598 =
      denoteGraphDistributedFaithful pm_goal_1 initPM 5598 := by
  have hi := cL19K_init_singleton_eq initSM initPM hInit initGoal_5598
    (by native_decide) 5598 rfl rfl rfl rfl
  have hs := cL19K_sm_weight_value initSM 5598 (by native_decide)
  have hp := cL19K_pm_weight_value initPM 5598 (by native_decide)
  rw [hs, hp, hi]

private theorem cL19K_proj_weight_shape (initPM : Store)
...
/-- The complete ordinary canonical L19 K graph relation `6094 ↔ 11158/11159`.
All projection-weight facts are derived from the generated init contracts. -/
theorem canonical_l19_k_ordinary_relation
    (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM)
    (hCache : Gather2Rel
  ...
  have hRmsW := cL19K_rms_weight_eq initSM initPM hInit
  have hKW := cL19K_proj_weight_eq initSM initPM hInit
  have hKShape := cL19K_proj_weight_shape initPM hPM
  have hSemantic := canonical_l19_k_ordinary_semantic hCache hRmsW hKW hKShape
  rw [canonical_l19_k_sm_reduce initSM]
  rw [canonical_l19_k_pm0_reduce initPM]
  rw [canonical_l19_k_pm1_reduce initPM]
  exact hSemantic

#print axioms canonical_l19_k_ordinary_relation
