...
import denote.yoco_goals.L6OrdinaryMoENorm
...
theorem l6_ordinary_moe_logits_from_norm_input (initSM initPM : Store)
    ...
    Gather2Rel
      ...
      [4096, 64] [2048, 64] := by
  have hwEq := l6OMr_weight_eq initSM initPM hInit
  have hwShape := l6OMr_weight_shape initPM hPM
  ...
  rw [l6OMr_red_sm5292 initSM, l6OMr_red_sm5290 initSM,
    l6OMr_red_sm8088 initSM, hwEq,
    l6OMr_red_pm8824 initPM, l6OMr_red_pm8825 initPM,
    l6OMr_red_pm5292 initPM, l6OMr_red_pm5290 initPM,
    l6OMr_red_pm11934 initPM, l6OMr_red_pm15344 initPM,
    l6OMr_red_pm15346 initPM,
    ...
    l6OMr_chunk_gather0 _ _ hp0Shape hp1Shape,
    l6OMr_chunk_gather1 _ _ hp0Shape hp1Shape]
  exact hLocal
...
theorem l6_ordinary_moe_router_from_norm_input (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks initGoals initSM initPM)
    (hNorm : Gather2Rel
      (denoteGraphDistributedFaithful sm_goal_1 initSM 5289)
    ...
    Gather2Rel
      ...
      [4096, 64] [2048, 64] := by
  have hLogits := l6_ordinary_moe_logits_from_norm_input initSM initPM hPM hInit hNorm
  ...
  rw [l6OMr_red_sm5293 initSM hLogits.full_shape,
    l6OMr_red_pm8826 initPM hLogits.shard0_shape,
    l6OMr_red_pm8827 initPM hLogits.shard1_shape,
    l6OMr_red_sm5294 initSM hLogits.full_shape,
    l6OMr_red_pm8828 initPM hLogits.shard0_shape,
    l6OMr_red_pm8829 initPM hLogits.shard1_shape]
  exact ⟨hProbs, hMap⟩

/-- The real scalar-gate branch preserves ordinary dim-0 layout. -/
theorem l6_ordinary_moe_gate_from_norm_input (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks initGoals initSM initPM)
    (hNorm : Gather2Rel
      (denoteGraphDistributedFaithful sm_goal_1 initSM 5289)
  ...
  have hSource : Gather2Rel
    ...
    rw [l6OMrg_red_sm8096 initSM, l6OMrg_red_pm13176 initPM,
      l6OMrg_red_pm13177 initPM]
    exact hNorm
  have hReshape : Gather2Rel
      ...
      [4096, 1024] [2048, 1024] := by
    rw [l6OMrg_red_sm5299 initSM, l6OMrg_red_pm8838 initPM,
      l6OMrg_red_pm8839 initPM]
    exact Gather2Rel.view_id hSource
  have hwEq := l6OMrg_weight_eq initSM initPM hInit
  have hwShape := l6OMrg_weight_shape initPM hPM
  have hLinear : Gather2Rel
      ...
      [4096, 1] [2048, 1] := by
    rw [l6OMrg_red_sm5301 initSM, l6OMrg_red_pm8842 initPM,
      l6OMrg_red_pm8843 initPM, hwEq]
  ...
  have hView : Gather2Rel
      ...
      [4096, 1] [2048, 1] := by
    rw [l6OMrg_red_sm5302 initSM, l6OMrg_red_pm8844 initPM,
      l6OMrg_red_pm8845 initPM]
    exact Gather2Rel.view_id hLinear
  rw [l6OMrg_red_sm5303 initSM, l6OMrg_red_pm8846 initPM,
    l6OMrg_red_pm8847 initPM]
...
theorem l6_ordinary_moe_router_from_attention_output (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    ...
    Gather2Rel
      ...
      (denoteGraphDistributedFaithful pm_goal_1 initPM 8829)
      [4096, 64] [2048, 64] := by
  have hNorm := l6_ordinary_moe_norm_from_attention_output
    initSM initPM hInit hAttention
  exact l6_ordinary_moe_router_from_norm_input initSM initPM hPM hInit hNorm

/-- The ordinary scalar gate is closed directly from attention output. -/
theorem l6_ordinary_moe_gate_from_attention_output (initSM initPM : Store)
    (hPM : StoreShapesHold initPM pm_goal_1InitEnv)
    (hInit : InitGoalsHold pm_goal_1.numRanks goal_1_full_initGoals initSM initPM)
    (hAttention : Gather2Rel
      (denoteGraphDistributedFaithful sm_goal_1 initSM 5287)
  ...
  have hNorm := l6_ordinary_moe_norm_from_attention_output
    initSM initPM hInit hAttention
  exact l6_ordinary_moe_gate_from_norm_input initSM initPM hPM hInit hNorm

#print axioms l6_ordinary_moe_router_from_norm_input
#print axioms l6_ordinary_moe_gate_from_norm_input
#print axioms l6_ordinary_moe_router_from_attention_output
#print axioms l6_ordinary_moe_gate_from_attention_output
