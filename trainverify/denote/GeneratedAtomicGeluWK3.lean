...
def pmGraph : GraphDecl := { numRanks := 3, nodes := [{ rank := 0, op := "OpName.BW_gelu", ins := [100, 200], outs := [300] }, { rank := 1, op := "OpName.BW_gelu", ins := [101, 201], outs := [301] }, { rank := 0, op := "OpName.CROSS_DP_WRED", ins := [600, 601, 602], outs := [600] }, { rank := 2, op := "OpName.BW_gelu", ins := [102, 202], outs := [302] }] }


private def fg : RelationFact :=
  .sharded 10 [100, 101, 102] 1 [2, 9] [2, 3]

private def fx : RelationFact :=
  .sharded 20 [200, 201, 202] 1 [2, 9] [2, 3]

private def fo : RelationFact :=
  .sharded 30 [300, 301, 302] 1 [2, 9] [2, 3]
...
private def segment_000000_pm_nodes : List NodeDecl := [{ rank := 0, op := "OpName.BW_gelu", ins := [100, 200], outs := [300] }, { rank := 1, op := "OpName.BW_gelu", ins := [101, 201], outs := [301] }, { rank := 0, op := "OpName.CROSS_DP_WRED", ins := [600, 601, 602], outs := [600] }, { rank := 2, op := "OpName.BW_gelu", ins := [102, 202], outs := [302] }]
...
private theorem segment_000000_hPmWriter2 (pmStore : Store) :
  ...
  have hfinal : (segment_000000_pm_final pmStore) = segment_000000_pm_nodes.foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore := by
    unfold segment_000000_pm_final
    rfl
  have hout_nodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 3) ++ [{ rank := 2, op := "OpName.BW_gelu", ins := [102, 202], outs := [302] }] ++ (segment_000000_pm_nodes.drop 4) := by
    native_decide
  have hout_prefix : (segment_000000_pm_final pmStore) 302 = bw_gelu (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore 102) (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore 202) := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_middle_writer SyntheticBWGeluWred.pmGraph pmStore
  ...
  have hout_read_0 : ((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore 102 = (segment_000000_pm_final pmStore) 102 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWGeluWred.pmGraph pmStore
      (segment_000000_pm_nodes.take 3) ({ rank := 2, op := "OpName.BW_gelu", ins := [102, 202], outs := [302] } :: (segment_000000_pm_nodes.drop 4)) 102
      (by native_decide) (by native_decide)
  have hout_read_1 : ((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore 202 = (segment_000000_pm_final pmStore) 202 := by
    rw [hfinal, hout_nodes]
    exact foldl_faithful_prefix_read_eq_final SyntheticBWGeluWred.pmGraph pmStore
      (segment_000000_pm_nodes.take 3) ({ rank := 2, op := "OpName.BW_gelu", ins := [102, 202], outs := [302] } :: (segment_000000_pm_nodes.drop 4)) 202
      (by native_decide) (by native_decide)
  have hout : (segment_000000_pm_final pmStore) 302 = bw_gelu ((segment_000000_pm_final pmStore) 102) ((segment_000000_pm_final pmStore) 202) := by
    calc
      _ = bw_gelu (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore 102) (((segment_000000_pm_nodes.take 3)).foldl (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore 202) := hout_prefix
...
private theorem segment_000000_wred_writer_0 (pmStore : Store) :
  ...
  have hfinal : segment_000000_pm_final pmStore = segment_000000_pm_nodes.foldl
    ...
    unfold segment_000000_pm_final
    rfl
  have hnodes : segment_000000_pm_nodes = (segment_000000_pm_nodes.take 2) ++ [{ rank := 0, op := "OpName.CROSS_DP_WRED", ins := [600, 601, 602], outs := [600] }] ++ (segment_000000_pm_nodes.drop 3) := by native_decide
  have hprefix : (segment_000000_pm_final pmStore) 600 =
      cross_dp_wred ([600, 601, 602].map ((segment_000000_pm_nodes.take 2).foldl
        (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore)) := by
    rw [hfinal, hnodes]
    exact foldl_faithful_middle_writer SyntheticBWGeluWred.pmGraph pmStore (segment_000000_pm_nodes.take 2) (segment_000000_pm_nodes.drop 3)
      ...
      (fun t => cross_dp_wred ([600, 601, 602].map t)) (by
        ...
        · exact applyNode_cross_dp_wred_out SyntheticBWGeluWred.pmGraph t 0 [600, 601, 602] 600
        · native_decide
        · native_decide
      ) (by native_decide) (by native_decide)
  ...
  have hread2 : ((segment_000000_pm_nodes.take 2).foldl
      (applyNodeDistributedFaithful SyntheticBWGeluWred.pmGraph) pmStore) 602 = pmStore 602 :=
    foldl_applyNodeDistributedFaithful_at_not_written SyntheticBWGeluWred.pmGraph
      (segment_000000_pm_nodes.take 2) pmStore 602 (by native_decide) (by native_decide)
...
private theorem segment_000000_sound (smStore pmStore : Store)
    ...
    change ShardedRel (smFinal 10) [pmFinal 100, pmFinal 101, pmFinal 102] 1 [2, 9] [2, 3] at hg
    have hx : fx.Holds smFinal pmFinal := hframe _ (by native_decide)
    change ShardedRel (smFinal 20) [pmFinal 200, pmFinal 201, pmFinal 202] 1 [2, 9] [2, 3] at hx
    have hgValue : smFinal 10 = allGatherPrimDimN 1 3 0 [pmFinal 100, pmFinal 101, pmFinal 102] := by
      simpa only [List.length_cons, List.length_nil] using hg.full_value
    have hxValue : smFinal 20 = allGatherPrimDimN 1 3 0 [pmFinal 200, pmFinal 201, pmFinal 202] := by
    ...
    have hOutShape2 : (pmFinal 302).shape = [2, 3] := by
      ...
      exact hx.shard_shapes _ (by simp)
    have hcomm := TrainVerify.Denote.bw_gelu_allGatherPrimDimN_eq 1 3 [pmFinal 100, pmFinal 101, pmFinal 102] [pmFinal 200, pmFinal 201, pmFinal 202] [2, 3]
    ...
    have hOutValue : smFinal 30 = allGatherPrimDimN 1 3 0 [pmFinal 300, pmFinal 301, pmFinal 302] := by
      rw [hSmWriter, hgValue, hxValue, hcomm]
      simp only [List.zipWith]
      rw [← hPmWriter0, ← hPmWriter1, ← hPmWriter2]
    have hOutValueList : smFinal 30 = allGatherPrimDimN 1 [pmFinal 300, pmFinal 301, pmFinal 302].length 0 [pmFinal 300, pmFinal 301, pmFinal 302] := by
      simpa only [List.length_cons, List.length_nil] using hOutValue
    have hFullShape : (smFinal 30).shape = [2, 9] := by
      rw [hSmWriter, bw_gelu_shape]
      exact hx.full_shape
    have hout : fo.Holds smFinal pmFinal := by
      change ShardedRel (smFinal 30) [pmFinal 300, pmFinal 301, pmFinal 302] 1 [2, 9] [2, 3]
      refine {
