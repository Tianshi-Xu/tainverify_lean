...
The local gradient, activation and weight shapes retain the checked 32-wide ABI. -/
theorem bw_linear_dx_weight_allGatherPrimDimN_dim1_rank3
    (g x : Tensor) (xs ws : List Tensor)
    (hne : xs ≠ []) (hlen : ws.length = xs.length)
    (hg : g.shape = [1, 8, 32]) (hx : x.shape = [1, 8, 32 * xs.length])
    (hxs : ∀ t ∈ xs, t.shape = [1, 8, 32])
    (hws : ∀ t ∈ ws, t.shape = [32, 32]) :
  ...
  have hwsget (r : Nat) (hr : r < ws.length) :
      (ws.getD r (zeroTensor [32, 32])).shape = [32, 32] := by
  ...
  have hxsget (r : Nat) (hr : r < xs.length) :
      (xs.getD r (zeroTensor [1, 8, 32])).shape = [1, 8, 32] := by
    rw [List.getD_eq_getElem?_getD, List.getElem?_eq_getElem hr]
    exact hxs xs[r] (List.getElem_mem _)
  have hWshape : (allGatherPrimDimN 1 ws.length 0 ws).shape = [32, 32 * xs.length] := by
    rw [allGatherPrimDimN_shape 1 ws.length ws [32, 32] hheadw]
    simp [List.set, List.getD, hlen]
  have hLshape : (bw_linear g x (allGatherPrimDimN 1 ws.length 0 ws)).1.shape =
      [1, 8, 32 * xs.length] :=
    bw_linear_3d_fst_shape 1 8 32 (32 * xs.length) _ _ _ hg hx hWshape
  ...
  have hpget (r : Nat) (hr : r < xs.length) :
      pieces.getD r (zeroTensor [1, 8, 32]) =
        (bw_linear g (xs.getD r (zeroTensor [1, 8, 32]))
          (ws.getD r (zeroTensor [32, 32]))).1 := by
  ...
  have hpiecehead : (pieces.head?.map (fun t => t.shape)).getD [] = [1, 8, 32] := by
    ...
    | cons a rest =>
      ...
      | nil => simp at hlen
      | cons w wrest =>
        simp [pieces, bw_linear_3d_fst_shape 1 8 32 32 g a w hg
          (hxs a (by simp)) (hws w (by simp))]
  have hRshape : (allGatherPrimDimN 2 xs.length 0 pieces).shape = [1, 8, 32 * xs.length] := by
    rw [allGatherPrimDimN_shape 2 xs.length pieces [1, 8, 32] hpiecehead]
    simp [List.set, List.getD]
  apply Tensor.ext (by rw [hLshape, hRshape])
  intro idx hidx
  rw [hLshape] at hidx
  have hbound : idx < 8 * (32 * xs.length) := by simpa [prodShape] using hidx
  have hfullpos : 0 < 32 * xs.length := by omega
  let P := idx / (32 * xs.length)
  let rem := idx % (32 * xs.length)
  let r := rem / 32
  let col := rem % 32
  have hP : P < 8 := Nat.div_lt_of_lt_mul (by simpa [Nat.mul_comm] using hbound)
  have hrem : rem < 32 * xs.length := Nat.mod_lt idx hfullpos
  have hr : r < xs.length := Nat.div_lt_of_lt_mul (by simpa [Nat.mul_comm] using hrem)
  have hcol : col < 32 := Nat.mod_lt rem (by decide)
  have hri : r * 32 + col = rem := by dsimp [r, col]; omega
  have heq : idx = P * (32 * xs.length) + (r * 32 + col) := by
    rw [hri]
    simpa only [P, rem, Nat.mul_comm] using (Nat.div_add_mod idx (32 * xs.length)).symm
  rw [heq, bw_linear_fst_valAt_1_8 _ _ _ 32 (32 * xs.length) hg hx hWshape
    P hP (r * 32 + col) (by omega)]
  rw [allGatherPrimDimN_dim2_3d_valAt pieces xs.length 1 8 32 P r col
    hK (by decide) (by simpa using hP) hr hcol hpiecehead, hpget r hr]
  have hwr : r < ws.length := by omega
  rw [bw_linear_fst_valAt_1_8_32_g134 _ _ _ 32 hg (hxsget r hr) (hwsget r hwr) P hP col hcol]
  apply Finset.sum_congr rfl
  intro j hj
  congr 1
  have hv := allGatherPrimDimN1_valAt_g240 ws.length 32 32 ws hW
    (by decide) (by decide) hheadw hwsget j (Finset.mem_range.mp hj) r hwr col hcol
