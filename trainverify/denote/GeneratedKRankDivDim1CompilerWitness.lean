...
def fact_in : RelationFact := .sharded 100 [201, 202, 203] 1 [1, 36, 256, 64] [1, 12, 256, 64]
def fact_out : RelationFact := .sharded 110 [301, 302, 303] 1 [1, 36, 256, 64] [1, 12, 256, 64]
...
private def segment_000000 :
  ...
  sound := by
    ...
    let smFinal := smNodes.foldl (applyNodeDistributedFaithful SyntheticDiv1.gSM) smStore
    let pmFinal := pmNodes.foldl (applyNodeDistributedFaithful SyntheticDiv1.gPM) pmStore
    ...
    change ShardedRel (smStore 100) [pmStore 201, pmStore 202, pmStore 203] 1 [1, 36, 256, 64] [1, 12, 256, 64] at hin
    have hinDiv : ShardedRel (smStore 100) [pmStore 201, pmStore 202, pmStore 203] 1 [1, 12 * [pmStore 201, pmStore 202, pmStore 203].length, 256, 64] [1, 12, 256, 64] := by
      simpa using hin
    have hSmWriter : smFinal 110 = fw_div (8 : Scalar) (smStore 100) := by
      change (smNodes.foldl (applyNodeDistributedFaithful SyntheticDiv1.gSM) smStore) 110 = _
      rw [show smNodes = smNodes.take 0 ++ [segment_000000_sm_node] ++ smNodes.drop 1 by native_decide]
      rw [foldl_faithful_middle_writer SyntheticDiv1.gSM smStore (smNodes.take 0) (smNodes.drop 1)
        segment_000000_sm_node 110 (fun t => fw_div (8 : Scalar) (t 100)) (by
          ...
          exact applyNode_fw_div_out_g67 SyntheticDiv1.gSM t 0 8 100 110
        ) (by native_decide) (by native_decide)]
      rw [foldl_applyNodeDistributedFaithful_at_not_written SyntheticDiv1.gSM (smNodes.take 0) smStore 100 (by native_decide) (by native_decide)]
    have hPmWriter0 : pmFinal 301 = fw_div (8 : Scalar) (pmStore 201) := by
      change (pmNodes.foldl (applyNodeDistributedFaithful SyntheticDiv1.gPM) pmStore) 301 = _
      rw [show pmNodes = pmNodes.take 0 ++ [segment_000000_pm_node_0] ++ pmNodes.drop 1 by native_decide]
      rw [foldl_faithful_middle_writer SyntheticDiv1.gPM pmStore (pmNodes.take 0) (pmNodes.drop 1)
        segment_000000_pm_node_0 301 (fun t => fw_div (8 : Scalar) (t 201)) (by
          intro t
          rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective
            (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
          simp [applyNodeDistributed, applyNodeRingAttn]
          exact applyNode_fw_div_out_g67 SyntheticDiv1.gPM t 0 8 201 301
        ) (by native_decide) (by native_decide)]
      rw [foldl_applyNodeDistributedFaithful_at_not_written SyntheticDiv1.gPM (pmNodes.take 0) pmStore 201 (by native_decide) (by native_decide)]
    have hPmWriter1 : pmFinal 302 = fw_div (8 : Scalar) (pmStore 202) := by
      change (pmNodes.foldl (applyNodeDistributedFaithful SyntheticDiv1.gPM) pmStore) 302 = _
      rw [show pmNodes = pmNodes.take 1 ++ [segment_000000_pm_node_1] ++ pmNodes.drop 2 by native_decide]
      rw [foldl_faithful_middle_writer SyntheticDiv1.gPM pmStore (pmNodes.take 1) (pmNodes.drop 2)
        segment_000000_pm_node_1 302 (fun t => fw_div (8 : Scalar) (t 202)) (by
          intro t
          rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective
            (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
          simp [applyNodeDistributed, applyNodeRingAttn]
          exact applyNode_fw_div_out_g67 SyntheticDiv1.gPM t 1 8 202 302
        ) (by native_decide) (by native_decide)]
      rw [foldl_applyNodeDistributedFaithful_at_not_written SyntheticDiv1.gPM (pmNodes.take 1) pmStore 202 (by native_decide) (by native_decide)]
    have hPmWriter2 : pmFinal 303 = fw_div (8 : Scalar) (pmStore 203) := by
      change (pmNodes.foldl (applyNodeDistributedFaithful SyntheticDiv1.gPM) pmStore) 303 = _
      rw [show pmNodes = pmNodes.take 2 ++ [segment_000000_pm_node_2] ++ pmNodes.drop 3 by native_decide]
      rw [foldl_faithful_middle_writer SyntheticDiv1.gPM pmStore (pmNodes.take 2) (pmNodes.drop 3)
        segment_000000_pm_node_2 303 (fun t => fw_div (8 : Scalar) (t 203)) (by
          intro t
          rw [applyNodeDistributedFaithful_eq_applyNodeDistributed_of_not_collective
            (hshuffle := by native_decide) (hunshuffle := by native_decide) (hattn := by native_decide)]
          simp [applyNodeDistributed, applyNodeRingAttn]
          exact applyNode_fw_div_out_g67 SyntheticDiv1.gPM t 2 8 203 303
        ) (by native_decide) (by native_decide)]
      rw [foldl_applyNodeDistributedFaithful_at_not_written SyntheticDiv1.gPM (pmNodes.take 2) pmStore 203 (by native_decide) (by native_decide)]
    have htransport := ShardedRel.fw_div_dim1_rank4 (h := hinDiv) (c := (8 : Scalar))
