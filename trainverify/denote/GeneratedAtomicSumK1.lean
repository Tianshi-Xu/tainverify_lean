...
def pmGraph : GraphDecl := { numRanks := 1, nodes := [{ rank := 0, op := "OpName.FW_float", ins := [8], outs := [1000] }, { rank := 0, op := "OpName.FW_sum", ins := [1000], outs := [2000] }, { rank := 0, op := "OpName.BW_sum", ins := [9, 1000], outs := [3000] }], replicaGroups := [] }
private def fact_g : RelationFact := .reduction 9 [9] [1]
private def fact_x : RelationFact := .sharded 10 [1000] 1 [2, 3, 5] [2, 3, 5]
private def fact_sum : RelationFact := .reduction 20 [2000] [1]
private def fact_bw : RelationFact := .sharded 30 [3000] 1 [2, 3, 5] [2, 3, 5]
...
private def segment_000000_pm_nodes : List NodeDecl := [{ rank := 0, op := "OpName.FW_sum", ins := [1000], outs := [2000] }, { rank := 0, op := "OpName.BW_sum", ins := [9, 1000], outs := [3000] }]
...
private theorem segment_000000_sound (smStore pmStore : Store)
  ...
  change ShardedRel (smFinal 10) [pmFinal 1000] 1 [2, 3, 5] [2, 3, 5] at hx
  change ReductionRel (smFinal 9) [pmFinal 9] [1] at hg
  have hxValue : smFinal 10 = allGatherPrimDimN 1 1 0 [pmFinal 1000] := by
  ...
  have hSumComm := TrainVerify.Denote.fw_sum_allGatherPrimDimN_eq_allReducePrim_fw_sum 1 1 [pmFinal 1000] rfl (by simp) [2, 3, 5]
    (by simpa using hXShape0) hx.shard_shapes hx.gather_dim_lt (by native_decide) (by native_decide)
  have hSumValue : smFinal 20 = allReducePrim [pmFinal 2000].length 0 [pmFinal 2000] := by
  ...
  have hSumOut : fact_sum.Holds smFinal pmFinal := by
    change ReductionRel (smFinal 20) [pmFinal 2000] [1]
  ...
  have hBwComm : bw_sum (smFinal 9)
      (allGatherPrimDimN 1 1 0 [pmFinal 1000]) =
      allGatherPrimDimN 1 1 0 [bw_sum (smFinal 9) (pmFinal 1000)] := by
    simpa only [List.length_cons, List.length_nil, List.map] using
      (TrainVerify.Denote.bw_sum_allGatherPrimDimN_dim1_rank3
        (smFinal 9) [pmFinal 1000] 2 3 5
        hx.shards_nonempty (by decide) (by decide) (by decide) hx.shard_shapes)
  have hBwOut : fact_bw.Holds smFinal pmFinal := by
    change ShardedRel (smFinal 30) [pmFinal 3000] 1 [2, 3, 5] [2, 3, 5]
    constructor
    · change smFinal 30 = allGatherPrimDimN 1 1 0 [pmFinal 3000]
