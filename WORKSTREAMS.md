...
- Historical source versions, all ignored/untracked data (including the six research probes and the saved-primal proof/cache collection), and prior worktree administration were preserved under `/home/v-zhouziyu/trainverify-audits/worktree-cleanup/single-worktree-20260913T181444Z/preserved/`. Git objects and archive refs remain intact in the promoted primary repository. The seven old `/home/v-zhouziyu/work/trainverify*` paths are compatibility symlinks to `preserved/snapshots/<basename>`, **not Git worktrees**. They remain valid for file-based proof receipts; do not develop or compile new outputs there.
- Historical Git-based runners must use their recorded commit through `git archive` or a disposable checkout instead of expecting Git metadata at an old compatibility path. Use private output/cache directories for new checks; archived checked objects are inputs, not a mutable build destination. Do not delete compatibility links until every current consumer has an explicit verified path replacement.
