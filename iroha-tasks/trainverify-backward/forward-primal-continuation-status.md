...
## Mixed post-transpose source handoff

Implementation `cf53bd9a60cec689f6cf22e087e7ae4da2524a56` plus retained-fact fix `8db687e1a20d5106be18087f423ceb41d13415fc` were merged into `integration/trainverify-forward-backward` by `b60e7288`. Both renderer/test blobs match the reviewed source exactly; the original worker checkout is preserved and clean.

- K's raw `torch.transpose(-2,-1)` produces SM1303 and PM222/525, PM828/1131, reconstructing on axis2.
- V's `allgather_reducescatter(dim2)` retains SM1302 and produces PM79/382, PM685/988. Its complete contract has `layout=replicated_within_dp`, `gather_axis=None`, global shape, every-local shape, and **every local output equal to its DP chunk**.
- Final-fix saved-source replay and independent raw census in the development checkout passed: **9 reads, 4 full facts, all 8 ordered frontier rows, 2 Q matmul deferrals and 2 residual carries**. The 13 read/unit declarations and all-eight mixed-layout joint passed kernel using only the standard three axioms. Final-fix proof bytes were unchanged and exact source/object/dependency reuse was checked.
- The retained-carry metadata gap was reproduced before repair: missing or coherently rewritten theorem identities could survive a candidate-against-itself comparison. The fix reconstructs the owning alias constructor's complete fact record from original fork/ADD/owner/slot provenance. Final-source narrow review passed. The final-fix public/retained mutation subset passed **46 tests**; the interrupted worker's full-suite summary is not counted as recovered acceptance evidence.
- The full joint rejected replacing one DP unit's V replicas with the other unit's same-shaped replicas. Four synthetic exact-index source-shaped K/V cases passed; these are not model-execution evidence.
- On integrated code commit `b60e7288`, the complete collected regression set passed **219 tests, zero failures/errors/skips**, split into three disjoint 73-case processes with JUnit and node-ID manifests. This is the full new suite plus two previous public regressions; it overlaps the earlier 46-case subset. Fresh integrated-source replay, independent raw census and the all-eight mixed joint passed. All 13 new declarations and their predecessor closure were reused only after exact source/object/dependency/axiom checks.
- Integration evidence remains at the original private evidence root in `post-transpose-integrated/` and `post-transpose-integrated-tests/`; the latter contains the complete collection, per-batch JUnit/logs and `result.json`.

Next is K's original AA(2,1), then the source-ordered Q/K score matmul join. V's replicated contract and both residual carries must remain intact. New implementation starts only after the integrated handoff is verified and its worktree is clean. Saved-X1317 remains unproved.

## Public and final-LN boundary

The historical sequence-alias bundle was rejected at 2,504,714 bytes against the 2,500,000-byte limit. The now-integrated forward v4 baseline subsequently resolved that alias-stage budget issue: its accepted bundle was 2,489,883 bytes, as recorded in WORKSTREAMS.md. This is not a size result for the newly assembled QKV/backward continuation. Local kernel-checked incremental modules above are **not** canonical whole-model/public acceptance, Torch refinement, or a proof of saved-X1317. No upper limit was raised and no theorem contract was shortened.

Private exact sources, raw checks, logs and receipts remain at `/home/v-zhouziyu/work/trainverify-backward-matmul-next/.hermes/forward-value-continuation/` (not under the integrated checkout): `alias/`, `alias-probe/`, `exchange/`, `input-linear/`, review JSONs and the original-input-linear census. `render_local.py` replays source stages and `kernel_local.py` checks incremental modules against the exact accepted forward closure. These are local diagnostics, not replacements for canonical publication gates.
