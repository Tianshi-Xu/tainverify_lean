"""Actual backward collective/cotangent fragment, reusing prior DAG certificates."""
import json
...
from Verdict.runtime_backward_cotangent_reads import render
...
text,detail=render(w,config,config['pm_run'],str(rank_code))
...
(A/'ActualBWCotangentRead.lean').write_text(header+text+'end\nend TrainVerify.Denote.RuntimeWorld\n')
(A/'actual-cotangent-reads.json').write_text(json.dumps(detail,indent=2))
print('actual cotangent connections',len(detail['reads']),'theorems',sum(len(r['theorems']) for r in detail['reads']))
