...
- Current source-read equations are local conditional facts, not distributed gradient equality. Existing generic column dX/dW lemmas remain to be composed with **final LayerNorm saved-X reconstruction**, **initial weight frame/layout**, and **BW_sum → reverse AllToAll cotangent values**.
- Next backward-owned slice: consume existing authenticated seed adapter and trace first cotangent producer chain into the same final Store. No default seed/normalizer/DP averaging may be inserted. Forward saved-X relation is an explicit obligation of the forward line.
- All emitted proof/kernel/public/Torch flags remain false; the external exact-byte receipts only certify these local kernel declarations. Canonical/main integration not done; no push, publication or cron.
