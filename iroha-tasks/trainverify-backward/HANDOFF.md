...
- Current source-read equations are local conditional facts, not distributed gradient equality. Existing generic column dX/dW lemmas remain to be composed with **final LayerNorm saved-X reconstruction**, **initial weight frame/layout**, and **reverse AllToAll cotangent values**.
- Seed → BW_sum values are now closed under the original successful seeded run. Next backward-owned slice: actual reverse AllToAll (raw fused primitive carries FW idim/odim; use authenticated backward scope, not its FW-looking kwargs). Forward saved-X relation is an explicit obligation of the forward line.
