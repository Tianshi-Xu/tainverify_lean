"""Original BW_div read fragment, with caller-owned successful Store/run."""
from trainverify.backward_div_authority import bind, _fail
...
def render_read(view, cells, snapshot, source_index, order, label):
    ...
    for original in (cell, cells[contract['fw_source_index']]):
        ...
        if not _same_typed(params, [contract['divisor']]):
            _fail('original scalar divisor lowered params mismatch')
        source_params[original_node] = params
    ...
    name = f'backwardDivRead_{label}_{source_index}_dx'
    proofs = [f'theorem {name} (s t : Store) (h : {label}DenoteWithInputs s = some t) :',
        f'    t {dx} = bw_div ({divisor} : Scalar) (t {ids[0]}) := by',
        f'  apply SourceBWDivRead.bw_div_value_of_split {label}Graph {label}Scope {label}Peers {label}Graph.nodes',
        f'    {requests} ({requests}.take {k}) ({requests}.drop {k+1}) {label}Node_{source_index}',
        f'    {node.rank} {ids[0]} {ids[1]} {dx} {divisor} s t rfl ?_ (by decide) ?_ h',
        '  · calc',
    ...
    return '\n'.join(proofs)+'\n', dict(theorems=[name], world=label,
        ...
        input_tids=ids, output_tid=dx, output_tids=[dx], input_refs=contract['inputs'],
        output_ref=contract['outputs'][0], output_refs=contract['outputs'],
