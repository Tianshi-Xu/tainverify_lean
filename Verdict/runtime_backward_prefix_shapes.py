...
def render(worlds,indices,prefix):
    ...
    rows=[];nodes=view.nodes()
    for index in indices:
        render_read(view,cells,snapshot,index,order,label)
        node=nodes[index]
        for role,tensor in [('cotangent',view.node_inputs(node)[0]),('saved_primal',view.node_inputs(node)[1]),('dx',view.node_outputs(node)[0])]:
