"""Original BW_gelu read in the SAME successful run's final Store."""
from trainverify.backward_gelu_authority import bind
...
def render_read(view, cells, snapshot, source_index, order, label):
    if label not in ('sm', 'pm'):
        raise ValueError('bw-gelu invalid world')
    ...
    if (not _same_typed([tuple(n) for n in nodes], [tuple(c.node) for c in cells])
        ...
        raise ValueError('bw-gelu original node inventory/owner mismatch')
    ...
    if not _same_typed(order.get('source_to_execution'), validated['source_to_execution']):
        raise ValueError('bw-gelu execution/source inverse order mismatch')
    for original in (cell, cells[contract['fw_source_index']]):
        ...
        for side in ('inputs', 'outputs'):
            ...
            if not _same_typed([tuple(view.source_tensor(t)) for t in ts], [tuple(r) for r in getattr(original, side)]):
                raise ValueError('bw-gelu lowered ordered fullref mismatch')
            if not _same_typed([tuple(view.tensor_shape(t)) for t in ts], [tuple(ir.shape) for ir in irs]):
                raise ValueError('bw-gelu lowered original shape mismatch')
    ...
    if any(set(ids + outs) & {t.tid for t in view.node_outputs(nodes[j])} for j in seq[k+1:]):
        raise ValueError('bw-gelu operand/output overwritten in complete execution suffix')
    if set(ids) & set(outs):
        raise ValueError('bw-gelu selected node overwrites operand')
    requests = f'{label}InputRequests'
    name = f'backwardGeluRead_{label}_{source_index}_dx'
    proof = [f'theorem {name} (s t : Store) (h : {label}DenoteWithInputs s = some t) :',
             f'    t {outs[0]} = bw_gelu (t {ids[0]}) (t {ids[1]}) := by',
             f'  apply SourceBWGeluRead.bw_gelu_value_of_split {label}Graph {label}Scope {label}Peers {label}Graph.nodes',
             f'    {requests} ({requests}.take {k}) ({requests}.drop {k+1}) {label}Node_{source_index}',
             '    ' + ' '.join(map(str, [cell.rank, *ids, *outs])) + ' s t rfl ?_ rfl ?_ ?_ h',
    ...
    return '\n'.join(proof) + '\n', dict(world=label, source_index=source_index, execution_index=k,
        theorems=[name], input_tids=ids, output_tids=outs,
