...
def _get_lng_slcmap(
    ...
    for slc, slc_Tps in slc2Tps.items():
        ...
        if is_attr and not is_fw:
            ...
            # Fallback for weight grads that are NOT fed into any
            # CROSS_DP_WRED (e.g. expert-parallel MoE weights where each
            # rank holds disjoint experts and the BW op's local weight grad
            # is already final — no cross-DP reduction is generated by
            # nnscaler). The audit lineage algorithm assumed every weight
            # grad would appear in `_shared_tensor_list` via a CROSS_DP_WRED
            # node, which produced KeyError for EP MoE traces. Treat any
            # missing (rank, tid) as its own singleton comm group so the
            # _group_by below still works and the grad is reported as a
            # per-rank-distinct copy (semantically correct for EP).
            def _comm_group_of(Tp) -> Hashable:
                key = (Tp.rank, Tp.tid)
                if key in comm_group_map:
                    return comm_group_map[key]
                # singleton: this rank's grad doesn't fuse with any other rank
                return ("__no_wred__",) + key
            if is_wred_lng:
                ...
                for rank, rank_Tps in rank2Tps.items():
                    ...
                    key = (rank, tid)
                    # Same EP-friendly fallback as above: dangling weight
                    # grads with no CROSS_DP_WRED stand alone in their own
                    # comm group.
                    if key in comm_group_map:
                        comm_group = comm_group_map[key]
                    else:
                        comm_group = ("__no_wred__",) + key
