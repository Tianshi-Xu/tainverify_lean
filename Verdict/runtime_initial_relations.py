...
def attach(world, sm, pm, parameter_inputs, lineages, validation):
    ...
    if 'input_feed' in world.receipt:
        ...
        from Verdict.runtime_frontier_gelu_values import render as render_frontier_gelu
        frontier_gelu_text, frontier_gelu_detail = render_frontier_gelu(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        if frontier_gelu_detail['reads']:
            entry += '\n' + frontier_gelu_text
        result['frontier_gelu_values'] = frontier_gelu_detail
        ...
        from Verdict.runtime_frontier_add_values import render as render_frontier_add
        frontier_add_text, frontier_add_detail = render_frontier_add(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        if frontier_add_detail['reads']:
            entry += '\n' + frontier_add_text
        result['frontier_add_values'] = frontier_add_detail
        from Verdict.runtime_frontier_next_alias_exchange_values import render as render_frontier_next_alias_exchange
        frontier_next_alias_exchange_text, frontier_next_alias_exchange_detail = render_frontier_next_alias_exchange(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        ...
        from Verdict.runtime_frontier_sequence_alias_values import render as render_frontier_sequence_alias
        frontier_sequence_alias_text, frontier_sequence_alias_detail = render_frontier_sequence_alias(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        ...
        from Verdict.runtime_frontier_middle_exchange_values import render as render_frontier_middle_exchange
        frontier_middle_exchange_text, frontier_middle_exchange_detail = render_frontier_middle_exchange(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        if frontier_middle_exchange_detail['reads']:
            entry += '\n' + frontier_middle_exchange_text
        result['frontier_middle_exchange_values'] = frontier_middle_exchange_detail
        from Verdict.runtime_frontier_score_matmul_values import render as render_frontier_score_matmul
        frontier_score_matmul_text, frontier_score_matmul_detail = render_frontier_score_matmul(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        if frontier_score_matmul_detail['reads']:
            entry += '\n' + frontier_score_matmul_text
        result['frontier_score_matmul_values'] = frontier_score_matmul_detail
        from Verdict.runtime_frontier_score_exchange_values import render as render_frontier_score_exchange
        frontier_score_exchange_text, frontier_score_exchange_detail = render_frontier_score_exchange(
            sm, pm, lineages, validation, bound, world.receipt['execution_order'])
        if frontier_score_exchange_detail['reads']:
            entry += '\n' + frontier_score_exchange_text
        result['frontier_score_exchange_values'] = frontier_score_exchange_detail
    ...
    read_version = (3 if world.receipt.get('seed_input', {}).get('parameter_inputs', {}).get('status')
                    == 'current-run-parameter-values-validated' else 2)
    entry = compact_reads(compact_binders(entry), version=read_version)
