"""Original head-sharded rank-four to rank-three source views, UNCOMPILED.
Fresh SAME-six contiguous-exchange rendering is the only frontier authority.
The parent owns captures, integration, imports and kernel acceptance.
"""
...
def render(sm, pm, lineages, validation, bound, execution_order):
    ...
    try:
        ...
        raise ValueError(f'malformed view-flatten original source: {exc}') from exc
...
def _authenticate_cell(index, cell):
    ...
    if (op(cell) != 'FW_view'
        ...
        raise ValueError('view-flatten requires immediate original global opcode')
    ...
    if snapshot is not None:
        ...
        for key in ('inputs', 'outputs'):
            if not _same_typed(writer[key], [dict(zip(fields, r)) for r in getattr(cell, key)]):
                raise ValueError('view-flatten writer ordered fullrefs mismatch')
        auth.update(source_writer=writer['export_id'], writer_ref=dict(ref))
    if not _same_typed(tuple(cell.node), tuple(node)):
        raise ValueError('view-flatten original typed node identity mismatch')
    for method, refs in [('node_inputs', cell.inputs), ('node_outputs', cell.outputs)]:
        ...
        if not _same_typed(actual, [tuple(r) for r in refs]):
            raise ValueError('view-flatten original ordered fullrefs mismatch')
    if len(cell.inputs) != 1 or len(cell.outputs) != 1:
        raise ValueError('view-flatten original unary arity mismatch')
    return auth
...
def _unary(index, cell, producer):
    ...
    if (originals is None or len(originals) != len(raw_producer.outputs)
        ...
        raise ValueError('view-flatten complete original producer/input/output metadata required')
    ...
    if (not _same_typed(post._port(index, producer.endpoint.ref, original), producer)
        ...
        raise ValueError('view-flatten original producer/consumer metadata mismatch')
    if (type(original.parent.tid) is not int or type(inputs[0].parent.tid) is not int
        ...
        raise ValueError('view-flatten original producer/consumer parent identity mismatch')
    node = projection._node(index, cell)
    ...
    if (not _same_typed(x, producer)
        ...
        raise ValueError('view-flatten original input/output fullref/owner/writer mismatch')
    C = x.parent_shape[3]
    if (len(y.parent_shape) != 3 or x.bounds[3] != (0, C)
            or not _same_typed(y.parent_shape, (*x.parent_shape[:2], x.parent_shape[2]*C))
            or not _same_typed(y.bounds, (*x.bounds[:2], tuple(n*C for n in x.bounds[2])))
            or not _same_typed(y.value_part, x.value_part)):
        raise ValueError('view-flatten original flatten shape/bounds/value partition mismatch')
    return routes.Step(tuple(node), 'FW_view', (x,), (y,))


def _kwargs(kw):
    if (type(kw) is not dict or set(kw) - {'size', '__consts'}
            or ('__consts' in kw and not _same_typed(kw['__consts'], []))):
        raise ValueError('view-flatten strict original kwargs mismatch')
    size = kw.get('size')
    if (type(size) not in (list, tuple) or len(size) != 3
            or any(type(n) is not int or n == 0 or n < -1 for n in size)
            or list(size).count(-1) > 1):
        raise ValueError('view-flatten strict original raw size mismatch')
    return list(size)


def _parameters(source, node):
    if (str(source.node_opname(node)).split('.')[-1] != 'FW_view'
            or node in getattr(source, 'collective_scopes', {})
            or len(source.node_inputs(node)) != 1 or len(source.node_outputs(node)) != 1):
        raise ValueError('view-flatten original opcode/global request/arity mismatch')
    size = _kwargs(source.node_kwargs(node))
    shape = tuple(source.tensor_shape(source.node_inputs(node)[0]))
    if len(shape) != 4 or any(type(n) is not int or n <= 0 for n in shape):
        raise ValueError('view-flatten positive integer rank-four input required')
    expected = [shape[0], shape[1], shape[2]*shape[3]]
    if -1 in size:
        divisor = prod(n for n in size if n != -1)
        if prod(shape) % divisor:
            raise ValueError('view-flatten raw size element count mismatch')
        size[size.index(-1)] = prod(shape)//divisor
    if not _same_typed(size, expected):
        raise ValueError('view-flatten raw size disagrees with input-derived flatten')
    output_shape = tuple(source.tensor_shape(source.node_outputs(node)[0]))
    if not _same_typed(output_shape, tuple(expected)):
        raise ValueError('view-flatten output shape disagrees with input-derived flatten')
    # Inspect extractor output before _ordinary's `params or []` coercion.
    raw = c._get_node_params(source, node, num_parts=0)
    if not _same_typed(raw, expected):
        raise ValueError('view-flatten raw params disagree with input-derived flatten')
    params, status = _ordinary(source, node, lambda *a, **k: raw)
    if status is not None or not _same_typed(params, expected):
        raise ValueError('view-flatten normalized params mismatch')
    return params
...
def _read(source, label, step, order):
    ...
    i, node = _one(((i, n) for i, n in enumerate(nodes) if _same_typed(tuple(n), step.node)),
        'view-flatten source writer missing/ambiguous')
    if step.op != 'FW_view':
        raise ValueError('view-flatten read step opcode mismatch')
    ...
    if (any(type(n) is not int for n in schedule) or sorted(schedule) != list(range(len(nodes)))):
        raise ValueError('view-flatten incomplete original execution order')
    k = schedule.index(i)
    ...
    for source_index in schedule[k:]:
        if inp in {t.tid for t in source.node_outputs(nodes[source_index])}:
            raise ValueError('view-flatten operand is written by selected node or suffix')
    params = _parameters(source, node)
    name, req = f'viewFlattenRead_{label}_{out}', f'{label}InputRequests'
...
def _unit(prior, global_, locals_, names):
    if prior['layout'] != 'sharded' or not _same_typed(prior['gather_axis'], 2):
        raise ValueError('view-flatten requires axis2 sharded predecessor')
    ...
    # S/H/C describe the actual post-exchange INPUT, not old AA dimensions.
    if len(gx.endpoint.shape) != 4 or not locals_ or len(locals_[0].inputs[0].endpoint.shape) != 4:
        raise ValueError('view-flatten rank-four input required')
    b, S, H, C = locals_[0].inputs[0].endpoint.shape
    if b != B:
        raise ValueError('view-flatten batch dimension mismatch')
    u = prior['unit']; gshape = [B*D, S, H*T*C]; lshape = [B, S, H*C]
    name = f'viewFlattenUnitFacts_{gy.endpoint.tid}_{u}'
    row = dict(theorem=name, facts_theorem=name, family='FW_view',
        ...
        input_gather_axis=2, gather_axis=2, global_shape=gshape, local_shape=lshape,
    ...
    proof = [f'theorem {name} (s p t q : Store)',
        ...
        f'    chunkPrimDimN 0 {D} {u} (t {gy.endpoint.tid}) = allGatherPrimDimN 2 {T} 0 {ys} := by',
        ...
        f'    {pairs}',
        f'  exact TrainVerify.Denote.source_view_flatten_head_unit_output_reconstruct {D} {T} {B} {S} {H} {C} {u}',
...
def _render(sm, pm, validation, order, prior):
    ...
    for i, unit in enumerate(old):
        owner = _one((o for o in validation._inputs[3]['config']['units']
            if _same_typed(o['unit'], unit['unit'])), 'view-flatten DP owner missing/ambiguous')
        if not _same_typed((owner['ranks'], owner['positions']), (unit['ranks'], unit['positions'])):
            raise ValueError('view-flatten source DP ownership/ordered ranks mismatch')
        ...
        if key in identities:
            raise ValueError('view-flatten duplicate original frontier identity')
        identities.add(key)
        ...
        for source, label, step, auth in [(sm, 'sm', global_, ga),
            ...
            if step.node in seen:
                if label != 'sm' or not _same_typed(seen[step.node], step):
                    raise ValueError('view-flatten duplicate/cross-unit source operation')
                continue
    ...
    return text, dict(status='source-view-flatten-values-emitted-uncompiled',
        ...
        deferred_stage='after-flatten-view: downstream unproved',
        cost_scope='new view-flatten fragment only; excludes predecessors, frame and imports',
