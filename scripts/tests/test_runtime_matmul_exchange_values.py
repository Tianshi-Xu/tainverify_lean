...
import pytest
from scripts.tests import test_runtime_matmul_values as previous
...
def api():
    assert importlib.util.find_spec('Verdict.runtime_matmul_exchange_values'), 'matmul exchange renderer missing'
    return importlib.import_module('Verdict.runtime_matmul_exchange_values')
...
def fixture(D=2, tp=2, seqlen=2, branches=3):
    ...
    sm, pm, authority = previous.fixture(D, tp, seqlen, branches, saved=True)
    old = copy.deepcopy(authority[2]); rows, adapters = [], []
    for producer in list(pm.cells):
        if producer.node.cid != 9000: continue
        ...
        bounds[1] = (0, x.parent.shape[1]); width = x.parent.shape[3]//tp
        bounds[3] = (j*width, (j+1)*width)
        out = IR(9500, x.parent.name, x.parent.shape, tuple(bounds))
        kwargs = dict(ranks=ranks, idim=1, odim=3)
        peers = [next(c for c in pm.cells if c.rank == r and c.node.cid == 9000) for r in ranks]
        cell = NS(node=N('p', producer.rank, 0, 9500, 'AllToAllPrim'), rank=producer.rank,
            opname='AllToAllPrim', inputs=[p.outputs[0] for p in peers],
            outputs=[T('p', producer.rank, 0, 9500, 1)], kwargs=kwargs,
        ...
        for consumer in pm.cells:
            if consumer.rank == producer.rank and producer.outputs[0] in consumer.inputs:
        ...
        row = dict(ref=dict(world='p', runtime_rank=cell.rank, microbatch=0, source_cid=9500,
        ...
        adapter['primitive'] = dict(kind='AllToAllPrim', forward=True, kwargs=copy.deepcopy(kwargs),
            signature='nnscaler.runtime.adapter.all_to_all', generated_inputs=['score'], generated_outputs=['exchanged'])
        params = ', '.join(f'{k}={v!r}' for k, v in kwargs.items())
        old['rank_sources'][str(cell.rank)] = old['rank_sources'][str(cell.rank)].replace(
            '\ndef _train_step', '\n        exchanged = nnscaler.runtime.adapter.all_to_all(score, '+params+')\ndef _train_step')
        adapters.append(adapter)
    def interleave(original, extra):
        # All peer matmuls must precede the collective's expanded input reads.
        early = [w for w in original if w['ref']['source_cid'] < 9100]
        late = [w for w in original if w['ref']['source_cid'] >= 9100]
        return [*early, *extra, *late]
    pm.cells[:] = ([c for c in pm.cells if c.node.cid < 9100]
        + [c for c in pm.cells if c.node.cid == 9500]
        + [c for c in pm.cells if c.node.cid >= 9100 and c.node.cid != 9500])
...
def prepared(D=2, tp=2, seqlen=2, branches=3):
    base = previous.previous.previous.previous.existing.existing.projection.norm.post.base
...
def test_next_pm_tracer():
    ...
    for old, new in zip(prior['frontier_units'], detail['frontier_units'], strict=True):
        if old['gather_axis'] is None:
            ...
            assert new['local_shape'] == [1, 4, 2, 1]
            assert new['gather_axis'] == 3
    ...
    for r in detail['reads']:
        assert (r['world'], r['gather_axis'], r['output_gather_axis'], r['params']) == ('pm', 1, 3, [1, 3])
...
def cells(args):
    return [c for c in args[3]._inputs[1] if c.node.cid == 9500]
...
def test_dimensions_and_complete_order(D, tp, seqlen, branches):
    ...
    for u in detail['units']:
        ...
        assert f"axis1_output_facts {D} {u['unit']} {tp} 1 2 {seqlen} {seqlen//tp}" in text
...
def test_boundary_independent_input_geometry(baseline, fault):
    ...
    if fault == 'order': ports.reverse()
        ...
        elif fault == 'cover': bounds[1] = (1, 3)
        elif fault == 'split': parent = (*parent[:-1], 3)
...
def test_order_is_all_old_frontier_not_slots(baseline):
    ...
    assert [u['gather_axis'] for u in detail['frontier_units']] == [None, 3, None, 3]
    for old, new in zip(prior['frontier_units'], detail['frontier_units'], strict=True):
        assert new['sm_output_ref'] == old['sm_output_ref']
...
def test_positive_exact_split_guard_not_peer_mismatch(baseline, width):
    ...
    args, prior = copy.deepcopy(baseline)
    index, old, ports, c = seam(args, prior)
    ports = [replace(p, parent_shape=(*p.parent_shape[:-1], width),
        bounds=(*p.bounds[:-1], (0, width)),
        endpoint=replace(p.endpoint, shape=(*p.endpoint.shape[:-1], width))) for p in ports]
    ...
    with pytest.raises(ValueError, match='nondivisible original split'):
...
def test_coordinated_output_owner_swap_after_accepted_predecessor(baseline):
    ...
    for c in cells(args):
        ...
        lo, hi = ir.indmap[3]; width = hi-lo
        other = (c.rank % 2+1) % 2
        ir.indmap = (*ir.indmap[:3], (other*width, (other+1)*width))
    assert predecessor.render(*args)[1]['frontier_units']
...
def test_value_order_oracle_is_split_each_source_then_concat():
    ...
    for u in range(D):
        ...
        inputs = np.split(batch, Tn, axis=1)
        outputs = [np.concatenate([np.split(x, Tn, axis=3)[j] for x in inputs], axis=1) for j in range(Tn)]
        assert all(y.shape == (B, S*Tn, H, C) for y in outputs)
        assert np.array_equal(np.concatenate(outputs, axis=3), batch)
        assert not np.array_equal(np.concatenate(outputs[::-1], axis=3), batch)
        wrong = [np.concatenate([np.split(x, Tn, axis=3)[j] for x in inputs[::-1]], axis=1) for j in range(Tn)]
        assert not np.array_equal(np.concatenate(wrong, axis=3), batch)
