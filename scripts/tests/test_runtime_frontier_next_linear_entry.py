...
def test_frontier_next_linear_same_six_text_and_detail_before_codecs():
    ...
    assert len(imports) == 1, 'canonical frontier linear attachment missing'
    ...
    assert any(isinstance(n, ast.Assign) and ast.unparse(n.value) == detail.id
               and any(isinstance(t, ast.Subscript) and ast.unparse(t) == "result['frontier_next_linear_values']"
