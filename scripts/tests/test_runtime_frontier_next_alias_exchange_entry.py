...
def test_frontier_next_alias_exchange_same_six_text_and_detail_before_codecs():
    ...
    imports = [n for n in ast.walk(attach) if isinstance(n, ast.ImportFrom)
               and n.module == 'Verdict.runtime_frontier_next_alias_exchange_values']
    assert len(imports) == 1, 'canonical next alias/exchange attachment missing'
    ...
    assert any(isinstance(n, ast.Assign) and ast.unparse(n.value) == detail.id
               and any(isinstance(t, ast.Subscript) and ast.unparse(t) == "result['frontier_next_alias_exchange_values']"
                       for t in n.targets) for n in ast.walk(attach))
    predecessor = next(n for n in ast.walk(attach) if isinstance(n, ast.ImportFrom)
                       and n.module == 'Verdict.runtime_frontier_add_values')
